<template>
  <el-table-column
    ...
    :filter-method="
      getColumnProp(column, 'filterMethod') ? handleFiltersMethod : undefined
    "
    ...
    <template slot-scope="{ row, $index }">
      ...
      <template v-else>
        ...
        <span v-else-if="'rate' === column.type">
          <avue-rate disabled v-model="row[column.prop]" />
        </span>
        <i
...
export default {
  ...
  created() {
    ...
    // console.log(this.crud.tableOption)
  },
  methods: {
    allParams(item) {
      return {
    ...
    handleChange(column, row) {
      this.$nextTick(() => {
        ...
        sendDic({
          ...
          //首次加载的放入队列记录
          if (!this.crud.formIndexList.includes(rowIndex))
            this.crud.formIndexList.push(rowIndex)
    ...
    getImgList(row, column) {
      ...
      let list = this.corArray(
        this.deepClone(row[column.prop]),
        column.separator
      )
