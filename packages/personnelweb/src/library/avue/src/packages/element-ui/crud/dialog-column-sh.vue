    ...
    <div
      ...
      <template>
        <div class="labelBox" v-for="(item, index) in list" :key="index">
          <template>
            <el-checkbox
              v-model="crudlist[item.prop].hide"
              @change="checkboxChange(item.prop)"></el-checkbox>
        ...
        <template v-for="(item, index) in crud.defaultColumn">
          <el-table-column
            ...
            <template slot-scope="{ row }">
              <i
                class="el-icon-close"
                @click="deleteItem(crudlist[row.prop])"></i>
            </template>
          </el-table-column>
        </template>
      </el-table>
  ...
  methods: {
    ...
    rowDrop() {
      const el = document.querySelectorAll(
        '.sortbox .el-table__body-wrapper tbody'
      ) //当打开多个带列显隐的页面时，document.querySelectorAll取最后一个就是当前tab的列显隐窗口
      // console.log(el)
      const _this = this
