...
export default {
  ...
  computed: {
    ...
    columnOption() {
      let column = this.deepClone(this.crud.columnOption).filter(
        item => item.hide == true
      )
      column.forEach(ele => {
  ...
  methods: {
    handleShow(data, searchFormat) {
      (this.box = true), (this.form.name = this.name)
      this.data = data
    ...
    async handleSum() {
      ...
      if (!this.crud.option.page) {
        ...
        for (let i = 1; i < num; i++) {
          await this.crud
            .excelAll({ ...this.crud.search, pageNo: i, pageSize: size })
            .then(data => {
              if (data) {
                list = [...list, ...data.data]
              }
