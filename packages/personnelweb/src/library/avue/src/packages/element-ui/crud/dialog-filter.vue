  ...
  <el-drawer
    ...
    <el-row :span="24">
      <div :class="b('filter-menu')">
        <el-button-group>
          <el-button
            ...
            @click="handleAdd"
            >{{ t('crud.filter.addBtn') }}</el-button
          >
          <el-button
            type="primary"
            :size="crud.isMediumSize"
            @click="handleClear"
            >{{ t('crud.filter.resetBtn') }}</el-button
          >
          <el-button
            type="primary"
            :size="crud.isMediumSize"
            @click="handleValueClear"
            >{{ t('crud.filter.clearBtn') }}</el-button
          >
        </el-button-group>
      </div>
      <el-col
        ...
        <form-temp
          :column="getColumnByIndex(columnList[index])"
          :size="crud.isMediumSize"
      ...
      <el-col :span="24" class="avue-form__menu avue-form__menu--right">
        ...
        >
        <el-button @click="box = false" :size="crud.isMediumSize">{{
...
export default create({
  ...
  computed: {
    ...
    columnOption() {
      return this.crud.propOption.filter(
        ele => ele.filter !== false && ele.showColumn !== false
      )
    }
  },
  created() {
