<template>
  <component
    ...
    <span
      ...
      <el-button
        v-if="vaildData(formOption.submitBtn, true) && !isView"
        @click="submit"
        :disabled="disabled"
      ...
      >
      <el-button
        v-if="vaildData(formOption.emptyBtn, true) && !isView"
        @click="reset"
        :disabled="disabled"
      ...
      const callback = () => {
        if (this.isEdit) {
          ...
          if (this.crud.isTree) {
            ...
            let hasChildrenKey =
              this.crud.treeProps['hasChildren'] || 'hasChildren'
