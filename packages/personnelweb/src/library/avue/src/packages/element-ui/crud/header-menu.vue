  ...
  <div :class="b('menu')">
    ...
    <div :class="b('right')">
      ...
      <el-button
        ...
        v-if="
          vaildData(crud.tableOption.excelBtn, config.excelBtn)
        "></el-button>
      <el-button
        ...
        v-if="
          vaildData(crud.tableOption.printBtn, config.printBtn)
        "></el-button>
      <el-button
        ...
        v-if="
          vaildData(crud.tableOption.refreshBtn, config.refreshBtn)
        "></el-button>
      <el-button
      ...
      <el-button
        ...
        v-if="
          (crud.$refs.headerSearch || {}).searchFlag &&
          vaildData(crud.tableOption.searchShowBtn, true)
        "></el-button>
      <el-button
        ...
        v-if="
          vaildData(crud.tableOption.filterBtn, config.filterBtn)
        "></el-button>
    </div>
  </div>
</template>
...
export default create({
  ...
  methods: {
    ...
    rowExcel() {
      let obj = {}
      let format = {}
      let searchForm = this.crud.$refs.headerSearch.searchForm
      let tableOption = this.crud.tableOption.column
      console.log(searchForm)
      for (let key in searchForm) {
        let filterElement = tableOption.filter(item => item.prop == key)[0]

        if (filterElement) {
          format[filterElement.label] = searchForm['$' + key] || searchForm[key]
          obj[filterElement.label] = searchForm[key]
        }
      }

      debugger
      this.crud.$refs.dialogExcel.handleShow(obj, format)
    },
    rowPrint() {
