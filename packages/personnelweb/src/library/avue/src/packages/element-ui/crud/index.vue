<template>
  <div :class="b({ card: !option.card })">
    ...
    <!-- <dialog-column ref="dialogColumn" @dialogClose="dialogFormClose(arguments)"></dialog-column> -->
    <dialog-column
      ref="dialogColumn"
      @dialogClose="dialogFormClose"></dialog-column>
    <!-- <dialog-column ref="dialogColumn" @dialogClose="dialogFormClose"></dialog-column> -->
...
import dialogColumn from './dialog-column-sh'
...
export default create({
  ...
  mounted() {
    if (this.tableOption.dialogColumn) {
      let colums = []

      this.tableOption.dialogColumn.forEach(item => {
        let element = this.defaultColumn.filter(item2 => item2.prop == item)[0]
        if (element) colums.push(element)
      })

      this.defaultColumn = colums
    }
    this.refreshTable(() => {
      //动态计算表格高度
      this.getTableHeight()
  ...
  methods: {
    vaildSearchForm(validate) {
      this.$refs.headerSearch.$refs.form.validate(validate)
    },
    vaildErrorItem(item, msg) {
      let form = this.$refs.headerSearch.$refs.form.$refs.form.fields.filter(
        item2 => item2.prop == item
      )[0]

      if (form) {
        form.validateMessage = msg
        form.validateState = 'error'
      }
    },
    getParameters() {
      let parameters = this.$refs.headerSearch.getParameters()
      let params = {}
    ...
    dialogFormClose(val) {
      // dialogFormClose(){
      this.$emit('hideChange', this.getHideColumns(val))
      // this.$emit("hideChange", this.getHideColumns())
    },
    ...
    getTableHeight() {
      if (this.isAutoHeight) {
        this.$nextTick(() => {
          ...
          const headerSearch = this.$refs.headerSearch
          if (!tableRef) return
          const tableStyle = tableRef.$el
          const headerSearchStyle = headerSearch.$el.offsetHeight + 50
    ...
    headerDragend(newWidth, oldWidth, column, event) {
      if (newWidth < column.minWidth) {
        this.objectOption[column.property].width = column.minWidth
        column.width = column.minWidth
        this.$emit('header-dragend', column.minWidth, oldWidth, column, event)
      } else {
    ...
    getHideColumns(val) {
      let datas = []
      for (var i in val) {
        if (val[i].hide == true) datas.push(val[i].prop)
      }

      this.aoption_column.filter(item => {
        if (item.showColumn) {
          item.hide = false
          datas.filter(item2 => {
            if (item.prop == item2) {
      ...
      this.$emit('row-del', row, index, () => {
        const callback = (list = []) => {
          let index = list.findIndex(
            ele => ele[this.rowKey] === row[this.rowKey]
          )
          list.splice(index, 1)
        }
        if (this.isTree) {
    ...
    tableSummaryMethod(param) {
      ...
      if (typeof this.summaryMethod === 'function') {
        this.sumsList = this.summaryMethod(param)
