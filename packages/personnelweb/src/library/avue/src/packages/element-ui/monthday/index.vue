<template>
  <div
    ...
    v-bind:class="[
      'normal_state',
      { active_state: myfocus, error_state: errorstate }
    ]">
    ...
    <el-row type="flex" justify="start">
      <el-col :span="5" class="typeSelect">
        <el-select
          ...
          :readonly="readonly"
          @focus="myfocus = true">
          <el-option label="按月" :value="true"> </el-option>
          <el-option label="按日" :value="false"> </el-option>
        </el-select>
      </el-col>
      <el-col>
        <avue-date
        ...
        disabledDate: time => {
          if (that.timeLimit && that.timeLimit.currentLimit) {
            if (that.mtype) {
              let months =
                moment()
                  .subtract(that.timeLimit.month, 'months')
                  .toDate()
                  .getMonth() + 1
              ...
              let month_temp =
                ...
                '-01 00:00:00'
              return (
                time.getTime() < new Date(month_temp).getTime() ||
              ...
              return (
                time.getTime() <
                  moment()
                    .subtract(that.timeLimit.day, 'days')
                    .toDate()
          ...
          if (maxDate && minDate && this.timeLimit) {
            ...
            if (that.mtype) {
              ...
              if (diff >= that.timeLimit.month) {
                ...
                that.$parent.$parent.validateMessage =
                  '时间范围不能超过' + that.timeLimit.month + '月'
                that.$parent.$parent.validateState = 'error'
                that.$parent.$parent.error =
                  that.$parent.$parent.validateMessage
    ...
    selectChange(value) {
      ...
      if (
        ...
        if (value) {
          ...
          this.mychange([
            this.defaultValue.month[0],
            this.defaultValue.month[1],
            value
          ])
        } else if (this.defaultValue.day != undefined) {
          ...
          this.mychange([
            this.defaultValue.day[0],
            this.defaultValue.day[1],
            value
          ])
        }
      }
