<template>
  <el-row :class="[b(), { 'avue--card': isCard }]" :span="24">
    <el-col
      :span="column.span || 24"
      ...
      <div :class="b('content')">
        ...
        <span
          ...
          :class="[
            b('tags'),
            { 'avue-search__tags--active': getActive(item, column) }
          ]"
          @click="handleClick(column, item)"
          v-for="item in DIC[column.prop]"
          :key="getKey(item, column.props, 'value')"
          >{{ getKey(item, column.props, 'label') }}</span
        >
      </div>
    </el-col>
  </el-row>
