    ...
    if (revealList)
      for (let key in revealList) {
        ...
        if (revealList[key] && revealList[key].length > 0) {
          if (
            Object.prototype.toString.call(revealList[key]) == '[object Array]'
          ) {
            if (revealList[key].length > 2 || revealList[key].length == 2) {
              //日期查询条件，在查范围月的时候会有三个值,把第三个true去掉
              let key_temp = revealList[key][0] + ' 至 ' + revealList[key][1]
              excelHeader[excelHeader.length - 1].push(
                key + ':',
                key_temp,
                '',
                ''
