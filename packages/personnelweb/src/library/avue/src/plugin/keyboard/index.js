export default class Keyboard {
  constructor(obj) {
    ...
    let html =
      ...
      (obj.fixedBottomCenter ? ' akeyboard-keyboard-fixedBottomCenter' : '') +
      '" style="' +
      keyboardStyle +
      '">'
    ...
    const keys = obj.keys || [
      ['`'].concat(numberKeys).concat(['-', '=', 'Delete']),
    ...
    if (containShift) {
      ...
      const elKeysEl = document.querySelectorAll(
        obj.el + ' .akeyboard-keyboard-keys-Shift'
      )
        ...
        el.onclick = function () {
          if (!this.isShift) {
            const caps = document.querySelector(
              obj.el + ' .akeyboard-keyboard-keys-Caps'
            )
            if (caps && caps.isCaps) {
              return
            }
            ...
            const keysInnerEl = document.querySelectorAll(
              obj.el + ' .akeyboard-keyboard-innerKeys'
            )
    ...
    if (containCaps) {
      const elCapsEl = document.querySelectorAll(
        obj.el + ' .akeyboard-keyboard-keys-Caps'
      )
  ...
  inputOn(inputEle, type, fn, customClick) {
    ...
    const inputEl = document.querySelector(inputEle),
      elKeysEl = document.querySelectorAll(
    ...
    for (let i = 0; i < elKeysEl.length; i++) {
      ...
      if (
        customClick &&
        typeof customClick === 'object' &&
        Object.keys(customClick).length > 0 &&
        customClick[elKeysEl[i].innerHTML]
      ) {
  ...
  onclick(btn, fn) {
    ...
    let elKeysEl = document.querySelector(
      this.obj.el + ' .akeyboard-keyboard-keys-' + btn
    )
