...
const addTextareaCursor = (textarea, cursor, text) => {
  ...
  if (textarea.setSelectionRange) {
    ...
    nValue =
      oValue.substring(0, cursor.start) + text + oValue.substring(cursor.end)
...
function printLine(format, keymap, Obj) {
  ...
  if (!array || array.length == 0) {
    ...
    let filed = keymap[arrayElement].filed
...
function printData(obj, printData, printKeys) {
  ...
  let printDataLines = printData.split('\n')
...
function formatContent(print, printitemData) {
  ...
  match.forEach(item => {
    let filter = printitemData.filter(item2 => item.indexOf(item2.name) != -1)
