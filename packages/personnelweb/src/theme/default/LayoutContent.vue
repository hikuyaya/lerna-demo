...
<!--          </el-badge>-->
          ...
          <el-dropdown>
            <img class="avatar" :src="headImgUrl? headImgUrl : require('../../assets/images/avatar.png')"
                 style="display: inline-block; margin-right: 15px;vertical-align: middle;width: 30px;height:30px;border-radius: 50%">
            <span class="userName" style="display: inline-block;">{{ userInfo.eename }}</span>
  ...
  methods: {
    async getUserInfo() {
      await service.user.userInfo().then(res => {
        ...
        let filter = res.data.menus.filter(item => item.url.indexOf("/report/") != -1);
        filter.forEach(item => {
          ...
          this.$router.$addRoutes({
                ...
                name: item.name,
                meta: {title: item.name, closable: true, auth: true, params: queryVariable},
