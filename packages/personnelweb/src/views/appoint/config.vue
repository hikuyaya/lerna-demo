  ...
  <div class="appoint-conf">
    <el-tabs v-model="activeTab" @tab-click="handleClick">
      <el-tab-pane label="预约规则设置" name="rule">
        <el-form
          ...
          <el-form-item
            ...
            <yid-ask>预约时间显示的开始结束时间段</yid-ask>
          </el-form-item>
          <el-form-item
            label="预约结束时间："
          ...
          <el-form-item label="到店分配预约天数：">
            <el-input-number
              v-model="ruleForm.maxshopday"
              placeholder="到店分配预约天数"
              :min="0"
              style="width: 200px"></el-input-number>
            <yid-ask
              >设置在顾客端小程序显示的天数，数值必须为大于0的整数</yid-ask
            >
          </el-form-item>
          <el-form-item label="到店分配最大预约次数：">
            <el-input-number
              v-model="ruleForm.alldaycount"
              placeholder="到店分配最大预约次数"
              :min="1"
              style="width: 200px"></el-input-number>
            <yid-ask
              >设置在顾客端小程序显示的天数，数值必须为大于0的整数</yid-ask
          ...
          <el-form-item label="自助预约需提前的天数：">
            <el-select
              ...
              <el-option
                ...
                :label="item.name"
                :value="item.id"
                v-for="item in earlierdays"></el-option>
            </el-select>
            天
            <yid-ask>顾客端预约如需提前的天数为0，则表示可预约当天</yid-ask>
          </el-form-item>
          <el-form-item label="当天预约需提前：">
            <el-select
              value-key="id"
              v-model="ruleForm.earliertime"
              clearable
              filterable
              placeholder="请选择">
              <el-option
                ...
                :label="item.name"
                :value="item.id"
                v-for="item in earliertimes"></el-option>
            </el-select>
            分钟
            <yid-ask
              >顾客端预约如需提前的分钟数为0，则表示不需提前，随时可约；门店创建预约不受该限制</yid-ask
            >
          </el-form-item>
          <el-form-item label="取消预约需提前：">
            <el-select
              ...
              <el-option
                ...
                :label="item.name"
                :value="item.id"
                v-for="item in canceltimes"></el-option>
            </el-select>
            分钟
            <yid-ask
              >离预约时间N分钟以内顾客可自主取消，如分钟数为0，则表示顾客可随时取消；门店可随时取消，不受该时间限制</yid-ask
            >
          </el-form-item>
          <el-form-item label="预约时长占用模式：">
            ...
            <yid-ask
              >注：叠加时以时间最长的项目为准占用，累计时以所有项目的总时长为准占用</yid-ask
            >
          </el-form-item>
          ...
          <el-form-item label="是否显示员工级别：">
            <el-radio-group
              v-model="ruleForm.showPslevel"
              @change="ifpslevelShow">
              <el-radio label="1">显示</el-radio>
              <el-radio label="0">不显示</el-radio>
            </el-radio-group>
            <yid-ask>控制顾客端中的员工列表及员工主页中级别的显示</yid-ask>
          </el-form-item>
          <el-form-item>
            <el-checkbox
          ...
          <el-form-item label="是否显示员工好评率：">
            ...
            <yid-ask>控制顾客端中的员工好评率的显示</yid-ask>
          </el-form-item>
          <el-form-item label="是否开启预约播报：">
            ...
            <yid-ask>控制门店端是否语音播报预约通知</yid-ask>
          </el-form-item>
          <el-form-item label="预约项目显示方式：">
            <el-radio-group v-model="ruleForm.serShowType">
              ...
              ><yid-ask
                >可前往“预约项目设置”模块设置具体可预约分类或项目</yid-ask
              >
            </el-radio-group>
          </el-form-item>
          <el-form-item label="预约限制：">
            ...
            <el-input
              ...
              placeholder=""
              @blur="check1"
              style="width: 50px; margin: 0 10px 0 10px"></el-input>
            <span>元，爽约不退还</span>
          ...
          <el-form-item>
            ...
            <span style="margin-left: 10px">连续爽约</span>
            <el-input
              v-model="ruleForm.restrictedDay"
              placeholder=""
              @blur="check3"
              style="width: 50px; margin: 0 10px 0 10px"></el-input>
            <span>次后顾客将在</span>
            <el-input
              v-model="ruleForm.miss"
              placeholder=""
              @blur="check3"
              style="width: 50px; margin: 0 10px 0 10px"></el-input>
            <span>天内不能在线预约</span>
      ...
      <el-tab-pane label="预约项目设置" name="project">
        ...
        <el-table
          ...
          <el-table-column
            ...
            label="分类编码"
            min-width="100"
            prop="scode"></el-table-column>
          <el-table-column
            align="center"
            label="分类名称"
            min-width="150"
            prop="sname"></el-table-column>
          <el-table-column
            align="center"
            label="预约显示名称"
            min-width="150"
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-checkbox
                ...
                @change="tableCheckboxChange"></el-checkbox>
            </template>
          </el-table-column>
        </el-table>
        <el-table
          ...
          <el-table-column
            ...
            label="项目编码"
            min-width="100"
            prop="servcode"></el-table-column>
          <el-table-column
            align="center"
            label="项目分类"
            min-width="150"
            prop="servtname"></el-table-column>
          <el-table-column
            align="center"
            label="项目名称"
            min-width="150"
            prop="servname"></el-table-column>
          <el-table-column
            align="center"
            label="预约显示名称"
            min-width="150"
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-checkbox
                v-model="scope.row.isyy"
                @change="tableCheckboxChange1"></el-checkbox>
            </template>
          </el-table-column>
          <el-table-column
            align="center"
      ...
      <el-tab-pane label="预约渠道设置" name="channel">
        <el-button @click="addChannel(false, '')" type="primary"
          >新增渠道</el-button
        >
        <yid-table pagination ref="channelTable" style="margin-top: 15px">
          <yid-table-column
            label="渠道编码"
            min-width="100"
            prop="yccode"></yid-table-column>
          <yid-table-column
            label="渠道名称"
            min-width="150"
            prop="ycname"></yid-table-column>
          <yid-table-column label="操作" min-width="100" prop="content">
            <template slot-scope="scope">
              <el-link
                type="primary"
                :disabled="getIsEdit(scope.row)"
                @click="addChannel(true, scope.row)"
                >编辑</el-link
              >
            </template>
          </yid-table-column>
    ...
    <yid-dialog
      ...
      <el-form ref="channelForm" :model="channelForm" label-width="140px">
        ...
        <el-form-item
          label="渠道名称："
          prop="ycname"
          :rules="[{ required: true, message: '渠道名称为空' }]">
          <el-input
            v-model="channelForm.ycname"
            placeholder="请输入5字以内"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <yid-dialog
      :title="pslevelDialog.title"
      :visible.sync="pslevelDialog.visible"
      width="450px">
...
export default {
  ...
  data() {
    return {
      ...
      tempRow: {},
      serviceMemo:
        '注：服务时长不设置则默认为一个预约区间；显示名称不设置默认为“分类名称”'
    }
  },
  mounted() {
    this.init()
  ...
  methods: {
          ...
          service.yy.config.getYYconfig().then(res => {
            if (res.resp_code == 200) {
              if (res.data.length > 0) {
                ...
                if (
                  this.ruleForm.deposit != null &&
                  this.ruleForm.deposit != ''
                ) {
                  this.depositCheck = true
                }
                if (
                  this.ruleForm.restricted != null &&
                  this.ruleForm.restricted != ''
                ) {
                  this.restrictedCheck = true
                }
                if (
                  (this.ruleForm.restrictedDay != null &&
                    this.ruleForm.restrictedDay != '') ||
                  (this.ruleForm.miss != null && this.ruleForm.miss != '')
                ) {
                  this.restrictedDayCheck = true
                }
                if (
                  this.ruleForm.dayLimit != null &&
    ...
    check3() {
      ...
      if (
        (restrictedDay != null && restrictedDay != '') ||
        (miss != null && miss != '')
      ) {
        this.restrictedDayCheck = true
      } else {
      ...
      this.$refs['ruleForm'].validate(valid => {
        if (valid) {
          ...
          if (this.ruleForm.stime > this.ruleForm.etime) {
            yid.util.alert('预约结束时间不得早于预约开始时间')
            return false
          }
          if (
            Number(this.ruleForm.earlierday) >= Number(this.ruleForm.maxday)
          ) {
            yid.util.alert('可预约天数必须大于自助预约需提前的天数！')
            return false
          }
          let pslevel = ''
    ...
    getServicetypeList() {
      service.servicetype.getServicetypes({ status: '1' }).then(res => {
        if (res.resp_code == 200) {
          if (res.data.length > 0) {
    ...
    getServiceinfoList() {
      service.serviceInfo
        .findServiceList({ isDel: '0', status: '1' })
        .then(res => {
          if (res.resp_code == 200) {
            if (res.data.length > 0) {
              res.data.forEach(item => {
    ...
    saveService() {
      if (this.yytype == '1') {
        ...
        service.servicetype
          .updateTypes({ types: this.servicetypeData })
          .then(res => {
            if (res.resp_code == 200) {
              this.getServicetypeList()
            }
        ...
        service.serviceInfo
          .updateInfos({ servs: this.serviceinfoData })
          .then(res => {
            if (res.resp_code == 200) {
              this.getServiceinfoList()
            }
    ...
    getIsEdit(row) {
      if (
        /*row.yccode == '01' || row.yccode=='02' || */ row.ycname == '小程序' ||
        row.ycname == '反预约'
      ) {
        return true
      } else {
    ...
    tableCheckboxChange() {
      setTimeout(() => {
        let isAllTrue = this.servicetypeData.every(item => item.isyy === true)
        let isAllFalse = this.servicetypeData.every(item => item.isyy === false)
        if (isAllTrue) {
          this.tableIsIndeterminate = false
          this.checkAll = true
        } else if (isAllFalse) {
    ...
    tableCheckboxChange1() {
      setTimeout(() => {
        let isAllTrue = this.serviceinfoData.every(item => item.isyy === true)
        let isAllFalse = this.serviceinfoData.every(item => item.isyy === false)
        if (isAllTrue) {
          this.tableIsIndeterminate = false
          this.checkAll = true
        } else if (isAllFalse) {
