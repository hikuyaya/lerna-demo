            ...
            <el-tab-pane label="预约规则设置" name="rule">
                <el-form ref="ruleForm" :model="ruleForm" :label-position="labelPosition" label-width="200px" >
                    ...
                    <el-form-item label="到店分配预约天数：">
                        <el-input-number v-model="ruleForm.maxshopday" placeholder="到店分配预约天数" :min="0" style="width: 200px;"></el-input-number>
                        <yid-ask>设置在顾客端小程序显示的天数，数值必须为大于0的整数</yid-ask>
                    </el-form-item>
                    <el-form-item label="到店分配最大预约次数：">
                        <el-input-number v-model="ruleForm.alldaycount" placeholder="到店分配最大预约次数" :min="1" style="width: 200px;"></el-input-number>
                        <yid-ask>设置在顾客端小程序显示的天数，数值必须为大于0的整数</yid-ask>
                    </el-form-item>
                    ...
                    <el-form-item label="当天预约需提前：">
                        ...
                        <yid-ask>顾客端预约如需提前的分钟数为0，则表示不需提前，随时可约；门店创建预约不受该限制</yid-ask>
                    </el-form-item>
                    <el-form-item label="取消预约需提前：">
                        ...
                        <yid-ask>离预约时间N分钟以内顾客可自主取消，如分钟数为0，则表示顾客可随时取消；门店可随时取消，不受该时间限制</yid-ask>
                    </el-form-item>
        ...
        <yid-dialog :title="addDialog.title" :visible.sync="addDialog.visible" @close="getchannelList" width="450px">
            <el-form ref="channelForm" :model="channelForm"  label-width="140px">
                <el-form-item label="渠道编码：" prop="yccode">
                    <el-input value="自动生成" placeholder="请输入5字以内" disabled="disabled"></el-input>
                </el-form-item>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            saveConfig(){
                ...
                this.$refs['ruleForm'].validate((valid) => {
                    if(valid){
                        ...
                        if(Number(this.ruleForm.earlierday) >= Number(this.ruleForm.maxday)){
            ...
            getServiceinfoList(){
                service.serviceInfo.findServiceList({isDel:'0',status:'1'}).then(res =>{
                    if(res.resp_code == 200){
                        if(res.data.length > 0){
                            res.data.forEach(item=>{
                                if(item.isyy == '1'){
