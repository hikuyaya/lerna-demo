<template>
  <div class="appoint-list">
    <el-form inline>
      ...
      <el-form-item label="预约渠道：">
        <el-select
          ...
          placeholder="请选择"
          style="width: 160px"
          v-model.trim="model.yccode">
          <el-option
            :key="'c_' + item.yccode"
            v-for="item in channel"
            :value="item.yccode"
            :label="item.ycname"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="门店：">
        <el-select
          clearable
          v-model.trim="model.shopid"
          @clear="clearFormShop"
          @blur="clearFormShop"
          filterable
    ...
    <yid-table pagination ref="table" :data="tableData">
      ...
      <yid-table-column label="预约项目" min-width="150" prop="sersnames">
        <template slot-scope="scope">
          {{ scope.row.sersnames != null ? scope.row.sersnames : '不选项目' }}
        </template>
      </yid-table-column>
      <yid-table-column
        label="服务时长(min)"
        min-width="80"
        prop="hjtimes"></yid-table-column>
      <yid-table-column
        label="人数"
        min-width="60"
        prop="perNum"></yid-table-column>
      <yid-table-column
        label="预约渠道"
        min-width="120"
    ...
    <yid-dialog
      ...
      <el-form
        ...
        <el-form-item label="预约员工：" prop="eename" label-width="100px">
          {{ yybillhead.eename }} </el-form-item
        ><br />
        ...
        <el-form-item label="预约时间：" prop="yydate" label-width="100px">
          ...
          <div>
            ...
            <el-button
              v-if="yybillhead.billstatus == '3' && editflag"
        ...
        <el-form-item label="性别：" prop="custsex" label-width="120px">
          <div style="width: 160px">
            {{ yybillhead.custsex == '1' ? '男' : '女' }}
          </div>
        </el-form-item>
        <el-form-item
          v-if="yybillhead.sersnames ? true : false"
          label="预约项目："
          prop="sersnames"
          label-width="100px">
        ...
        <el-form-item
          v-if="yybillhead.sersnames ? false : true"
          label="预约时长："
          prop="hjtimes"
          label-width="100px">
          <div>{{ yybillhead.hjtimes }}min</div> </el-form-item
        ><br />
        <el-form-item label="人数：" prop="exprite" label-width="120px">
          <div style="width: 160px">{{ yybillhead.perNum }}</div>
        </el-form-item>
        <el-form-item label="服务时长：" prop="channel" label-width="100px">
          <div style="width: 160px">
            {{ yybillhead.hjtimes }}min
          </div> </el-form-item
        ><br />
        ...
        <el-form-item label="备注：" prop="memo" label-width="120px">
          <div>{{ yybillhead.memo }}</div> </el-form-item
        ><br />
        <el-form-item label="创建时间：" prop="createdTime" label-width="100px">
          <div>{{ yybillhead.createdTime }}</div>
        </el-form-item>
        <el-row>
          <el-col :span="3" :offset="3" v-if="yybillhead.billstatus == '3'"
            ><el-button @click="yycancel()">取消预约</el-button></el-col
          >
          <el-col :span="6" :offset="4" v-if="yybillhead.billstatus == '3'"
            ><el-button @click="yysure()" type="primary"
              >到店</el-button
            ></el-col
          >
          <el-col :span="3"
            ><el-button @click="lookupLog()" type="primary"
              >预约状态日志</el-button
            ></el-col
          >
        </el-row>
...
export default {
  ...
  methods: {
    ...
    getYybill(row) {
      service.yy.yylist
        .getYybill({ id: row.id, shopid: row.shopid })
        .then(res => {
    ...
    yysure() {
      service.yy.yylist
        .yysure({ id: this.yybillhead.id, revision: this.yybillhead.revision })
        .then(res => {
          if (res.resp_code == '200') {
            this.searchDialog.visible = false
            this.queryYyBilllist()
    ...
    lookupLog() {
      service.yy.yylist
        .lookYyBillLogs({ billid: this.yybillhead.id })
        .then(res => {
