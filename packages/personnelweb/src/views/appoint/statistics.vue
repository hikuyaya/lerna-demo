      ...
      <el-tab-pane label="门店" name="store">
        <el-form inline ref="model" :model="model">
          ...
          <el-form-item style="margin-bottom: 0">
            <el-button @click="getStatisShopList" type="primary"
              >查询</el-button
            >
        ...
        <yid-table
          ...
          <yid-table-column
            label="预约次数"
            min-width="150"
            prop="yyNum"></yid-table-column>
          <yid-table-column
            label="到店次数"
            min-width="150"
            prop="doneNum"></yid-table-column>
          <yid-table-column
            label="服务客次"
            min-width="150"
            prop="serviceNum"></yid-table-column>
      ...
      <el-tab-pane label="员工" name="staff">
        <el-form inline ref="model2" :model="model2">
          ...
          <el-form-item label="门店：">
            <el-select
              clearable
              v-model.trim="model2.shopid"
              @clear="clearFormShop2"
              @blur="clearFormShop2"
              filterable
              :filter-method="filterShop"
              placeholder="请选择门店"
          ...
          <el-form-item style="margin-bottom: 0">
            <el-button @click="getStatisEmployeeList" type="primary"
              >查询</el-button
            >
  ...
  methods: {
    excel() {
      this.$refs['model'].validate(valid => {
        if (valid) {
          if (
            this.model.createdTime1 == '' &&
            this.model.createdTime1.length == 0
          ) {
            yid.util.alert('开始时间不能为空')
            return false
          }
          if (
            this.model.createdTime2 == '' &&
            this.model.createdTime2.length == 0
          ) {
            yid.util.alert('结束时间时间不能为空')
            return false
          }
          download(
            $yid.config.API.BASE + 'api-yy/statisshop/list/excel',
            this.model
          )
        }
      })
    ...
    excel2() {
      this.$refs['model'].validate(valid => {
        if (valid) {
          ...
          download(
            $yid.config.API.BASE + 'api-yy/statisemployee/list/excel',
            this.model
          )
        }
      })
    ...
    getStatisEmployeeList() {
      this.$refs['model2'].validate(valid => {
        if (valid) {
          if (
            this.model2.createdTime1 == '' &&
            this.model2.createdTime1.length == 0
          ) {
            yid.util.alert('开始时间不能为空')
            return false
          }
          if (
            this.model2.createdTime2 == '' &&
            this.model2.createdTime2.length == 0
          ) {
            yid.util.alert('结束时间时间不能为空')
            return false
          }
