            ...
            <el-tab-pane label="员工" name="staff">
                <el-form inline ref="model2" :model="model2">
                    ...
                    <el-form-item label="门店：">
                        ...
                        <el-select clearable v-model.trim="model2.shopid" @clear="clearFormShop2" @blur="clearFormShop2" filterable :filter-method="filterShop" placeholder="请选择门店" style="width: 160px;">
                            <el-option :key="item.id" :label="item.shopname" :value="item.id" v-for="item in filterShopList">
                                <span style="float: left">{{ item.shopcode }}</span>
                                <span style="float: right; color: #8492a6; font-size: 13px">{{ item.shopname }}</span>
                            </el-option>
