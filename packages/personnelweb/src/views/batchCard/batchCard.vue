  ...
  <div>
      ...
      <div class="batchCard" v-show="editCardShow">
        ...
        <yid-table pagination ref="bkTable" style="margin-top: 15px">
          ...
          <yid-table-column label="服务人员" prop="eename">
            <template slot-scope="scope">
              <span v-for="royaltys in scope.row.yjsList"
                >{{ royaltys.eename }}|{{ royaltys.yjje }}<br
              /></span>
            </template>
          </yid-table-column>
          ...
          <yid-table-column label="详情">
            <template slot-scope="scope">
              <el-link type="primary" @click="checkCard(scope.row)"
                >详情</el-link
              >
            </template>
          </yid-table-column>
    ...
    <el-collapse-transition>
      <div class="editCard" v-show="!editCardShow">
        <div class="editCardHead">
          ...
          <el-button @click="save" type="primary" :disabled="true"
            >保存并结账</el-button
          >
        </div>
          ...
          <el-form
            ...
            <div class="cardForm_left">
              <el-form-item
                ...
                :rules="[{ required: true }]">
                <el-input
                  :class="this.redborder == 'memname' ? 'redborder' : ''"
                  clearable
                  filterable
                  v-model="cardInfo.memname"></el-input>
              </el-form-item>
              ...
              <el-form-item
                ...
                :rules="[{ required: true }]">
                <el-input
                  :class="this.redborder == 'money' ? 'redborder' : ''"
                  clearable
                  filterable
                  v-model="cardInfo.money"
                  @input="moneyChange"></el-input>
              </el-form-item>
              <el-form-item
                class="must"
                label="开卡数量： "
                :rules="[{ required: true }]">
                <el-input
                  :class="this.redborder == 'cardnum' ? 'redborder' : ''"
                  clearable
                  filterable
                  v-model="cardInfo.cardnum"
                  @input="cardnumChange"></el-input>
              </el-form-item>
            </div>
            <div class="cardForm_right">
              ...
              <el-form-item
                ...
                <div style="width: 300px">
                  <el-select
                    clearable
                    filterable
                    v-model.trim="cardInfo.pays[0].payname"
                    @change="checkpay"
                    :class="this.redborder == 'paylist' ? 'redborder' : ''">
                    <el-option
                  ...
                  <el-input
                    class="payje"
                    :class="this.redborder == 'payje' ? 'redborder' : ''"
                    v-show="
                      cardInfo.pays[0].paycode !== '52' &&
                      cardInfo.pays[0].paycode !== ''
                    "
                    clearable
                    filterable
                    v-model="cardInfo.pays[0].payje"
                    @input="payjeChange"></el-input>
                  <!--  银联通付款方式  选择的流水单号 -->
                  <span
                    :class="this.redborder == 'paycode' ? 'redborder' : ''"
                    v-show="cardInfo.pays[0].paycode == '52'"
                    >{{ cardInfo.pays[0].paymentNo }}</span
                  >
                </div>
              </el-form-item>
              <!-- 第一工位的服务人员  -->
              <el-form-item :label="gwlist[0].name + ': '">
                <div class="ygbox">
                  <div
                    class="ygcontent"
                    v-if="
                      cardInfo.yjs[0][0].eecode === '' &&
                      cardInfo.yjs[0].length == 1
                    ">
                    <div class="yg">
                      <!-- <span>服务人员</span> -->
                      <el-select
                        filterable
                        v-model.trim="cardInfo.yjs[0][0].eename">
                        <el-option
                          v-for="(item, index) in eelist[0]"
                          :key="index"
                          :label="item.eename"
                          :value="item.eename"
                          style="padding-left: 18px">
                    ...
                    <div class="yj">
                      ...
                      <el-input
                        clearable
                        filterable
                        v-model="cardInfo.yjs[0][0].je"></el-input>
                    </div>
                    <span
                      class="del_yg"
                      v-show="cardInfo.yjs[0].length > 1"
                      @click="deleteyg(cardInfo.yjs[0][0], '1', 0)"
                      >删除</span
                    >
                  </div>
                  <!-- 保存之后打开，有多个人的情况  -->
                  <div
                    class="ygcontent"
                    v-else
                    v-for="(item, index) in cardInfo.yjs[0]"
                    :key="index">
                    <div class="yg">
                      <!-- <span>服务人员</span> -->
                      <el-select filterable v-model.trim="item.eename">
                        <el-option
                          v-for="(item1, index) in eelist[0]"
                          :key="index"
                          :label="item1.eename"
                          :value="item1.eename"
                          style="padding-left: 18px">
                    ...
                    <div class="yj">
                      ...
                      <el-input
                        clearable
                        filterable
                        v-model="item.je"></el-input>
                    </div>
                    <span
                      class="del_yg"
                      v-show="cardInfo.yjs[0].length > 1"
                      @click="deleteyg(item, '1', index)"
                      >删除</span
                    >
                  </div>
                  <div style="display: flex">
                    <!-- <span class="addyg" @click="addyg('1')">添加</span> -->
                    <span class="addWarning" v-show="firstWarningFlag1"
                      >该工位下最多只允许添加{{
                        this.gwlist[0].pepnum
                      }}人!</span
                    >
                  </div>
                </div>
              ...
              <!-- 第二工位的服务人员  -->
              <el-form-item
                v-if="gwlist.length > 1"
                :label="gwlist[1].name + ': '">
                <div class="ygbox">
                  <div
                    class="ygcontent"
                    v-if="
                      cardInfo.yjs[1][0].eecode === '' &&
                      cardInfo.yjs[1].length == 1
                    ">
                    <div class="yg">
                      <!-- <span>服务人员</span> -->
                      <el-select
                        filterable
                        v-model.trim="cardInfo.yjs[1][0].eename">
                        <el-option
                          v-for="(item, index) in eelist[1]"
                          :key="index"
                          :label="item.eename"
                          :value="item.eename"
                          style="padding-left: 18px">
                    ...
                    <div class="yj">
                      ...
                      <el-input
                        clearable
                        filterable
                        v-model="cardInfo.yjs[1][0].je"></el-input>
                    </div>
                    <span
                      class="del_yg"
                      v-show="cardInfo.yjs[1].length > 1"
                      @click="deleteyg(cardInfo.yjs[1][0], '2', 0)"
                      >删除</span
                    >
                  </div>
                  <!-- 保存之后打开，有多个人的情况  -->
                  <div
                    class="ygcontent"
                    v-else
                    v-for="(item, index) in cardInfo.yjs[1]"
                    :key="index">
                    <div class="yg">
                      <!-- <span>服务人员</span> -->
                      <el-select filterable v-model.trim="item.eename">
                        <el-option
                          v-for="(item1, index) in eelist[1]"
                          :key="index"
                          :label="item1.eename"
                          :value="item1.eename"
                          style="padding-left: 18px">
                    ...
                    <span
                      class="del_yg"
                      v-show="cardInfo.yjs[1].length > 1"
                      @click="deleteyg(item, '2', index)"
                      >删除</span
                    >
                  </div>
                  <div style="display: flex">
                    <!-- <span class="addyg" @click="addyg('2')">添加</span> -->
                    <span class="addWarning" v-show="firstWarningFlag2"
                      >该工位下最多只允许添加{{
                        this.gwlist[1].pepnum
                      }}人!</span
                    >
                  </div>
                </div>
          ...
          <el-table
            ...
            <yid-table-column
              ...
              <template slot-scope="scope">
                <span
                  :class="
                    scope.row.cardcmcode == '' || scope.row.cardcmcode == null
                      ? 'relation'
                      : ''
                  ">
                  {{
                    scope.row.cardcmcode == '' || scope.row.cardcmcode == null
                      ? '关联实体卡'
                      : scope.row.cardcmcode
                  }}
                </span>
              </template>
            </yid-table-column>
            <yid-table-column label="类型" min-width="100"
              >开卡</yid-table-column
            >
            <yid-table-column
              label="开卡金额"
            ...
            <yid-table-column label="服务人员" min-width="100" prop="yjs">
              <template slot-scope="scope">
                <span
                  class="serveyg"
                  v-for="(item, index) in scope.row.yjs"
                  :key="index">
    ...
    <yid-dialog
      ...
      <checkPay
        ref="paySelectPage"
        :ysje="cardInfo.ysje"
        @onClose="closedProductDialog"></checkPay>
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      if (
        $yid.util.isEmpty(this.searchForm.saleTimeRange[0]) ||
        $yid.util.isEmpty(this.searchForm.saleTimeRange[1])
      ) {
        $yid.util.alert('请选择日期再进行查询!')
        return
      }
      let arg = this.DateMinus(
        this.searchForm.saleTimeRange[0],
        this.searchForm.saleTimeRange[1]
      )
      if (Number(arg) > 30) {
    ...
    //单卡金额change
    moneyChange() {
      this.cardInfo.ysje = isNaN(
        Number(this.cardInfo.money) * Number(this.cardInfo.cardnum)
      )
        ? ''
        : Number(this.cardInfo.money) * Number(this.cardInfo.cardnum)
    },
    ...
    save() {
      ...
      if ($yid.util.isEmpty(this.cardInfo.pays[0].paycode)) {
        ...
        this.cardInfo.pays[0].paycode != '52' &&
        $yid.util.isEmpty(this.cardInfo.pays[0].payje)
      ) {
        this.redborder = 'payje'
        return yid.util.error('付款方式金额必填!')
      } else if (
        this.cardInfo.pays[0].paycode == '52' &&
        $yid.util.isEmpty(this.cardInfo.pays[0].paymentNo)
      ) {
      ...
      if (
        this.cardInfo.yjs[0].length == 1 &&
        $yid.util.isEmpty(this.cardInfo.yjs[0][0].eecode) &&
        this.cardInfo.yjs[1].length == 1 &&
        $yid.util.isEmpty(this.cardInfo.yjs[1][0].eecode)
      ) {
        ...
        for (let i = 0; i < this.cardInfo.yjs.length; i++) {
          ...
          for (let m = 0; m < this.cardInfo.yjs[i].length; m++) {
            if (this.cardInfo.yjs[i][m].eecode != '') {
              if (
                serveje + Number(this.cardInfo.yjs[i][m].je) <
                  this.cardInfo.ysje ||
                serveje + Number(this.cardInfo.yjs[i][m].je) ==
                  this.cardInfo.ysje
              ) {
      ...
      this.formDisableFlag = true
      let payname = this.paylist.filter(
        item => item.paycode == this.cardInfo.pays[0].paycode
      )[0].payname
      let yjs = []
      let eeid, eename
      for (let i = 0; i < this.cardInfo.yjs.length; i++) {
        for (let m = 0; m < this.cardInfo.yjs[i].length; m++) {
          if (
            this.cardInfo.yjs[i][m].eecode != '' &&
            this.cardInfo.yjs[i][m].eecode != '00'
          ) {
            let eeitem = this.eeAllList.filter(
              item => item.eecode == this.cardInfo.yjs[i][m].eecode
            )
            yjs.push({
              eeid: eeitem[0].id,
              ...
              postion: i == 0 ? 1 : 2,
              je:
                this.cardInfo.yjs[i][m].je == ''
                  ? 0
                  : this.cardInfo.yjs[i][m].je
            })
          } else if (this.cardInfo.yjs[i][m].eecode == '00') {
            //公单
        ...
        pays: [
          {
            ...
            paymentNo:
              this.cardInfo.pays[0].paycode == '52'
                ? this.cardInfo.pays[0].paymentNo
                : ''
          }
        ],
        yjs: yjs,
          ...
          for (let i = 0; i < this.gwlist.length; i++) {
            for (let m = 0; m < this.gwlist[i].pslist.length; m++) {
              for (let n = 0; n < this.eeAllList.length; n++) {
                if (
                  this.gwlist[i].pslist[m].pscode == this.eeAllList[n].pscode
                ) {
                  if (i == 0) {
    ...
    addyg(type) {
      ...
      if (
        this.gwlist[Number(type) - 1].pepnum >
          this.cardInfo.yjs[Number(type) - 1].length + 1 ||
        this.gwlist[Number(type) - 1].pepnum ==
          this.cardInfo.yjs[Number(type) - 1].length + 1
      ) {
