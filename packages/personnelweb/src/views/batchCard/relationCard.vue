...
export default {
  ...
  methods: {
    ...
    call(command, parms) {
      ...
      // callback: "callback",
      // timeout: 10000,
      Vue.jsonp('http://127.0.0.1:7765', params)
        .then(mdata => {
          console.log(mdata)
          var printmsg = mdata
          ...
          if (command == 'readcard') {
            ...
            if (cardno.substring(0, 4) == '[ER]') {
              if (
                cardno.substring(0, 8) == '[ER][08]' ||
                cardno.substring(0, 8) == '[ER][09]' ||
                cardno.substring(0, 8) == '[ER][10]' ||
                cardno.substring(0, 8) == '[ER][11]'
              ) {
            ...
            if (this.cardNum == cardno) {
              ...
              return yid.util.error(
                '请仔细检查填写卡号与实体卡卡片上的卡号是否一致!'
              )
            }
          }
        })
