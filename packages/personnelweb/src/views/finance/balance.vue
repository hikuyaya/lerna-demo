<template>
  <div class="balance">
    <el-tabs v-model="activeName" @tab-click="handleClick">
      <el-tab-pane label="门店结存月报表" name="shopBalance">
        <el-form :model="searchForm" ref="ruleForm" inline>
          ...
          <el-form-item label="">
            <el-button @click="getshopBalanceList" type="primary"
              >查询</el-button
            >
        ...
        <el-table
          ref="shopBalancetable"
          style="margin-top: 15px"
          :data="shopBalanceDate"
          :row-class-name="$yid.util.getTableClass">
          <el-table-column
            v-for="item in dicBalanceList"
            align="center"
            :prop="item.colName"
            :label="item.name">
          </el-table-column>
      ...
      <el-tab-pane label="结存字段管理" name="dicBalance">
        <el-button @click="dicBalanceAlert(false, '')" type="primary"
          >添加</el-button
        >
        ...
        <yid-table
          ...
          <yid-table-column
            label="报表展示顺序"
            min-width="120"
            prop="colOrder"></yid-table-column>
          <yid-table-column label="操作" min-width="120" prop="content">
            <template slot-scope="scope">
              <el-link type="primary" @click="dicBalanceAlert(true, scope.row)"
                >编辑</el-link
              >
              <el-link
                type="primary"
                style="margin: 0 10px 0 10px"
                @click="dicBalanceDelete(scope.row)"
    ...
    <yid-dialog
      :title="dicBalanceDialog.title"
      :visible.sync="dicBalanceDialog.visible"
      width="450px">
      <el-form ref="dicBalanceForm" :model="dicBalanceForm" label-width="140px">
        <el-form-item
          ...
          <el-input
            clearable
            placeholder="字段名称"
            v-model="dicBalanceForm.name"
            :disabled="dicBalanceForm.isDefault == 1"
            style="width: 200px" />
        </el-form-item>
        <el-form-item
          label="对应报表字段："
          prop="colName"
          :rules="[{ required: true, message: '请选择对应报表字段' }]">
          <el-select
            clearable
            placeholder="请选择报表字段"
            v-model.trim="dicBalanceForm.colName"
            style="width: 160px"
            filterable
            :disabled="isEditCol">
            <el-option
              v-show="checkColName(val.value)"
              v-for="val in colNameList"
              :key="val.value"
              :value="val.value"
              :label="val.label" />
          </el-select>
        ...
        <el-form-item
          label="是否默认字段："
          prop="isDefault"
          :rules="[{ required: true, message: '请选择是否默认字段' }]">
          <el-select
            placeholder="请选择是否默认字段"
            v-model.trim="dicBalanceForm.isDefault"
            style="width: 160px"
            :disabled="dicBalanceForm.isDefault == 1">
            <el-option
              v-for="val in isDefaultList"
              :key="val.value"
              :value="val.value"
              :label="val.label" />
          </el-select>
        ...
        <el-form-item
          label="是否报表展示："
          prop="isShow"
          :rules="[{ required: true, message: '请选择是否报表展示' }]">
          <el-select
            placeholder="请选择是否报表展示"
            v-model.trim="dicBalanceForm.isShow"
            style="width: 160px"
            :disabled="dicBalanceForm.isDefault == 1">
            <el-option
              v-for="val in isShowList"
              :key="val.value"
              :value="val.value"
              :label="val.label" />
          </el-select>
        ...
        <el-form-item
          label="报表展示顺序："
          prop="colOrder"
          :rules="[{ required: true, message: '报表展示顺序不能为空' }]">
          <el-input-number
            placeholder="报表展示顺序"
            :min="1"
            v-model="dicBalanceForm.colOrder"
            style="width: 160px" />
        </el-form-item>
        <el-form-item>
    ...
    <yid-dialog
      ...
      <el-form :model="importDialog.model" label-width="120px" ref="copyForm">
        ...
        <el-form-item>
          <el-button @click="saveImportShopBalances" type="primary"
            >确定</el-button
          >
...
export default {
  ...
  data() {
    return {
      ...
      colNameList: [
        ...
        { value: 'col5', label: 'col5' },
        { value: 'col6', label: 'col6' },
        { value: 'col7', label: 'col7' },
        { value: 'col8', label: 'col8' },
        { value: 'col9', label: 'col9' },
        { value: 'col10', label: 'col10' },
        { value: 'col11', label: 'col11' },
        { value: 'col12', label: 'col12' },
        { value: 'col13', label: 'col13' },
        { value: 'col14', label: 'col14' },
        { value: 'col15', label: 'col15' },
        { value: 'col16', label: 'col16' },
        { value: 'col17', label: 'col17' },
        { value: 'col18', label: 'col18' },
        { value: 'col19', label: 'col19' },
        { value: 'col20', label: 'col20' },
        { value: 'col21', label: 'col21' },
        { value: 'col22', label: 'col22' },
        { value: 'col23', label: 'col23' },
        { value: 'col24', label: 'col24' },
        { value: 'col25', label: 'col25' },
        { value: 'col26', label: 'col26' },
        { value: 'col27', label: 'col27' },
        { value: 'col28', label: 'col28' },
        { value: 'col29', label: 'col29' },
        { value: 'col30', label: 'col30' },
        { value: 'col31', label: 'col31' },
        { value: 'col32', label: 'col32' },
        { value: 'col33', label: 'col33' },
        { value: 'col34', label: 'col34' },
        { value: 'col35', label: 'col35' },
        { value: 'col36', label: 'col36' },
        { value: 'col37', label: 'col37' },
        { value: 'col38', label: 'col38' },
        { value: 'col39', label: 'col39' },
        { value: 'col40', label: 'col40' },
        { value: 'col41', label: 'col41' },
        { value: 'col42', label: 'col42' },
        { value: 'col43', label: 'col43' },
        { value: 'col44', label: 'col44' },
        { value: 'col45', label: 'col45' },
        { value: 'col46', label: 'col46' },
        { value: 'col47', label: 'col47' },
        { value: 'col48', label: 'col48' },
        { value: 'col49', label: 'col49' },
        { value: 'col50', label: 'col50' }
      ],
      isDefaultList: [
        {
          value: '0',
  ...
  mounted() {
    ...
    this.excelConvertUrl =
      $yid.config.API.BASE + 'api-finance/dicbalance/convertSystem'
  ...
  methods: {
    getdicBalanceList() {
      ...
      this.pageInfo.limit =
        this.$refs.dicBalancetable.Pagination.internalPageSize
    ...
    queryDicBalanceList() {
      service.finance.dicBalance
        .listAll({ isDel: '0', isShow: '1' })
        .then(res => {
    ...
    dicBalanceSave() {
      this.$refs['dicBalanceForm'].validate(valid => {
        if (valid) {
          ...
          service.finance.dicBalance
            .listAllNoId({
              id: this.dicBalanceForm.id,
              isDel: '0',
              name: this.dicBalanceForm.name
            })
            .then(res => {
              if (res.resp_code == 200) {
                ...
                service.finance.dicBalance
                  .listAllNoId({
                    id: this.dicBalanceForm.id,
                    isDel: '0',
                    colName: this.dicBalanceForm.colName
                  })
                  .then(res => {
                    if (res.resp_code == 200) {
                      if (res.data.length > 0) {
                      ...
                      service.finance.dicBalance
                        .listAllNoId({
                          id: this.dicBalanceForm.id,
                          isDel: '0',
                          colOrder: this.dicBalanceForm.colOrder
                        })
                        .then(res => {
                          if (res.resp_code == 200) {
                            if (res.data.length > 0) {
    ...
    downExcelTemplate() {
      //动态生成导入模版
      download(
        yid.config.API.BASE + 'api-finance/dicbalance/excel/exportMb',
        this.searchForm
      )
    },
    saveImportShopBalances() {
      if (
        this.importDialog.shopbalances == null ||
        this.importDialog.shopbalances.length == 0
      ) {
        yid.util.error(
      ...
      service.finance.dicBalance
        .saveSystem(this.importDialog.shopbalances)
        .then(res => {
          if (res.resp_code == '200') {
            this.queryDicBalanceList()
            this.getshopBalanceList()
