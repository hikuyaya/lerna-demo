...
-->
                ...
                <div style="display: flex; justify-content: space-between; padding: 15px 0">
                    ...
                    <span>
                                        <el-checkbox v-model="billForm.cardChecked" disabled>会员卡储值金额为0时自动注销会员卡</el-checkbox>
                                        <el-tooltip class="item" effect="dark" content="勾选此选项后，退款复核成功后自动判断会员卡余额和赠送金余额为0时，注销此会员卡（判断所有门店的卡金批次）。" placement="right-start">
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            showDetail(type){
                type=='cards'?this.$parent.$parent.detailDialog.title = '卡金明细':
                    type=='packages'?this.$parent.$parent.detailDialog.title = '套餐明细':
                        type=='products'?this.$parent.$parent.detailDialog.title = '寄存记录':
                            type=='coupons'?this.$parent.$parent.detailDialog.title = '优惠券记录':
                                type=='redpacks'?this.$parent.$parent.detailDialog.title = '红包记录':''
                if(type =="cards"){
                    const res = new Map();
