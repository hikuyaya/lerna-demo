<template>
  <div>
    <el-form :model="formData" label-width="120px" ref="importForm">
      ...
      <p>1、点击链接：下载模板；</p>
      <p>
        2、下载完成并保存文件，然后打开文件并输入需要导入的数据，请不要修改模板中的列名；
      </p>
...
export default {
  ...
  methods: {
    downExcelTemplate() {
      download(
        yid.config.API.BASE +
          'api-finance/finance/global/downSysTemplate/' +
          this.excelName,
        {}
      )
    },
    ...
    beforeAvatarExcelUpload(file) {
      const isExcel =
        file.type.indexOf('sheet') >= 0 || file.type.indexOf('excel') >= 0
    ...
    cancelImp() {
      ...
      if (this.importCancel) this.importCancel()
    }
  }
}
</script>
