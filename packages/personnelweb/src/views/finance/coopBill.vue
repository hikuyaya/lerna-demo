            ...
            <div v-show="showList">
                ...
                <el-form ref="searchForm" inline :model="searchForm">
                    ...
                    <el-form-item label="项目：" label-width="100px">
                        <el-select @clear="clearFormCoop" @blur="clearFormCoop" clearable v-model.trim="searchForm.coopid" filterable
                                   :filter-method="filterCoop" placeholder="请选择" style="width: 220px;">
            ...
            <div v-show="!showList">
                ...
                <el-form ref="billForm" :model="billForm" style="margin-top: 16px" :rules="rules" label-width="100px" label-position="right">
                    ...
                    <div class="container">
                        ...
                        <div style="width: 220px;display: inline-block;">合计金额：{{billForm.totalMoney}}</div>
                    </div>
                </el-form>
                ...
                <yid-table v-show="showButton" ref="itemTable" style="margin-top: 15px;" :data="billForm.details">
                    ...
                    <yid-table-column label="项目名称" min-width="150" prop="coopname"></yid-table-column>
                    <yid-table-column label="金额" min-width="120" prop="money">
                        <template slot-scope="scope">
                            <el-input @blur="calcMoney(scope.row)" v-model="scope.row.money"></el-input>
                        </template>
                    </yid-table-column>
                    <yid-table-column label="扣减比例" min-width="80" prop="shoprate"></yid-table-column>
                    <yid-table-column label="合作方业绩扣减" min-width="130" prop="smoney"></yid-table-column>
                    <yid-table-column label="剩余业绩" min-width="80" prop="rmoney"></yid-table-column>
                    ...
                    <yid-table-column label="操作" min-width="100" fixed="right">
                        <template slot-scope="scope">
                            <el-button type="text" @click="deleteCoop(scope.$index,scope.row)" style="margin-left: 6px;color: red">删除</el-button>
                        </template>
                    </yid-table-column>
                </yid-table>
                ...
                <yid-table v-show="!showButton" ref="showTable" style="margin-top: 15px;" :data="billForm.details">
                    ...
                    <yid-table-column label="合作方业绩扣减" min-width="130" prop="smoney"></yid-table-column>
                    <yid-table-column label="剩余业绩" min-width="80" prop="rmoney"></yid-table-column>
                    <yid-table-column label="备注" min-width="120" prop="memo"></yid-table-column>
                </yid-table>
            </div>
        </el-collapse-transition>
        ...
        <yid-dialog :title="importDialog.title" :visible.sync="importDialog.visible" @close="importCancel" width="700px">
            <el-form :model="importDialog.model"  label-width="120px" ref="copyForm">
                ...
                <el-form-item label="下载导入模板：" align="left">
                    <el-link @click="downExcelTemplate" type="primary">合作方业绩扣减模板</el-link>
                </el-form-item>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.excelConvertUrl = $yid.config.API.BASE + "api-finance/coopbillhead/convertSystem";
            this.getShopList();
            this.getDicCoopList();
        },
        computed:{
        ...
        methods: {
                         ...
                         if(!yid.util.isEmpty(billcode)){
                             yid.util.confirm("您确定要审核["+billcode+"]的单据?","","",()=>{
                                 if(status == '2'){
                                     ...
                                     //调用审核单据接口
                                     service.finance.coopBill.auditBill({id:id,billcode:billcode}).then(res=>{
                                         if(res.resp_code=="200"){
            ...
            downExcelTemplate(){
                download(yid.config.API.BASE + 'api-finance/coopbillhead/downSysTemplate', {})
            },
            saveImportCoops(){
                if(this.importDialog.coops.length == 0){
                    yid.util.info("无导入数据");
                }else{
                    service.finance.coopBill.saveSystem(this.importDialog.coops).then(res =>{
                        if(res.resp_code=="200"){
            ...
            calcMoney(row){
                ...
                if(!yid.util.isEmpty(money)){
                    ...
                    let smoney = math.round(math.chain(money).multiply(Number(shoprate)).divide(100).done(),2);
                    let rmoney = math.round(math.chain(money).subtract(Number(smoney)).done(),2);
                    row.smoney = smoney
                    row.rmoney = rmoney
                    //计算合计金额
                    this.getTotalMoney();
            ...
            getTotalMoney(){
                ...
                if(this.billForm.details.length > 0){
                    this.billForm.details.forEach(d=>{
                        totalmoney = math.round(math.chain(totalmoney).add(Number(d.money)).done(),2);
                    })
                }
