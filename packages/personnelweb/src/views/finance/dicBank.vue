    ...
    <div class="dicBank">
            ...
            <div v-show="showList">
                <el-form ref="searchForm" inline :model="searchForm">
                    ...
                    <el-form-item label="开户行名称：" prop="bank">
                        <el-input clearable v-model="searchForm.bank" placeholder="名称" style="width: 200px;"></el-input>
                    </el-form-item>
                    <el-form-item label="">
                ...
                <yid-table ref="table" pagination style="width: 100%;margin-top: 10px;">
                    <yid-table-column prop="code" label="开户行编号" width="300"></yid-table-column>
                    <yid-table-column prop="bank" label="开户行名称" width="400"></yid-table-column>
        ...
        <el-collapse-transition>
            <div v-show="!showList">
                ...
                <el-form ref="bankForm" :model="bankForm" style="margin-top: 16px" :rules="rules"
                         label-width="100px" label-position="right">
                    <el-form-item label="开户行编码：" prop="code" label-width="150px">
                        <el-input v-model="bankForm.code" clearable placeholder="编码自动生成" disabled="disabled" style="width: 200px;"></el-input>
                    </el-form-item>
                    <el-form-item label="开户行名称：" prop="bank" label-width="150px" :rules="[{ required: true, message: '名称不能为空'}]">
                        <el-input v-model="bankForm.bank" placeholder="名称"
                                  @blur="checkBank" style="width: 200px;"></el-input>
                    </el-form-item>
                    <el-form-item label="状态：" prop="status" label-width="150px" :rules="[{ required: true, message: '状态不能为空'}]">
                        <el-select style="width: 200px" v-model="bankForm.status" clearable placeholder="请选择">
                            <el-option
        ...
        methods: {
            ...
            checkBank(){
                ...
                if(!yid.util.isEmpty(bank)){
                    ...
                    service.finance.dicBank.listAll({bank:bank,isDel:'0'}).then(res=>{
                        if (res.resp_code == 200) {
                            if(res.data.length > 0){
                                const bankObj = res.data[0];
                                if(bankObj.code != this.bankForm.code){
                                    this.bankForm.bank = "";
                                    yid.util.error("名称为:["+bankObj.bank+"]的开户行已存在请重新输入!");
                                }
                            }
                        }else{
                            this.bankForm.bank = "";
            ...
            deleteAction(row){
                yid.util.confirm("您确定要删除["+row.bank+"]的开户行吗?","","",()=>{
                    row.isDel = '1';
