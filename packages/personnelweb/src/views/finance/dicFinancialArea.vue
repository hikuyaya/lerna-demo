  ...
  <div class="salaryRewardDetail">
          ...
          <yid-table-column label="操作" min-width="150" prop="content">
            <template slot-scope="scope">
              ...
              <el-link
                ...
                >{{ scope.row.status == '1' ? '停用' : '恢复' }}</el-link
              >
            </template>
          </yid-table-column>
        </yid-table>
    ...
    <el-collapse-transition>
      <div v-show="!showList">
        ...
        <el-form
          ref="financialAreaForm"
          :model="financialAreaForm"
          label-width="140px">
          <el-row :gutter="20">
            <el-col :span="8">
              <el-form-item
                label="财务片区编码："
                prop="code"
                :rules="[{ required: true, message: '财务片区编码为空' }]">
              ...
              <el-form-item
                label="财务片区名称："
                prop="name"
                :rules="[{ required: true, message: '财务片区名称为空' }]">
                <el-input
                  v-model="financialAreaForm.name"
                  maxlength="32"
              ...
              <el-form-item
                ...
                <el-select
                  value-key="id"
                  placeholder="请选择"
                  v-model.trim="financialAreaForm.status">
                  <el-option label="正常" value="1"></el-option>
                  <el-option label="停用" value="2"></el-option>
                </el-select>
              </el-form-item>
      ...
      this.$refs['financialAreaForm'].validate(valid => {
        if (valid) {
          service.finance.dicFinancialArea
            .saveJob(this.financialAreaForm)
            .then(res => {
