    ...
    <div class="salaryRewardDetail">
        <el-collapse-transition>
            <div v-show="showList">
                ...
                <yid-table ref="table" pagination style="width: 100%;margin-top: 10px;">
                    <yid-table-column prop="code" label="项目编号" width="120"></yid-table-column>
                    <yid-table-column prop="name" label="项目名称" width="250"></yid-table-column>
                    <yid-table-column prop="shoprate" label="门店业绩扣减比率" width="150"></yid-table-column>
                    <yid-table-column prop="ygrate" label="员工业绩扣减比率" width="150"></yid-table-column>
        ...
        <el-collapse-transition>
            <div v-show="!showList">
                ...
                <el-form ref="cooperateForm" :model="cooperateForm" style="margin-top: 16px" :rules="rules"
                         label-width="100px" label-position="right">
                    <el-form-item label="项目编码：" prop="code" label-width="150px">
                        <el-input v-model="cooperateForm.code" placeholder="自动生成" disabled="true"
                                  style="width: 200px;"></el-input>
                    </el-form-item>
                    <el-form-item label="项目名称：" prop="name" label-width="150px" :rules="[{ required: true, message: '项目名称不能为空'}]">
                        <el-input v-model="cooperateForm.name" placeholder="项目名称"
                                  style="width: 200px;"></el-input>
                    </el-form-item>
                    <el-form-item label="门店业绩扣减比率：" prop="shoprate" label-width="150px" :rules="[{ required: true, message: '门店业绩扣减比率不能为空'}]">
                        <el-input v-model="cooperateForm.shoprate" @blur="checkShopRate()" placeholder="门店业绩扣减比率"
                                  style="width: 200px;"></el-input>（输入范围：0-100）
                    </el-form-item>
                    <el-form-item label="员工业绩扣减比率：" prop="ygrate" label-width="150px" :rules="[{ required: true, message: '员工业绩扣减比率不能为空'}]">
                        <el-input v-model="cooperateForm.ygrate" @blur="checkYgRate()" placeholder="员工业绩扣减比率"
                                  style="width: 200px;"></el-input>（输入范围：0-100）
                    </el-form-item>
                    <el-form-item label="备注：" prop="name" label-width="150px">
                        <el-input v-model="cooperateForm.memo"  type="textarea" :rows="2" style="width: 300px;resize: none"></el-input>
                    </el-form-item>
                </el-form>
            </div>
        </el-collapse-transition>
...
<script>
    ...
    export default {
        ...
        methods: {
                    ...
                    if (valid) {
                        if(!yid.util.isEmpty(this.cooperateForm.shoprate)){
                            if(!this.isNumber(this.cooperateForm.shoprate)){
                                yid.util.error("门店业绩扣减比率必须输入整数或小数点后2位之内小数");
                                return
                            }
                            if(Number(this.cooperateForm.shoprate) < 0 || Number(this.cooperateForm.shoprate) > 100){
                        ...
                        if(!yid.util.isEmpty(this.cooperateForm.ygrate)){
                            if(!this.isNumber(this.cooperateForm.ygrate)){
                                yid.util.error("员工业绩业绩扣减比率必须输入整数或小数点后2位之内小数");
                                return
                            }
                            if(Number(this.cooperateForm.ygrate) < 0 || Number(this.cooperateForm.ygrate) > 100){
            ...
            deleteAction(row){
                yid.util.confirm("您确定要删除["+row.name+"]的合作方项目?","","",()=>{
                    row.isDel = '1';
            ...
            checkShopRate(){
                ...
                if(!yid.util.isEmpty(rate)){
                    ...
                    if(Number(this.cooperateForm.shoprate) >= 100 || Number(this.cooperateForm.shoprate) <= 0){
            ...
            checkYgRate(){
                ...
                if(!yid.util.isEmpty(rate)){
                    ...
                    if(Number(this.cooperateForm.ygrate) >= 100 || Number(this.cooperateForm.ygrate) <= 0){
