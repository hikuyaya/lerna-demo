  ...
  <div class="Rq">
        ...
        <yid-table
          ...
          <yid-table-column label="修改金额" min-width="120" prop="umoney">
            <template slot-scope="scope">
              <el-input
                ...
                v-model="scope.row.umoney"></el-input>
            </template>
          </yid-table-column>
          <yid-table-column
            label="修改后金额"
          ...
          <yid-table-column label="操作" min-width="100" fixed="right">
            <template slot-scope="scope">
              <el-button
                type="text"
                @click="deleteDj(scope.$index, scope.row)"
    ...
    <yid-dialog
      ...
      <el-form :model="importDialog.model" label-width="120px" ref="copyForm">
        <p>批量导入门店日常拨款单方法</p>
        <p>1、点击链接：下载门店日常拨款单模板；</p>
        <p>2、下载完成并保存文件，然后打开文件并输入门店日常拨款单信息；</p>
...
export default {
  ...
  mounted() {
    this.excelConvertUrl =
      $yid.config.API.BASE + 'api-finance/djbillhead/convertSystem'
    this.getShopList()
    this.getDicDjList()
  },
  computed: {
  ...
  methods: {
    ...
    getDicDjList() {
      service.finance.dicDjitem.listAll({ isDel: '0', isBk: '1' }).then(res => {
        if (res.resp_code == 200) {
    ...
    showImportDialog() {
      if (yid.util.isEmpty(this.billForm.rq)) {
        yid.util.error('请选择日期再进行导入')
        return
      }
    ...
    downExcelTemplate() {
      download(
        yid.config.API.BASE + 'api-finance/djbillhead/downSysTemplaterc',
        {}
      )
    },
    saveImportDjs() {
      if (this.importDialog.djs.length == 0) {
        ...
        service.finance.djBill.saveSystem(this.importDialog.djs).then(res => {
          if (res.resp_code == '200') {
        ...
        if (!yid.util.isEmpty(smoney)) {
          ...
          service.finance.dicDjitem.listAll({ code: row.djcode }).then(res => {
            if (res.resp_code == '200') {
              ...
              if (type == '2') {
                row.rmoney = math.round(
                  math.chain(umoney).add(smoney).done(),
                  2
                )
              } else {
      ...
      if (this.billForm.details.length > 0) {
        this.billForm.details.forEach(d => {
          totalmoney = math.round(
            math.chain(totalmoney).add(Number(d.umoney)).done(),
            2
          )
        })
      }
