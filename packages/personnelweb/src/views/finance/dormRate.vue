    ...
    <div class="dormRate">
            ...
            <div v-show="showList">
                <el-form ref="searchForm" inline :model="searchForm">
                    ...
                    <el-form-item label="门店：" prop="shopid">
                        <el-select clearable v-model.trim="searchForm.shopid"  filterable :filter-method="filterShop" placeholder="请选择门店" style="width: 200px;">
                ...
                <yid-table ref="table" pagination style="width: 100%;margin-top: 10px;">
                    <yid-table-column prop="shopcode" label="门店编码" width="120px"></yid-table-column>
                    <yid-table-column prop="shopname" label="门店名称" width="250px"></yid-table-column>
                    <yid-table-column prop="rate" label="宿舍扣款比率" width="200px"></yid-table-column>
        ...
        <el-collapse-transition>
            <div v-show="!showList">
                ...
                <el-form ref="dormRateFrom" :model="dormRateFrom" style="margin-top: 16px" :rules="rules"
                         label-width="100px" label-position="right">
                    <el-form-item label="门店编码：" prop="shopcode" label-width="150px" :rules="[{ required: true, message: '门店编码不能为空'}]">
                        <el-input v-model="dormRateFrom.shopcode" clearable placeholder="门店编码"
                                  @blur="getShopByCode" style="width: 200px;"></el-input>
                    </el-form-item>
                    <el-form-item label="门店名称：" prop="shopname" label-width="150px" :rules="[{ required: true, message: '名称名称不能为空'}]">
                        <el-input v-model="dormRateFrom.shopname" placeholder="门店名称" disabled="disabled"
                                  style="width: 200px;"></el-input>
                    </el-form-item>
                    <el-form-item label="宿舍扣款比率：" prop="rate" label-width="150px" :rules="[{ required: true, message: '宿舍扣款比率不能为空'}]">
                        <el-input @blur="checkRate" v-model="dormRateFrom.rate" placeholder="宿舍扣款比率"
                                  style="width: 200px;"></el-input>
                    </el-form-item>
                </el-form>
            </div>
        ...
        <yid-dialog :title="importDialog.title" :visible.sync="importDialog.visible" @close="importCancel" width="700px">
            <el-form :model="importDialog.model"  label-width="120px" ref="copyForm">
                ...
                <el-form-item label="下载导入模板：" align="left">
                    <el-link @click="downExcelTemplate" type="primary">宿舍扣款比率模板</el-link>
                </el-form-item>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.excelConvertUrl = $yid.config.API.BASE + "api-finance/dormrate/convertSystem";
            this.getShopList();
            this.search();
        },
        methods: {
            ...
            saveAction(){
                this.$refs['dormRateFrom'].validate((valid) => {
                    if (valid) {
                        ...
                        if(Number(this.dormRateFrom.rate) >= 100 || Number(this.dormRateFrom.rate) <= 0){
            ...
            deleteAction(row){
                yid.util.confirm("您确定要删除["+row.shopname+"]店的宿舍扣款比率?","","",()=>{
                    row.isDel = '1';
            ...
            saveImportRates(){
                if(this.importDialog.rates.length == 0){
                    ...
                    service.finance.dormRate.saveSystem(this.importDialog.rates).then(res =>{
                        if(res.resp_code=="200"){
                            this.search();
                            this.importDialog.visible = false
                        }else{
