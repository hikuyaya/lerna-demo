<template>
    <div class="dormRentRecords">
        <el-collapse-transition>
            <div v-show="showList">
                <el-button v-if="isCancel" @click="cancelAuditBill" type="primary">取消审批</el-button>
                ...
                <el-form ref="searchForm" inline :model="searchForm">
                    ...
                    <el-form-item label="审核人：" prop="code" label-width="100px">
                        <el-input clearable v-model="searchForm.auditor" placeholder="审核人" style="width: 160px;"></el-input>
                    </el-form-item>
                    ...
                    <el-form-item label="单据状态：" prop="status" label-width="100px">
                        <el-select clearable style="width: 120px" v-model="searchForm.status" placeholder="请选择">
                            <el-option
                    ...
                    <el-form-item label="支付方式：" prop="paytype" label-width="100px">
                        <el-select style="width: 120px" v-model="searchForm.paytype" clearable placeholder="请选择">
                            <el-option
                    ...
                    <el-form-item label="汇总类型：" prop="sumtype" label-width="100px">
                        <el-select style="width: 120px" v-model="searchForm.sumtype" clearable placeholder="请选择">
                            <el-option
                ...
                <yid-table ref="table" pagination style="width: 100%;margin-top: 10px;" @selection-change="handleSelectionChange">
                    ...
                    <yid-table-column prop="createdBy" label="制单人" width="100"></yid-table-column>
                    ...
                    <yid-table-column prop="paytype" label="支付方式" width="120">
                        <template slot-scope="scope">
                            <span v-if='scope.row.paytype=="00"'>营业款</span>
                            <span v-if='scope.row.paytype=="03"'>预留其他</span>
                            <span v-if='scope.row.paytype=="99"'>风险金</span>
                        </template>
                    </yid-table-column>
        ...
        <el-collapse-transition>
            <div v-show="!showList">
                ...
                <el-button v-show="showButtonS" @click="cancelAuditButton" type="primary">取消审批</el-button>
                <el-button v-show="showButton" @click="openRejectDialog" type="primary">驳回</el-button>
                <el-button style="margin-left:25px;" @click="$Print('#printId');">打印</el-button>
                <div ref="printId" id="printId" cellspacing="0" cellpadding="0" border="0">
                    <el-form ref="billForm" :model="billForm" style="margin-top: 16px" :rules="rules" label-width="100px" label-position="right">
                        <div class="container">
                            <div style="width: 250px;display: inline-block;">单据编码：{{billForm.billcode}}</div>
                            <div style="width: 250px;display: inline-block;">单据状态：
                                <el-select style="width: 100px" v-model="billForm.status" disabled="disabled">
                                    <el-option
                                            v-for="item in statusList"
                                            :key="item.value"
                                            :label="item.label"
                            ...
                            <div style="display: inline-block;">审核时间：{{billForm.auditdate}}</div>
                        </div>
                        <div class="container">
                            <div style="width: 250px;display: inline-block;">门店编码：
                                {{billForm.shopcode}}
                            </div>
                            <div style="width: 250px;display: inline-block;">门店名称：{{billForm.shopname}}</div>
                        </div>
                        <div class="container" style="color: green">
                            <div style="width: 250px;display: inline-block;">房东名称：{{billForm.name}}</div>
                            <div style="width: 250px;display: inline-block;">合同开始日期：{{billForm.dlstartdateStr}}</div>
                            <div style="width: 210px;display: inline-block;">合同结束日期：{{billForm.dlenddateStr}}</div>
                        </div>
                        <div class="container" style="color: green">
                            <div style="width: 250px;display: inline-block;">收款人姓名：{{billForm.payee}}</div>
                            <div style="width: 250px;display: inline-block;">银行账号：{{billForm.bankno}}</div>
                            <div style="width: 210px;display: inline-block;">开户行：{{billForm.bank}}</div>
                        </div>
                    </el-form>
                    ...
                    <el-row type="flex" justify="start">
                        <el-col :span="12">
                            <el-row :gutter="24">
                                <el-col :span="10">
                                    <div style="width: 250px;display: inline-block;color: red;margin-top: 10px">开始年月日：
                                    ...
                                    <div style="width: 250px;display: inline-block;margin-top: 10px">缴纳天数：{{this.billForm.cdays}}</div>
                                    <div style="width: 250px;display: inline-block;margin-top: 10px;color: red">汇款方式：
                                        <el-select style="width: 150px" v-model="billForm.remittype" clearable placeholder="请选择" disabled="disabled">
                                            <el-option
                                    ...
                                    <div style="width: 250px;display: inline-block;margin-top: 10px;color: red">支付方式：
                                        <el-select style="width: 150px" v-model="billForm.paytype"  disabled="disabled" clearable placeholder="请选择">
                                            <el-option
                                                    v-for="item in paytypeList"
                                                    :key="item.value"
                                                    :label="item.label"
                                ...
                                <el-col :span="10">
                                    <div style="width: 250px;display: inline-block;color: red;margin-top: 10px">结束年月日：
                                        <el-date-picker placeholder
                                                        disabled="disabled"
                                                        style="width: 150px"
                                    ...
                                    <div style="width: 250px;display: inline-block;margin-top: 10px">缴纳月数：{{this.billForm.cmonths}}</div>
                                    <div style="width: 250px;display: inline-block;margin-top: 10px;">汇总类型：
                                        <el-select style="width: 150px" v-model="billForm.sumtype" placeholder="请选择" :disabled="!showButton" @change="changeSumType()">
                                            <el-option
                                                    v-for="item in sumtypeList"
                                                    :key="item.value"
                                                    :label="item.label"
                                    ...
                                    <div style="width: 250px;display: inline-block;margin-top: 10px;color: red">金额：
                                        <el-input type="number" v-model="billForm.money" placeholder="金额"
                                                  style="width: 150px;" disabled="disabled"></el-input>
                                    </div>
                                </el-col>
                            </el-row>
                            <el-row :gutter="24">
                                <el-col :span="20">
                                    <div style="width: 600px;display: inline-block;margin-top: 10px">
                                        说明：<el-input v-model="billForm.memo"  disabled="disabled" type="textarea" :rows="2" style="width: 500px;resize: none"></el-input>
                                    </div>
                                </el-col>
                            </el-row>
                        </el-col>
                        <el-col :span="6" :offset="1">
                            <yid-table ref="shopReserveTable" style="margin-top: 10px;" :data="this.shopReserveMoneys">
                                <yid-table-column label="款项" min-width="120" prop="reservename"></yid-table-column>
                                <yid-table-column label="余额" min-width="100" prop="money"></yid-table-column>
                            </yid-table>
                        </el-col>
                    </el-row>
                </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                printObj:{
                    id:"printId",
                    popTitle:"",
                    extraHead: '<meta http-equiv="Content-Language"content="zh-cn"/>,<style>  #printId { width: 100%;} <style>'
                },
                rejectDialog : {
                    title : '',
                    visible : false
        ...
        computed:{
            searchFormReq: function () {
                ...
                reqObj.paytype =  this.searchForm.paytype;
        ...
        methods: {
            ...
            getData(reqParams) {
                const fetch = service.finance.dormRentRecords.billListForChain
            ...
            importBill(){
                ...
                service.finance.dormRentRecords.billListForChain(params).then(res=>{
                    if(res.resp_code=="200"){
                        if(res.data.length > 0){
                            download($yid.config.API.BASE + 'api-finance/dormrentrecords/billExportForChain', this.searchFormReq)
                        }else{
            ...
            getShopReseveMoney(shopcode){
                ...
                service.finance.reserveMoney.getShopReseveMoney({shopcode:shopcode}).then(res => {
                    if (res.resp_code == 200) {
                        this.shopReserveMoneys = res.data;
                        //获取当前月的营业款数据
                        service.finance.dayshoppay.shopRcjy({shopcode:shopcode,srq:srq,erq:erq}).then(res=>{
                            if (res.resp_code == 200) {
                                this.yyk = res.data;
                                const yyklist = {reserveid:"0",reservecode:"00",reservename:"营业款",money:res.data,shopcode:shopcode};
            ...
            rejectBill(){
                if(!yid.util.isEmpty(this.billForm.billcode)){
                    ...
                    service.finance.dormRentRecords.rejectBill(this.billForm).then(res=>{
                        if(res.resp_code=="200"){
            ...
            changeSumType(){
                if(!yid.util.isEmpty(this.billForm.billcode)){
                    service.finance.dormRentRecords.changeSumType(this.billForm).then(res=>{
                        if(res.resp_code=="200"){
            ...
            editBill(row){
                ...
                this.billForm.paytype = row.paytype
            ...
            deleteBill(){
                if(!yid.util.isEmpty(this.billForm.billcode)){
                    yid.util.confirm("您确定要删除["+this.billForm.billcode+"]的单据?","","",()=>{
                        service.finance.dormRentRecords.deleteBill(this.billForm).then(res =>{
                            if(res.resp_code=="200"){
                ...
                if(!yid.util.isEmpty(billcode)){
                    yid.util.confirm("您确定要审核["+billcode+"]的单据?","","",()=>{
                        if(status == '1'){
                            ...
                            //调用审核单据接口
                            service.finance.dormRentRecords.auditBill({id:id,billcode:billcode,memo:memo}).then(res=>{
                                if(res.resp_code=="200"){
            ...
            cancelAuditBill(){
                ...
                if(billids.length > 0){
                    ...
                    yid.util.confirm("您确定要取消审批所选的单据?","","",()=>{
                        //调用取消审核单据接口
                        service.finance.dormRentRecords.cancelAuditBill({billids:billids}).then(res=>{
                            if(res.resp_code=="200"){
