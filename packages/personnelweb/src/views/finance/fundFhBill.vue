<template>
    <div class="Rq">
        <el-collapse-transition>
            <div v-show="showList">
                <el-button @click="cancleFhBilllBatch" type="primary">取消复核</el-button>
                <el-button @click="openRject(2)" type="primary">批量驳回</el-button>
                <el-button @click="auditFhBilllBatch" type="primary">批量复核</el-button>
                <el-button @click="importBill" type="primary">导出</el-button>
                <el-button @click="setLimitbillBatch" type="primary">设置支出上限设置</el-button>
                <el-button @click="selectChangeGather()" type="success">修改汇总类型</el-button>
                <el-button @click="lockrq" type="success">锁定</el-button>
                <el-button type="text" @click="dialogLogTableVisible = true">查看操作记录</el-button>
                ...
                <el-form ref="searchForm" inline :model="searchForm">
                    ...
                    <el-form-item label="审批部门：" prop="orgcode" label-width="100px">
                        <el-select clearable v-model="searchForm.orgcode" style="width: 180px">
                    ...
                    <el-form-item label="是否上传：" prop="isphoto">
                        <el-select clearable v-model="searchForm.isphoto" style="width: 180px">
                            <el-option label="是" value="Y"></el-option>
                            <el-option label="否" value="N"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="申请原因：" prop="reason" label-width="100px">
                        <el-input clearable v-model="searchForm.reason" placeholder="申请原因" style="width: 180px;"></el-input>
                    </el-form-item>
                    <br>
                    <el-form-item label="申请金额：" prop="money" label-width="100px">
                        <el-input clearable v-model="searchForm.startmoney"  style="width: 80px;"></el-input>
                        至
                        <el-input clearable v-model="searchForm.endmoney"  style="width: 80px;"></el-input>
                    </el-form-item>


                    <el-form-item label="财务片区：" prop="status" label-width="100px">
                        <el-select style="width: 200px" v-model="searchForm.areacode" multiple placeholder="请选择">
                            <el-option
                    ...
                    <el-form-item label="支出项目：" prop="status">
                        <el-select style="width: 200px" v-model="searchForm.outcode" multiple  filterable
                                   remote
                                   reserve-keyword
                                   placeholder="请输入关键词"
                ...
                <yid-table ref="table" pagination style="width: 100%;margin-top: 10px;" @selection-change="handleSelectionChange">
                    ...
                    <yid-table-column prop="shopcode" label="门店" width="100"></yid-table-column>
                    <yid-table-column prop="money" label="合计金额" width="80"></yid-table-column>
                    <yid-table-column prop="ioitemName" label="支出项目" width="100"></yid-table-column>
                    <yid-table-column prop="reason" label="申请原因" width="320"></yid-table-column>
                    ...
                    <yid-table-column prop="gathertype" label="汇总类型" width="100">
                        <template slot-scope="scope">
                            <el-select placeholder="请选择" :disabled="false"  @change="changeGathertype(scope.row)" v-model="scope.row.gathertype">
                                <el-option label="单笔" value="1"></el-option>
                                <el-option label="汇总" value="2"></el-option>
                                <el-option label="工资" value="3"></el-option>
                    ...
                    <yid-table-column prop="billcode" label="单据编码" width="135"></yid-table-column>
                    ...
                    <yid-table-column prop="auditor" label="审核人" width="100"></yid-table-column>
                    ...
                    <yid-table-column prop="checkor" label="复核人" width="100"></yid-table-column>
                    <yid-table-column prop="checkdate" label="复核时间" width="100">
                        <template slot-scope="scope">
                            {{$yid.util.formatTime(scope.row.checkdate)}}
                        </template>
                    </yid-table-column>
                    <yid-table-column prop="rejectmemo" label="驳回原因" width="100"></yid-table-column>

                    <yid-table-column label="操作" min-width="70" fixed="right">
                        <template slot-scope="scope">
                            <el-link  type="primary" @click="editBill(scope.row)">详情</el-link>
   ...
   -->
        ...
        <el-collapse-transition>
            <div v-show="!showList">
                ...
                <el-button v-show="isEdit" @click="auditBill" type="primary">复核</el-button>
                <el-button v-show="isZc" @click="setLimitbillInfo" type="primary">设置支出上限</el-button>
                <el-button v-show="isEdit" @click="openRject(1)" type="primary">驳回</el-button>
                ...
                <el-form ref="billForm" :model="billForm" style="margin-top: 16px" :rules="rules" label-width="100px" label-position="right">
                    ...
                    <div class="container">
                        <div style="width: 210px;display: inline-block;">审核状态：<el-select style="width: 100px" v-model="billForm.status" disabled="disabled">
                            <el-option
                                    v-for="item in statusList"
                                    :key="item.value"
                                    :label="item.label"
                ...
                <el-form ref="billForm"   :model="billForm"  label-width="130px">
                    <el-row :gutter="20">
                        <el-col :span="16">
                            ...
                            <el-form-item label="备用金类型：" prop="type" style="display: inline-block;" :disabled="!isEdit" :rules="[{ required: true, message: '请选择备用金类型'}]">
                                <el-select value-key="id" placeholder="请选择"  disabled  v-model.trim="billForm.type" >
                                    <el-option label="日常备用金" value="1"></el-option>
                                    <el-option label="大额备用金" value="2"></el-option>
                                </el-select>
                            </el-form-item>

                            <el-form-item label="申请金额："  width="150" style="display: inline-block;" prop="money" :disabled="!isEdit" :rules="[{ required: true, message: '申请金额为空'}]">
                                <el-input type ='number' v-model="billForm.money" disabled></el-input>
                            </el-form-item>
                            <el-form-item label="一级科目：" width="150" style="display: inline-block;" prop="ioitemPcode" :disabled="!isEdit" v-show="typeCheck" :rules="[{ required: true, message: '请选择一级科目'}]">
                                <el-select value-key="ioitemPcode" placeholder="请选择" disabled v-model="billForm.ioitemPcode" >
                                    <el-option :key="item.id" :value="item.code" :label="item.name" v-for="item in ioitemPDataAll"/>
                                </el-select>

                            </el-form-item>
                            <el-form-item label="二级科目：" width="150" style="display: inline-block;" prop="ioitemCode"  v-show="typeCheck"  :rules="[{ required: true, message: '请选择二级科目'}]">
                                <el-select value-key="ioitemCode" placeholder="请选择"    disabled v-model="billForm.ioitemCode" >
                                    <el-option :key="item.id" :value="item.code" :label="item.name" v-for="item in ioitemDataAll"/>
                                </el-select>
                            </el-form-item>
                            <el-form-item label="二级科目备注：" prop="reason"  v-show="typeCheck"  :disabled="true" >
                            ...
                            <el-form-item label="申请费所属周期：" prop="sTimeRange" :disabled="true" v-show="typeCheck" width="150">
                            ...
                            <el-form-item label="申请原因：" prop="reason" :disabled="!isEdit" :rules="[{ required: true, message: '申请原因为空'}]">
                                <el-input disabled
                                          type="textarea"
                                          :autosize="{ minRows: 2, maxRows: 4}"
                                          placeholder="请输入内容"
                            ...
                            <el-form-item label="备注：" prop="reason" v-show="!typeCheck"  :disabled="!isEdit" >
                                <el-input disabled
                                          type="textarea"
                                          :autosize="{ minRows: 2, maxRows: 4}"
                                          placeholder="请输入内容"
                            ...
                            <el-form-item label="收款账户人："  width="150" style="display: inline-block;" prop="payee" disabled v-show="typeCheck"  >
                                <el-input v-model="billForm.payee" disabled></el-input>

                            </el-form-item>
                            <el-form-item label=""  width="150"  disabled style="display: inline-block;" v-show="typeCheck">
                            <el-select value-key="landCode" placeholder="请选择"  disabled v-model="billForm.landCode">
                                <el-option :key="item.id" :value="item.code" :label="item.payee" v-for="item in accountDataAll"/>
                            </el-select>
                            </el-form-item>
                            <el-form-item label="收款银行账号："  width="150"  prop="bankno" :disabled="!isbankEdit" v-show="typeCheck" >
                                <el-input v-model="billForm.bankno" disabled></el-input>
                            </el-form-item>
                            <el-form-item label="收款开户银行："  width="150"  prop="bank" :disabled="!isbankEdit" v-show="typeCheck" >
                                <el-input v-model="billForm.bank" disabled></el-input>
                            </el-form-item>
                            <el-form-item label="收款银行支行："  width="150"  prop="branchbank" :disabled="!isbankEdit" v-show="typeCheck" >
                                <el-input v-model="billForm.branchbank" disabled></el-input>
                            </el-form-item>
                            <el-form-item label="审批部门：" prop="orgcode" disabled="true" v-show="typeCheck" >
                                <el-select value-key="id" placeholder="请选择" disabled="true" v-model.trim="billForm.orgcode" >
                            ...
                            <el-form-item label="收款人是否匹配：" prop="isMatched" :disabled="!isEdit"  v-show="typeCheck" >
                                <el-select value-key="id" placeholder="请选择"  :disabled="true"  v-model.trim="billForm.isMatched" >
                            ...
                            <el-form-item label="上传图片：" prop="photo">
                                <div v-if="billForm.fundImages && billForm.fundImages.length > 0" class="block" style="width: 360px; margin-top: 16px;">

                                    <el-image
                                            v-for="item in billForm.fundImages" :key="item"
        ...
        <yid-dialog :title="lockRqDialog.title" :visible.sync="lockRqDialog.visible" @close="lockRqCancel" @opened="queryLockrq"
                    width="900px">
            <el-row>
                <el-col>
                    <yid-table pagination   ref="tablelockRq" style="margin-top: 10px;" class="store-table"  >
                        ...
                        <yid-table-column label="解锁标志" min-width="200" prop="status">
                            <template slot-scope="scope">
                                <el-select placeholder="请选择" :disabled="true" v-model="scope.row.status">
                                    <el-option label="未锁定" value="0"></el-option>
                                    <el-option label="已锁定" value="1"></el-option>
                                </el-select>
                            </template>
                        </yid-table-column>
                        <yid-table-column label="操作人" min-width="120" prop="createdBy"></yid-table-column>
            ...
            <el-row justify="space-between" style="margin-top: 10px;">
                <el-col>
                    <el-button type="primary" @click="lockrqSave()" style="margin-left: 300px;">确定</el-button>
        ...
        <yid-dialog  title="修改汇总类型" :visible.sync="gatherVigsible" >
                ...
                <el-col>
                    <el-form :model="billForm">
                        ...
                        <el-form-item label="" label-width="10px">
                            已选中<span style="color: red">{{selectedNum}}</span>条单据，选中的单据批量修改为
            ...
            <el-row justify="space-between" style="margin-top: 10px;">
                <el-col>
                    <el-button @click="gatherVigsible = false" style="margin-left: 200px;">取 消</el-button>
                    <el-button type="primary" @click="changeGathertypeBatch()">确 定</el-button>
                </el-col>
            </el-row>
        </yid-dialog>
        <yid-dialog title="查看操作记录" :visible.sync="dialogLogTableVisible"  @opened="queryLockLog"
                    width="900px">
            <el-tabs  v-model="activeName" @tab-click="querygatherLockLog">
                ...
                <el-tab-pane label="修改汇总类型记录" name="second">
                    <el-row>
                        <el-col>
                            <yid-table pagination   ref="tablegatherRq" style="margin-top: 10px;" class="store-table"  >
                                <yid-table-column prop="gathertype" label="修改汇总类型" width="200">
                                    <template slot-scope="scope">
                                ...
                                <yid-table-column label="单据编码" min-width="120" prop="billcode"></yid-table-column>
                                <yid-table-column label="操作人" min-width="120" prop="createdBy"></yid-table-column>
                                <yid-table-column label="操作时间" min-width="120" prop="createdTime"></yid-table-column>
                            </yid-table>
                        </el-col>
    ...
    export default {
        ...
        computed:{
            searchFormReq: function () {
                ...
                if(checkTimeRange!=null && checkTimeRange.length>0){
                    ...
                    reqObj.sCreatedTimeStart =  this.getcreateDate(new Date(reqObj.sCheckStart));
                }else{
                    reqObj.sCheckStart = "";
                    reqObj.sCheckEnd = "";
                }
                if(auditTimeRange!=null && auditTimeRange.length>0&&checkTimeRange!=null && checkTimeRange.length>0){
                    reqObj.sCreatedTimeStart =  this.getcreateDate(new Date(reqObj.auditTimeStart));
                }
        ...
        methods: {
            ...
            queryFundshopData(shopcode){
                service.finance.fundFhBill.queryFundShop({shopcode:shopcode}).then(res =>{
                ...
                if(!yid.util.isEmpty(billcode)){
                    yid.util.confirm("您确定要复核["+billcode+"]的单据?","","",()=>{
                        if(status == '4'){
                            ...
                            //调用审核单据接口
                            service.finance.fundFhBill.auditFhBill({id:id,billcode:billcode}).then(res=>{
                                if(res.resp_code=="200"){
                ...
                if(!yid.util.isEmpty(billcode)){
                    yid.util.confirm("您确定要取消复核["+billcode+"]的单据?","","",()=>{
                        if(status != '4'){
                            ...
                            //调用取消复核单据接口
                            service.finance.fundFhBill.cancleFhBilllBatch({ids:id,billcodes:billcode}).then(res=>{
                                if(res.resp_code=="200"){
            ...
            changeGathertype(item){
                ...
                if(!yid.util.isEmpty(billcode)){
                    //调用单据接口
                    service.finance.fundFhBill.changeGathertype({id:id,billcode:billcode,gathertype:gathertype}).then(res=>{
                        if(res.resp_code=="200"){
            ...
            changeGathertypeBatch(){
               let gathertype =this.billForm.gathertypes;
                ...
                if(!yid.util.isEmpty(billcode)){
                    //调用单据接口
                    service.finance.fundFhBill.changeGathertypeBatch({ids:id,billcodes:billcode,gathertype:gathertype}).then(res=>{
                        if(res.resp_code=="200"){
                            yid.util.success("调整成功");
            ...
            setLimitbill(row){
                ...
                if(!yid.util.isEmpty(billcode)){
                    //调用单据接口
                    service.finance.fundFhBill.setLimitbillBatch({ids:id,billcodes:billcode}).then(res=>{
                        if(res.resp_code=="200"){
           ...
           async queryLockrq(){
            ...
            auditFhBilllBatch(){
                ...
                if(!yid.util.isEmpty(billcode)){
                    //调用审核单据接口
                    service.finance.fundFhBill.auditFhBilllBatch({ids:id,billcodes:billcode}).then(res=>{
                        if(res.resp_code=="200"){
                            yid.util.success("审核成功");
                            this.search();
                        }else{
            ...
            rejectBill(){
                ...
                //调用驳回单据接口
                service.finance.fundFhBill.rejectBill({id:id,billcode:billcode,rejectmemo:rejectmemo}).then(res=>{
                    if(res.resp_code=="200"){
            ...
            rejectBillBatch(){
                ...
                //调用批量驳回单据接口
                service.finance.fundFhBill.rejectBillBatch({ids:id,billcodes:billcode,rejectmemo:rejectmemo}).then(res=>{
                    if(res.resp_code=="200"){
                        yid.util.success("驳回成功");
                        this.showList = true;
                        this.search();
                ...
                service.finance.fundFhBill.billListForchain(params).then(res=>{
                    if(res.resp_code=="200"){
                        if(res.data.length > 0){
                            download($yid.config.API.BASE + 'api-finance/fundbillhead/expRecordsListFh', this.searchFormReq)
                        }else{
