<template>
    <div class="Rq">
        <el-collapse-transition>
            <div v-show="showList">
                <el-button @click="auditSpBilllBatch" type="primary">批量审批</el-button>
                ...
                <el-form ref="searchForm" inline :model="searchForm">
                    <el-form-item label="门店：" prop="shopcode" label-width="100px">
                        <el-input clearable v-model="searchForm.shopcode" placeholder="门店编码或门店名称"  style="width: 180px;"></el-input>
                    </el-form-item>
                    <el-form-item label="区：" prop="shopType" label-width="100px">
                        <el-select clearable v-model="searchForm.reginCode" style="width: 180px">
                    ...
                    <el-form-item label="业务类型：" prop="shopType" label-width="100px">
                        <el-select clearable v-model="searchForm.shopType" style="width: 180px">
                    ...
                    <el-form-item label="门店提交时间：" prop="sTimeRange" label-width="120px">
                        <div class="block">
                            <el-date-picker
                                    :clearable = true
                ...
                <yid-table ref="table" pagination style="width: 100%;margin-top: 10px;" @selection-change="handleSelectionChange">
                    ...
                    <yid-table-column prop="auditor" label="审批人" width="120"></yid-table-column>
                    <yid-table-column prop="auditdate" label="审批时间" width="150"></yid-table-column>
                    ...
                    <yid-table-column prop="checkor" label="复核人" width="120"></yid-table-column>
                    <yid-table-column prop="checkdate" label="复核时间" width="150"></yid-table-column>
                    ...
                    <yid-table-column prop="money" label="金额" width="100"></yid-table-column>
                    <yid-table-column prop="ioitemName" label="支出项目" width="100"></yid-table-column>
                    <yid-table-column prop="reason" label="申请原因" width="100"></yid-table-column>
                    <yid-table-column prop="rejectmemo" label="审批/驳回原因" width="100"></yid-table-column>
        ...
        <el-collapse-transition>
            <div v-show="!showList">
                ...
                <el-button v-show="showbutton" @click="openRject(1)" type="primary">驳回</el-button>
                <el-form ref="billForm" :model="billForm" style="margin-top: 16px" :rules="rules" label-width="100px" label-position="right">
                    ...
                    <div class="container">
                        <div style="width: 210px;display: inline-block;">审核状态：{{billForm.status=='2'?'已提交':(billForm.status=='3'?'已审批':(billForm.status=='9'?'已作废':(billForm.status=='4'?'已复核':(billForm.status=='5'?'已驳回':'未提交'))))}}</div>
                ...
                <el-form ref="billForm"   :model="billForm"  label-width="130px">
                    <el-row :gutter="20">
                        <el-col :span="16">
                            <el-form-item label="门店编码：" prop="shopcode"  :disabled="!isEdit">
                                {{billForm.shopcode}}
                            </el-form-item>

                            <el-form-item label="备用金类型：" prop="type" style="display: inline-block;" :disabled="!isEdit" :rules="[{ required: true, message: '请选择备用金类型'}]">
                                <el-select value-key="id" placeholder="请选择"  :disabled="!isEdit"  v-model.trim="billForm.type" >
                            ...
                            <el-form-item label="申请金额："  width="150" style="display: inline-block;" prop="money" :disabled="!isEdit" :rules="[{ required: true, message: '申请金额为空'}]">
                                <el-input type ='number' v-model="billForm.money" :disabled="!isEdit"></el-input>
                            </el-form-item>
                            <el-form-item label="一级科目：" width="150" style="display: inline-block;" prop="ioitemPcode" :disabled="!isEdit" v-show="typeCheck" :rules="[{ required: true, message: '请选择一级科目'}]">
                                <el-select value-key="ioitemPcode" placeholder="请选择"  :disabled="!isEdit" v-model="billForm.ioitemPcode" >
                            ...
                            <el-form-item label="二级科目：" width="150" style="display: inline-block;" prop="ioitemCode"  v-show="typeCheck"  :rules="[{ required: true, message: '请选择二级科目'}]">
                                <el-select value-key="ioitemCode" placeholder="请选择"   :disabled="!isEdit" v-model="billForm.ioitemCode" >
                            ...
                            <el-form-item label="收款账户人："  width="150" style="display: inline-block;" prop="payee" :disabled="!isbankEdit" v-show="typeCheck"  >
                                <el-input v-model="billForm.payee" :disabled="!isEdit"></el-input>

                            </el-form-item>

                            <el-form-item label="收款银行账号："  width="150"  prop="bankno" :disabled="!isbankEdit" v-show="typeCheck" >
                                <el-input v-model="billForm.bankno" :disabled="!isEdit"></el-input>
                            </el-form-item>
                            <el-form-item label="收款开户银行："  width="150"  prop="bank" :disabled="!isbankEdit" v-show="typeCheck">
                                <el-input v-model="billForm.bank" :disabled="!isEdit"></el-input>
                            </el-form-item>
                            <el-form-item label="收款银行支行："  width="150"  prop="branchbank" :disabled="!isbankEdit" v-show="typeCheck" >
                                <el-input v-model="billForm.branchbank" :disabled="!isEdit"></el-input>
                            </el-form-item>
    ...
    export default {
        ...
        computed:{
            searchFormReq: function () {
                ...
                if(commitRange!=null && commitRange.length>0){
                    ...
                    reqObj.sCreatedTimeStart =  this.getcreateDate(new Date(reqObj.commitStart));
                }else{
                    reqObj.commitStart = '';
                    reqObj.commitEnd = '';
        ...
        methods: {
            ...
            queryFundshopData(shopcode){
                service.finance.fundSpBill.queryFundShop({shopcode:shopcode}).then(res =>{
                ...
                if(!yid.util.isEmpty(billcode)){
                    yid.util.confirm("您确定要审核["+billcode+"]的单据?","","",()=>{
                        if(status == '3'){
                            ...
                            //调用审核单据接口
                            service.finance.fundSpBill.auditSpBill({id:id,billcode:billcode}).then(res=>{
                                if(res.resp_code=="200"){
            ...
            auditSpBilllBatch(){
                ...
                if(!yid.util.isEmpty(billcode)){
                    //调用审核单据接口
                    service.finance.fundSpBill.auditSpBilllBatch({ids:id,billcodes:billcode}).then(res=>{
                        if(res.resp_code=="200"){
                            yid.util.success("审核成功");
                            this.search();
                        }else{
            ...
            rejectBill(){
                ...
                //调用驳回单据接口
                service.finance.fundSpBill.rejectBill({id:id,billcode:billcode,rejectmemo:rejectmemo}).then(res=>{
                    if(res.resp_code=="200"){
            ...
            rejectBillBatch(){
                ...
                //调用批量驳回单据接口
                service.finance.fundSpBill.rejectBillBatch({ids:id,billcodes:billcode,rejectmemo:rejectmemo}).then(res=>{
                    if(res.resp_code=="200"){
                        yid.util.success("驳回成功");
                        this.showList = true;
                        this.search();
                ...
                service.finance.fundSpBill.billListForchainSp(params).then(res=>{
                    if(res.resp_code=="200"){
                        if(res.data.length > 0){
                            download(yid.config.API.BASE + 'api-finance/fundbillhead/expRecordsList', this.searchFormReq)
                        }else{
