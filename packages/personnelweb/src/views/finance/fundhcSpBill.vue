    ...
    <div class="Rq">
            ...
            <div v-show="showList">
                ...
                <el-form ref="searchForm" inline :model="searchForm">
                    ...
                    <el-form-item label="门店编码：" prop="shopcode" label-width="100px">
                        <el-input clearable v-model="searchForm.shopcode" placeholder="门店编码" style="width: 180px;"></el-input>
                    </el-form-item>
                    <el-form-item label="">
                ...
                <yid-table ref="table" pagination style="width: 100%;margin-top: 10px;">
                    ...
                    <yid-table-column label="操作" min-width="150" fixed="right">
                        <template slot-scope="scope">
                            <el-link type="primary" v-if="scope.row.status != '1'" @click="editBill(scope.row)">查看</el-link>
                        </template>
                    </yid-table-column>
                </yid-table>
        ...
        <el-collapse-transition>
            <div v-show="!showList">
                ...
                <el-button v-show="isEdit" @click="openRject"  style="background-color: red;color: white">驳回</el-button>
                <el-button v-show="isCancel" @click="cancelFhBill"  style="background-color: red;color: white">取消复核</el-button>
                <el-form ref="billForm" :model="billForm" style="margin-top: 16px" :rules="rules" label-width="100px" label-position="right">
                    ...
                    <div class="container">
                        <div style="width: 210px;display: inline-block;">审核状态：{{billForm.status=='2'?'已审核':(billForm.status=='3'?'已复核':(billForm.status=='9'?'已作废':'未审核'))}}</div>
                        ...
                        <div style="width: 250px;display: inline-block;">日&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;期：
                            <el-date-picker
                                    :disabled="true"
                                    :clearable = true
                                    style="width: 150px"
                ...
                <el-form ref="billForm"   :model="billForm"  label-width="140px">
                    <el-row :gutter="20">
                        <el-col :span="16">
                            ...
                            <el-form-item label="申请金额："  width="150"  prop="money"  :rules="[{ required: true, message: '申请金额为空'}]">
                                <el-input v-model="billForm.money" :disabled="true"></el-input>
                            </el-form-item>
                            <el-form-item label="">
                                本店余额为：大额备用金余额为：{{billForm.breservfund}} ; 日常备用金余额为：{{billForm.sreservfund}}
                            </el-form-item>
                            <el-form-item label="经办人" width="150" prop="eecode" :rules="[{ required: true, message: '请选择经办人'}]">
                                <el-input v-model="billForm.eename" :disabled="true"></el-input>
                            </el-form-item>
                            <el-form-item label="证明人" width="150" prop="zeecode"  :rules="[{ required: true, message: '请选择证明人'}]">
                                <el-input v-model="billForm.zeename" :disabled="true"></el-input>
                            </el-form-item>
                            <el-form-item label="备用金类型：" prop="type"   :rules="[{ required: true, message: '请选择备用金类型'}]">
                                <el-select value-key="id" placeholder="请选择"  :disabled="true"  v-model.trim="billForm.type" >
                                    <el-option label="日常备用金" value="1"></el-option>
                                    <el-option label="大额备用金" value="2"></el-option>
                                </el-select>
                            </el-form-item>
                            <el-form-item label="申请原因：" prop="reason" :rules="[{ required: true, message: '申请原因为空'}]">
    ...
    export default {
        ...
        methods: {
            ...
            queryFundshopData(shopcode){
                service.finance.fundhcSpBill.queryFundShop({shopcode:shopcode}).then(res =>{
                ...
                if(!yid.util.isEmpty(billcode)){
                    yid.util.confirm("您确定要复核["+billcode+"]的单据?","","",()=>{
                        if(status == '3'){
                            ...
                            //调用审核单据接口
                            service.finance.fundhcSpBill.auditFhBill({id:id,billcode:billcode}).then(res=>{
                                if(res.resp_code=="200"){
                ...
                if(!yid.util.isEmpty(billcode)){
                    yid.util.confirm("您确定要取消复核["+billcode+"]的单据?","","",()=>{
                        if(status == '2'){
                            ...
                            //调用取消复核单据接口
                            service.finance.fundhcSpBill.cancelFhBill({id:id,billcode:billcode}).then(res=>{
                                if(res.resp_code=="200"){
            ...
            rejectBill(){
                ...
                //调用驳回单据接口
                service.finance.fundhcSpBill.rejectBill({id:id,billcode:billcode,rejectmemo:rejectmemo}).then(res=>{
                    if(res.resp_code=="200"){
