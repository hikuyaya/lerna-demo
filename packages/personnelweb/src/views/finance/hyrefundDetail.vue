<template>
  <div>
    <el-row>
      ...
      <el-button
        v-show="isSp"
        @click="saveBillBefore"
        type="primary"
        style="margin-left: 60px"
        >审批</el-button
      >
      <el-button v-show="isSp" @click="saveBill" type="primary">保存</el-button>
      <el-button v-show="isSp" @click="openRject(1)" type="primary"
        >驳回</el-button
      >
      <el-button
        v-print="printObj"
      ...
      <el-form
        ...
        <el-row>
          ...
          <span style="margin-left: 80px"
            ><b>店号：</b>{{ billForm.shopcode }}</span
          >
          <span style="margin-left: 80px"
            ><b>会员姓名：</b>{{ billForm.memname }}</span
          >
          <span style="margin-left: 80px"
            ><b>会员手机号：</b>{{ billForm.mobile }}</span
          >
          <span style="margin-left: 80px"
            ><b>会员id：</b>{{ billForm.memid }}</span
          >
        </el-row>
        <el-row style="margin-top: 10px">
          <b>是否跨店承担退款：</b>
          <label>{{
            billForm.isKd == 0 ? '否' : billForm.isKd == 1 ? '是' : ''
          }}</label>
        </el-row>
        <el-row style="margin-top: 10px">
          <b style="font-size: 16px">退款明细：</b>
          <label style="cursor: pointer; color: #00aaee" @click="showHyDetail"
            >查看注销资产明细 +
          </label>
        </el-row>
        <div style="margin-top: 10px">
          <el-row>
            <el-col :span="8"
              >会员卡注销金额：<label style="font-size: 16px; font-weight: bold"
                >￥{{ billForm.cardMoney }}</label
              ></el-col
            >
            <el-col :span="8"
              >可用套餐注销金额：<label
                style="font-size: 16px; font-weight: bold"
                >￥{{ billForm.packageMoney }}</label
              ></el-col
            >
            <el-col :span="8"
              >寄存产品注销金额：<label
                style="font-size: 16px; font-weight: bold"
                >￥{{ billForm.productMoney }}</label
              ></el-col
            >
          </el-row>
          <el-row style="margin-top: 20px">
            <span style="font-size: 16px; font-weight: bold">
              ...
              合计实退金额：<label style="font-size: 20px; color: red"
                ><b>{{
                  billForm.totalMoney > 0
                    ? '￥' + billForm.totalMoney
                    : billForm.totalMoney
                }}</b></label
              >
        ...
        <el-row style="margin-top: 15px">
          <el-col :span="8">
            <div>
              <label style="color: red; font-weight: bold; margin-right: 5px"
                >*</label
              >退款联系人：
              <el-input
                clearable
                placeholder="姓名"
                disabled
                v-model.trim="billForm.tkname"
                style="width: 100px"></el-input>
              <el-input
                clearable
                placeholder="手机号"
                disabled
                v-model.trim="billForm.tkmobile"
            ...
            <div
              ...
              <el-select
                ...
                :disabled="!isSp"
                v-model.trim="billForm.tktype"
                @change="checkTktype">
                <el-option
                  key="'k_'+1"
                  value="1"
                  label="店内支出卡"></el-option>
                <el-option
                  key="'k_'+2"
                  value="2"
                  label="顾客银行卡"></el-option>
              ...
              <el-input
                ...
                placeholder="户名"
                :disabled="!isShopext"
                v-model.trim="billForm.payee"
                style="width: 100px; margin-right: 10px"></el-input>
              <el-input
                clearable
                placeholder="账户"
                :disabled="!isShopext"
                v-model.trim="billForm.bankno"
          ...
          <el-col :span="8">
            <div
              ...
              <el-select
                ...
                disabled
                style="width: 300px"
                v-model.trim="billForm.tkattr">
                <el-option
                  key="'tk_'+1"
                  value="1"
                  label="仅注销无退款"></el-option>
            ...
            <div style="display: flex; flex-direction: column">
              <div>
                ...
                <el-select
                  ...
                  :disabled="!isSp"
                  style="width: 200px"
                  v-model.trim="billForm.hstkattr">
                  <el-option
                    key="'tk_'+1"
                    value="1"
                    label="仅注销无退款"></el-option>
                  <el-option
                    key="'tk_'+2"
                    value="2"
                    label="员工离职"></el-option>
                  <el-option
                    key="'tk_'+3"
                    value="3"
                    label="个人原因"></el-option>
                  <el-option
                    key="'tk_'+4"
                    value="4"
                    label="技术原因"></el-option>
                  <el-option
                    key="'tk_'+5"
                    value="5"
                    label="消费原因"></el-option>
                  <el-option
                    key="'tk_'+6"
                    value="6"
                    label="服务及接待"></el-option>
...
export default {
  ...
  data() {
    return {
      billForm: {
        ...
        proChecked: false,
        totalMoney: 0, //实退金额
        status: 1, //1 未提交  2 已提交  3 已审批  4 已复核
        cardMoney: 0,
        packageMoney: 0,
        productMoney: 0,
        redpacksMoney: 0,
  ...
  methods: {
    async initData(obj) {
      //   await this.restData();
      this.billForm.hstkattr = ''
      this.billForm.hstkcontent = ''
      ...
      this.billForm.brandId = headData.brandId
      this.billForm.brandCode = headData.brandCode
      ...
      this.showButton = headData.status == '2'
      this.isSp = headData.status == '2'
      this.isShopext = headData.status == '2'
      if (this.billForm.tktype == '1') {
        this.isShopext = false
      } else {
        this.isShopext = true
      }
      this.photosrcList = []
    ...
    checkTktype() {
      if (this.billForm.tktype == '1') {
        this.billForm.payee = this.shopext.payee
        this.billForm.bank = this.shopext.bank
        this.billForm.bankno = this.shopext.bankno
        this.isShopext = false
      } else {
        this.billForm.payee = ''
        this.billForm.bank = ''
    ...
    searchShopext() {
      ...
      service.finance.shopExt
        .listAll({ shopcode: this.billForm.shopcode, isDel: '0' })
        .then(res => {
          if (res.resp_code == '200') {
            if (res.data.length > 0) {
              this.shopext = res.data[0]
            }
          }
        })
    },
    saveBill() {
      let hstkattr = this.billForm.hstkattr
      if (hstkattr == null || hstkattr == '') {
        yid.util.alert('请选择核实退款属性')
        return
      }
      let hstkcontent = this.billForm.hstkcontent
      if (hstkcontent == null || hstkcontent == '') {
        yid.util.alert('请填写核实退卡原因')
        return
      }

      service.finance.hyrefund.savesp(this.billForm).then(res => {
        if (res.resp_code == '200') {
        ...
        yid.util.confirm(
          ...
          () => {
            if (status == '1') {
              ...
              yid.util.error('已审批的单据不能进行审核')
              return false
            } else if (status == '4') {
              yid.util.error('已复核单据不能进行审核')
              return false
            } else {
              //调用审核单据接口
              service.finance.hyrefund
                .censor({ id: id, billcode: billcode })
                .then(res => {
                  if (res.resp_code == '200') {
    ...
    showDetail(type) {
      type == 'cards'
        ? (this.detailDialog.title = '卡金明细')
        : type == 'packages'
        ? (this.detailDialog.title = '套餐明细')
        : type == 'products'
        ? (this.detailDialog.title = '寄存记录')
        : type == 'coupons'
        ? (this.detailDialog.title = '优惠券记录')
        : type == 'redpacks'
        ? (this.detailDialog.title = '红包记录')
        : ''
