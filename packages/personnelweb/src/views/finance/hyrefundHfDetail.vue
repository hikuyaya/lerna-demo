  ...
  <div>
    <el-row>
      ...
      <el-button v-show="showButton" @click="auditBill" type="primary"
        >确认提交</el-button
      >
      <el-button v-show="showButton" @click="censorforM" type="primary"
        >免回访</el-button
      >
    </el-row>
    <el-form
      ...
      <el-row style="margin-top: 10px">
        ...
        <el-tooltip
          class="item"
          effect="dark"
          content="当跨店承担为否，则其他门店批次的卡金不可选中退款"
      ...
      <div style="position: relative">
        <el-tabs v-model="activetab">
          <el-tab-pane
            :label="'会员储值卡' + billForm.cards.length"
            name="first">
            <div>
              <yid-table
                :data="billForm.cards"
          ...
          <el-tab-pane
            :label="'可用套餐' + billForm.packages.length"
            name="second">
            <div>
              <yid-table
                :data="billForm.packages"
                ref="hypackages"
                :row-class-name="rowClass"
                height="300">
                <yid-table-column
          ...
          <el-tab-pane
            :label="'寄存产品' + billForm.products.length"
            name="third">
            <div>
              <yid-table
                :data="billForm.products"
                ref="hyproducts"
                :row-class-name="rowClass"
                height="300">
                <yid-table-column
      ...
      <div>
        <el-row>
          ...
          >
          <el-col :span="5"
            >可用套餐注销金额：<label style="font-size: 16px; font-weight: bold"
              >￥{{ billForm.packageMoney }}</label
            ></el-col
          >
          <el-col :span="5"
            >寄存产品注销金额：<label style="font-size: 16px; font-weight: bold"
              >￥{{ billForm.productMoney }}</label
            ></el-col
          >
          <el-col :span="5"
            >注销优惠券数：<label style="font-size: 16px; font-weight: bold">{{
              billForm.couponnum
            }}</label></el-col
          >
          <el-col :span="4"
            >注销红包金额：<label style="font-size: 16px; font-weight: bold"
              >￥{{ billForm.redpacksMoney }}</label
            ></el-col
          >
        </el-row>
        <el-row style="margin-top: 20px">
          <span style="font-size: 16px; font-weight: bold">
            ...
            <el-input
              ...
              type="number"
              disabled
              v-model.trim="billForm.totalMoney"
            ...
            <div>
              <yid-table
                ...
                <yid-table-column
                  label="核实原因"
                  min-width="140"
                  prop="hstkcontent"></yid-table-column>
                <yid-table-column
                  label="备注"
                  min-width="140"
      ...
      <el-row v-if="activetab == 'first'">
        <el-form ref="record" label-width="100px" label-position="right">
          ...
          <el-select
            ...
            placeholder="请选择"
            style="width: 300px"
            v-model.trim="formCard.cardid"
            @change="changeCardinfo">
            <el-option
              v-for="(card, cardindex) in cardLists"
              :key="card.cardid"
              :value="card.cardid"
          ...
          <el-col :span="16">
            ...
            <yid-table :data="rechanges" height="300">
              ...
              <yid-table-column
                label="充值时间"
                min-width="120"
                prop="updateTime"></yid-table-column>
              <yid-table-column
                label="充值金额"
                min-width="70"
                prop="money"></yid-table-column>
      ...
      <el-row v-if="activetab == 'second'">
        <el-row>
          ...
          <el-select
            ...
            placeholder="请选择"
            style="width: 150px"
            v-model.trim="formCard.shopcode"
            @change="changCard">
            <el-option
              v-for="shop in shopLists"
              :key="shop.shopcode"
              :value="shop.shopcode"
              :label="shop.shopname"></el-option>
          </el-select>
          ...
          <el-select
            ...
            placeholder="请选择"
            style="width: 150px"
            v-model.trim="formCard.lessNum"
            @change="changCard">
            <el-option
              v-for="less in lessType"
              :key="less.id"
              :value="less.id"
              :label="less.value"></el-option>
          </el-select>
          <el-button
        ...
        <yid-table
          :data="packages"
          pagination
          style="margin-top: 20px; min-height: 443px">
          <yid-table-column
            label="门店编码"
            min-width="60"
            prop="shopcode"></yid-table-column>
...
export default {
  ...
  methods: {
    ...
    rowClass(scope) {
      if (
        this.billForm.isKd == 0 &&
        scope.row.shopcode != this.userInfo.shopcode
      ) {
    ...
    searchRechages() {
      ...
      service.finance.hyrefund
        .queryCardRecords({ cardid: this.formCard.cardid, sdate, edate })
        .then(res => {
      ...
      if (!yid.util.isEmpty(billcode)) {
        yid.util.confirm(
          ...
          () => {
            ...
            //调用审核单据接口
            service.finance.hyrefundhf
              .censor({ id: id, billcode: billcode, status: '3' })
              .then(res => {
                if (res.resp_code == '200') {
