      ...
      <div v-if="showList">
        ...
        <el-form
          ...
          <el-form-item label="会员：">
            <el-select
              ...
              placeholder="请选择"
              style="width: 110px"
              v-model.trim="searchForm.memType">
              <el-option
            ...
            <el-input
              clearable
              v-model.trim="searchForm.cmcode"
              placeholder="卡号"
              style="width: 150px"></el-input>
          </el-form-item>
          <el-form-item label="项目分类：">
            <el-select
              ...
              placeholder="请选择"
              style="width: 160px"
              @change="clearFormShop1"
              v-model.trim="searchForm.serveType">
              <el-option
          ...
          <el-form-item label="是否关店：" prop="isClose" label-width="100px">
            <el-select
              clearable
              filterable
              placeholder="请选择"
              style="width: 120px"
              v-model.trim="searchForm.isClose">
              <el-option
                v-for="item in isCloselist"
                :key="item.value"
                :label="item.label"
                :value="item.value">
              </el-option>
          ...
          <el-form-item label="所属公司：" prop="ftype" label-width="100px">
            ...
            <ftype-select :code.sync="searchForm.ftype"></ftype-select>
          </el-form-item>
          <el-form-item>
            <el-button @click="queryYyBilllist()" type="primary"
              >查询</el-button
            >
          </el-form-item>
        </el-form>
        <yid-table pagination ref="table" style="width: 100%">
          ...
          <yid-table-column
            label="驳回时间"
            min-width="150"
            prop="rejectTime"></yid-table-column>
          <yid-table-column
            label="驳回人"
            min-width="80"
            prop="rejectBy"></yid-table-column>
    ...
    <el-collapse-transition>
      <div v-if="!showList">
        <hyrefundOlineDetailfh
          ref="detailPage"
          @onClose="closeDetailPage"></hyrefundOlineDetailfh>
      </div>
    </el-collapse-transition>
    <!--变更记录-->
...
export default {
  ...
  computed: {
    searchFormReq: function () {
      ...
      reqObj.isClose = this.searchForm.isClose
  ...
  methods: {
    getData(reqParams) {
      const fetch = service.finance.xxtkRecords.billListfh
    ...
    changeLog(billcode) {
      ...
      service.finance.xxtkOperlog
        .listAll({ billcode: billcode, isDel: '0' })
        .then(res => {
    ...
    importBill() {
      ...
      service.finance.xxtkRecords.billListfh(params).then(res => {
        if (res.resp_code == '200') {
          if (res.data.length > 0) {
            download(
              $yid.config.API.BASE +
                'api-finance/xxtkrecords/billExportForChainfh',
              this.searchFormReq
            )
          } else {
