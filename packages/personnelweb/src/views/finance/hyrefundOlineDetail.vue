  ...
  <div class="hyrefundOlineDetail">
    <el-row>
      ...
      <el-button
        ...
        >驳回</el-button
      >
      <el-button @click="auditBill" v-show="!this.showButton" type="primary"
        >审核</el-button
      >
      <el-button v-print="printObj" v-show="true" @click="handleprint(printObj)"
        >打印</el-button
      >
    </el-row>
    <div ref="printId" id="printId" cellspacing="0" cellpadding="0" border="0">
      <el-form
        ...
        <el-row>
          <span
            ><b>单据编码：</b
            >{{ billForm.billcode ? billForm.billcode : '自动生成' }}</span
          >
        ...
        <el-row style="margin: 15px 0">
          ...
          <span style="margin-left: 80px"
            ><b>会员手机号：</b>{{ billForm.memTel }}</span
          >
          <span style="margin-left: 80px"
        ...
        <el-row>
          <el-col :span="8">
            <div class="item">
              ...
              <el-select
                ...
                placeholder="请选择"
                @change="showBillcode"
                v-show="billForm.shopType == 1"
                :disabled="true"
                style="width: 150px"
                v-model.trim="billForm.serveTypeMf">
                <el-option
              ...
              <el-select
                ...
                placeholder="请选择"
                @change="showBillcode"
                v-show="billForm.shopType == 2"
                :disabled="true"
                style="width: 150px"
                v-model.trim="billForm.serveTypeMr">
                <el-option
              ...
              <el-input
                clearable
                placeholder="请输入订单号"
                v-show="showtkbillcode"
                :disabled="true"
                v-model.trim="billForm.tkBillcode"
            ...
            <div>
              <span class="labtitle" style="margin-bottom: 15px"
                ><label>*</label>打款方式：</span
              >
              <el-select
                filterable
                placeholder="请选择"
                @change="getTkInfo"
                style="width: 120px; margin-right: 10px; margin-bottom: 15px"
                :disabled="this.showPayType"
                v-model.trim="billForm.payType">
                <el-option
          ...
          <el-col :span="8">
            <div class="item">
              <span class="labtitle"><label></label>卡号：</span>
              <el-input
                clearable
                placeholder="卡号"
                :disabled="true"
                v-model.trim="billForm.cmcode"
                style="width: 150px"></el-input>
            </div>
            <div class="item">
          ...
          <el-col :span="8">
            <div class="item">
              <span class="labtitle"><label>*</label>实退金额：</span>
              <el-input
                clearable
                placeholder=""
                @blur="checkTkmoney"
                :disabled="true"
                v-model.trim="billForm.tkMoney"
                ...
                <el-upload
                  ...
                  <div style="width: 120px; height: 20px; position: relative">
                    ...
                    <i
                      ...
                      v-show="billForm.photo"
                      @click.stop="handlePictureCardPreview(billForm.photo)"
                      style="
                        position: absolute;
                        left: 50px;
                        top: 50px;
  ...
  methods: {
    async initData(obj) {
      ...
      if (this.billForm.status == '2') {
        ...
        if (this.billForm.serveTypeMr == '3') {
          //微慕
    ...
    saveBill() {
      ...
      if (this.billForm.memType == '1') {
        if (
          yid.util.isEmpty(this.billForm.memId) ||
          yid.util.isEmpty(this.billForm.memName) ||
          yid.util.isEmpty(this.billForm.memTel)
        ) {
      ...
      if (
        yid.util.isEmpty(this.billForm.tkName) ||
        yid.util.isEmpty(this.billForm.tkMobile)
      ) {
      ...
      if (this.billForm.payType == '1' || this.billForm.payType == '2') {
        if (
          yid.util.isEmpty(this.billForm.payee) ||
          yid.util.isEmpty(this.billForm.bank) ||
          yid.util.isEmpty(this.billForm.bankno)
        ) {
          yid.util.warning(
            '打款方式为店内支出卡或者顾客银行卡时,对应的银行账户信息不能为空!'
          )
          return
        }
            ...
            yid.util.confirm(
              ...
              () => {
                if (status == '1') {
                  ...
                  //调用审核单据接口
                  service.finance.xxtkRecords
                    .auditBillsp({ id: id, billcode: billcode })
                    .then(res => {
                      if (res.resp_code == '200') {
