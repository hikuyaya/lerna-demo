<template>
  <div class="hyrefundOlineDetailfh">
    <el-row>
  ...
  methods: {
    ...
    rejectBillsp() {
      if (!yid.util.isEmpty(this.billForm.billcode)) {
        ...
        service.finance.xxtkRecords.rejectBillfh(this.billForm).then(res => {
    ...
    auditBill() {
      ...
      if (!yid.util.isEmpty(billcode)) {
        yid.util.confirm(
          ...
          () => {
            if (status == '1') {
              yid.util.error('未提交的单据不能再次进行复核')
              return
            } else if (status == '2') {
              yid.util.error('已提交的单据不能再次进行复核')
              return
            } else if (status == '4') {
              yid.util.error('已复核的单据不能再次进行复核')
              return
            } else if (status == '9') {
              yid.util.error('已作废的单据不能再次进行复核')
              return
            } else if (status == '0') {
              yid.util.error('已驳回的单据不能再次进行复核')
              return
            } else {
              //调用审核单据接口
