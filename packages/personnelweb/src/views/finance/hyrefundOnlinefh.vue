    ...
    <div class="hyrefund">
        <el-collapse-transition>
            ...
            <div v-if="showList">
                ...
                <el-button @click="fhBillZeor" type="primary">自动复核零元退卡单</el-button>
                <el-form inline style="margin-top: 20px">
                    ...
                    <el-form-item label="会员：">
                        <el-select clearable filterable placeholder="请选择" style="width: 110px;" v-model.trim="searchForm.type">
                            <el-option
                                    key="01"
                                    label="会员姓名"
                                    value="01">
                        ...
                        <el-input clearable v-model.trim="searchForm.hyinfo" placeholder=""  style="width: 150px;"></el-input>
                    </el-form-item>
                    ...
                    <el-form-item label="退款属性：">
                        <el-select clearable filterable placeholder="请选择" style="width: 160px;" v-model.trim="searchForm.tkattr">
                            <el-option
                    ...
                    <el-form-item label="退款方式：">
                        <el-select clearable filterable placeholder="请选择" style="width: 160px;" v-model.trim="searchForm.tkway">
                            <el-option :key="'c_'+item.id" v-for="item in tkLists" :value="item.id" :label="item.label"></el-option>
                        </el-select>
                    </el-form-item>
                    ...
                    <el-form-item label="实退金额为0：">
                        <el-select clearable filterable placeholder="请选择" style="width: 160px;" v-model.trim="searchForm.isZero">
                            <el-option key="'c_'+1" value="1" label="是"></el-option>
                            <el-option key="'c_'+0" value="0" label="否"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="有无备注：">
                        <el-select clearable filterable placeholder="请选择" style="width: 160px;" v-model.trim="searchForm.isMemo">
                    ...
                    <el-form-item label="跨店承担：">
                        <el-select clearable filterable placeholder="请选择" style="width: 160px;" v-model.trim="searchForm.isKd">
                            <el-option key="'c_'+1" value="1" label="是"></el-option>
                            <el-option key="'c_'+0" value="0" label="否"></el-option>
                        </el-select>
                    </el-form-item>
                    ...
                    <el-form-item label="审批人：">
                        <el-input clearable v-model.trim="searchForm.auditor" placeholder=""  style="width: 150px;"></el-input>
                    </el-form-item>
                    ...
                    <el-form-item label="制单人：">
                        <el-input  v-model.trim="searchForm.createdBy" placeholder=""  style="width: 150px;"></el-input>
                    </el-form-item>
                    ...
                    <el-form-item label="所属公司：" prop="ftype" label-width="100px">
                        <el-select clearable filterable placeholder="请选择" style="width: 120px;" v-model.trim="searchForm.ftype">
                            <el-option
                    ...
                    <el-form-item label="是否关店：" prop="isClosed" label-width="100px">
                        <el-select clearable v-model="searchForm.isClosed" style="width: 150px">
                ...
                <yid-table pagination ref="table"  :data="tableData" width="2200">
                    <yid-table-column label="单据编码" min-width="140" prop="billcode"></yid-table-column>
                    <yid-table-column label="店号" min-width="80" prop="shopcode"></yid-table-column>
                    <yid-table-column label="店号" min-width="80" prop="shopname"></yid-table-column>
                    <yid-table-column label="制单人" min-width="80" prop="createdBy"></yid-table-column>
                    <yid-table-column label="制单时间" min-width="150" prop="createdTime"></yid-table-column>
                    <yid-table-column label="联系人姓名" min-width="120" prop="tkname"></yid-table-column>
                    <yid-table-column label="手机号码" min-width="140" prop="tkmobile"></yid-table-column>
                    <yid-table-column label="合计注销金额" min-width="120" >
                        <template slot-scope="scope">
                            {{scope.row.cardMoney+scope.row.packageMoney+scope.row.productMoney}}
                        </template>
                    </yid-table-column>
                    <yid-table-column label="合计实退金额" min-width="120" prop="totalMoney"></yid-table-column>
                    <yid-table-column label="审批人" min-width="80" prop="auditor"></yid-table-column>
                    <yid-table-column label="审批时间" min-width="150" prop="auditdate"></yid-table-column>
                    <yid-table-column label="审批状态" min-width="120" prop="status">
                        <template slot-scope="scope">
                            {{scope.row.status==1?'未提交':scope.row.status==2?'已提交':scope.row.status==3?'已审批':
                            scope.row.status==4?'已复核':scope.row.status==5?'已驳回':scope.row.status==9?'已作废':''}}
                        </template>
                    </yid-table-column>
                    <yid-table-column label="备注" min-width="150" prop="hsmemo"></yid-table-column>
        ...
        <el-collapse-transition>
            <div v-if="!showList"><hyrefundDetailfh  ref="detailPage" :visible.sync="showList"></hyrefundDetailfh></div>
        </el-collapse-transition>
...
<script>
    ...
    import hyrefundDetailfh from './hyrefundDetailfh'
    ...
    export default {
        name: "hyrefund",
        components: {hyrefundDetailfh,cardtkbilllogs},
        data() {
            return {
        ...
        computed: {
            ...
            searchFormReq: function () {
                ...
                if(auditTimeRange != null && auditTimeRange.length>0){
                    reqObj.sCheckStart = this.searchForm.updatedTimeRange[0];
                    reqObj.sCheckEnd = this.searchForm.updatedTimeRange[1];
                ...
                reqObj.ftype  = this.searchForm.ftype;
        ...
        methods:{
            ...
            changeLog(billcode){
                ...
                service.finance.hyrefund.findstatuslovVOs({billcode:billcode,isDel:'0'}).then(res=>{
                    this.$refs.cardtkbilllogs.billlogs = res.data
                })
            },
            //调用0元审核单据接口
            fhBillZeor(){
                service.finance.hyrefund.fhBillZeor().then(res=>{
                ...
                service.finance.hyrefund.list(params).then(res => {
                    if (res.resp_code == "200") {
                        if (res.data.length > 0) {
                            download($yid.config.API.BASE + "api-finance/cardtkhead/billExportForChainfh", this.searchFormReq);
                        } else {
