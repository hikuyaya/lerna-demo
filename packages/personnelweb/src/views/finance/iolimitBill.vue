  ...
  <div class="reserveBill">
      ...
      <div v-show="showList">
        ...
        <el-form ref="searchForm" inline :model="searchForm">
          ...
          <el-form-item label="项目名称：" label-width="100px">
            <el-select
              @clear="clearFormReserve"
              clearable
              v-model.trim="searchForm.ioid"
              filterable
              :filter-method="filterReserve"
              placeholder="请选择"
              style="width: 220px">
              <el-option
                :key="item.id"
    ...
    <el-collapse-transition>
      <div v-show="!showList">
        ...
        <el-button v-show="showButton" @click="showLoginShop" type="primary"
          >选择门店</el-button
        >
        <el-button
          v-show="showButton"
        ...
        <el-button v-show="showButton" @click="saveBill" type="primary"
          >保存</el-button
        >
        <el-button v-show="showButton" @click="saveBillBefore" type="primary"
          >审核</el-button
        >
        <el-button v-show="showButton" @click="deleteBill" type="primary"
          >删除</el-button
        >
        <el-form
          ref="billForm"
          ...
          <div class="container">
            ...
            <div style="width: 250px; display: inline-block">
              ...
              <el-select
                ...
                style="width: 100px">
                <el-option label="期初设置" value="1"></el-option>
                <el-option label="后期调整" value="2"></el-option> </el-select
              >&nbsp;&nbsp;<font color="red">*</font>
            </div>
          </div>
          <div class="container">
        ...
        <yid-table
          ...
          <yid-table-column
            label="限制金额"
            min-width="100"
            prop="yrmoney"></yid-table-column>
          <yid-table-column label="调整金额" min-width="120" prop="umoney">
            <template slot-scope="scope">
              <el-input
                v-model="scope.row.umoney"
                @blur="calcMoney(scope.row)"></el-input>
            </template>
          </yid-table-column>
          <yid-table-column
            label="修改后金额"
          ...
          <yid-table-column label="操作" min-width="100" fixed="right">
            <template slot-scope="scope">
              <el-button
                type="text"
                @click="deleteReserve(scope.$index, scope.row)"
      ...
      <el-row type="flex" justify="space-between">
        <el-form inline>
          ...
          <el-form-item label="支出项目名称：">
            <el-select clearable v-model="shopDialog.iocode">
              <el-option
                ...
                :label="item.name"
                :value="item.code"
                v-for="item in ioitemDataAll"></el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="财务片区：">
            <el-select
              clearable
              v-model="shopDialog.facode"
              style="width: 100px">
              <el-option
                :key="item.id"
                :label="item.name"
                :value="item.code"
                v-for="item in financeAreaList"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="">
...
export default {
  ...
  mounted() {
    this.excelConvertUrl =
      $yid.config.API.BASE + 'api-finance/iolimitbillhead/convertSystem'
    this.getShopList()
  ...
  methods: {
    ...
    queryShop() {
      if (
        yid.util.isEmpty(this.shopDialog.iocode) &&
        yid.util.isEmpty(this.shopDialog.shopcode)
      ) {
    ...
    downExcelTemplate() {
      download(
        yid.config.API.BASE + 'api-finance/iolimitbillhead/downSysTemplate',
        {}
      )
    },
    saveImportIolimit() {
