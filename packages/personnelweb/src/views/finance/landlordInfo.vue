    ...
    <div class="landlordInfo">
            ...
            <div v-show="showList">
                <el-form ref="searchForm" inline :model="searchForm">
                    ...
                    <el-form-item label="房东姓名：" prop="code">
                        <el-input clearable v-model="searchForm.name" placeholder="名称" style="width: 150px;"></el-input>
                    </el-form-item>
                    <el-form-item label="房东电话：" prop="tel">
                        <el-input clearable v-model="searchForm.tel" placeholder="电话" style="width: 150px;"></el-input>
                    </el-form-item>
                ...
                <yid-table ref="table" pagination style="width: 100%;margin-top: 10px;">
                    <yid-table-column prop="code" label="房东编号" width="150"></yid-table-column>
                    <yid-table-column prop="name" label="房东名称" width="150"></yid-table-column>
                    <yid-table-column prop="bank" label="开户银行" width="200"></yid-table-column>
                    <yid-table-column prop="bankno" label="银行帐号" width="200"></yid-table-column>
                    <yid-table-column prop="payee" label="开户人姓名" width="150"></yid-table-column>
        ...
        <el-collapse-transition>
            <div v-show="!showList">
                ...
                <el-form ref="landlordInfoFom" :model="landlordInfoFom" style="margin-top: 16px" :rules="rules"
                         label-width="100px" label-position="right">
                    <el-form-item label="房东编码：" prop="code" label-width="150px" :rules="[{ required: true, message: '编码不能为空'}]">
                        <el-input v-model="landlordInfoFom.code" clearable placeholder="编码" :disabled="!editCode"
                                  @blur="checkCode" style="width: 200px;"></el-input>
                    </el-form-item>
                    <el-form-item label="房东姓名：" prop="name" label-width="150px" :rules="[{ required: true, message: '房东姓名不能为空'}]">
                        <el-input v-model="landlordInfoFom.name" placeholder="名称"
                                  style="width: 200px;"></el-input>
                    </el-form-item>
                    <el-form-item label="房东电话：" prop="tel" label-width="150px">
                        <el-input v-model="landlordInfoFom.tel" placeholder="电话" @blur="checkTel"
                                  style="width: 200px;"></el-input>
                    </el-form-item>
                    <el-form-item label="开户银行：" prop="bank" label-width="150px" :rules="[{ required: true, message: '开户银行不能为空'}]">
                        <el-select style="width: 200px" v-model="landlordInfoFom.bank" placeholder="请选择">
                            <el-option
                    ...
                    <el-form-item label="银行帐号：" prop="bankno" label-width="150px" :rules="[{ required: true, message: '银行帐号不能为空'}]">
                        <el-input v-model="landlordInfoFom.bankno" placeholder="银行帐号"
                                  @blur="checkBankno" oninput="value=value.replace(/[^0-9-]/g,'')" style="width: 200px;"></el-input>
                    </el-form-item>
                    <el-form-item label="收款人姓名：" prop="payee" label-width="150px" :rules="[{ required: true, message: '收款人姓名不能为空'}]">
                        <el-input v-model="landlordInfoFom.payee" placeholder="收款人姓名"
                                  style="width: 200px;"></el-input>
                    </el-form-item>
                    <el-form-item label="是否过期：" prop="status" label-width="150px" :rules="[{ required: true, message: '是否过期不能为空'}]">
                        <el-select style="width: 100px" v-model="landlordInfoFom.status" placeholder="请选择">
                            <el-option
                                    v-for="item in statusList"
                                    :key="item.value"
                                    :label="item.label"
                    ...
                    <el-form-item label="备注：" prop="name" label-width="150px">
                        <el-input v-model="landlordInfoFom.memo"  type="textarea" :rows="2" style="width: 300px;resize: none"></el-input>
                    </el-form-item>
                </el-form>
            </div>
        </el-collapse-transition>
        ...
        <yid-dialog :title="importDialog.title" :visible.sync="importDialog.visible" @close="importCancel" width="700px">
            <el-form :model="importDialog.model"  label-width="120px" ref="copyForm">
                ...
                <el-form-item label="下载导入模板：" align="left">
                    <el-link @click="downExcelTemplate" type="primary">门面房东资料模板</el-link>
                </el-form-item>
                ...
                <el-form-item>
                    <el-button @click="saveImportLandlords" type="primary">确定</el-button>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.excelConvertUrl = $yid.config.API.BASE + "api-finance/landlordinfo/convertSystem";
            this.getBankList();
            this.search();
        },
        methods: {
            ...
            checkCode(){
                ...
                if(!yid.util.isEmpty(code)){
                    ...
                    service.finance.landlordInfo.listAll({checkcode:code,isDel:"0"}).then(res=>{
                        if (res.resp_code == 200) {
            ...
            checkBankno(){
                ...
                if(!yid.util.isEmpty(bankno)){
                    ...
                    service.finance.landlordInfo.checkBankno({id:this.landlordInfoFom.id,bankno:bankno}).then(res=>{
                        if (res.resp_code == 200) {
                            if(res.data.length > 0){
                                this.landlordInfoFom.bankno = "";
                                yid.util.error("银行帐号为:["+bankno+"]的房东已存在请重新输入!");
                            }
                        }else{
                            this.landlordInfoFom.bankno = "";
                            yid.util.error(res.resp_msg);
            ...
            downExcelTemplate(){
                download(yid.config.API.BASE + 'api-finance/landlordinfo/downSysTemplate', {})
            },
            saveImportLandlords(){
                if(this.importDialog.landlords.length == 0){
                    yid.util.info("无导入数据");
                }else{
                    service.finance.landlordInfo.saveSystem(this.importDialog.landlords).then(res =>{
                        if(res.resp_code=="200"){
