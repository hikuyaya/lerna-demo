            ...
            <div v-show="showList">
                <el-form ref="searchForm" inline :model="searchForm">
                    ...
                    <el-form-item label="审核人：" prop="code" label-width="100px">
                        <el-input clearable v-model="searchForm.auditor" placeholder="审核人" style="width: 120px;"></el-input>
                    </el-form-item>
        ...
        <el-collapse-transition>
            <div v-show="!showList">
                ...
                <el-form ref="billForm" :model="billForm" style="margin-top: 16px" :rules="rules" label-width="100px" label-position="right">
                    <div class="container">
                        <div style="width: 220px;display: inline-block;">单据编码：{{billForm.billcode}}</div>
                        <div style="width: 200px;display: inline-block;">单据状态：
                        ...
                        <div style="width: 150px;display: inline-block;">审核人：{{billForm.auditor}}</div>
                        <div style="width: 250px;display: inline-block;">审核时间：{{billForm.auditdate}}</div>
                        <div style="width: 150px;display: inline-block;">合计金额：{{billForm.money}}</div>
                        <div style="display: inline-block;">门店编码：{{billForm.shopcode}}</div>
                    </div>
                </el-form>
                    ...
                    <el-col :span="13">
                        <yid-table ref="showTable" style="margin-top: 10px;" :data="billForm.details">
                            <yid-table-column label="转出类型" min-width="150" prop="ormcode">
                                <template slot-scope="scope">
                                    <el-select v-model.trim="scope.row.ormcode" placeholder="请选择" style="width: 150px;" disabled="disabled">
                            ...
                            <yid-table-column label="转入类型" min-width="150" prop="nrmcode">
                                <template slot-scope="scope">
                                    <el-select v-model.trim="scope.row.nrmcode" placeholder="请选择" style="width: 150px;" disabled="disabled">
                                        <el-option :key="item.id" :label="item.name" :value="item.code" v-for="item in reserveList"></el-option>
                                    </el-select>
                                </template>
                            </yid-table-column>
    ...
    export default {
        ...
        methods: {
            ...
            getShopReseveMoney(code){
                ...
                service.finance.reserveMoney.getShopReseveMoney({shopcode:code}).then(res => {
                    if (res.resp_code == 200) {
               ...
               if(!yid.util.isEmpty(billcode)){
                   yid.util.confirm("您确定要审核["+billcode+"]的单据?","","",()=>{
                       if(status == '1'){
                           ...
                           //调用审核单据接口
                           service.finance.reserveAdjBill.auditBillsp({id:id,billcode:billcode}).then(res=>{
                               if(res.resp_code=="200"){
            ...
            rejectBill(){
                if(!yid.util.isEmpty(this.billForm.billcode)){
                    ...
                    if(this.billForm.status == '1'){
                        ...
                        return;
                    }else if(this.billForm.status == '3'){
