        ...
        <el-form ref="searchForm" inline :model="searchForm">
          ...
          <el-form-item label="预留款类型：" label-width="100px">
            <el-select
              ...
              @blur="clearFormReserve"
              clearable
              v-model.trim="searchForm.reserveid"
              filterable
      ...
      <div v-show="!showList">
        ...
        <yid-table
          ...
          <yid-table-column label="预留款类型" min-width="180" prop="reserveid">
            <template slot-scope="scope">
              <el-select
                v-model.trim="scope.row.reservecode"
                placeholder="请选择"
                style="width: 150px"
                @change="getReseveJe(scope.row)">
                <el-option
                  :key="item.id"
                  :label="item.name"
                  :value="item.code"
                  v-for="item in allReservList"></el-option>
              </el-select>
            </template>
          </yid-table-column>
          <yid-table-column
            label="原金额"
            min-width="80"
            prop="je"></yid-table-column>
          <yid-table-column label="调整金额" min-width="120" prop="money">
            <template slot-scope="scope">
              <el-input
                v-model="scope.row.money"
                @blur="calcBalance(scope.row)"></el-input>
            </template>
          </yid-table-column>
          <yid-table-column
            label="修改后金额"
...
export default {
  ...
  mounted() {
    this.excelConvertUrl =
      $yid.config.API.BASE + 'api-finance/reservebillhead/convertSystem'
    this.getShopList()
  ...
  methods: {
    ...
    queryShop() {
      if (
        yid.util.isEmpty(this.shopDialog.type) &&
        yid.util.isEmpty(this.shopDialog.shop) &&
        yid.util.isEmpty(this.shopDialog.brand)
      ) {
    ...
    downExcelTemplate() {
      download(
        yid.config.API.BASE + 'api-finance/reservebillhead/downSysTemplate',
        {}
      )
    },
    saveImportReserves() {
      if (this.importDialog.reserves.length == 0) {
        ...
        service.finance.reserveBill
          .saveSystem(this.importDialog.reserves)
          .then(res => {
            if (res.resp_code == '200') {
