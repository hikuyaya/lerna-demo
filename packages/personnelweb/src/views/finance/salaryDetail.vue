  ...
  <div class="salaryRewardDetail">
    ...
    <el-table style="width: 100%" :data="tableData">
      ...
      <el-table-column align="center" label="提成">
        <el-table-column
          align="center"
          :label="type.prtname + '业绩'"
          v-for="(type, i) in rTypeList">
          <template slot-scope="scope">
            <span>{{
              scope.row.penalties.find(e => rTypeList[i].id == e.prtid)
                ? scope.row.penalties.find(e => rTypeList[i].id == e.prtid)
                    .achievement
                : 0
            }}</span>
      ...
      <el-table-column align="center" label="奖金">
        <el-table-column
          v-for="(item, i) in awardList"
          align="center"
          :label="item.padname">
          <template slot-scope="scope">
            <span>{{
              scope.row.awards.find(e => awardList[i].id == e.padid)
                ? scope.row.awards.find(e => awardList[i].id == e.padid).value
                : 0
            }}</span>
          </template>
        </el-table-column>
      </el-table-column>
      <el-table-column align="center" label="扣罚项">
        <el-table-column
          align="center"
          v-for="(item, i) in penaltyList"
          :label="item.padname">
          <template slot-scope="scope">
            <span>{{
              scope.row.awards.find(e => penaltyList[i].id == e.padid)
                ? scope.row.awards.find(e => penaltyList[i].id == e.padid).value
                : 0
            }}</span>
          </template>
        </el-table-column>
  ...
  methods: {
    ...
    downExcel() {
      ...
      if (this.ruleForm.month instanceof Date)
        this.ruleForm.month = this.ruleForm.month.formatDate('yyyy-MM')
      download(
        $yid.config.API.BASE + 'api-pers/employeeroyalty/excel/export',
        this.ruleForm
      )
    },
    getShopList(params) {
