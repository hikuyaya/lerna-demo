  ...
  <div class="salaryManage">
    ...
    <yid-table
      ...
      <yid-table-column
        label="编码"
        min-width="150"
        prop="padcode"></yid-table-column>
      <yid-table-column
        label="名称"
        min-width="150"
        prop="padname"></yid-table-column>
    ...
    <yid-dialog
      :title="awardValidateForm.title"
      :visible.sync="showAwardForm"
      @close="closeDialog"
      width="450px">
      <el-form
        :model="awardValidateForm"
        label-width="140px"
        ref="awardValidateForm">
        <el-form-item prop="padtype" label="奖/扣罚项类型：">
          <el-select
            clearable
            placeholder="请选择类型"
            style="width: 200px"
            v-model.trim="awardValidateForm.padtype">
            <el-option
              :key="item.type"
              :label="item.name"
              :value="item.type"
              v-for="item in awardValidateForm.list"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item
          prop="padname"
          label="奖扣罚项名称："
          :rules="[{ required: true, message: '奖扣罚项名称不能为空' }]">
          <el-input
            maxlength="8"
            v-model="awardValidateForm.padname"
            placeholder="8个字以内"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button @click="saveAward('awardValidateForm')" type="primary"
            >保存</el-button
          >
        </el-form-item>
      </el-form>
