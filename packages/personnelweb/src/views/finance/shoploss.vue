<template>
    <div class="shoploss">
        ...
        <yid-table ref="shoplosstable" style="margin-top: 15px;" :data="shoplossDate" :row-class-name="$yid.util.getTableClass">
            ...
            <yid-table-column prop="shopname" label="门店名称" width="130"></yid-table-column>
            <yid-table-column prop="ks" label="亏损" width="100"></yid-table-column>
            <yid-table-column prop="orgjkyl" label="管理处借款（预留）" width="100"></yid-table-column>
            <yid-table-column prop="shopjk" label="分店内借款" width="100"></yid-table-column>
            <yid-table-column prop="jfxj" label="借风险金" width="100"></yid-table-column>
            <yid-table-column prop="gcjk" label="还工程公司借款" width="100"></yid-table-column>
            <yid-table-column prop="wfgz" label="未发工资" width="100"></yid-table-column>
            <yid-table-column prop="fzqk" label="房租缺口" width="100"></yid-table-column>
            <yid-table-column prop="totalks" label="亏损合计" width="100"></yid-table-column>
        </yid-table>
        ...
        <yid-dialog :title="importDialog.title" :visible.sync="importDialog.visible" @close="importCancel" width="700px">
            <el-form :model="importDialog.model"  label-width="120px" ref="copyForm">
                ...
                <el-form-item label="下载导入模板：" align="left">
                    <el-link @click="downExcelTemplate" type="primary">分店亏损情况表模板</el-link>
                </el-form-item>
                ...
                <el-form-item>
                    <el-button @click="saveImportshoplosss" type="primary">确定</el-button>
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.excelConvertUrl = $yid.config.API.BASE + "api-finance/shopmonthloss/convertSystem";
        ...
        methods: {
            getshoplossList(){
                ...
                service.finance.shoploss.listAll({isDel:'0',year:Number(years),month:Number(month),shopcode:this.searchForm.shopcode}).then(res=>{
            ...
            downExcelTemplate(){
                //动态生成导入模版
                download(yid.config.API.BASE + 'api-finance/shopmonthloss/downSysTemplate',{})
            },
            saveImportshoplosss(){
                if(this.importDialog.shoplosss == null || this.importDialog.shoplosss.length == 0){
                ...
                service.finance.shoploss.saveSystem(this.importDialog.shoplosss).then(res =>{
                    if(res.resp_code=="200"){
                        this.getshoplossList();
                        this.importCancel();
                    }else{
