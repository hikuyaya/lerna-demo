<template>
  <div class="dept">
      ...
      <div class="meminfo" v-show="!listShow">
        ...
        <table border="1" cellspacing="0">
          <tr>
            ...
            <td width="25%">
              <label style="margin-left: 5px">{{ memberDesc.mobile }}</label>
            </td>
            <td align="right" width="25%">姓名:</td>
            <td width="25%">
              <label style="margin-left: 5px">{{ memberDesc.memname }}</label>
            </td>
          </tr>
          <tr>
            ...
            <td width="25%">
              <label style="margin-left: 5px">{{
                $yid.util.formatDate(memberDesc.birthday)
              }}</label>
            </td>
        ...
        <yid-table
          ...
          <yid-table-column label="套餐名称" prop="mpname">
            <template slot-scope="scope">
              <div>
                {{ scope.row.mpname }}({{
                  memberDesc.shopid == scope.row.shopid
                    ? '本店'
                    : scope.row.shopname
                }})
              </div>
            ...
            <template slot-scope="scope">
              <yid-table
                ...
                <yid-table-column
                  ...
                  label="到期日期"
                  min-width="160"
                  :formatter="formatter"></yid-table-column>
                  ...
                  <template slot-scope="scopes">
                    <div v-if="memberDesc.shopid == scope.row.shopid">
                      <el-input
                        type="number"
                        min="0"
                        v-model="scopes.row.tkNum"></el-input>
                    </div>
                  </template>
                </yid-table-column>
                <yid-table-column
        ...
        <el-form inline class="margTop">
          <el-form-item label="合计退款金额："
            >￥{{ memberDesc.packageMoney }}</el-form-item
          >
          <el-form-item label="退款手续费："
            ><el-input
              type="number"
              min="0"
              v-model="memberDesc.freePackage"></el-input
          ></el-form-item>
          <el-form-item label="合计套餐退款总金额："
            >￥{{ memberDesc.packageJe }}
        ...
        <yid-table
          ...
          <yid-table-column label="退卡金额度" prop="tkje" min-width="100">
            <template slot-scope="scope">
              <div>
                <el-input type="number" min="0" v-model="scope.row.tkje" />
              </div>
            </template>
          </yid-table-column>
          <yid-table-column label="扣除赠送金额度" prop="tzje" min-width="100">
            <template slot-scope="scope">
              <div>
                <el-input type="number" min="0" v-model="scope.row.tzje" />
              </div>
            </template>
          </yid-table-column>
          <yid-table-column
          ...
          <yid-table-column
            ...
            <template slot-scope="scope">
              <div
                v-for="card in scope.row.cardshops"
                v-if="memberDesc.shopid != card.shopid">
                {{ card.money }}
              </div>
            </template>
          </yid-table-column>
        ...
        <el-form inline class="margTop">
          <el-form-item label="合计退卡金："
            >￥{{ memberDesc.cardMoney }}</el-form-item
          >
          <el-form-item label="合计退卡赠送："
            >￥{{ memberDesc.giftMoney }}</el-form-item
          >
          <el-form-item label="退款后卡金和赠送金余额为0时自动销卡："
            ><el-checkbox v-model="memberDesc.autokcard"></el-checkbox
          ></el-form-item>
          <el-form-item label="退款手续费："
            ><el-input type="number" v-model="memberDesc.freeCard"></el-input>
            <yid-ask
              >已使用的赠送金和以消耗卡金的折扣成本可以通过折算成手续费扣除出来，门店可根据实际情况填写，
              手续费金额最高等于合计应退卡金，
              最低为0.当手续费金额等于合计应退卡金时，
              合计会员卡退款总金额等于0.</yid-ask
            ></el-form-item
          >
          <el-form-item label="合计套餐退款总金额："
            >￥{{ memberDesc.cardJe }}
        ...
        <yid-table
          ...
          <yid-table-column label="开卡/续卡金额" min-width="120">
            <template slot-scope="scope">
              <div>
                <el-button @click="lookupLog(scope.row.id)">查看</el-button>
              </div>
            </template>
          </yid-table-column>
          <yid-table-column
        ...
        <el-form inline class="margTop">
          <el-form-item label="退款后资格卡过期将自动销卡"
            ><el-checkbox v-model="memberDesc.autozkcard"></el-checkbox
          ></el-form-item>
          <el-form-item label="退款手续费："
            ><el-input type="number" v-model="memberDesc.freeCardzk"></el-input>
            <yid-ask
              >已使用的资格卡的折扣成本可以通过折算成手续费扣除出来，门店可根据实际情况填写，
              手续费金额最高等于合计应退款，
              最低为0.当手续费金额等于合计应退款时，
              合计资格卡退款总金额等于0.</yid-ask
            ></el-form-item
          >
          <el-form-item label="合计资格卡退款总金额："
            >￥{{ memberDesc.cardzJe }}</el-form-item
          >
        </el-form>
        <el-divider></el-divider>
        <div>退寄存商品</div>
        ...
        <el-form inline class="margTop">
          <el-form-item label="合计退货金额："
            >￥{{ memberDesc.productMoney }}</el-form-item
          >
          <el-form-item label="退款手续费：">
            <el-input type="number" v-model="memberDesc.freeProduct"></el-input>
            <yid-ask
              >已使用的套餐折扣成本可以通过手续费的形式扣除出来，
              门店操作时可根据实际情景填写。手续费金额最高等于合计应退金额，
              当手续费等于合计应退金额时，合计套餐退款金额为0</yid-ask
            ></el-form-item
          >
          <el-form-item label="合计寄存退款总金额："
            >￥{{ memberDesc.productJe }}
            <yid-ask
              >合计寄存退款金额 = 合计应退金额 - 手续费</yid-ask
            ></el-form-item
          >
        </el-form>
        <el-divider></el-divider>
        ...
        <el-form>
          <el-form-item label="">
            <el-row>
              ...
              >
              <el-col :span="3"
                ><el-checkbox v-model="memberDesc.isPrint"
                  >打印退款单据</el-checkbox
                ></el-col
              >
            </el-row>
    ...
    <el-dialog
      :title="memDialog.title"
      :visible="memDialog.visible"
      @close="closeDialog(1)"
      width="600px">
      <el-row>
        <el-col>请输入顾客手机号：</el-col>
        <el-col class="margTop"
          ><el-input
            v-model.trim="memDialog.mobile"
            placeholder="顾客手机号"></el-input
        ></el-col>
        <el-col class="margTop"
          ><el-button type="primary" @click="seachMember()"
            >下一步</el-button
          ></el-col
        >
      </el-row>
    ...
    <yid-dialog
      :title="memCardlog.title"
      :visible.sync="memCardlog.visible"
      @close="closeDialog(3)"
      width="800px">
        ...
        <yid-table-column width="50">
          <template slot-scope="scope">
            <el-checkbox
              ...
              v-show="scope.row.status != '已过期'"></el-checkbox>
          </template>
        </yid-table-column>
        <yid-table-column
          label="类型"
          min-width="60"
          prop="type"></yid-table-column>
        <yid-table-column
          label="金额"
          min-width="90"
      ...
      <el-row style="margin-top: 10px">
        <el-col :span="1" :offse="3"
          ><el-button type="primary" @click="setCardzkje()"
            >确定</el-button
          ></el-col
        >
      </el-row>
    ...
    <yid-dialog
      :title="lookupdetailDialog.title"
      :visible.sync="lookupdetailDialog.visible"
      @close="closeDialog(4)"
      width="1120px">
      <div class="meminfo">
        <div>会员基础信息</div>
        ...
        <el-form inline class="margTop">
          ...
          >
          <el-form-item label="退款后卡金和赠送金余额为0时自动销卡：">{{
            memberDesc.autokcard == '1' ? '是' : '否'
          }}</el-form-item>
          <el-form-item label="退款手续费："
            >￥{{ memberDesc.freeCard }}
            <yid-ask
              >已使用的赠送金和以消耗卡金的折扣成本可以通过折算成手续费扣除出来，
              门店可根据实际情况填写，手续费金额最高等于合计应退卡金，
              最低为0.当手续费金额等于合计应退卡金时，合计会员卡退款总金额等于0.</yid-ask
            ></el-form-item
          >
          <el-form-item label="合计套餐退款总金额："
            >￥{{ memberDesc.cardJe }}
            <yid-ask
              >合计会员卡退款总金额 = 合计应退卡金- 手续费</yid-ask
            ></el-form-item
          >
        </el-form>
        <el-divider></el-divider>
        ...
        <el-form inline class="margTop">
          <el-form-item label="退款后资格卡过期将自动销卡">{{
            memberDesc.autozkcard == '1' ? '是' : '否'
          }}</el-form-item>
          <el-form-item label="退款手续费："
            >￥{{ memberDesc.freeCardzk }}
            <yid-ask
              >已使用的资格卡的折扣成本可以通过折算成手续费扣除出来，门店可根据实际情况填写，
              手续费金额最高等于合计应退款，
              最低为0.当手续费金额等于合计应退款时，
        ...
        <el-rol>
          <el-col :span="4" :offset="8"
            ><el-button type="primary" @click="closeDialog(4)"
              >返回</el-button
            ></el-col
          >
          <el-col :span="4"
            ><el-button type="primary">打印退款单据</el-button></el-col
          >
        </el-rol>
      </div>
    </yid-dialog>
      ...
      handler(n) {
        ...
        n.packageData.map(pack => {
          pack.descList.map(desc => {
            if (desc.tkNum > desc.snum) desc.tkNum = desc.snum
            if (desc.tkNum == desc.snum) desc.tkje = desc.pprice
            if (desc.tkNum) {
              if (desc.isGift == '1') {
                desc.tkje = 0
              } else {
        ...
        this.memberDesc.packageJe =
          this.memberDesc.packageMoney -
          (this.memberDesc.freePackage ? this.memberDesc.freePackage : 0)
        ...
        this.memberDesc.cardJe =
          this.memberDesc.cardMoney -
          (this.memberDesc.freeCard ? this.memberDesc.freeCard : 0)
        ...
        if (
          this.memberDesc.freeCardzk &&
          Number(this.memberDesc.freeCardzk) > tzkje
        ) {
          this.memberDesc.freeCardzk = tzkje
          this.memberDesc.cardzJe = 0
        } else {
          this.memberDesc.cardzJe =
            tzkje -
            (this.memberDesc.freeCardzk ? this.memberDesc.freeCardzk : 0)
        }
        ...
        this.memberDesc.productJe =
          this.memberDesc.productMoney -
          (this.memberDesc.freeProduct ? this.memberDesc.freeProduct : 0)
        if (this.memberDesc.productJe < 0) {
          this.memberDesc.freeProduct = this.memberDesc.productMoney
          this.memberDesc.productJe = 0
        }
        this.memberDesc.totalJe =
          Number(this.memberDesc.packageJe) +
          Number(this.memberDesc.cardJe) +
          Number(this.memberDesc.cardzJe) +
          Number(this.memberDesc.productJe)
      },
      deep: true
    }
  },
  methods: {
    ...
    seach() {
      ...
      if (this.searchForm.meminfo) {
        if (
          !isNaN(this.searchForm.meminfo) &&
          this.searchForm.meminfo.length == 11
        ) {
    ...
    seachMember() {
      ...
      service.member.refund
        .getDiffMember({ mobile: this.memDialog.mobile })
        .then(res => {
