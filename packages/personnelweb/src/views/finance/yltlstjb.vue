<template>
    <div>
        ...
        <yid-table ref="elTable" :data="tableData" style="margin-top: 15px;" :height="wheight" :row-class-name="$yid.util.getTableClass">
            ...
            <yid-table-column label="本期流水" min-width="100" prop="m1"></yid-table-column>
            <yid-table-column label="扣除手续费后" min-width="100" prop="m2"></yid-table-column>
            <yid-table-column label="本期已开单" min-width="100" prop="m3"></yid-table-column>
            <yid-table-column label="本期未开单" min-width="100" prop="m4"></yid-table-column>
            <yid-table-column label="本期流水差额" min-width="100" prop="c2">
                <template slot-scope="scope">
                    <el-tooltip class="item" effect="dark" content="本期流水差额=本期流水-本期已开单-本期未开单" placement="top-start">
                        <label :style="{ color: scope.row.c2 != 0? 'red' : 'blue'}">{{scope.row.c2}}</label>
                    </el-tooltip>
                </template>
            </yid-table-column>
            <yid-table-column label="本期在他" min-width="100" prop="m9"></yid-table-column>
            <yid-table-column label="本期在本" min-width="100" prop="m5"></yid-table-column>
            <yid-table-column label="本期开单差额" min-width="100" prop="c3">
                <template slot-scope="scope">
                    <el-tooltip class="item" effect="dark" content="本期开单差额=本期已开单-本期在本-本期在他" placement="top-start">
                        <label :style="{ color: scope.row.c3 != 0? 'red' : 'blue'}">{{scope.row.c3}}</label>
                    </el-tooltip>
                </template>
            </yid-table-column>
            <yid-table-column label="他期在本" min-width="100" prop="m6"></yid-table-column>
            <yid-table-column label="对账表金额" min-width="100" prop="m7"></yid-table-column>
            <yid-table-column label="对账差额" min-width="100" prop="c1">
                <template slot-scope="scope">
                    <el-tooltip class="item" effect="dark" content="对账差额=本期在本+他期在本-对账表金额" placement="top-start">
                        <label :style="{ color: scope.row.c1 != 0? 'red' : 'blue'}">{{scope.row.c1}}</label>
                    </el-tooltip>
                </template>
            </yid-table-column>
            <yid-table-column label="对账表手续费后" min-width="100" prop="m8"></yid-table-column>
    ...
    export default {
        ...
        methods: {
            ...
            excel() {
                ...
                params.shopcode = this.searchForm.shopcode;
                params.facode = this.searchForm.facode;
                download(yid.config.API.BASE + 'api-cashier/prepaidorder/report/yltlstjb/excel', params)
            },
        },
    }
