  ...
  <div>
    ...
    <el-form
      ...
      <el-form-item label="券编码：" prop="cpcode">
        {{ settingForm.cpcode
        }}<label style="color: silver">编号限定从0001开始的整数,自动叠加</label>
      </el-form-item>
      <el-form-item label="优惠券名称：" prop="cpname">
        <el-input
          :disabled="!isCanEdit"
          v-model="settingForm.cpname"
          placeholder="10字以内"
          style="width: 150px" />
      </el-form-item>
      <el-form-item label="优惠券形式：" prop="couponType">
        <el-row>
          <el-radio
            v-model="settingForm.couponType"
            :disabled="!isCanEdit"
            :label="1"
      ...
      <el-row style="padding-left: 130px">
        <div v-if="settingForm.couponType == '2'" style="margin-top: 8px">
          ...
          <el-input
            placeholder=">0的数"
            v-model="settingForm.maxMoney"
            style="width: 150px" />
          <el-tag type="info">元</el-tag>
          （不填表示无上限）
        </div>
        <div v-if="settingForm.couponType == '2'" style="margin-top: 8px">
          <el-radio v-model="forService" :label="0" @change="changService"
            >针对单个项目</el-radio
          >
          <el-radio v-model="forService" :label="1" @change="changService"
            >针对多个项目</el-radio
          >
          <br /><label style="color: silver"
            >说明:设置为针对单个项目，表示优惠券的优惠仅针对单据中优惠额度最高的可用项目进行折扣，单据内一个项目对应一张券</label
          >
          <br /><label style="color: silver"
            >按折扣优惠力度从高到低依次自动对应抵扣;如果设置为针对多个项目，则表示该单据内所有优惠券指定项目之和享受折扣，</label
          >
          <br /><label style="color: silver"
            >最高减额按整单算。一个项目仅可使用一张折扣优惠券，不可叠加使用(不分单项多项)</label
          >
        </div>
        <div v-if="settingForm.couponType == '2'" style="margin-top: 8px">
      ...
      <el-form-item label="有效期：" prop="timeType">
        <el-radio-group
          ...
          <div>
            ...
            <el-input
              type="number"
              :disabled="settingForm.timeType != '2'"
              style="width: 150px; margin-left: 5px"
          ...
          <div>
            ...
            <el-input
              type="number"
              :disabled="settingForm.timeType != '3'"
              style="width: 150px; margin-left: 5px"
              v-model="settingForm.someDay"
              @input="onInputTitle()" />
            <el-tag style="margin-left: 3px" type="info">天后起用</el-tag>
            <label style="color: silver">（0表示当天起用）</label>
            <el-tag type="info">，有效期</el-tag>
            <el-input
              ...
              :disabled="settingForm.timeType != '3'"
              style="width: 150px; margin-left: 5px"
              v-model="settingForm.someMoth"
              @input="onInputTitle()"
              placeholder=">0，=<36的数，" />
            <el-tag type="info">月</el-tag>
            <br /><label style="color: silver"
              >说明:按月设置表示从领券启用之时开始的自然月份开始计算有效期数量。例如领券启用时间为2月8日，设置有效月为1月则</label
            >
            <br /><label style="color: silver"
              >券有效期为2月28日。如果设置有效月为2月，则券有效期从领券启用日期的当月开始延续到2个月，则券有效期为3月31日</label
            >
      ...
      <el-form-item label="业绩补贴：" prop="yjSubsidy">
        <el-input
          ...
          min="0"
          max="100"
          v-model="settingForm.yjSubsidy"
          placeholder="0"
          style="width: 100px"
          @input="onInputTitle()" />
        %<br />
        <label style="color: silver"
          >说明:业绩补贴是指优惠券折扣的部分金额贾计算业绩，可输入0-100的整数，默认设置为0时表示抵扣部分不计业绩</label
        ><br />
        <label style="color: silver"
          >设量大于0的比率，则抵扣部分按设置的比率计入服务业绩</label
        >
      </el-form-item>
      <el-form-item label="使用场景及规则：">
        ...
        <el-row>
          ...
          <label style="color: silver" v-if="settingForm.isSelectProject"
            >说明：如果不选择则默认全部项目</label
          ></el-row
        >
        ...
        <el-row>
          ...
          <label style="color: silver" v-if="settingForm.isSelectGood"
            >说明：如果不选择则默认全部产品</label
          ></el-row
        >
        ...
        <el-row>
          <el-checkbox
            label="购买套餐可用"
            v-model="settingForm.isSelectCombo"
            :disabled="!isCanEdit"
            @change="clearComboData(comboAddDialog)"></el-checkbox>
          <label style="color: silver" v-if="settingForm.isSelectCombo"
            >说明：如果不选择则默认全部套餐</label
          ></el-row
        >
        <el-row v-if="settingForm.isSelectCombo">
          <el-button
            type="primary"
            @click="showComboDialog()"
            :disabled="!isCanEdit"
            >选择套餐</el-button
          >
        </el-row>
        <div
          ...
          "
          v-if="
            settingForm.isSelectCombo &&
            comboAddDialog.selectComboList.length > 0
          ">
          <el-scrollbar style="height: 100%; width: 100%">
            <ul
              style="
              ...
              ">
              <li
                v-for="(item, index) in comboAddDialog.selectComboList"
                style="width: 25%">
                <el-row
                  ><i
                    class="el-icon-circle-close"
                    style="margin-right: 6px"
                    @click="deleteCombo(index)"></i>
                  {{ item.spname }}
                </el-row>
              </li>
            </ul>
        ...
        <el-row>
          ...
          <el-input
            ...
            v-model.trim="settingForm.maxAddnum"
            style="width: 120px"
            :disabled="isEditMaxAddNum"></el-input>
          张
        </el-row>
        <el-row>
          <el-checkbox
            label="不支持以下支付方式"
            v-model="settingForm.isNopaytype"
            :disabled="!isCanEdit"
        ...
        <div
          ...
          "
          v-if="
            settingForm.isNopaytype &&
            paytypeAddDialog.selectPaytypeList.length > 0
          ">
          <el-scrollbar style="height: 100%; width: 100%">
            <ul
              style="
              ...
              ">
              <li
                v-for="(item, index) in paytypeAddDialog.selectPaytypeList"
                style="width: 25%">
                <el-row
                  ><i
                    class="el-icon-circle-close"
                    style="margin-right: 6px"
                    @click="deletePaytype(index)"></i
                  >({{ item.paycode }}) {{ item.payname }}
                </el-row>
              </li>
            </ul>
          </el-scrollbar>
      ...
      <el-form-item label="优惠券说明：" prop="cpcode">
        <el-input
          v-model.trim="settingForm.introduction"
          placeholder="请填写优惠券说明"
          maxlength="300"
    ...
    <yid-dialog
      :title="paytypeAddDialog.title"
      :visible.sync="paytypeAddDialog.visible"
      width="700px">
      <el-form label-width="90px" ref="copyForm2">
        <el-form-item label="筛选：">
          <el-select
            clearable
            v-model="paytypeAddDialog.payMold"
            placeholder="请选择分类"
            style="margin-left: 5px; width: 140px">
            <el-option
          ...
          <div class="selected">
            <table border="1" style="width: 100%" class="serve-table">
              <tr>
                <th width="30%">
                  <el-checkbox
                    v-model="paytypeAddDialog.checkbox"
                    @change="checkAllPaytype"></el-checkbox>
                  编码
                </th>
    ...
    screenService() {
      return function (item) {
        ...
        if (this.proAddDialog.sername) {
          if (
            item.sdname.indexOf(this.proAddDialog.sername) !== -1 ||
  ...
  methods: {
    ...
    addProject(tag) {
      if (tag == 1) {
        ...
        this.proAddDialog2.serviceTypes.forEach(item => {
          if (
            item.isSelect &&
            (that.proAddDialog2.selectProjectList.length == 0 ||
              that.proAddDialog2.selectProjectList.findIndex(
        ...
        that.proAddDialog.selectProjectList =
          that.proAddDialog2.selectProjectList
        this.proAddDialog2.visible = false
      }
    },
    selectType(e, val) {
    ...
    addpaytype() {
      ...
      this.paytypeAddDialog.paytypes.forEach(item => {
        if (
          item.isSelect &&
          (that.paytypeAddDialog.selectPaytypeList.length == 0 ||
            that.paytypeAddDialog.selectPaytypeList.findIndex(
    ...
    toSave(status) {
      this.$refs['settingForm'].validate(valid => {
        ...
        if (reqParam.couponType == '1') {
          if (
            !this.settingForm.discount ||
            parseInt(this.settingForm.discount) <= 0
          ) {
          ...
          if (this.settingForm.discount.toString()) {
            if (
              parseInt(this.settingForm.discount) < 0 ||
              parseInt(this.settingForm.discount) > 10
            ) {
          ...
          if (
            this.settingForm.discount.indexOf('.') != -1 &&
            this.settingForm.discount.split('.')[1].length > 1
          ) {
            $yid.util.warning('只能有一位小数的数字')
            return
          }
          if (
            this.settingForm.zeroTitle &&
            this.settingForm.zeroTitle.length > 5
          ) {
        ...
        if (reqParam.timeType == '1') {
          ...
          reqParam.afterDay = this.settingForm.someDay
            ? this.settingForm.someDay
            : 0
          if (parseInt(reqParam.afterDay) < 0) {
            $yid.util.warning('领取天数不能为负数')
            return
          ...
          if (
            !this.settingForm.someMoth ||
            parseInt(this.settingForm.someMoth) <= 0 ||
            parseInt(this.settingForm.someMoth) > 36
          ) {
        ...
        if (
          Number(this.settingForm.maxAddnum) < 0 ||
          Number(this.settingForm.maxAddnum) > 100
        ) {
