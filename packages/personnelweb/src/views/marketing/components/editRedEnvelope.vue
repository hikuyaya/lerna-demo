  ...
  <div>
    ...
    <el-form :model="settingForm" :rules="settingFormRules" label-width="150px" ref="settingForm"
      ...
      <el-form-item label="编码：" prop="rcode">
        {{ settingForm.rcode }}
        <explain style="color:silver">编号限定从0001开始的整数,自动叠加</explain>
      </el-form-item>
      <el-form-item label="红包名称：" prop="rname">
        <el-input :disabled="!isCanEdit" v-model="settingForm.rname" placeholder="10字以内" style="width: 150px"/>
      </el-form-item>
      <el-form-item label="红包价值：" prop="money">
        <el-input :disabled="!isCanEdit" type="number" v-model="settingForm.money" style="width: 150px"/>
      </el-form-item>
      <el-form-item label="有效期：" prop="timeType">
        <el-radio-group :disabled="!isCanEdit" v-model="settingForm.timeType" @change="changeTimeType">
          ...
          <div>
            ...
            <el-input type="number" :disabled="settingForm.timeType!='2' || !isCanEdit"
                      style="width: 150px ; margin-left: 5px" v-model="settingForm.afterDay"/>
            <el-tag style="margin-left: 3px" type="info">天后起用</el-tag>
            <explain style="color:silver">（0表示当天起用）</explain>
            <el-tag type="info">，有效期</el-tag>
            <el-input type="number" :disabled="settingForm.timeType!='2' || !isCanEdit"
                      style="width: 150px ; margin-left: 5px" v-model="settingForm.expiryDay" placeholder=">0的数"/>
      ...
      <el-form-item label="使用场景及规则：">
        ...
        <el-row>
          <el-checkbox label="可分多次使用" v-model="settingForm.isSelectManyTimes" :disabled="!isCanEdit"></el-checkbox>
        </el-row>
        <el-row>
        ...
        <el-row>
          ...
          <explain style="color:silver" v-if="settingForm.isSelectProject">说明：如果不选择则默认全部项目</explain>
        </el-row>
          ...
          <el-scrollbar style="height:100%;width: 100%">
            <ul style="width:100%;list-style: none;text-align:left; display: flex;flex-direction: row;flex-wrap: wrap">
              <li v-for="(item, index) in proAddDialog.selectProjectList" style="width: 25%">
                <el-row><i class="el-icon-circle-close" style="margin-right: 6px" @click="deleteProject(index)"></i>
                  {{ item.sdname }}
                </el-row>
              </li>
            </ul>
        ...
        <el-row>
          ...
          <explain style="color:silver" v-if="settingForm.isSelectGood">说明：如果不选择则默认全部产品</explain>
        </el-row>
        <el-row v-if="settingForm.isSelectGood">
          <el-button type="primary" @click="showGoodDialog()" :disabled="!isCanEdit">选择产品</el-button>
        </el-row>
        <div style="height:200px;width: 100%;margin-top: 6px;background-color: azure"
          ...
          <el-scrollbar style="height:100%;width: 100%">
            <ul style="width:100%;list-style: none;text-align:left; display: flex;flex-direction: row;flex-wrap: wrap">
              <li v-for="(item, index) in goodAddDialog.selectProducts" style="width: 25%">
                <el-row><i class="el-icon-circle-close" style="margin-right: 6px" @click="deleteGood(index)"></i>
                  {{ item.sdname }}
                </el-row>
              </li>
            </ul>
  ...
  data() {
    return {
      ...
      settingFormRules: {
        ...
        rname: [{required: true, message: '请填写红包名称'}, {min: 0, max: 10, message: '红包名称在10个字符以内'}],
  ...
  methods: {
    ...
    addProject(tag) {
      if (tag == 1) {
        ...
        this.proAddDialog2.serviceTypes.forEach(item => {
          if (item.isSelect && (this.proAddDialog2.selectProjectList.length == 0 || this.proAddDialog2.selectProjectList.findIndex(list => item.id == list.servid) == -1)) {
        ...
        this.proAddDialog.selectProjectList = this.proAddDialog2.selectProjectList
        this.proAddDialog2.visible = false
      }
    },
