    ...
    <div>
        <el-collapse-transition>
            <div v-show="showList">
                ...
                <el-form inline :model="searchForm"  ref="searchForm" style="margin-top: 8px">
                    <el-form-item label="发放渠道：" prop="isOnline">
                        <el-select style="width: 120px" v-model="searchForm.isOnline" clearable placeholder="请选择">
                            <el-option value="1" label="线上"></el-option>
                            <el-option value="0" label="线下"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="优惠券：">
                        <el-input clearable v-model="searchForm.pcodename" placeholder="红包名称/编码" style="width: 160px;"></el-input>
                    </el-form-item>
                    ...
                    <el-form-item label="状态：" prop="status">
                        <el-select style="width: 120px" v-model="searchForm.status" clearable placeholder="请选择">
                ...
                <yid-table pagination ref="table" style="margin-top: 10px;">
                    ...
                    <yid-table-column label="编码" min-width="80" prop="cpcode"></yid-table-column>
                    <yid-table-column label="门店品牌" min-width="80" prop="brandName"></yid-table-column>
                    <yid-table-column label="优惠券名称" min-width="80" prop="cpname"></yid-table-column>
                    ...
                    <yid-table-column label="门店使用" min-width="200">
                        <template slot-scope="scope">
                            ...
                            <div v-if="scope.row.conIsproduct != '1'">
                                {{scope.row.conIsproduct == "2" ? "全部产品可用": "部分产品可用"}}
                            </div>
                            <div v-if="scope.row.conIspackage != '1'">
                                {{scope.row.conIspackage == "2" ? "全部套餐可用": "部分套餐可用"}}
                            </div>
                        </template>
                    </yid-table-column>
                    <yid-table-column label="领取量" min-width="80" prop="code"></yid-table-column>
                    <yid-table-column label="已使用" min-width="80" prop="code"></yid-table-column>
                    ...
                    <yid-table-column label="修改时间" min-width="110" prop="updatedTime"></yid-table-column>
                    <yid-table-column label="操作" min-width="120" prop="code">
                        <template slot-scope="scope">
                            <el-link type="primary" @click="showEditPage(scope.row)">
                                {{scope.row.status == "0" || scope.row.status == "1" || scope.row.status == "2"? "编辑" : "查看"}}</el-link>
                            <el-link type="primary" v-if="scope.row.status !='3' " @click="changeStatus(scope.row)" style="margin-left: 6px">
                                {{scope.row.status == "0" || scope.row.status == "2"? "发布" : "停用"}}</el-link>
                            <el-link style="margin-left: 6px" type="primary" @click="showRecordsDialog(scope.row)">明细</el-link>
                        </template>
                    </yid-table-column>
                </yid-table>
            </div>
        ...
        <yid-dialog :title="recordsDialog.title" :visible.sync="recordsDialog.visible" @close="cancelRecordsDialog" width="1100px">
            <el-form inline :model="recordSearchForm"  ref="recordSearchForm">
                <el-form-item label="活动名称：" prop="maname">
                    <el-select clearable v-model.trim="recordSearchForm.maname" @clear="clearFormRecordName" filterable :filter-method="filterActivity" placeholder="请选择活动" style="width: 215px;" >
                        <el-option :key="item.id" :label="item.name" :value="item.name" v-for="item in filterActivityList">
                            <span style="float: right; color: #8492a6; font-size: 13px">{{ item.name }}</span>
                        </el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="状态" prop="status">
                    <el-select clearable v-model="recordSearchForm.status"  placeholder="请选择状态" style="width: 120px;">
                ...
                <el-form-item label="发券门店：">
                    <el-select clearable v-model.trim="recordSearchForm.shopid" placeholder="请选择" style="width: 160px;"
            ...
            <yid-table  pagination ref="recordsTable" style="margin-top: 10px;">
                <yid-table-column label="活动名称" min-width="80" prop="maname"></yid-table-column>
                <yid-table-column label="优惠券券码" min-width="80" prop="bcode"></yid-table-column>
                <yid-table-column label="状态" min-width="80" prop="status">
                    <template slot-scope="scope">
                        {{getUseStatusName(scope.row.status)}}
                    </template>
                </yid-table-column>
                <yid-table-column label="领取时间" min-width="80" prop="createdTime"></yid-table-column>
                <yid-table-column label="会员id" min-width="80" prop="memid"></yid-table-column>
                <yid-table-column label="会员名" min-width="80" prop="memname"></yid-table-column>
                <yid-table-column label="手机号" min-width="80" prop="mobile"></yid-table-column>
                <yid-table-column label="发券单号" min-width="80" prop="sbillcode"></yid-table-column>
                <yid-table-column label="发券门店" min-width="80" prop="shopname"></yid-table-column>
                <yid-table-column label="消费单号" min-width="80" prop="billcode"></yid-table-column>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            changeStatus(row){
                let tips = row.status == "0" || row.status == "2" ? "保存发布后，优惠券将不能再次编辑！" : "停用后，活动中有赠送该券的将停止赠送，但不影响已领取优惠券的使用，确定要停用吗？"
            ...
            getTimeTypeWord(row){
                if(row.timeType == "1"){
                    return $yid.util.formatDate(row.sdate) + "至" + $yid.util.formatDate(row.edate)
                }else if(row.timeType == "2"){
                    let afterDay = row.afterDay
                    let expiryDay = row.expiryDay
                    return afterDay == 0 ? "当天有效，有效期" + expiryDay + "天": "领取" + afterDay + "天后起用，有效期"  + expiryDay + "天"
                } else if(row.timeType == "3"){
                  let afterDay = row.afterDay
                  let expiryDay = row.expiryDay
                  return afterDay == 0 ? "当天有效，有效期" + expiryDay + "个月": "领取" + afterDay + "天后起用，有效期"  + expiryDay + "个月"
