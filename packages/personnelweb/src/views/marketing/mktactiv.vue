  ...
  <div>
      ...
      <div v-show="showList">
        ...
        <el-form :model="searchForm" inline style="margin-top: 15px">
          <el-form-item label="活动：">
            <el-input clearable placeholder="活动名称/编码" v-model.trim="searchForm.name" style="width: 150px;"></el-input>
          </el-form-item>
          <el-form-item label="活动类型">
            <el-select clearable filterable placeholder="请选择" style="width: 140px;" v-model.trim="searchForm.matype">
              <el-option :key="'matype'+item.value" :label="item.name" :value="item.value"
                         v-for="item in matypes"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="活动状态">
            <el-select clearable filterable placeholder="请选择" style="width: 130px;" v-model.trim="searchForm.status">
              <el-option :key="'mastatus'+item.value" :label="item.name" :value="item.value"
                         v-for="item in mastatuss"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="门店品牌">
            <el-select clearable filterable placeholder="请选择" style="width: 160px;"
                       @change="filterBrand(searchForm.brandId)" v-model.trim="searchForm.brandId">
              <el-option :key="'brand'+item.id" :label="item.name" :value="item.id" v-for="item in brands"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="门店：">
            <el-select @clear="clearFormShop" @blur="clearFormShop" clearable v-model.trim="searchForm.shop" filterable
                       :filter-method="filterShop" placeholder="门店名称/编码"
                       style="width: 160px;">
        ...
        <yid-table pagination ref="mktactivTable" style="margin-top: 15px;">
          <yid-table-column label="编码" min-width="60" prop="billcode" fixed></yid-table-column>
          <yid-table-column label="门店品牌" min-width="90" prop="brandName" fixed></yid-table-column>
          <yid-table-column label="活动名称" min-width="120" prop="name"></yid-table-column>
          <yid-table-column label="活动类型" width="120" prop="matype">
            <template slot-scope="scope">
              {{ scope.row.matype | formatValue(matypes) }}
            </template>
          </yid-table-column>
          <yid-table-column label="活动时间" width="200" prop="timeType" :formatter="dateFormat"></yid-table-column>
          <yid-table-column label="创建人" min-width="100" prop="createdBy"></yid-table-column>
          <yid-table-column label="创建日期" min-width="100" prop="createdTime">
            <template slot-scope="scope">
              {{ $yid.util.formatDate(scope.row.createdTime) }}
            </template>
          </yid-table-column>
          <yid-table-column label="修改人" min-width="100" prop="updatedBy"></yid-table-column>
          ...
          <yid-table-column label="操作" min-width="120" fixed="right">
            <template slot-scope="scope">
              <el-button v-if="scope.row.status!='4'" @click="showEdit(scope.row, false)" type="text"><i
                  class="edit"></i>编辑
              </el-button>
              <el-button v-if="scope.row.status=='4'" @click="showEdit(scope.row, true)" type="text"><i
                  class="edit"></i>查看
              </el-button>
              <el-button v-if="scope.row.status=='1' || scope.row.status=='2'" @click="updateStatus(scope.row.id, '3')"
                         type="text"><i class="edit"></i>暂停
              </el-button>
              <el-button v-if="scope.row.status=='3'" @click="updateStatus(scope.row.id, '2')" type="text"><i
                  class="edit"></i>启用
              </el-button>
              <el-button @click="copyTemplate(scope.row, false)" type="text"><i class="edit"></i>复制</el-button>
            </template>
          </yid-table-column>
        </yid-table>
      </div>
    ...
    <el-collapse-transition>
      <div v-show="!showList">
        ...
        <el-button @click="saveMktactiv('2')" type="primary" isDisabled="isDisabled">保存并启用</el-button>
        <el-button @click="saveMktactiv('3')" type="primary" isDisabled="isDisabled">仅保存</el-button>
        <el-form ref="mktactivForm" :model="mktactivForm" label-width="80px" :disabled="isSelect"
                 style="margin-top: 15px">
          <el-form-item label="门店品牌" prop="brandId" :rules="[{ required: true, message: '门店品牌不能为空'}]">
            <el-select placeholder="请选择" style="width: 160px;" v-model.trim="mktactivForm.brandId" @change="changBrnad"
                       :disabled="mktactivForm.id!=''">
              <el-option :key="'brand2'+item.id" :label="item.name" :value="item.id" v-for="item in brands"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="活动类型" prop="matype" :rules="[{ required: true, message: '活动类型不能为空'}]">
            <el-select placeholder="请选择" style="width: 140px;" v-model.trim="mktactivForm.matype"
                       :disabled="mktactivForm.id!=''">
              <el-option :key="'matype2'+item.value" :label="item.name" :value="item.value"
                         v-for="item in matypes"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="编码" prop="billcode">
            <el-input placeholder="自动生成" v-model.trim="mktactivForm.billcode" :disabled="true"
                      style="width: 200px;"></el-input>
          </el-form-item>
          <el-form-item label="活动名称" prop="name" :rules="[{ required: true, message: '活动名称不能为空'}]">
            <el-input v-model.trim="mktactivForm.name" placeholder="20字以内" maxlength="20"
                      style="width: 320px;"></el-input>
          </el-form-item>
          <el-form-item label="活动时间">
            <el-date-picker style="width:320px;"
                            v-model="mktactivForm.mktdate" value-format="yyyy-MM-dd"
                            type="daterange" @change="changMktdate"
                            start-placeholder="开始日期" range-separator="至" end-placeholder="结束日期">
            </el-date-picker>
            <el-checkbox v-model="mktactivForm.expType" @change="changMktdate2" style="margin-left: 5px;">长期
            </el-checkbox>
          </el-form-item>
          <el-form-item label="参与条件" v-if="mktactivForm.matype=='2'">
            <el-checkbox @change="editCktype" v-model="mktactivForm.firstOrderFlag">首次开卡(当前品牌)</el-checkbox>
          </el-form-item>
          <el-form-item label="充值累计" v-if="mktactivForm.matype=='2'">
            <el-radio-group v-model="mktactivForm.cktype" :disabled="editCktypeFlag">
              <el-radio label="0">不可累计</el-radio>
              <el-radio label="1">可累计</el-radio>
            </el-radio-group>
            <br>
            <div v-if="mktactivForm.cktype=='1'" style="margin-top: 3px;">
              <el-radio label="1" v-model="mktactivForm.ckdatetype" @change="changeCkdatetype">固定时间</el-radio>
              <el-date-picker
            ...
            <div v-if="mktactivForm.cktype=='1'" style="margin-top: 3px;">
              <el-radio label="2" v-model="mktactivForm.ckdatetype" @change="changeCkdatetype">相对时间</el-radio>
              <el-input type="number" min="1" v-model="mktactivForm.relatime"
          ...
          <el-form-item label="活动门店" prop="shop">
            ...
            {{ mktactivForm.shoplist ? '共选中' + mktactivForm.shoplist.length + '家门店' : '' }}
          </el-form-item>
          <el-form-item label="使用门店" prop="useshopType" v-if="mktactivForm.matype!='1'">
            <el-radio-group v-model="mktactivForm.useshopType">
              <el-radio label="1">所有活动门店</el-radio>
              <el-radio label="2">仅赠送门店</el-radio>
            </el-radio-group>
            <span style="margin-left: 30px;color:grey;font-size: 12px;" v-if="mktactivForm.matype=='2'">注：赠送红包/优惠券的使用门店设置，赠送项目/产品默认仅赠送门店使用</span>
          </el-form-item>
          <el-form-item label="活动业绩折扣" prop="discount" label-width="120px" v-if="mktactivForm.matype=='2'"
                        :rules="[{ required: true, message: '活动业绩折扣不能为空'}]">
            <el-input-number v-model.trim="mktactivForm.discount" :min="0" :max="100"
                             style="width: 150px;"></el-input-number>&nbsp;&nbsp;（输入范围：0-100）
          </el-form-item>
          <el-form-item label="优先级" prop="priority" label-width="120px" v-if="mktactivForm.matype=='4'">
            <el-input-number v-model.trim="mktactivForm.priority" :min="0" :max="100"
                             style="width: 150px;"></el-input-number>&nbsp;&nbsp;按照数字从大到小匹配活动,不填默认为0
          </el-form-item>
...
<!--          </el-form-item>-->
          <el-form-item label="限制条件" v-if="mktactivForm.matype=='2' || mktactivForm.matype=='3'">
            <div class="rechargeitem">
              <div style="margin-bottom: 10px">
                <el-checkbox v-model="mktactivForm.isPnum" @change="isSelectNum(mktactivForm.isPnum)" style="margin-left: 5px;">每个顾客可参与</el-checkbox>
                <el-input type="number" min="1" v-model="mktactivForm.pnum" style="width:100px; margin: 0 10px"
                          oninput="value=value.replace(/[^\d]/g,'')"/>
                次&nbsp;&nbsp;
              </div>
              <div style="margin-bottom: 10px">
                <el-checkbox v-model="mktactivForm.isNopaytype" style="margin-left: 5px;">不支持以下支付方式</el-checkbox>
                <br>
                <el-button type="primary" @click="selectPaytype">添加支付方式</el-button>
                <div class="walletitem ser1" v-show="mktactivForm.nopaytypes && mktactivForm.nopaytypes.length>0">
                  <div class="item1" :key="'pay'+item.payid" v-for="item in mktactivForm.nopaytypes">
                    <i class="el-icon-error iconshanchu" @click="deletePaytype(item.payid)"
                       style="vertical-align: middle"></i>
                    <ul style="display: inline-block">{{ item.payname }}</ul>
                  </div>
                </div>
              </div>
            </div>
          ...
          <el-form-item label="活动规则" v-if="mktactivForm.matype=='1'">
            ...
            <el-form-item label="赠送红包" label-width="70px" style="margin-top: 10px">

              <div style="list-style: none;text-align:left;padding: 0; margin: 0">

                <ul :key="'red1'+item.redid" v-for="item in mktactivForm.redenvs" class="walletitem">
                  <i class="el-icon-error iconshanchu" @click="deleteRedenv(item.redid)"></i>
                  <div class="right">
                    <el-row :gutter="24" type="flex">
                      <el-col :span="14">{{ item.redname }}{{ item.status == '' ? '' : (item.status) }}</el-col>
                      <el-col :span="10" style="text-align: right">{{ item.expTime }}</el-col>
                    </el-row>
              ...
              <el-button type="primary" @click="selectRedenv">增加红包</el-button>
            </el-form-item>
          </el-form-item>
          <el-form-item label="活动规则" v-if="mktactivForm.matype=='2'">
            <div :key="'rule'+itemRule.clevel" v-for="itemRule in mktactivForm.mktactivRules" class="rechargeitem">
              <div style="margin-bottom: 10px">
                <span style="color:red;">*</span>&nbsp;&nbsp;规则名称
                <el-input type="text" placeholder="20字以内" maxlength="20" v-model="itemRule.recharge.rulename"
                          style="width:320px; margin: 0 10px" clearable/>&nbsp;&nbsp;
                <el-checkbox label="预售" v-model="itemRule.recharge.presale"
                             v-if="mktactivForm.cktype==='1'"></el-checkbox>
                <el-input type="number" min="0" maxlength="8" v-model="itemRule.recharge.minMoney"
                          v-if="mktactivForm.cktype==='1'" style="width:200px; margin: 0 10px" clearable></el-input>
              </div>
              <div style="margin-bottom: 10px">
                开卡充值满
                <el-input type="number" min="1" maxlength="8" v-model="itemRule.recharge.money"
                          style="width:100px; margin: 0 10px" clearable/>
                元&nbsp;&nbsp;
                赠送
                <el-input :disabled="itemRule.recharge.isCustom" type="number" min="0" maxlength="8"
                          v-model="itemRule.recharge.giftMoney" style="width:100px; margin: 0 10px" clearable/>
                元 &nbsp;&nbsp;
                <el-checkbox @change="changeCustom($event,itemRule)" v-model="itemRule.recharge.isCustom">自定义金额:
                </el-checkbox>
                最高赠送
                <el-input :disabled="!itemRule.recharge.isCustom" type="number" min="0" maxlength="8"
                          v-model="itemRule.recharge.highestMoney" placeholder=">0,不填无上限"
                          style="width:140px; margin: 0 10px"/>
                元
                <span style="float: right"><el-link type="primary" style="color: red;"
                                                    @click="deleteRule(itemRule.clevel)">删除规则</el-link></span>
              </div>
              <el-form-item label="赠送红包" label-width="85px">
                <el-button type="primary" @click="selectRedenv2(itemRule.clevel)">增加红包</el-button>
                <ul style="list-style: none;text-align:left;margin-top: 10px">
                  <div :key="'red2'+item.clevel+item.rindex" v-for="item in itemRule.redenvs" class="walletitem">
                    <i class="el-icon-error iconshanchu" @click="deleteRedenv2(item.clevel, item.rindex)"></i>
                    <div class="right">
                      <el-row :gutter="24" type="flex">
                        <el-col :span="14">{{ item.redname }}{{ item.status == '' ? '' : (item.status) }}</el-col>
                        <el-col :span="10" style="text-align: right">{{ item.expTime }}</el-col>
              ...
              <el-form-item label="赠送优惠券" label-width="85px">
                <el-button type="primary" @click="selectCoupon2(itemRule.clevel)">增加优惠券</el-button>
                <ul style="list-style: none;text-align:left;margin-top: 10px">
                  <div :key="'cou2'+item.clevel+item.cindex" v-for="item in itemRule.coupons" class="walletitem">
                    <i class="el-icon-error iconshanchu" @click="deleteCoupon2(item.clevel, item.cindex)"></i>
                    <div class="right">
                      <el-row :gutter="24" type="flex">
                        <el-col :span="14">{{ item.cpname }}{{ item.status == '' ? '' : (item.status) }}</el-col>
                        <el-col :span="10" style="text-align: right">{{ item.expTime }}</el-col>
                      </el-row>
                      <el-row> {{ item.rule }}</el-row>
                    </div>
              ...
              <el-form-item label="赠送项目" label-width="85px">
                <el-button type="primary" @click="selectServ(itemRule.clevel)" style="margin-bottom: 15px">增加项目
                </el-button>
                <div class="walletitem ser" v-show="itemRule.servs && itemRule.servs.length>0">
                  <div class="item first">
                    <i></i>
                    <ul class="one">项目名称</ul>
                    <ul class="two">次数</ul>
                    <ul class="three">总价</ul>
                    <ul class="four">有效期</ul>
                  </div>
                  <div class="item" :key="'serv'+item.clevel+''+item.servid" v-for="item in itemRule.servs">
                    <i class="el-icon-error iconshanchu" @click="deleteServ(item.clevel, item.servid)"></i>
                    <ul class="one">{{ item.servname }}</ul>
                    <ul class="two">
                      <el-input type="number" min="1" v-model="item.totalNum" style="width:60px;margin-right:5px"
                                placeholder="次数"/>
                      次
                    </ul>
                    <ul class="three">
                      <el-input type="number" min="1" v-model="item.pprice" style="width:70px; margin-right:5px"
                                placeholder="总价"/>
                      元
                    </ul>
                    <ul class="four">
                      <el-input type="number" min="1" v-model="item.timenum" @input="checkServPrice((item.clevel))"
                                style="width:70px; margin-right:5px" placeholder="有效期"/>
                      <el-select v-model="item.timeunit" style="width: 50px; margin-right: 5px">
                        <el-option :key="'unit'+itemUnit.value" :label="itemUnit.name" :value="itemUnit.value"
                                   v-for="itemUnit in timeunits"></el-option>
                      </el-select>
                      <el-checkbox style="width: 50px; margin-right: 5px" v-model="item.timeType2Checked"
                                   @change="changSerstime((item.clevel))">长期
                      </el-checkbox>
                      <el-checkbox style="width: 80px; margin-right: 5px" v-model="item.timeType3Checked"
                                   @change="changSersdate((item.clevel))">截止日期
                      </el-checkbox>
                      <el-date-picker
                          v-model="item.edate"
              ...
              <el-form-item label="赠送产品" label-width="85px">
                <el-button type="primary" @click="selectGood(itemRule.clevel)" style="margin-bottom: 15px">增加产品
                </el-button>
                <div class="walletitem ser" v-show="itemRule.products && itemRule.products.length>0">
                  <div class="item first">
                    <i></i>
                    <ul class="one">产品名称</ul>
                    <ul class="two">次数</ul>
                    <ul class="three">总价</ul>
                    <ul class="four">有效期</ul>
                  </div>
                  <div class="item" :key="'prod'+item.clevel+''+item.pid" v-for="item in itemRule.products">
                    <i class="el-icon-error iconshanchu" @click="deleteGood(item.clevel, item.pid)"></i>
                    <ul class="one">{{ item.pname }}</ul>
                    <ul class="two">
                      <el-input type="number" min="1" v-model="item.totalNum" style="width:60px;margin-right:5px"
                                placeholder="次数"/>
                      次
                    ...
                    <ul class="four">
                      <el-input type="number" min="1" v-model="item.timenum" @input="checkGoodPrice(item.clevel)"
                                style="width:70px; margin-right:5px" placeholder="有效期"/>
                      <el-select v-model="item.timeunit" style="width: 60px; margin-right: 5px">
                        <el-option :key="'unit2'+itemUnit.value" :label="itemUnit.name" :value="itemUnit.value"
                                   v-for="itemUnit in timeunits"></el-option>
                      </el-select>
                      <el-checkbox v-model="item.timeType2Checked" style="width: 50px; margin-right: 5px"
                                   @change="changGoodtime(item.clevel)">长期
                      </el-checkbox>
                      <el-checkbox v-model="item.timeType3Checked" style="width: 80px; margin-right: 5px"
                                   @change="changGooddate((item.clevel))">截止日期
                      </el-checkbox>
                      <el-date-picker
                          v-model="item.edate"
                          type="date" value-format="yyyy-MM-dd HH:mm:ss"
          ...
          <el-form-item label="活动规则" v-if="mktactivForm.matype=='3'">
            ...
            <el-form-item label="指定套餐" label-width="85px">
              <div class="walletitem pack" v-show="mktactivForm.packages && mktactivForm.packages.length>0">
                <div class="pitem" :key="'pkg3'+item.spid" v-for="item in mktactivForm.packages">
                  <i class="el-icon-error iconshanchu" @click="deletePack(item.spid)"></i>
                  <label>({{ item.spcode }}){{ item.spname }}</label>
                </div>
              </div>
              <el-button type="primary" @click="selectPack()">选择套餐</el-button>

            </el-form-item>
            <el-form-item label="赠送优惠券" label-width="85px">
              <ul style="list-style: none;text-align:left;margin: 0; padding: 0">
                <div :key="'cou3'+item.cpid" v-for="item in mktactivForm.coupons" class="walletitem">
                  <i class="el-icon-error iconshanchu" @click="deleteCoupon(item.cpid)"></i>
                  <div class="right">
                    <el-row :gutter="24" type="flex">
                      <el-col :span="14">{{ item.cpname }}{{ item.status == '' ? '' : (item.status) }}</el-col>
                      <el-col :span="10" style="text-align: right">{{ item.expTime }}</el-col>
              ...
              <el-button type="primary" @click="selectCoupon()">增加优惠券</el-button>
            </el-form-item>
            <el-form-item label="赠送红包" label-width="85px">

              <ul style="list-style: none;text-align:left;margin: 0; padding: 0">
                <div :key="'red3'+item.redid" v-for="item in mktactivForm.redenvs" class="walletitem">
                  <i class="el-icon-error iconshanchu" @click="deleteRedenv(item.redid)"></i>
                  <div class="right">
                    <el-row :gutter="24" type="flex">
                      <el-col :span="14">{{ item.redname }}{{ item.status == '' ? '' : (item.status) }}</el-col>
          ...
          <el-form-item label="活动规则" v-if="mktactivForm.matype=='4'">
            <div class="rechargeitem">
              <div style="margin-bottom: 10px">
                <span style="color:red;">*</span>&nbsp;&nbsp;<font style="font-weight: bolder">触发条件</font>&nbsp;&nbsp;
              </div>
              <div style="margin-bottom: 10px">
                <el-checkbox v-model="mktactivForm.isJe" style="margin-left: 5px;">消费金额满</el-checkbox>
                <el-input type="number" min="0" v-model="mktactivForm.minje" style="width:100px; margin: 0 10px"
                          oninput="value=value.replace(/[^\d]/g,'')"/>
                元&nbsp;&nbsp;
              </div>
              <div style="margin-bottom: 10px">
                <el-checkbox v-model="mktactivForm.isBirthday" style="margin-left: 5px;" @change="changeBirthday">生日当天
                </el-checkbox>
              </div>
              <div style="margin-bottom: 10px">
                <el-checkbox v-model="mktactivForm.isServzd" style="margin-left: 5px;" @change="addFirstzd()">指定项目
                </el-checkbox>
                ...
                <el-popover style="padding-left: 10px;"
                  ...
                  <ul class="tips-content">
                    <li>添加单个指定项目时，表示开单时任选一个或一个以上指定项目，可以享受优惠。</li>
                    <li>如果添加多个组合，则表示开单时，必须在每个组合中任选一个或以上，才能享受优惠。</li>
                    <li>最多可支持创建5个指定项目组.</li>
                  </ul>
                </el-popover>
              </div>
              <div :key="'rule4'+itemRule.servzdid" v-for="itemRule in mktactivForm.zscomlist">
                指定{{ itemRule.servzdid }}&nbsp;&nbsp;<el-select placeholder="请选择" style="width: 100px;"
                                                                v-model.trim="itemRule.type"
                                                                @change="selectType($event,itemRule.type)">
                <el-option :key="item.value" :label="item.name" :value="item.value"
                           v-for="item in classifi"></el-option>
              </el-select>&nbsp;&nbsp;&nbsp;<el-button type="primary"
                                                       @click="selectServ2(itemRule.servzdid, itemRule.type)">添加
              </el-button>
                <i class="el-icon-error iconshanchu" @click="deleteZscom(itemRule.servzdid)"></i>
                <div class="walletitem ser1" v-show="itemRule.zscoms && itemRule.zscoms.length>0">
                  <div class="item1" :key="'serv'+item.servzdid+''+item.servid" v-for="item in itemRule.zscoms">
                    <i class="el-icon-error iconshanchu" @click="deleteServ2(item.servzdid, item.servid)"
                       style="vertical-align: middle"></i>
                    <ul style="display: inline-block">{{ item.servname }}</ul>
                  </div>
                </div>
              </div>
              <div style="margin-bottom: 10px">
              ...
              <div style="margin-bottom: 10px">
                优惠形式：
                <el-select placeholder="请选择" style="width: 120px;" v-model.trim="mktactivForm.ptype">
                  <el-option :key="item.value" :label="item.name" :value="item.value"
                             v-for="item in ptypes"></el-option>
                </el-select>&nbsp;&nbsp;
                <el-input type="number" style="width:100px" min="0" v-model="mktactivForm.pvalue"/>
                <span v-if="mktactivForm.ptype=='2'">折</span>
                <span v-if="mktactivForm.ptype=='1'">元</span>
              </div>
              <div style="margin-bottom: 10px">
                最高减额：
                <el-input :disabled = "mktactivForm.ptype=='1'" type="number" min="0" v-model="mktactivForm.maxsub" style="width: 120px"></el-input>
                元&nbsp;（默认为0.0表示不限）
              </div>
              <div style="margin-bottom: 10px">
              ...
              <el-form-item label="赠送优惠券" label-width="85px">
                <ul style="list-style: none;text-align:left;margin: 0; padding: 0">
                  <div :key="'cou4'+item.cpid" v-for="item in mktactivForm.coupons3" class="walletitem">
                    <i class="el-icon-error iconshanchu" @click="deleteCoupon3(item.cpid)"></i>
                    <div class="right">
                      <el-row :gutter="24" type="flex">
                        <el-col :span="14">{{ item.cpname }}{{ item.status == '' ? '' : (item.status) }}</el-col>
                        <el-col :span="10" style="text-align: right">{{ item.expTime }}</el-col>
                ...
                <el-button type="primary" @click="selectCoupon3">增加优惠券</el-button>
              </el-form-item>
              <el-form-item label="赠送红包" label-width="85px">

                <ul style="list-style: none;text-align:left;margin: 0; padding: 0">
                  <div :key="'red4'+item.redid" v-for="item in mktactivForm.redenvs3" class="walletitem">
                    <i class="el-icon-error iconshanchu" @click="deleteRedenv3(item.redid)"></i>
                    <div class="right">
                      <el-row :gutter="24" type="flex">
                        <el-col :span="14">{{ item.redname }}{{ item.status == '' ? '' : (item.status) }}</el-col>
                        <el-col :span="10" style="text-align: right">{{ item.expTime }}</el-col>
                ...
                <el-button type="primary" @click="selectRedenv3">增加红包</el-button>
              </el-form-item>

              <el-form-item label="赠送项目" label-width="85px">
                <el-button type="primary" @click="selectServ3" style="margin-bottom: 15px">增加项目</el-button>
                <div class="walletitem ser" v-show="mktactivForm.servs3 && mktactivForm.servs3.length>0">
                  <div class="item first">
                    <i></i>
                    <ul class="one">项目名称</ul>
                    <ul class="two">次数</ul>
                  ...
                  <div class="item" :key="'serv'+item.servid" v-for="item in mktactivForm.servs3">
                    <i class="el-icon-error iconshanchu" @click="deleteServ3(item.servid)"></i>
                    <ul class="one">{{ item.servname }}</ul>
                    <ul class="two">
                      <el-input type="number" min="1" v-model="item.totalNum" style="width:60px;margin-right:5px"
                                placeholder="次数"/>
                    ...
                    <ul class="four">
                      ...
                      <el-checkbox style="width: 50px; margin-right: 5px" v-model="item.timeType2Checked"
                                   @change="changSerstime3()">长期
                      </el-checkbox>
                      <el-checkbox style="width: 80px; margin-right: 5px" v-model="item.timeType3Checked"
                                   @change="changSersdate3()">截止日期
                      </el-checkbox>
                      <el-date-picker
                          v-model="item.edate"
                          type="date" value-format="yyyy-MM-dd HH:mm:ss"
              ...
              <div style="margin-bottom: 10px">
                <el-checkbox v-model="mktactivForm.isShared" style="margin-left: 5px;">不可与其他优惠同享</el-checkbox>
                <el-popover style="padding-left: 10px;"
                            placement="right"
                            title="提示"
                            width="380"
                  ...
                  <el-button slot="reference"><i class="el-icon-question"></i></el-button>
                  <ul class="tips-content">
                    <li>有指定优惠券，则仅不可与指定优惠券同享。 无指定券，则不可与所有券同享。</li>
                  </ul>
                </el-popover>
              </div>
              <el-form-item label="选择优惠券" label-width="85px">
                <ul style="list-style: none;text-align:left;margin: 0; padding: 0">
                  <div :key="'cou5'+item.cpid" v-for="item in mktactivForm.couponZsdiffs" class="walletitem">
                    <i class="el-icon-error iconshanchu" @click="deleteCoupon4(item.cpid)"></i>
                    <div class="right">
                      <el-row :gutter="24" type="flex">
                        <el-col :span="14">{{ item.cpname }}{{ item.status == '' ? '' : (item.status) }}</el-col>
                        <el-col :span="10" style="text-align: right">{{ item.expTime }}</el-col>
                ...
                <el-button type="primary" @click="selectCoupon4">增加优惠券</el-button>
              </el-form-item>
              <el-form-item label="选择红包" label-width="85px">

                <ul style="list-style: none;text-align:left;margin: 0; padding: 0">
                  <div :key="'red5'+item.redid" v-for="item in mktactivForm.redenvZsdiffs" class="walletitem">
                    <i class="el-icon-error iconshanchu" @click="deleteRedenv4(item.redid)"></i>
                    <div class="right">
                      <el-row :gutter="24" type="flex">
                        <el-col :span="14">{{ item.redname }}{{ item.status == '' ? '' : (item.status) }}</el-col>
                        <el-col :span="10" style="text-align: right">{{ item.expTime }}</el-col>
                ...
                <el-button type="primary" @click="selectRedenv4">增加红包</el-button>
              </el-form-item>
              <div style="margin-bottom: 10px">
                <el-checkbox v-model="mktactivForm.isPd" style="margin-left: 5px;" :disabled="isEditPd">支持拼单
                </el-checkbox>
                <br>
                最少拼单人数
                <el-input type="number" min="0" v-model="mktactivForm.minpdnum" style="width:100px;"
                          oninput="value=value.replace(/[^\d]/g,'')"/>
                人
                <el-popover style="padding-left: 10px;"
                            placement="right"
                  ...
                  <el-button slot="reference"><i class="el-icon-question"></i></el-button>
                  <ul class="tips-content">
                    <li>表示可多个单据合并结账时，判断所有单据中的内容合并起来是否符合触发条件。如果符合则每个单据都可以享受优惠。</li>
                    <li>设置最少拼单人数，则需要合并结账单据数量满足最少拼单人数时，才开始参与活动。</li>
                    <li>默认值为0，当设置为0时，表示不限制最少拼单数。</li>
                    <li>例如：最少拼单数为3，则表示，在拼单情况下买单，需要达到最少3个单据一起结账才可以享受优惠。</li>
                  </ul>
                </el-popover>
              </div>
              <div style="margin-bottom: 10px">
          ...
          <el-form-item label="活动描述"
                        v-if="mktactivForm.matype=='2' || mktactivForm.matype=='3' || mktactivForm.matype=='4'">
            <template>
    ...
    <yid-dialog :title="redenvDialog.title" :visible.sync="redenvDialog.visible" @close="redenvDialog.visible=false"
                width="700px">
      <el-form label-width="100px" ref="copyForm1">
        <el-form-item label="红包：">
          <el-input clearable v-model="redenvDialog.rname" placeholder="红包名称/编码" style="width: 160px;"></el-input>
          <el-button type="primary" @click="queryRedenv()" style="margin-left: 20px;">查询</el-button>
          <el-button type="primary" @click="setRedenv()" style="margin-left: 20px;">确认选择</el-button>
        </el-form-item>
        <yid-table ref="tableRedenv" style="margin-top: 10px;" :data="redenvDialog.redenvs"
                   @selection-change="handleSelectionChangeRedenv">
          <el-table-column type="selection" width="55"></el-table-column>
          <yid-table-column label="编码" min-width="60" prop="rcode"></yid-table-column>
          <yid-table-column label="红包名称" min-width="100" prop="rname"></yid-table-column>
          <yid-table-column label="价值" min-width="70" prop="money"></yid-table-column>
          <yid-table-column label="有效期" min-width="160" prop="expTime"></yid-table-column>
          <yid-table-column label="门店使用" min-width="160" prop="rule"></yid-table-column>
        </yid-table>
      </el-form>
    </yid-dialog>

    <yid-dialog :title="redenvDialog2.title" :visible.sync="redenvDialog2.visible" @close="redenvDialog2.visible=false"
                width="700px">
      <el-form label-width="100px" ref="copyForm10">
        <el-form-item label="红包：">
          <el-input clearable v-model="redenvDialog2.rname" placeholder="红包名称/编码" style="width: 160px;"></el-input>
          <el-button type="primary" @click="queryRedenv2()" style="margin-left: 20px;">查询</el-button>
          <el-button type="primary" @click="setRedenv2()" style="margin-left: 20px;">确认选择</el-button>
        </el-form-item>
        <yid-table ref="tableRedenv" style="margin-top: 10px;" :data="redenvDialog2.redenvs"
    ...
    <yid-dialog :title="redenvDialog3.title" :visible.sync="redenvDialog3.visible" @close="redenvDialog3.visible=false"
                width="700px">
      <el-form label-width="100px" ref="copyForm11">
        <el-form-item label="红包：">
          <el-input clearable v-model="redenvDialog3.rname" placeholder="红包名称/编码" style="width: 160px;"></el-input>
          <el-button type="primary" @click="queryRedenv3()" style="margin-left: 20px;">查询</el-button>
          <el-button type="primary" @click="setRedenv3()" style="margin-left: 20px;">确认选择</el-button>
        </el-form-item>
        <yid-table ref="tableRedenv" style="margin-top: 10px;" :data="redenvDialog3.redenvs"
                   @selection-change="handleSelectionChangeRedenv">
          <el-table-column type="selection" width="55"></el-table-column>
          <yid-table-column label="编码" min-width="60" prop="rcode"></yid-table-column>
          <yid-table-column label="红包名称" min-width="100" prop="rname"></yid-table-column>
    ...
    <yid-dialog :title="redenvDialog4.title" :visible.sync="redenvDialog4.visible" @close="redenvDialog4.visible=false"
                width="700px">
      <el-form label-width="100px" ref="copyForm1">
        <el-form-item label="红包：">
          <el-input clearable v-model="redenvDialog4.rname" placeholder="红包名称/编码" style="width: 160px;"></el-input>
          <el-button type="primary" @click="queryRedenv4()" style="margin-left: 20px;">查询</el-button>
          <el-button type="primary" @click="setRedenv4()" style="margin-left: 20px;">确认选择</el-button>
        </el-form-item>
        <yid-table ref="tableRedenv" style="margin-top: 10px;" :data="redenvDialog4.redenvs"
                   @selection-change="handleSelectionChangeRedenv">
          <el-table-column type="selection" width="55"></el-table-column>
          <yid-table-column label="编码" min-width="60" prop="rcode"></yid-table-column>
          <yid-table-column label="红包名称" min-width="100" prop="rname"></yid-table-column>
    ...
    <yid-dialog :title="servDialog.title" :visible.sync="servDialog.visible" @close="servDialog.visible=false"
                width="800px">
      <el-form label-width="100px" ref="copyForm3">
        <el-form-item label="筛选：">
          <el-select clearable v-model="servDialog.branch"
                     @change="changeBranch(servDialog.branch,'1')" placeholder="选择部门" style="width: 140px;">
            <el-option v-for="item in servDialog.branchs"
                       :key="'bra'+item.id"
                       :value="item.id"
                       :label="item.bname"></el-option>
          </el-select>
          <el-select clearable v-model="servDialog.serviceType" placeholder="请选择分类"
          ...
          <el-input clearable v-model="servDialog.sername" placeholder="项目名称/编码"
                    style="margin-left: 5px;width: 160px;"></el-input>
          <el-button type="primary" @click="setServs()" style="margin-left: 10px;">确认添加</el-button>
          <el-button type="primary" @click="servDialog.visible=false">取消</el-button>
        </el-form-item>
        <el-form-item label="">
          <table border="1" style="width: 100%" class="serve-table">
            <tr>
              ...
              <th width="25%">分类</th>
              <th width="55%">
                <el-checkbox v-model="servDialog.checkbox" @change="checkServ"></el-checkbox>
                (编号）项目名称
              </th>
            </tr>
            <tr :key="'servs'+item.servid"
                v-for="item in servDialog.services"
                v-if="screenService(item)">
              <td>{{ item.bname }}</td>
              <td>{{ item.typename }}</td>
              <td>
                <el-checkbox v-model="item.type" :value="item.servid"></el-checkbox>
    ...
    <yid-dialog :title="servDialog2.title" :visible.sync="servDialog2.visible" @close="servDialog2.visible=false"
                width="800px">
      <el-form label-width="100px" ref="copyForm4">
        <el-form-item label="筛选：">
          <el-select clearable v-model="servDialog2.branch"
                     @change="changeBranch2(servDialog2.branch,'1')" placeholder="选择部门" style="width: 140px;">
            <el-option v-for="item in servDialog2.branchs"
                       :key="'bra'+item.id"
                       :value="item.id"
                       :label="item.bname"></el-option>
          </el-select>
          <el-select clearable v-model="servDialog2.serviceType" placeholder="请选择分类"
                     style="margin-left: 5px; width: 140px;">
            <el-option
          ...
          <el-input clearable v-model="servDialog2.sername" placeholder="项目名称/编码"
                    style="margin-left: 5px;width: 160px;"></el-input>
          <el-button type="primary" @click="setServs2(1)" style="margin-left: 10px;">确认添加</el-button>
          <el-button type="primary" @click="servDialog2.visible=false">取消</el-button>
        </el-form-item>
        <el-form-item label="">
          <table border="1" style="width: 100%" class="serve-table">
            <tr>
              ...
              <th width="25%">分类</th>
              <th width="55%">
                <el-checkbox v-model="servDialog2.checkbox" @change="checkSer2"></el-checkbox>
                (编号）项目名称
              </th>
            </tr>
            <tr :key="'servs'+item.servid"
    ...
    <yid-dialog :title="servDialog3.title" :visible.sync="servDialog3.visible" @close="servDialog3.visible=false"
                width="800px">
      <el-form label-width="100px" ref="copyForm10">
        <el-form-item label="筛选：">
          <el-select clearable v-model="servDialog3.branch"
                     @change="changeBranch3(servDialog3.branch,'1')" placeholder="选择部门" style="width: 140px;">
            <el-option v-for="item in servDialog3.branchs"
                       :key="'bra'+item.id"
                       :value="item.id"
                       :label="item.bname"></el-option>
          </el-select>
          <el-select clearable v-model="servDialog3.serviceType" placeholder="请选择分类"
                     style="margin-left: 5px; width: 140px;">
            <el-option
          ...
          <el-input clearable v-model="servDialog3.sername" placeholder="项目名称/编码"
                    style="margin-left: 5px;width: 160px;"></el-input>
          <el-button type="primary" @click="setServs3()" style="margin-left: 10px;">确认添加</el-button>
          <el-button type="primary" @click="servDialog3.visible=false">取消</el-button>
        </el-form-item>
        <el-form-item label="">
          <table border="1" style="width: 100%" class="serve-table">
            <tr>
              ...
              <th width="25%">分类</th>
              <th width="55%">
                <el-checkbox v-model="servDialog3.checkbox" @change="checkSer3"></el-checkbox>
                (编号）项目名称
              </th>
            </tr>
            <tr :key="'servs'+item.servid"
    ...
    <yid-dialog :title="servDialog4.title" :visible.sync="servDialog4.visible" @close="servDialog4.visible=false"
                width="800px">
      <el-form label-width="100px" ref="copyForm4">
        <el-form-item label="筛选：">
          <el-select clearable v-model="servDialog4.branch"
                     @change="changeBranch2(servDialog4.branch,'2')" placeholder="选择部门" style="width: 140px;">
            <el-option v-for="item in servDialog4.branchs"
                       :key="'bra'+item.id"
                       :value="item.id"
                       :label="item.bname"></el-option>
          </el-select>
          <el-button type="primary" @click="setServs2(2)" style="margin-left: 10px;">确认添加</el-button>
          <el-button type="primary" @click="servDialog4.visible=false">取消</el-button>
        </el-form-item>
        <el-form-item label="">
          <table border="1" style="width: 100%" class="serve-table">
            <tr>
              <th width="20%">所属部门</th>
              <th width="25%">
                <el-checkbox v-model="servDialog4.checkbox" @change="checkSer4"></el-checkbox>
                分类
              </th>
            </tr>
            <tr :key="item.id"
                v-for="item in servDialog4.serviceTypes">
              <td>{{ item.branchName }}</td>
              <td>
                <el-checkbox @change="selectOk($event,item)" v-model="item.type" :value="item.id"></el-checkbox>
    ...
    <yid-dialog :title="goodDialog.title" :visible.sync="goodDialog.visible" @close="goodDialog.visible=false"
                width="800px">
      <el-form label-width="120px" ref="copyForm4">
        <el-form-item label="筛选：">
          <el-select clearable v-model="goodDialog.branch"
                     @change="changeBranch(goodDialog.branch,'2')" placeholder="所属部门" style="width: 140px;">
            <el-option v-for="item in goodDialog.branchs"
                       :key="'br'+item.id"
                       :value="item.id"
                       :label="item.bname"></el-option>
          </el-select>
          <el-select clearable v-model="goodDialog.productType" placeholder="请选择分类"
                     style="margin-left: 5px; width: 140px;">
            <el-option
          ...
          <el-input clearable v-model="goodDialog.goodname" placeholder="产品名称/编码"
                    style="margin-left: 5px;width: 160px;"></el-input>
          <el-button type="primary" @click="setGoods()" style="margin-left: 10px;">确认添加</el-button>
          <el-button type="primary" @click="goodDialog.visible=false">取消</el-button>
        </el-form-item>
        <el-form-item label="">
          <table border="1" style="width: 100%" class="serve-table">
            <tr>
            ...
            <tr :key="'prd'+item.pid" v-for="item in goodDialog.products"
    ...
    <yid-dialog :title="packDialog.title" :visible.sync="packDialog.visible" @close="packDialog.visible = false"
                width="700px">
      <el-form label-width="120px" ref="copyForm5">
        <el-form-item label="套餐：">
          <el-input placeholder="套餐名称/编码" style="width: 200px" v-model="packDialog.pcodename" clearable></el-input>
        </el-form-item>
        <el-form-item label="">
          <table border="1" style="width: 100%" class="serve-table">
            <tr>
              <th width="25%">所属品牌</th>
              <th width="55%">
                <el-checkbox @change="checkPack" v-model="packDialog.checkbox"></el-checkbox>
                (编号）套餐名称
              </th>
            </tr>
            <tr :key="'packa'+item.spid"
                v-for="item in packDialog.packages"
                v-if="screenPackage(item)">
              <td>{{ item.brandname }}</td>
              <td>
                <el-checkbox v-model="item.type" :value="item.spid"></el-checkbox>
        ...
        <el-form-item label="">
          ...
          <el-button type="primary" @click="packDialog.visible = false">取消</el-button>
        </el-form-item>
      </el-form>
    </yid-dialog>

    <yid-dialog :title="couponDialog.title" :visible.sync="couponDialog.visible" @close="couponDialog.visible=false"
                width="700px">
      <el-form label-width="100px" ref="copyForm6">
        <el-form-item label="优惠券：">
          <el-input clearable v-model="couponDialog.cpcodename" placeholder="优惠券名称/编码" style="width: 160px;"></el-input>
          <el-button type="primary" @click="queryCoupon()" style="margin-left: 20px;">查询</el-button>
          <el-button type="primary" @click="setCoupons()" style="margin-left: 20px;">确认选择</el-button>
        </el-form-item>
        <yid-table ref="tableCoupon" style="margin-top: 10px;" :data="couponDialog.coupons"
                   @selection-change="handleSelectionChangeCoupon">
          <el-table-column type="selection" width="50"></el-table-column>
          <yid-table-column label="编码" min-width="60" prop="cpcode"></yid-table-column>
          <yid-table-column label="优惠券名称" min-width="100" prop="cpname"></yid-table-column>
          <yid-table-column label="价值" min-width="80" prop="couponType">
            <template slot-scope="scope">
              <div v-if="scope.row.couponType =='1' ">{{ "定额" + scope.row.discount + "元" }}</div>
              <div v-if="scope.row.couponType =='2' ">{{ "折扣" + scope.row.discount + "折" }}</div>
            </template>
          </yid-table-column>
          <yid-table-column label="有效期" min-width="160" prop="expTime"></yid-table-column>
          <yid-table-column label="门店使用" min-width="160" prop="rule"></yid-table-column>
    ...
    <yid-dialog :title="couponDialog2.title" :visible.sync="couponDialog2.visible" @close="couponDialog2.visible=false"
                width="700px">
      <el-form label-width="100px" ref="copyForm7">
        <el-form-item label="优惠券：">
          <el-input clearable v-model="couponDialog2.cpcodename" placeholder="优惠券名称/编码"
                    style="width: 160px;"></el-input>
          <el-button type="primary" @click="queryCoupon2()" style="margin-left: 20px;">查询</el-button>
          <el-button type="primary" @click="setCoupons2()" style="margin-left: 20px;">确认选择</el-button>
        </el-form-item>
        <yid-table ref="tableCoupon2" style="margin-top: 10px;" :data="couponDialog2.coupons"
    ...
    <yid-dialog :title="couponDialog3.title" :visible.sync="couponDialog3.visible" @close="couponDialog3.visible=false"
                width="700px">
      <el-form label-width="100px" ref="copyForm8">
        <el-form-item label="优惠券：">
          <el-input clearable v-model="couponDialog3.cpcodename" placeholder="优惠券名称/编码"
                    style="width: 160px;"></el-input>
          <el-button type="primary" @click="queryCoupon3()" style="margin-left: 20px;">查询</el-button>
          <el-button type="primary" @click="setCoupons3()" style="margin-left: 20px;">确认选择</el-button>
        </el-form-item>
        <yid-table ref="tableCoupon3" style="margin-top: 10px;" :data="couponDialog3.coupons"
                   @selection-change="handleSelectionChangeCoupon">
          <el-table-column type="selection" width="50"></el-table-column>
          <yid-table-column label="编码" min-width="60" prop="cpcode"></yid-table-column>
          <yid-table-column label="优惠券名称" min-width="100" prop="cpname"></yid-table-column>
    ...
    <yid-dialog :title="couponDialog4.title" :visible.sync="couponDialog4.visible" @close="couponDialog4.visible=false"
                width="700px">
      <el-form label-width="100px" ref="copyForm9">
        <el-form-item label="优惠券：">
          <el-input clearable v-model="couponDialog4.cpcodename" placeholder="优惠券名称/编码"
                    style="width: 160px;"></el-input>
          <el-button type="primary" @click="queryCoupon4()" style="margin-left: 20px;">查询</el-button>
          <el-button type="primary" @click="setCoupons4()" style="margin-left: 20px;">确认选择</el-button>
        </el-form-item>
        <yid-table ref="tableCoupon4" style="margin-top: 10px;" :data="couponDialog4.coupons"
                   @selection-change="handleSelectionChangeCoupon">
          <el-table-column type="selection" width="50"></el-table-column>
          <yid-table-column label="编码" min-width="60" prop="cpcode"></yid-table-column>
          <yid-table-column label="优惠券名称" min-width="100" prop="cpname"></yid-table-column>
    ...
    <yid-dialog :title="paytypeDialog.title" :visible.sync="paytypeDialog.visible" @close="paytypeDialog.visible=false"
                width="800px">
      <el-form label-width="100px" ref="copyForm12">
        <el-form-item label="筛选：">
          <el-select clearable v-model="paytypeDialog.payMold" placeholder="请选择分类"
                     style="margin-left: 5px; width: 140px;">
            <el-option
          ...
          <el-button type="primary" @click="setpaytypes()" style="margin-left: 10px;">确认添加</el-button>
          <el-button type="primary" @click="paytypeDialog.visible=false">取消</el-button>
        </el-form-item>
        <el-form-item label="">
          <table border="1" style="width: 100%" class="serve-table">
            <tr>
              <th width="30%">
                <el-checkbox v-model="paytypeDialog.checkbox" @change="checkPaytype"></el-checkbox>
                编码
              </th>
              <th width="50%">付款方式名称</th>
              <th width="20%">类型</th>
            </tr>
            <tr :key="'paytypes'+item.id"
...
export default {
  ...
  data() {
    return {
      ...
      type: '',
      duoflag: false,
      matypes: [{name: '生日赠送', value: '1'}, {name: '开卡充值赠送', value: '2'}, {name: '购买套餐赠送', value: '3'}, {
        name: '服务开单赠送',
        value: '4'
      }],
      mastatuss: [{name: '未开始', value: '1'}, {name: '进行中', value: '2'}, {name: '暂停中', value: '3'}, {
        name: '已结束',
        value: '4'
      }],
      timeunits: [{name: '年', value: '1'}, {name: '月', value: '2'}, {name: '日', value: '3'}],
      ptypes: [{name: '固定金额', value: '1'}, {name: '折扣', value: '2'}],
      pageInfo: {
        page: 1,
        limit: 10,
      },
  ...
  methods: {
    ...
    showEdit(row, value) {
      ...
      if (this.mktactivForm.matype == "2") {
        ...
        this.mktactivForm.mktactivRules.forEach(rule => {
          //rule.recharge.presale = rule.recharge.ispresale==="1"?true:false;
          this.$set(rule.recharge, 'presale', rule.recharge.ispresale === "1" ? true : false)
    ...
    saveMktactiv(status) {
      ...
      this.$refs["mktactivForm"].validate(valid => {
        if (valid) {
          ...
          if (that.mktactivForm.timeType == '2' && that.mktactivForm.mktdate.length == 0) {
          ...
          let errmsg = "5001";
          //使用门店类型：所有门店
          if (that.mktactivForm.matype == "1") {//生日赠送
          ...
          } else if (that.mktactivForm.matype == "2") {//开卡充值赠送
            if (that.mktactivForm.firstOrderFlag) {
              that.mktactivForm.firstOrder = '1'
            } else {
              that.mktactivForm.firstOrder = '0'
            }
            if (that.mktactivForm.cktype == '1') {// 可累计
              if (that.mktactivForm.ckdatetype == "1") {// 固定时间
              ...
              } else if (that.mktactivForm.ckdatetype == "2") {//相对时间
            ...
            try {
              ...
              if (this.mktactivForm.isNopaytype) {
                if (this.mktactivForm.nopaytypes.length == 0) {
                  yid.util.error("勾选了'不支持使用以下支付方式'后必须添加一个或多个支付方式!");
                  return false;
                }
              }
              ...
              that.mktactivForm.mktactivRules.forEach((rule, index) => {
                ...
                if (rule.recharge.isCustom) {
                  ...
                  if (rule.recharge.giftMoney === '' && rule.redenvs.length == 0 && rule.servs.length == 0 && rule.products.length == 0) {
                ...
                rule.servs.forEach(item => {
                  ...
                  if (!item.timenum && !item.timeType2Checked && !item.timeType3Checked) {
                    temp += '有效期没选填，';
                  }
          ...
          } else if (that.mktactivForm.matype == "3") {//购买套餐赠送
            that.mktactivForm.firstOrder = '';
            if (yid.util.isEmpty(that.mktactivForm.isPnum)) {
              that.mktactivForm.isPnum = '0'
            } else {
            ...
            if (that.mktactivForm.coupons.length == 0 && that.mktactivForm.redenvs.length == 0) {
          ...
          } else if (that.mktactivForm.matype == "4") {//服务开单活动
            if (yid.util.isEmpty(that.mktactivForm.minje) && yid.util.isEmpty(that.mktactivForm.zscomlist)) {
    ...
    deleteShop(row, name) {
      yid.util.confirm('删除【' + row.shopname + '】后，本店将不再参与【' + name + '】且已参与的赠送在本店也不能继续使用，确定要删除吗？', '', '', () => {
        let i = -1;
...
//                    })
    ...
    setServs() {
      ...
      that.servDialog.services.forEach(item => {
        if (item.type && (!that.servDialog.branch || item.branch == that.servDialog.branch)
            && (!that.servDialog.serviceType || item.servtid == that.servDialog.serviceType)) {
          let flag = true;
          servs.forEach(serv => {
    ...
    setServs2(tag) {
      if (tag == 1) {
        ...
        that.servDialog2.services.forEach(item => {
          if (item.type && (!that.servDialog2.branch || item.branch == that.servDialog2.branch)
              && (!that.servDialog2.serviceType || item.servtid == that.servDialog2.serviceType)) {
            let flag = true;
            zscoms.forEach(serv => {
              if (item.servid == serv.servid) {
                flag = false;
        ...
        this.servDialog4.serviceTypes.forEach(item => {
          if (item.type && (!this.servDialog4.branch || item.branch.toString() == this.servDialog4.branch)) {
            let flag = true;
            zscoms.forEach(serv => {
    ...
    setServs3() {
      ...
      that.servDialog3.services.forEach(item => {
        if (item.type && (!that.servDialog3.branch || item.branch == that.servDialog3.branch)
            && (!that.servDialog3.serviceType || item.servtid == that.servDialog3.serviceType)) {
          let flag = true;
          servs3.forEach(serv => {
            if (item.servid == serv.servid) {
              flag = false;
    ...
    setpaytypes() {
      ...
      that.paytypeDialog.paytypes.forEach(item => {
        if (item.type && (!that.paytypeDialog.payMold || item.ptid == that.paytypeDialog.payMold)) {
          let flag = true;
    ...
    setGoods() {
      ...
      that.goodDialog.products.forEach(item => {
        if (item.type && (!that.goodDialog.branch || item.branch == that.goodDialog.branch)
            && (!that.goodDialog.productType || item.servtid == that.goodDialog.productType)) {
          let flag = true;
    ...
    checkPack(val) {
      this.packDialog.packages.forEach(item => {
        if (!this.packDialog.pcodename) {
          item.type = val;
        } else if (this.packDialog.pcodename == item.spcode || item.spname.indexOf(this.packDialog.pcodename) != -1) {
          item.type = val;
        }
      })
    ...
    setPacks() {
      ...
      that.packDialog.packages.forEach(item => {
        if (item.type && (!that.packDialog.brandId || item.brandId == that.packDialog.brandId)) {
          let flag = true;
    ...
    init() {
      ...
      service.chain.servicepackage.findAll({isDel: '0', status: '1'}).then(res => {
  ...
  computed: {
    screenService() {
      return function (item) {
        ...
        if (this.servDialog.sername) {
          if (item.sdname.indexOf(this.servDialog.sername) !== -1 ||
    ...
    screenService3() {
      return function (item) {
        ...
        if (this.servDialog3.sername) {
          if (item.sdname.indexOf(this.servDialog3.sername) !== -1 ||
    ...
    screenProduct() {
      return function (item) {
        ...
        if (this.goodDialog.goodname) {
          if (item.sdname.indexOf(this.goodDialog.goodname) !== -1 ||
    ...
    screenPackage() {
      return function (item) {
        ...
        if (this.packDialog.pcodename == item.spcode || item.spname.indexOf(this.packDialog.pcodename) != -1) {
          flag = true
        } else {
          flag = false
        }
