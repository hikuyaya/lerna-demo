<template>
    <div>
            ...
            <div v-show="showList">
                ...
                <el-form inline :model="searchForm" ref="searchForm" style="margin-top: 8px">
                    ...
                    <el-form-item label="红包：">
                        <el-input clearable v-model="searchForm.pcodename" placeholder="红包名称/编码"
                ...
                <yid-table ref="table" style="margin-top: 10px;" pagination>
                    ...
                    <yid-table-column label="编码" min-width="80" prop="rcode"></yid-table-column>
                    <yid-table-column label="门店品牌" min-width="80" prop="brandName"></yid-table-column>
                    <yid-table-column label="红包名称" min-width="80" prop="rname"></yid-table-column>
                    ...
                    <yid-table-column label="门店使用" min-width="200">
                        <template slot-scope="scope">
                            <div v-if="scope.row.ismanytimes == '1'">
                                可分多次消费
                            </div>
                    ...
                    <yid-table-column label="操作" min-width="120" prop="code">
                        <template slot-scope="scope">
                            <el-link type="primary" @click="showEditPage(scope.row)">
                                {{scope.row.status == "0" || scope.row.status == "1" || scope.row.status == "2" ? "编辑" : "查看"}}
                            </el-link>
                            <el-link v-if="scope.row.status !='3' " type="primary" @click="changeStatus(scope.row)"
                                     style="margin-left: 6px">
                                {{scope.row.status == "0" || scope.row.status == "2" ? "发布" : "停用"}}
                            </el-link>
                            <el-link style="margin-left: 6px" type="primary" @click="showRecordsDialog(scope.row)">明细
                            </el-link>
                        </template>
                    </yid-table-column>
                </yid-table>
        ...
        <el-collapse-transition>
            <div v-show="showEdit">
                <EditRedEnvelope ref="editRedEnvelope" @onClose="closeEditPage($event)"/>
            </div>
        </el-collapse-transition>

        <yid-dialog :title="recordsDialog.title" :visible.sync="recordsDialog.visible" @close="cancelRecordsDialog"
                    width="1100px">
            <el-form inline :model="recordSearchForm" ref="recordSearchForm">
                <el-form-item label="活动名称：" prop="maname">
                    <el-select clearable v-model.trim="recordSearchForm.maname" @clear="clearFormRecordName" filterable
                               :filter-method="filterActivity" placeholder="请选择活动" style="width: 215px;">
            ...
            <yid-table ref="recordsTable" style="margin-top: 10px;" pagination>
                ...
                <yid-table-column label="红包码" min-width="80" prop="bcode"></yid-table-column>
                ...
                <yid-table-column label="使用记录" min-width="80">
                    <template slot-scope="scope">
                        <el-button type="text" @click="expandRecord(scope.row)">查看</el-button>
                    </template>
                </yid-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            changeStatus(row) {
                let tips = row.status == "0" || row.status == "2" ? "保存发布后，红包将不能再次编辑！" : "停用后，活动中有赠送该红包的将停止赠送，但不影响已领取红包的使用，确定要停用吗？"
