<template>
  <div class="assignment">
    <div class="aside">
      ...
      <div class="linkWrap">
        <el-button
          v-for="(item, key) in sortActionbutton"
          :key="key"
          @click="getMemByeeid(item.id)">
    ...
    <yid-dialog
      :title="turnDialog.title"
      :visible.sync="turnDialog.visible"
      @close=""
      width="400px">
      <el-form label-width="140px" ref="copyForm">
        <el-form-item label="原所属员工：">
          <el-select
            ...
            value-key="id"
            clearable
            v-model="turnForm.oldEmployeeObj"
            filterable
            placeholder="请选择"
            style="width: 160px">
            <el-option
        ...
        <el-form-item label="现所属员工：">
          <el-select
            value-key="id"
            clearable
            v-model="turnForm.newEmployeeObj"
            filterable
            placeholder="请选择"
            style="width: 160px">
            <el-option
  ...
  methods: {
    ...
    getEeList() {
      if (this.shops != null && this.shops != '') {
        ...
        //获取已分配员工个数
        service.employee
          .geteeNum({ isDel: '0', shopid: this.shops })
          .then(res => {
            if (res.resp_code == 200) {
    ...
    findEenum() {
      ...
      if (oldeeid != null) {
        service.employee
          .geteeNum({ isDel: '0', shopid: this.shops, id: oldeeid })
          .then(res => {
            if (res.resp_code == 200) {
