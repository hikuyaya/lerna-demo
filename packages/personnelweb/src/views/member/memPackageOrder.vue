  ...
  <div class="memPackageOrder">
    ...
    <el-collapse-transition>
      <div v-show="!showList">
        ...
        会员卡号：
        <el-input
          v-model="billForm.cardno"
          placeholder="请输入卡号"
          style="width: 150px"></el-input>
        &nbsp;&nbsp;
        <el-link type="primary" @click="queryMemInfo" v-show="showButton"
          >查询</el-link
        >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 会员姓名：{{ billForm.memname
        }}<br /><br />
        套餐项目信息：<el-button
          v-show="showButton"
          @click="alertPackageDialog"
          type="primary"
          >选择项目</el-button
        >
        ...
        <yid-table
          ...
          <yid-table-column label="操作" min-width="60" fixed="right">
            <template slot-scope="scope">
              <el-button
                type="text"
                @click="deleteServ(scope.$index, scope.row)"
                style="margin-left: 6px; color: red"
                >删除</el-button
              >
            </template>
        ...
        <yid-table
          ...
          <yid-table-column
            label="现到期日期"
            min-width="150"
            prop="nexpiryTime"></yid-table-column>
        </yid-table>
      </div>
    </el-collapse-transition>
      ...
      <div style="display: flex">
        <el-form ref="searchpackageForm" inline :model="searchpackageForm">
          <el-form-item label="项目：" prop="scodename">
            <el-input
              v-model="searchpackageForm.scodename"
              clearable
              style="width: 150px"></el-input>
          </el-form-item>
          <el-form-item label="">
        ...
        <yid-table
          ...
          <yid-table-column label="项目名称" min-width="120" prop="servname">
            <template slot-scope="scope">
              ...
              <span v-if="scope.row.isGift == 1"
                ><font color="red">(赠)</font>{{ scope.row.servname }}</span
              >
            </template>
          </yid-table-column>
          <yid-table-column
...
export default {
  ...
  methods: {
    ...
    searchPackages() {
      ...
      service.member.packagedesc
        .memPackagedescList({ memid: memid, scodename: scodename })
        .then(res => {
          ...
          for (let i = 0; i < this.orderitemData.length; i++) {
            for (let n = 0; n < this.packageData.length; n++) {
              if (
                this.orderitemData[i].scode == this.packageData[n].scode &&
                this.orderitemData[i].batchno == this.packageData[n].batchno
              ) {
