    ...
    <div class="member-relation">
        ...
        <yid-dialog :title="meminfoDialog.title" :visible.sync="meminfoDialog.visible" width="1150px">
            <el-row style="padding-left: 45px;margin: 0;" width="980px" justify="center">
                ...
                <el-button @click="meminfoDialog.showNum=3" type="primary" :plain="meminfoDialog.showNum!=3"
                           v-if="false">账户调整
                </el-button>
            ...
            <div class="meminfo" v-show="meminfoDialog.showNum==1">
                ...
                <table border="1" cellspacing="0">
                    ...
                    <tr>
                        ...
                        <td>
                            <el-link v-if="cardinfo.status=='1'" @click="openChangeCard(cardinfo.cardno)" type="primary"
                                     class="marg5">迁移卡
                            </el-link>
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td><label class="marg5" :class="cardinfo.type == '1' ? '' : 'relationCardText'" @click="relationCard(cardinfo.type)">{{cardinfo.type == '1' ? cardinfo.dawncmcode : String(cardinfo.type) === '' ? '' : '关联实体卡'}}</label></td>
                    </tr>
                </table>
                <div>套餐信息</div>
            ...
            <div class="meminfo" v-show="meminfoDialog.showNum==2">
                ...
                <el-row>
                    <el-col :offset="22" :span="1">
                        <el-button type="primary" @click="saveMemberCard()">确认修改</el-button>
                    </el-col>
                </el-row>
            </div>
            ...
            <div class="meminfo" v-show="meminfoDialog.showNum==7">
                ...
                <yid-table pagination ref="memberlog">
                    ...
                    <yid-table-column label="更改人" min-width="80" prop="createdBy"></yid-table-column>
                    <yid-table-column label="店号" min-width="60" prop="shopcode"></yid-table-column>
    ...
    export default {
        ...
        data() {
            return {
                ...
                inchanels: [{value: "1", name: '门店业务端'}, {value: "2", name: '门店管理端'},
                    {value: "3", name: '顾客手机端'}, {value: "4", name: '设计师端'}, {value: "8", name: '大系统同步'}],
                ...
                membercard: {
                    memid: "", memname: "", mobile: "", sex: "", birthday: "", cardid: "", cardtype: '',
            ...
            jsonStringFormate(str) {
                if (str) {
                    ...
                    try {
                        ...
                        list.forEach(l => {
                            info = info + l.payname + ":" + l.money + ":" + (l.payno ? l.payno : "") + " "
                        })
                        return info;
                    } catch (e) {
                        return str
        ...
        methods: {
            ...
            queryMemberStatic(yearmonth, offset) {
                ...
                service.member.memberinfo.queryMemberStatis({
                    shopid: this.shopid,
...
//                })
            ...
            searchMemberMobile(mobile) {
                ...
                service.member.memberinfo.queryMemberMobile({mobile: mobile}).then(res => {
                    if (res.resp_code == 200) {
                        if (null != res.data) {
            ...
            async sureVercode(mobile, code) {
                ...
                await service.member.memberinfo.sureVercode({mobile: mobile, code: code}).then(res => {
                    if (res.resp_code != 200) {
                        yid.util.error(res.resp_msg)
                        return
                    }
                })
                await service.member.memberinfo.checkMember({cardno: this.changeCardDialog.cardno}).then(res => {
                    if (res.resp_code != 200) {
                        yid.util.error(res.resp_msg)
                        return
                    }
            ...
            sureChangeCard() {
                this.sureVercode(this.changeCardDialog.mobile, this.changeCardDialog.vercode)
            },
            //查看卡备注
            queryCardContext(row) {
                this.cardMemo.visible = true
                this.cardMemo.title = '卡备注';
                service.member.memberinfo.queryCardShopMemo({
                    cardno: row.cardno,
                    shopid: row.shopid,
                    memid: this.usermemid
                }).then(res => {
            ...
            checkUpdateLog(row) {
                ...
                service.member.memberinfo.queryCardMemo({cardno: row.cardno, shopcode: row.shopcode}).then(res => {
                    this.updateMemo.data = res.data
                })
            },
            //点击关联实体卡
            relationCard(type){
                if(type == 0){              //0代表未关联，只允许未关联的进行设置
