<template>
    <div class="member-relation">
        <el-tabs v-model="activeName" @tab-click="handleClick">
            <el-tab-pane label="全部会员(总部)" name="chainMember">
                <el-form ref="chainMemberForm" inline :model="chainMemberForm" :rules="memrules">
                    <el-form-item label="请选择筛选条件：" prop="name">
                        <el-select style="width: 100px" v-model="chainMemberForm.code" placeholder="请选择">
                    ...
                    <el-form-item>
                        <el-input clearable v-model.trim="chainMemberForm.name" placeholder="手机号/卡号/会员姓名/会员ID"
                ...
                <yid-table ref="chainMemberTable">
                    ...
                    <yid-table-column prop="name" label="操作" width="70">
                        <template slot-scope="scope">
                            <el-link type="primary" style="margin: 0 10px 0 10px;" @click="getMember(scope.row)">详细
                            </el-link>
                        </template>
                    </yid-table-column>
                </yid-table>
            ...
            <el-tab-pane label="全部会员(门店)" name="shopMember">
                <el-form ref="shopMembertable" inline :model="shopMemberForm" :rules="rules">
                    ...
                    <el-form-item label="会员查询：">
                        <el-input clearable v-model.trim="shopMemberForm.name" placeholder="姓名/手机号/卡号"
                ...
                <div style="padding-bottom: 5px">
                    本次查询出{{memberHj.count}}位会员，卡金总余额:{{memberHj.zkj}}元, 赠送金总余额:{{memberHj.zzs}}元
                </div>
                <el-table :data="memberTables" :span-method="objectSpanMethod" style="width: 100%">
                    ...
                    <el-table-column label="卡账户信息" align="center">
                        ...
                        <el-table-column prop="status" label="状态" width="80">
                            <template slot-scope="scope">
                                ...
                                {{scope.row.status == "2" ? "挂失" : "" }}
                                {{scope.row.status == "3" ? "冻结" : "" }}
                                {{scope.row.status == "4" ? "作废" : "" }}
                            </template>
                        </el-table-column>
                    ...
                    <el-table-column prop="name" label="操作" width="60">
                        <template slot-scope="scope">
                            ...
                            <el-link type="primary" style="margin: 0 10px 0 10px;" @click="removeMem(scope.row)">移除
                            </el-link>
                        </template>
                    </el-table-column>
                </el-table>
            ...
            <el-tab-pane label="套餐会员" name="assignment">
                <el-form ref="memberpackfrom" inline :model="memberpackfrom" :rules="rules">
                    ...
                    <el-form-item label="套餐名称：">
                        <el-input clearable v-model="memberpackfrom.mpname" placeholder="套餐名称"
                                  style="width: 180px;"></el-input>
                    </el-form-item>
                    <el-form-item label="项目/产品名称：">
                        <el-input clearable v-model="memberpackfrom.servname" placeholder="项目/产品名称"
                                  style="width: 180px;"></el-input>
                    </el-form-item>
                ...
                <div style="padding-bottom: 5px">
                    本次查询出的所有会员套餐项目总剩次数:{{memberpackhj.servsum}}次， 总剩余金额:{{memberpackhj.servje}}元；产品总剩余数量{{memberpackhj.packsum}}个
                </div>
                <el-table ref="memberpacktable" :data="memberpackData" :span-method="objectSpanMethod2"
                          style="width: 100%">
                    ...
                    <el-table-column label="套餐明细" align="center">
                        ...
                        <el-table-column prop="servname" label="项目/产品名称" width="240">
                            <template slot-scope="scope">
                                {{scope.row.servname }} <label style="color: red">
                                {{scope.row.status | formatStatus}}</label>
                            </template>
                        </el-table-column>
            ...
            <el-tab-pane label="已注销会员" name="cancellMembers">
                <el-form ref="membercancellfrom" inline :model="membercancellfrom" :rules="rules">
                    <el-form-item label="会员查询：">
                        <el-input clearable v-model="membercancellfrom.name" placeholder="姓名/手机号"
                                  style="width: 180px;"></el-input>
                    </el-form-item>
                    <el-form-item label="移除日期：">
                        <el-date-picker style="width: 240px;"
                                        v-model="membercancellfrom.date"
                                        value-format="yyyy-MM-dd"
                                        type="daterange"
                                        unlink-panels
                                        range-separator="至"
                    ...
                    <el-form-item style="margin-bottom:0">
                        <el-button @click="queryMemberCancells()" type="primary">查询</el-button>
                    </el-form-item>
                </el-form>
                <el-table :data="membercancellTables" :span-method="objectSpanMethod" style="width: 100%">
                    <el-table-column prop="memname" label="姓名" width="150"></el-table-column>
                    ...
                    <el-table-column prop="cardno" label="卡号" width="150"></el-table-column>
                    <el-table-column prop="cardname" label="卡名称" width="200"></el-table-column>
                    <el-table-column prop="moneyAll" label="储值总额" width="100"></el-table-column>
                    <el-table-column prop="money" label="卡金余额" width="100"></el-table-column>
                    <el-table-column prop="gmoney" label="赠送金余额" width="100"></el-table-column>
                    <el-table-column prop="updatedTime" label="注销时间" width="180">
                    </el-table-column>
                    <el-table-column prop="updatedBy" label="操作人" width="150">
                    </el-table-column>
                    <el-table-column prop="content" label="操作" min-width="120" fixed="right">
                        <template slot-scope="scope">
                            <el-link type="primary" style="margin: 0 10px 0 10px;" @click="getMember(scope.row)">详细
                            </el-link>
                            <el-link type="primary" style="margin: 0 10px 0 10px;" @click="restoreMem(scope.row)">恢复
                            </el-link>
                        </template>
                    </el-table-column>
                </el-table>
            ...
            <el-tab-pane label="已移除会员" name="removeMember">
                <el-form ref="memberDelfrom" inline :model="memberDelfrom" :rules="rules">
                    ...
                    <el-form-item label="会员查询：">
                        <el-input clearable v-model="memberDelfrom.name" placeholder="姓名/手机号"
                                  style="width: 180px;"></el-input>
                    </el-form-item>
                    <el-form-item label="移除日期：">
                ...
                <yid-table pagination ref="memberDelTable">
                    ...
                    <yid-table-column label="操作" min-width="80" prop="content" fixed="right">
                        <template slot-scope="scope">
                            <el-link type="primary" style="margin: 0 10px 0 10px;" @click="recoverMem(scope.row)">恢复
                            </el-link>
                        </template>
                    </yid-table-column>
                </yid-table>
            ...
            <el-tab-pane label="已过期卡会员" name="expiredCard">
                <el-form ref="memberExporfrom" inline :model="memberExporfrom" :rules="rules">
                    ...
                    <el-form-item label="会员查询：">
                        <el-input clearable v-model="memberExporfrom.name" placeholder="姓名/手机号/卡号"
                                  style="width: 180px;"></el-input>
                    </el-form-item>
                    <el-form-item label="到期日期：">
                        <el-date-picker class="margl5" style="width: 240px;"
            ...
            <el-tab-pane label="会员导入" name="improtMember">
                <table style="border:1px dashed #000; padding: 10px 5px;">
                    <tr>
                        <td style="border:0px;">操作提示：</td>
                    </tr>
                    <tr>
                        <td class="tableTd">1.可从管理平台批量导入会员，并下载对应模板填写相关信息后保存提交上传。</td>
                    </tr>
                    <tr>
                        <td class="tableTd">2.储值卡会员和资格卡会员导入时，手机号已存在且会员卡号已存在的则不导入；
                            套餐会员导入时，手机号已存在且套餐已存在，则导入的套餐将覆盖原有套餐，请谨慎操作！
                        </td>
                    </tr>
                    <tr>
                        <td class="tableTd">3.标识“*”的选项为必填项。</td>
                    </tr>
                    <tr>
                        <td class="tableTd">4.导入会员后可从全部会员列表中找到该条会员数据。</td>
                    </tr>
                </table>
                <el-row class="margsx10">
                    <el-col :span="3" :offset="2"><i style="color: red">*</i>下载导入模板：</el-col>
                    <el-col :span="3">
                        <el-link type="primary" @click="downExcelTemplate1()">储值卡会员模板</el-link>
                    </el-col>
                    <el-col :span="3">
                        <el-link type="primary" @click="downExcelTemplate2()">资格卡会员模板</el-link>
                    </el-col>
                    <el-col :span="3">
                        <el-link type="primary" @click="downExcelTemplate3()">套餐会员模板</el-link>
                    </el-col>
                </el-row>
                <el-row class="margsx10">
                    <el-row class="margsx10">
                    ...
                    <el-row class="margsx10">
                        ...
                        <el-col :span="6">
                            <el-select v-model="memberImport.type">
                    ...
                    <el-col :span="12">
                        <el-upload
                            ...
                            <label style="margin-left: 10px">{{memberImport.filename}}</label>
                ...
                <el-row>
                    <el-col :span="3" :offset="5">
                        <el-button @click="sureImportMembers()" type="primary">确定导入</el-button>
                    </el-col>
                </el-row>
                <yid-table pagination ref="memberImportTable" style="margin-top: 20px;">
                    ...
                    <yid-table-column label="失败数" min-width="80" prop="fails">
                        <template slot-scope="scope">
                            ...
                            <div v-show="scope.row.fails > 0">
                                <el-link type="primary" @click="exportServs(scope.row)">下载</el-link>
                            </div>
                        </template>
                    </yid-table-column>
        ...
        <yid-dialog :title="chainSeniorForm.title" :visible.sync="chainSeniorForm.visible" width="860px">
            <el-form label-width="140px" ref="chainSeniorForm" :inline="true" :model="chainSeniorForm"
                ...
                <el-form-item label="储值总额：" prop="smoney" label-width="90px">
                    <div style="width: 300px;">
                        <el-input clearable type="number" v-model="chainSeniorForm.ssmoney"
                                  style="width: 120px;"></el-input>
                        -
                        <el-input clearable type="number" v-model="chainSeniorForm.esmoney"
                                  style="width: 120px;"></el-input>
                    </div>
                </el-form-item>
                ...
                <el-form-item label="赠送余额：" prop="gmoney" label-width="90px">
                    <div style="width: 300px;">
                        <el-input clearable type="number" v-model="chainSeniorForm.sgmoney"
                                  style="width: 120px;"></el-input>
                        -
                        <el-input clearable type="number" v-model="chainSeniorForm.egmoney"
                                  style="width: 120px;"></el-input>
                    </div>
                </el-form-item>
                <el-form-item label="消费总额：" prop="servTotal" label-width="90px">
                    <div style="width: 300px;">
                        <el-input clearable type="number" v-model="chainSeniorForm.stotal"
                                  style="width: 120px;"></el-input>
                        -
                        <el-input clearable type="number" v-model="chainSeniorForm.etotal"
                                  style="width: 120px;"></el-input>
                    </div>
                </el-form-item>
                ...
                <el-form-item label="当前积分：" prop="servNum" label-width="90px">
                    <div style="width: 300px;">
                        <el-input clearable type="number" v-model="chainSeniorForm.jf1"
                                  style="width: 120px;"></el-input>
                        -
                        <el-input clearable type="number" v-model="chainSeniorForm.jf2"
                                  style="width: 120px;"></el-input>
                    </div>
                </el-form-item>
                <el-form-item label="消费次数：" prop="servNum" label-width="90px">
                    <div style="width: 300px;">
                        <el-input clearable type="number" v-model="chainSeniorForm.snum"
                                  style="width: 120px;"></el-input>
                        -
                        <el-input clearable type="number" v-model="chainSeniorForm.enum"
                                  style="width: 120px;"></el-input>
                    </div>
                </el-form-item>
                <el-form-item label="顾客来源：" prop="channel" label-width="90px">
                    <div style="width: 300px;">
                        <el-select clearable value-key="id" filterable placeholder="请选择"
                                   v-model.trim="chainSeniorForm.channel">
                ...
                <el-form-item label="录入渠道：" prop="channel" label-width="90px">
                    <div style="width: 300px;">
                        <el-select clearable value-key="id" filterable placeholder="请选择"
                                   v-model.trim="chainSeniorForm.inchanel">
                ...
                <el-form-item label="生日方式：" label-width="90px">
                    <div style="width: 300px;">
                        ...
                        <el-select clearable v-model="chainSeniorForm.month" style="width:180px;"
                                   v-if="chainSeniorForm.type=='month'?true:false">
                ...
                <el-row>
                    <el-col :span="6" :offset="6">
                        <el-button @click="seniorMemberSearch" type="primary">提交</el-button>
                    </el-col>
                    <el-col :span="6" :offset="4">
                        <el-button @click="chainSeniorForm.visible=false">取消</el-button>
                    </el-col>
                </el-row>
            </el-form>
        </yid-dialog>
        ...
        <yid-dialog :title="shopSerniorForm.title" :visible.sync="shopSerniorForm.visible" width="860px">
            <el-form label-width="140px" ref="copyForm" :inline="true" class="demo-form-inline">
                ...
                <el-form-item label="注册日期：" prop="regdate" label-width="90px">
                    <div style="width: 300px;">
                        <el-date-picker clearable style="width: 140px;" v-model="shopSerniorForm.sregdate"
                                        value-format="yyyy-MM-dd" type="date" placeholder="选择日期"></el-date-picker>
                        -
                        <el-date-picker clearable style="width: 140px;" v-model="shopSerniorForm.eregdate"
                                        value-format="yyyy-MM-dd" type="date" placeholder="选择日期"></el-date-picker>
                    </div>
                </el-form-item>
                <el-form-item label="储值总额：" prop="smoney" label-width="90px">
                    <div style="width: 300px;">
                        <el-input clearable type="number" v-model="shopSerniorForm.ssmoney"
                                  style="width: 120px;"></el-input>
                        -
                        <el-input clearable type="number" v-model="shopSerniorForm.esmoney"
                                  style="width: 120px;"></el-input>
                    </div>
                </el-form-item>
                <el-form-item label="未来店：" prop="daynum" label-width="90px">
                    <div style="width: 300px;">
                        <el-input clearable type="number" v-model="shopSerniorForm.daynum"
                ...
                <el-form-item label="赠送余额：" prop="gmoney" label-width="90px">
                    <div style="width: 300px;">
                        <el-input clearable type="number" v-model="shopSerniorForm.sgmoney"
                                  style="width: 120px;"></el-input>
                        -
                        <el-input clearable type="number" v-model="shopSerniorForm.egmoney"
                                  style="width: 120px;"></el-input>
                    </div>
                </el-form-item>
                <el-form-item label="消费总额：" prop="servTotal" label-width="90px">
                    <div style="width: 300px;">
                        <el-input clearable type="number" v-model="shopSerniorForm.stotal"
                                  style="width: 120px;"></el-input>
                        -
                        <el-input clearable type="number" v-model="shopSerniorForm.etotal"
                                  style="width: 120px;"></el-input>
                    </div>
                </el-form-item>
                <el-form-item label="到期日期：" prop="exprite" label-width="90px">
                    <div style="width: 300px;">
                        <el-date-picker clearable style="width: 140px;" v-model="shopSerniorForm.sexprite"
                                        value-format="yyyy-MM-dd" type="date" placeholder="选择日期"></el-date-picker>
                        -
                        <el-date-picker clearable style="width: 140px;" v-model="shopSerniorForm.eexprite"
                                        value-format="yyyy-MM-dd" type="date" placeholder="选择日期"></el-date-picker>
                    </div>
                </el-form-item>
                <el-form-item label="消费次数：" prop="servNum" label-width="90px">
                    <div style="width: 300px;">
                        <el-input clearable type="number" v-model="shopSerniorForm.snum"
                                  style="width: 120px;"></el-input>
                        -
                        <el-input clearable type="number" v-model="shopSerniorForm.enum"
                                  style="width: 120px;"></el-input>
                    </div>
                </el-form-item>
                <el-form-item label="顾客来源：" prop="channel" label-width="90px">
                    <div style="width: 300px;">
                        <el-select clearable value-key="id" filterable placeholder="请选择"
                                   v-model.trim="shopSerniorForm.channel">
                            <el-option :key="item.id" :label="item.dcname" :value="item.id"
                                       v-for="item in distChannelList"></el-option>
                        </el-select>
                    </div>
                ...
                <el-form-item label="录入渠道：" prop="channel" label-width="90px">
                    <div style="width: 300px;">
                        <el-select clearable value-key="id" filterable placeholder="请选择"
                                   v-model.trim="shopSerniorForm.inchanel">
                            <el-option :key="item.value" :label="item.name" :value="item.value"
                                       v-for="item in inchanels"></el-option>
                        </el-select>
                    </div>
                ...
                <el-form-item label="会员ID：" prop="memid" label-width="90px">
                    <div style="width: 300px;">
                        <el-input clearable type="number" v-model="shopSerniorForm.memberid"
                                  style="width: 180px;"></el-input>
                    </div>
                </el-form-item>
                <el-row>
                    <el-col :span="6" :offset="8">
                        <el-button @click="shopSeniorSearch()" type="primary">提交</el-button>
                    </el-col>
                    <el-col :span="6" :offset="2">
                        <el-button @click="shopSerniorForm.visible=false">取消</el-button>
                    </el-col>
                </el-row>
            </el-form>
        </yid-dialog>
        ...
        <yid-dialog :title="meminfoDialog.title" :visible.sync="meminfoDialog.visible" width="1150px">
            <el-row style="padding-left: 45px;margin: 0;" width="980px" justify="center">
                <el-button @click="meminfoDialog.showNum=1" type="primary" :plain="meminfoDialog.showNum!=1">基础资料
                </el-button>
                <el-button @click="meminfoDialog.showNum=2" type="primary" :plain="meminfoDialog.showNum!=2">资料修改
                </el-button>
                <!--<el-button @click="meminfoDialog.showNum=3" type="primary" :plain="meminfoDialog.showNum!=3">账户调整</el-button>-->
                <el-button @click="meminfoDialog.showNum=4" type="primary" :plain="meminfoDialog.showNum!=4">卡金流水
                </el-button>
                <el-button @click="meminfoDialog.showNum=5" type="primary" :plain="meminfoDialog.showNum!=5">套餐记录
                </el-button>
                <el-button @click="meminfoDialog.showNum=6" type="primary" :plain="meminfoDialog.showNum!=6">消费记录
                </el-button>
                <el-button @click="meminfoDialog.showNum=7" type="primary" :plain="meminfoDialog.showNum!=7">资料修改记录
                </el-button>
                <el-button @click="meminfoDialog.showNum=8" type="primary" :plain="meminfoDialog.showNum!=8">寄存记录
                </el-button>
                <el-button @click="meminfoDialog.showNum=9" type="primary" :plain="meminfoDialog.showNum!=9">优惠券
                </el-button>
                <el-button @click="meminfoDialog.showNum=10" type="primary" :plain="meminfoDialog.showNum!=10">红包
                </el-button>
                <el-button @click="meminfoDialog.showNum=11" type="primary" :plain="meminfoDialog.showNum!=11">共享记录
                </el-button>
                <el-button @click="meminfoDialog.showNum=12" type="primary" :plain="meminfoDialog.showNum!=12">积分流水
                </el-button>
                <el-button @click="meminfoDialog.showNum=13" type="primary" :plain="meminfoDialog.showNum!=13">实体卡
                </el-button>
            </el-row>
            <div class="meminfo" v-show="meminfoDialog.showNum==1">
                ...
                <table border="1" cellspacing="0">
                    <tr>
                        <td align="right" width="22%">手机号码:</td>
                        <td width="28%"><label class="marg5">{{memberDesc.mobile}}</label></td>
                        <td align="right" width="22%">姓名:</td>
                        <td><label class="marg5" width="28%">{{memberDesc.memname}}</label></td>
                    </tr>
                    <tr>
                        <td align="right">生日:</td>
                        <td><label class="marg5">{{memberDesc.birthday}}</label></td>
                        <td align="right">性别:</td>
                        <td><label class="marg5">{{memberDesc.sex == "1" ? "男" : "女"}}</label></td>
                    </tr>
                    <tr>
                        <td align="right">顾客来源:</td>
                    ...
                    <tr>
                        ...
                        <td><label class="marg5">{{memberDesc.nickname}}</label></td>
                        <td align="right">当前积分:</td>
                        <td><label class="marg5">{{integral}}</label></td>
                    </tr>
                    <tr>
                        <td align="right">住址:</td>
                        <td><label class="marg5">{{memberDesc.address}}</label></td>
                        <td align="right">累计获取总积分:</td>
                        <td><label class="marg5">{{integral_all}}</label></td>
                    </tr>
                    <tr>
                        <td align="right">状态:</td>
                        <td colspan="3"><label class="marg5">
                            {{memberDesc.status == "1" ? "正常" : "" }}
                            {{memberDesc.status == "2" ? "冻结" : "" }}
                            {{memberDesc.status == "3" ? "已注销" : "" }}
                        </label></td>
                    </tr>
                    ...
                    <tr v-for="member in memberDesc.shopmembers">
                        <td align="right">{{member.shopname}} 备注:</td>
                        <td colspan="3"><label class="marg5">{{member.memo}}</label></td>
                    </tr>
                </table>
                <div>卡信息:
                    <el-select v-model="cardinfo.id" @change="changeCardinfo()" style="width: 260px;">
                        <el-option v-for="card in memberDesc.cards" :key="card.id" :value="card.id"
                                   :label="card.cardno+'('+card.cardname+')'+' '+card.stk"></el-option>
                    </el-select>
                </div>
                <table border="1" cellspacing="0">
                    <tr>
                        <td align="right" width="22%">卡号:</td>
                        ...
                        <td width="28%"><label class="marg5">{{cardinfo.money}}
                            <el-link v-if="cardinfo.cardtype=='1'"
                                     @click="queryShopCardMoney(cardinfo.id)" type="primary" class="marg5">明细
                            </el-link>
                        </label>
                        </td>
                    </tr>
                    <tr>
                        <td align="right">储值总额:</td>
                        <td><label class="marg5">{{cardinfo.smoney}}
                            <el-link v-if="cardinfo.cardtype=='1'"
                                     @click="queryShopCardMoney(cardinfo.id)" type="primary" class="marg5">明细
                            </el-link>
                        </label></td>
                    ...
                    <tr>
                        ...
                        <td>
                            <el-link v-if="cardinfo.status=='1' && memberDesc.status=='1'" @click="openChangeCard(cardinfo.cardno)" type="primary"
                                     class="marg5">迁移卡
                            </el-link>&nbsp;&nbsp;&nbsp;&nbsp;
                            <el-link v-if="checkCardCancell(cardinfo)" @click="cancellCard(cardinfo)" type="primary"
                                     class="marg5">注销卡
                            </el-link>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td><label class="marg5">
                            {{cardinfo.status == "1" ? "正常" : "" }}
                            {{cardinfo.status == "2" ? "冻结" : "" }}
                            {{cardinfo.status == "3" ? "挂失" : "" }}
                            {{cardinfo.status == "4" ? "作废" : "" }}
                        </label></td>
                        <td align="right">会员卡备注:</td>
                        <td>
                            <el-link @click="queryCardContext(cardinfo)" type="primary"
                                     class="marg5">查看卡备注
                            </el-link>
                        </td>
                    </tr>
                    <!-- 关联实体卡 -->
                    <tr>
                        <td align="right">关联实体卡</td>
                        <td><label class="marg5" :class="(cardinfo.dawncmcode != '' && cardinfo.dawncmcode != null) ? '' : 'relationCardText'" @click="relationCard(cardinfo)">{{(cardinfo.dawncmcode != '' && cardinfo.dawncmcode != null) ? cardinfo.dawncmcode : cardinfo.cardno=='' ? '' : '关联实体卡'}}</label></td>
                    </tr>
                </table>
                <div>套餐信息</div>
                <table border="1" cellspacing="0" class="taoctable">
                    <tr>
                        <td rowspan="2" width="22%">套餐名称</td>
                        <td colspan="6">套餐明细</td>
                    </tr>
                    <tr>
                        <td>购买门店</td>
                        <td>项目/产品名称</td>
                    ...
                    <tr v-for="item in memberDesc.packages" style="height: 26px;">
                        <td>{{item.mpname}} </td>
                        <td>{{item.shopname}} </td>
                        <td>{{item.servname}}<label style="color: #ee9900"> {{item.isGift == '1' ? '赠' : ''}}</label>
                            <label style="color: red"> {{item.status | formatStatus}}</label>
                        </td>
                        <td>{{item.total}}</td>
                        <td>{{item.snum}}</td>
                        <td>{{item.lessmoney}}</td>
                ...
                <table border="1" cellspacing="0">
                    <tr>
                        <td align="right" width="22%">兴趣爱好:</td>
                    ...
                    <tr>
                        ...
                        <td colspan="3"><label class="marg5">{{memberDesc.others}}</label></td>
                    </tr>
                </table>
                ...
                <table border="1" cellspacing="0" v-if="activeName!='chainMember'">
                    ...
                    <tr>
                        ...
                        <td><label class="marg5">{{memberStatic.cyNum}}</label></td>
                    </tr>
                    <tr>
                        <td align="right">平均月消费金额:</td>
            ...
            <div class="meminfo" v-show="meminfoDialog.showNum==2">
                ...
                <table border="1" cellspacing="0">
                    <tr>
                        ...
                        <td width="30%">
                            <el-input v-model="membercard.mobile"/>
                        </td>
                        <td align="right" width="20%">姓名:</td>
                        <td width="30%">
                            <el-input v-model="membercard.memname"/>
                        </td>
                    </tr>
                    <tr>
                        <td align="right">生日:</td>
                        <td>
                            <el-date-picker v-model="membercard.birthday" type="date" placeholder="选择日期"
                    ...
                    <tr>
                        <td align="right">住址:</td>
                        <td>
                            <el-input v-model="membercard.address"/>
                        </td>
                        <td align="right">操作:</td>
                        <td>
                            <el-link v-if="memberDesc.status=='1'" @click="cancellMember(membercard)" type="primary"
                                     class="marg5">注销会员资料
                            </el-link>
                        </td>
                    </tr>
                ...
                <div>卡信息:
                    <el-select style="width: 260px;" v-model="membercard.cardid" @change="changeMemberCard()">
                        <el-option v-for="card in memberDesc.cards" :key="card.id" :value="card.id"
                ...
                <table border="1" cellspacing="0">
                    <tr>
                        ...
                        <td width="30%"><label class="margl0">{{membercard.cardno}}</label></td>
                        <td align="right" width="20%">会员卡级别:</td>
                        <td width="30%">
                            <el-select :disabled="true" v-model="membercard.cpcode" width="300px">
                    ...
                    <tr>
                        <td align="right">到期日期:</td>
                        <td>
                            <el-date-picker v-model="membercard.exprite" type="date" placeholder="选择日期"
                                            value-format="yyyy-MM-dd HH:mm:ss"
                                            :disabled="true">
                            </el-date-picker>
                            <br/>
                            <lable style="font-size: 10px">*如此卡没有有效期请不要填</lable>
                        </td>
                        <td align="right">卡备注:</td>
                        <td>
                            <el-input v-model="membercard.cardmemo"/>
                        </td>
                    </tr>
                </table>
                <el-row>
                    <el-col :offset="22" :span="1">
                        <el-button v-if="memberDesc.status=='1'" type="primary" @click="saveMemberCard()">确认修改</el-button>
                        <el-button v-if="memberDesc.status == '2'">资料已冻结,不可修改</el-button>
                        <el-button v-if="memberDesc.status == '3'">资料已注销,不可修改</el-button>
                    </el-col>
                </el-row>
            </div>
            <div class="meminfo" v-show="meminfoDialog.showNum==3">
                <div>修改卡内余额及赠送金额:
                    <el-select v-model="cardmoney.id" @change="changeCardmoney()">
                ...
                <table border="1" cellspacing="0">
                    <tr>
                        <td align="right" width="25%">会员卡内可用金额:</td>
                        <td><label>{{cardmoney.money}} </label>
                            <el-link @click="editShopCardMoney(cardmoney.id)" type="primary" class="marg5">修改</el-link>
                        </td>
                        <td align="right" width="25%">赠送可用金额:</td>
                        <td><label>{{cardmoney.gmoney}}</label>
                            <el-link @click="editShopCardMoney(cardmoney.id)" type="primary" class="marg5">修改</el-link>
                        </td>
                    </tr>
                    <tr>
                        <td align="left" colspan="4">提示：卡内余额是指充值卡内原本剩下的金额。此功能用于恢复使用本平台以前的会员卡里的卡金，
                            此处填入的数字不进入当日的店内现金报表，但此操作将被记录在"会员资料修改日志”和“卡金流水”中。
                        </td>
                    </tr>
                </table>
                ...
                <el-row>
                    <el-col :offset="22" :span="1"></el-col>
                </el-row>
            </div>
            <div class="meminfo" v-show="meminfoDialog.showNum==4">
                <div>
                    ...
                    <el-select clearable v-model="cardrecordform.btype" style="width:140px;"
                               v-if="cardrecordform.cardtype=='1'">
                    ...
                    <el-select clearable v-model="cardrecordform.mtype" style="width:100px;"
                               v-if="cardrecordform.cardtype=='1'">
                        <el-option value="1" label="卡金"></el-option>
                        <el-option value="2" label="赠送金"></el-option>
                    </el-select>
                    <label style="color: red">*</label>发生日期:
                    <el-date-picker class="margl5"
                                    v-model="cardrecordform.date"
                                    value-format="yyyy-MM-dd"
                                    type="daterange"
                                    unlink-panels
                                    range-separator="至"
            ...
            <div class="meminfo" v-show="meminfoDialog.showNum==5">
                <div>
                    套餐名称:
                    <el-input v-model="packrecordform.mpname" style="width:160px;" placeholder="套餐名称"/>
                    项目/产品名称:
                    <el-input v-model="packrecordform.name" style="width:160px;" placeholder="项目/产品名称"/>
                    <label style="color: red">*</label>发生日期:
                    <el-date-picker class="margl5"
                                    v-model="packrecordform.date"
                                    value-format="yyyy-MM-dd"
                                    type="daterange"
                                    unlink-panels
                                    range-separator="至"
            ...
            <div class="meminfo" v-show="meminfoDialog.showNum==6">
                <div>
                    <label style="color: red">*</label>发生日期:<el-date-picker class="margl5"
                ...
                <yid-table pagination ref="memberRedcords">
                    ...
                    <yid-table-column label="合计折扣" min-width="80" prop="zkeTotal">
                        <template slot-scope="scope">
                            {{ scope.row.zkeTotal > 0 ? ("-" + scope.row.zkeTotal) : 0}}
                        </template>
                    </yid-table-column>
            ...
            <div class="meminfo" v-show="meminfoDialog.showNum==7">
                <div>
                    ...
                    操作类型：
                    <el-select clearable v-model="memberlogfrom.type" style="width:150px;">
                        <el-option value="0" label="修改资料"></el-option>
                        <el-option value="1" label="转赠/迁移"></el-option>
                        <el-option value="2" label="退款"></el-option>
                        <el-option value="3" label="状态变更"></el-option>
                        <el-option value="4" label="删除或撤单"></el-option>
                    </el-select>
                ...
                <yid-table pagination ref="memberlog">
                    ...
                    <yid-table-column label="操作类型" min-width="140" prop="type">
                        <template slot-scope="scope">
                            <div v-if="scope.row.type && scope.row.type == '0'">修改资料</div>
                            <div v-if="scope.row.type && scope.row.type == '1'">转赠/迁移</div>
                            <div v-if="scope.row.type && scope.row.type == '2'">退款</div>
                            <div v-if="scope.row.type && scope.row.type == '3'">状态变更</div>
                            <div v-if="scope.row.type && scope.row.type == '4'">删除或撤单</div>
                        </template>
                    </yid-table-column>
            ...
            <div class="meminfo" v-show="meminfoDialog.showNum==8">
                <div>
                   购买时间:<el-date-picker class="margl5"
                    ...
                    订单编号:
                    <el-input clearable v-model="storefrom.billcode" placeholder="订单编号"
                              style="width: 130px;"></el-input>
                    商品编号:
                    <el-input clearable v-model="storefrom.pcode" placeholder="商品编号" style="width: 120px;"></el-input>
                    商品名称:
                    <el-input clearable v-model="storefrom.pname" placeholder="商品名称" style="width: 130px;"></el-input>
                        ...
                        <template slot-scope="scope">
                            <yid-table ref="memberStorelogs" style="margin-top: 5px;" :data="scope.row.logs">
                                ...
                                <yid-table-column label="操作方式" min-width="80">
                                    <template slot-scope="scope">
                                        {{scope.row.btype == '3' ? '支付' : (scope.row.btype == '4' ? '退款':'提货')}}
                                    </template>
                                </yid-table-column>
                                <yid-table-column prop="btype" label="类型" min-width="80">
                                    ...
                                    <template slot-scope="scope">{{scope.row.btype == '4' ? '退款' : ''}}</template>
                                </yid-table-column>
            ...
            <div class="meminfo" v-show="meminfoDialog.showNum==9">
                <div>
                    活动名称:
                    <el-input clearable v-model="coupon.maname" placeholder="活动名称" style="width: 130px;"></el-input>
                    状态:
                    <el-select clearable v-model="coupon.status" style="width:110px;">
                    ...
                    领用时间:
                    <el-date-picker class="margl5" style="width: 200px;"
                ...
                <yid-table pagination ref="couponlogs">
                    ...
                    <yid-table-column label="发券单号" min-width="140" prop="sbillcode"></yid-table-column>
                    ...
                    <yid-table-column label="发券门店" min-width="100" prop="shopname"></yid-table-column>
                    ...
                    <yid-table-column label="使用时间" min-width="120" prop="usedDate">
                        <template slot-scope="scope">
                            {{ scope.row.usedDate ? scope.row.usedDate : '' }}
                        </template>
                    </yid-table-column>
            ...
            <div class="meminfo" v-show="meminfoDialog.showNum==10">
                <div>
                    活动名称:
                    <el-input clearable v-model="redenvelope.maname" placeholder="活动名称"
                              style="width: 130px;"></el-input>
                    状态:
                    <el-select clearable v-model="redenvelope.status" style="width:140px;">
                        <el-option value="1" label="未使用"></el-option>
                        <el-option value="2" label="已使用"></el-option>
                        <el-option value="3" label="已过期"></el-option>
                        <el-option value="4" label="已退款"></el-option>
            ...
            <div class="meminfo" v-show="meminfoDialog.showNum==11">
                <div>我邀请的共享</div>
                <table border="1" cellspacing="0" class="taoctable">
                    <tr>
                        <td>账户id</td>
                    ...
                    <tr v-for="item in memberDesc.shareInfo" style="height: 16px;">
                        <td>{{item.rmemid}}</td>
                        <td>{{item.rmemname}}</td>
                        <td>{{item.mobile}}</td>
                        <td>{{item.acceptDate}}</td>
                        <td width="28%">
                            <el-link @click="queryShareContext(item.id)" type="primary" class="marg5">查看共享内容
                            </el-link>
                        </td>
                    </tr>
                </table>
                ...
                <table border="1" cellspacing="0" class="taoctable">
                    ...
                    <tr v-for="item in memberDesc.rshareInfo" style="height: 26px;">
                        <td>{{item.memid}}</td>
                        <td>{{item.memname}}</td>
                        <td>{{item.rmobile}}</td>
                        <td>{{item.acceptDate}}</td>
                ...
                <table border="1" cellspacing="0" class="taoctable">
                    ...
                    <tr v-for="item in memberDesc.cancelShare" style="height: 26px;">
                        <td>{{item.cancel}}</td>
                        <td>{{item.rmemid}}</td>
                        <td>{{item.rmemname}}</td>
                        <td>{{item.rmobile}}</td>
                        <td>{{item.acceptDate}}</td>
                        <td>{{item.unbindDate}}</td>
                        <td>{{item.updatememo}}</td>
                    </tr>
                </table>
            </div>
            <div class="meminfo" v-show="meminfoDialog.showNum==12">
                <div style="margin:auto">
                    类型:
                    <el-select clearable value-key="id" filterable placeholder="请选择"
                               v-model.trim="integralForm.behavior">
                        <el-option :key="item" :label="item" :value="item"
                                   v-for="item in integralType"></el-option>
                    </el-select>
                    发生日期:
                    <el-date-picker class="margl5" style="width: 250px;"
                                    v-model="integralForm.date" value-format="yyyy-MM-dd"
                                    type="daterange"
                                    unlink-panels
                                    range-separator="至"
                                    start-placeholder="开始日期"
                                    end-placeholder="结束日期"></el-date-picker>
                    <el-button type="primary" @click="queryIntegral()" class="margl5">查询</el-button>
                </div>
                <yid-table pagination ref="integral" :data="this.integralData">
                    <yid-table-column label="发生时间" prop="createdTime"></yid-table-column>
                    <yid-table-column label="业务类型" prop="behavior"></yid-table-column>
                    <yid-table-column label="变动积分" prop="direction">
                        <template slot-scope="scope">
                            {{scope.row.direction == 1 ? '+' + scope.row.optmoney : '-' + scope.row.optmoney}}
                        </template>
                    </yid-table-column>
                    <yid-table-column label="结存积分" prop="balance"></yid-table-column>
                    <yid-table-column label="业务单号" prop="refbill"></yid-table-column>
                </yid-table>
            </div>
            <div class="meminfo" v-show="meminfoDialog.showNum==13">
            <div>已关联实体卡</div>
              <yid-table ref="relevanceTable">
                <yid-table-column label="电子会员卡" min-width="160" prop="dcardno"></yid-table-column>
                <yid-table-column label="关联实体卡" min-width="160" prop="scardno"></yid-table-column>
                <yid-table-column label="创建关联时间" min-width="100" prop="createdTime"></yid-table-column>
                <yid-table-column label="恢复原因" min-width="160" prop="reason"></yid-table-column>
                <yid-table-column label="操作门店" min-width="80" prop="shopname"></yid-table-column>
                <yid-table-column label="操作人" min-width="80" prop="updatedBy"></yid-table-column>
                <yid-table-column label="操作" min-width="80">
                  <template slot-scope="scope">
                    <el-link type="primary" style="margin: 0 10px 0 10px;" @click="relieveRelevance(scope.row)">解除关联
                    </el-link>
                  </template>
                </yid-table-column>
              </yid-table>
            <div>已解除关联实体卡</div>
              <yid-table ref="relieveRelevanceTable">
                <yid-table-column label="电子会员卡" min-width="160" prop="dcardno"></yid-table-column>
                <yid-table-column label="关联实体卡" min-width="160" prop="scardno"></yid-table-column>
                <yid-table-column label="解除关联时间" min-width="100" prop="updatedTime"></yid-table-column>
                <yid-table-column label="解除类型" min-width="80" prop="type">
                  <template slot-scope="scope">
                  {{scope.row.type == 1 ? "挂失实体卡" : "" }}
                  {{scope.row.type == 2 ? "坏卡" : "" }}
                  {{scope.row.type == 3 ? "顾客要求" : ""}}
                    {{scope.row.type == 4 ? "其他" : ""}}
                  </template>
                </yid-table-column>
                <yid-table-column label="解除原因" min-width="160" prop="reason"></yid-table-column>
                <yid-table-column label="操作门店" min-width="80" prop="shopname"></yid-table-column>
                <yid-table-column label="操作人" min-width="80" prop="updatedBy"></yid-table-column>
                <yid-table-column label="操作" min-width="80">
                  <template slot-scope="scope">
                    <el-link type="primary" style="margin: 0 10px 0 10px;" @click="recoverRelevance(scope.row)">恢复关联
                    </el-link>
                  </template>
                </yid-table-column>
              </yid-table>
        ...
        <yid-dialog class="meminfo" :visible.sync="boolShare" width="1150px">
            <div>共享的会员卡</div>
            <table border="1" cellspacing="0" class="taoctable">
                <tr>
                    <td>卡类型</td>
                ...
                <tr v-for="item in shareCardPackage.shareCard" style="height: 26px;">
                    <td>{{item.cardtype}}</td>
                    <td>{{item.cardname}}</td>
                    <td>{{item.cardno}}</td>
                </tr>
            ...
            <div>共享的套餐</div>
            <table border="1" cellspacing="0" cellpadding="0" width="100%" class="taoctable"
                   id="shareCardPackage.sharePackage" style="width: 980px">
                <tr>
                    <td width="245">套餐名称</td>
                    <td width="245">项目名称</td>
                    <td width="245">剩余数量</td>
                    <td width="245">有限期</td>
                </tr>
                <tr v-for="item in shareCardPackage.sharePackage">
                    <td>{{item.mpname}}</td>
                    <td colspan="3">
                        <table border="0" cellspacing="0" cellpadding="0" class="taoctable taocan" style="width: 745px">
                            <tr v-for="obj in item.list">
                                <td width="245">{{obj.servname}}</td>
                                <td width="245">{{obj.snum}}</td>
                                <td width="245">{{obj.expiryTime == null ? "长期有限" : obj.expiryTime}}</td>
                            </tr>
                        </table>
                    </td>
                </tr>
        ...
        <yid-dialog :title="changeCardDialog.title" :visible.sync="changeCardDialog.visible" width="650px">
            <el-form>
                <el-form-item label="迁移卡号：" prop="cardno" label-width="160px">
                    <div>
                        {{changeCardDialog.cardno}}
                    </div>
                </el-form-item>
                <el-form-item label="请选择目标用户：" prop="memname" label-width="160px">
                    <div>
                        <el-input clearable v-model="mobile" placeholder="请输入用户手机号"
                                  style="width: 160px;"></el-input>
                        <el-button @click="searchMemberMobile(mobile)" type="primary" style="margin-left: 6px">查询
                        </el-button>
                    </div>
                </el-form-item>
                <el-form-item label="" prop="member" label-width="160px">
                    <div>
                        <img style="border-radius:50%" min-width="100" height="100" :src="changeCardDialog.photo"/><br>
                        {{changeCardDialog.memname}}
                        {{changeCardDialog.mobile}}
                    </div>
                </el-form-item>
                <el-form-item label="输入会员姓名：" prop="vercode" label-width="160px"
                              v-if="!changeCardDialog.memname && changeCardDialog.mobile">
                    <el-input clearable v-model.trim="changeCardDialog.nmemname" placeholder="会员姓名"
                              style="width: 160px;"></el-input>
                </el-form-item>
                <el-form-item label="输入验证码：" prop="vercode" label-width="160px">
                    <div>
                        <el-input clearable v-model="changeCardDialog.vercode" placeholder="验证码"
                                  style="width: 140px;"></el-input>
                        <el-button
                                v-if="this.changeCardDialog.memname || this.changeCardDialog.nmemname"
                                :disabled="this.changeCardDialog.count>0"
                                @click="sendVercode(changeCardDialog.mobile)" type="primary"><label
                                v-if="this.changeCardDialog.count>0">{{this.changeCardDialog.count}}秒后</label>发送验证码
                        </el-button>
                    </div>
                </el-form-item>
                <el-row>
                    <el-col :span="6" :offset="6">
                        <el-button @click="sureChangeCard" type="primary">确定</el-button>
                    </el-col>
                    <el-col :span="6" :offset="4">
                        <el-button @click="changeCardDialog.visible=false">取消</el-button>
                    </el-col>
                </el-row>
            </el-form>
        </yid-dialog>

        <yid-dialog :title="cardMemo.title" :visible.sync="cardMemo.visible" width="650px">
            <yid-table ref="cardTable" :data="cardMemo.data">
                <yid-table-column prop="memo" label="备注" min-width="100"></yid-table-column>
                <yid-table-column prop="shopname" label="所属门店" min-width="100"></yid-table-column>
                <yid-table-column prop="shopcode" label="门店编码" min-width="100"></yid-table-column>
                <yid-table-column prop="updatedTime" label="备注日期" min-width="100"></yid-table-column>
                <yid-table-column label="修改记录">
                    <template slot-scope="scope">
                        <el-button type="primary" @click="checkUpdateLog(scope.row)">查看</el-button>
                    </template>
                </yid-table-column>
            </yid-table>
        </yid-dialog>

        <yid-dialog :title="updateMemo.title" :visible.sync="updateMemo.visible" width="650px">
            <yid-table ref="updateMemoTable" :data="updateMemo.data">
                <yid-table-column prop="memo" label="备注" min-width="100"></yid-table-column>
                <yid-table-column prop="createdBy" label="操作人" min-width="100"></yid-table-column>
                <yid-table-column prop="createdTime" label="修改日期" min-width="100"></yid-table-column>
            </yid-table>
        </yid-dialog>

      <yid-dialog :title="relieveDialog.title" :visible.sync="relieveDialog.visible" width="550px">
        <el-form>
          <el-form-item label="会员：" label-width="180px">
            <div>
              {{relieveDialog.memname}}&nbsp;{{relieveDialog.mobile}}
            </div>
          </el-form-item>
          <el-form-item label="电子会员卡：" label-width="180px">
            <div>
              {{relieveDialog.dcardno}}
            </div>
          </el-form-item>
          <el-form-item label="关联实体卡：" label-width="180px">
            <div>
              {{relieveDialog.scardno}}
            </div>
          </el-form-item>
          <el-form-item label="解除类型：" label-width="180px">
            <el-select v-model="relieveDialog.type" placeholder="请选择" style="width: 140px;">
              <el-option label="挂失实体卡" value="1"></el-option>
              <el-option label="坏卡" value="2"></el-option>
              <el-option label="顾客要求" value="3"></el-option>
              <el-option label="其他" value="4"></el-option>
              <el-option label="卡转赠" value="5"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="解除原因：" label-width="180px">
            <el-input
                type="textarea"
                :rows="2"
                ...
                style="width: 300px;"
                v-model="relieveDialog.reason">
            </el-input>
          </el-form-item>
          <el-row>
            <el-col :span="9" :offset="9">
              <el-button @click="sureRelieve" type="primary">确定</el-button>
              <el-button @click="cancelRelieve">取消</el-button>
            </el-col>
          </el-row>
        </el-form>
        </yid-dialog>

      <yid-dialog :title="recoverDialog.title" :visible.sync="recoverDialog.visible" width="600px">
        <el-form>
          <el-form-item label="会员：" label-width="180px">
            <div>
              {{recoverDialog.memname}}&nbsp;{{recoverDialog.mobile}}
            </div>
          </el-form-item>
          <el-form-item label="电子会员卡：" label-width="180px">
            <div>
              {{recoverDialog.dcardno}}
            </div>
          </el-form-item>
          <el-form-item label="关联实体卡：" label-width="180px">
            <div>
              {{recoverDialog.scardno}}
            </div>
          </el-form-item>
          <el-form-item label="恢复原因：" label-width="180px">
            <el-input
                type="textarea"
                :rows="2"
                placeholder="请输入1-30字"
                ...
                :maxlength="20" clearable
                style="width: 300px;"
                v-model="recoverDialog.reason">
            </el-input>
          </el-form-item>
          <el-form-item label="输入验证码：" label-width="180px">
            <el-input v-model.trim="recoverDialog.verifyCode" placeholder="请输入验证码" maxlength="10" clearable
                      style="width: 210px;">
            </el-input>
            <el-button @click="sendVercode(recoverDialog.mobile)" class="button" style="background-color: #00aaee;color: #ffffff">{{"发送验证码"}}
            </el-button>
          </el-form-item>
          <el-row>
            <el-col :span="9" :offset="9">
              <el-button @click="recoverRelieve" type="primary">确定</el-button>
              <el-button @click="cancelRelieve">取消</el-button>
            </el-col>
          </el-row>
        </el-form>
        ...
        <yid-dialog :title="dialogRelation.title" v-if="dialogRelation.visible" :visible.sync="dialogRelation.visible"  width="500px">
            <relationCard :sitem="cardinfo" :memid="memberDesc.memid" @onClose="closedRelation"></relationCard>
        </yid-dialog>
    </div>
</template>
...
import math from "mathjs";
 //关联实体卡组件
import relationCard from "../batchCard/relationCard.vue"
...
export default {
  ...
  data() {
    return {
      ...
      impShopList: [],
      rules: {
        shopid: { required: true, message: "请选择门店", trigger: "red" }
      },
      memrules: {
        name: {
          required: true,
          message: "请输入姓名/手机号/卡号",
          trigger: "red"
        }
      },
      ...
      timer: "",
      cardMemo: { title: "", visible: false, data: [] },
      updateMemo: { title: "", visible: false, data: [] },
      cardId: null,
      integral: 0,
      integral_all: 0,
      relieveDialog: {
  ...
  mounted() {
    const datetime = new Date().setMonth(new Date().getMonth() - 12);
    const sdate = this.getToday(new Date(datetime));
    const edate = this.getToday(new Date());
  ...
  methods: {
    init() {
      ...
      service.member.memberinfo.getLogtypeList({}).then(res => {
        if (res.resp_code == 200) {
    ...
    //查询主动共享卡套餐信息
    queryShareContext(id) {
      service.member.memberinfo.queryShareCard(id).then(res => {
        if (res.resp_code == "200") {
          this.shareCardPackage = res.data;
        }
      });
      this.boolShare = true;
    },
    ...
    chainMemberSearch() {
      if (!this.chainMemberForm.name) {
        if (this.chainMemberForm.code == "1") yid.util.error("请输入手机号");
        if (this.chainMemberForm.code == "2") yid.util.error("请输入卡号");
        if (this.chainMemberForm.code == "3") yid.util.error("请输入会员姓名");
        if (this.chainMemberForm.code == "4") yid.util.error("请输入会员ID");
        return;
      }
      const params = {
        page: 1,
      ...
      if (this.chainMemberForm.code == "1")
        params.mobile = this.chainMemberForm.name;
      if (this.chainMemberForm.code == "2")
        params.cardno = this.chainMemberForm.name;
      if (this.chainMemberForm.code == "3")
        params.memname = this.chainMemberForm.name;
      if (this.chainMemberForm.code == "4")
        params.memid = this.chainMemberForm.name;
    ...
    queryMemberCancells(page = 1) {
      this.pageInfo.page = page;
      const membercancellquery = {};
      membercancellquery.name = this.membercancellfrom.name;
      if (this.membercancellfrom.date != null) {
        if (this.membercancellfrom.date.length > 0) {
          membercancellquery.updatedTimeStart =
            this.membercancellfrom.date[0] + " 00:00:00";
          membercancellquery.updatedTimeEnd =
            this.membercancellfrom.date[1] + " 23:59:59";
        } else {
          membercancellquery.updatedTimeStart = "";
          membercancellquery.updatedTimeEnd = "";
        }
      ...
      const params = { ...membercancellquery, ...this.pageInfo, status: "3" };
      this.membercancellQuery(params);
      this.pageInfo.type = "1";
    },
    ...
    membercancellQuery(params) {
      service.member.memberinfo.pagecancellMembers(params).then(res => {
        if (res.resp_code == "200") {
          this.membercancellTables = res.data;
          this.pageInfo.total = res.count;
        }
      });
    },
    ...
    getMember(row) {
      ...
      service.member.memberinfo.queryMember({memid: row.memid || row.id}).then(res => {
            if (res.resp_code == "200") {
              ...
              if (this.memberDesc.cards && this.memberDesc.cards.length > 0) {
                ...
                this.cardinfo.id = id;
                this.memberDesc.cards.forEach(item => {
                  item.stk = '';
                  if (item.dawncmcode) {
                    item.stk = "-关联实体卡:" + item.dawncmcode;
                  }
                })
              }
              this.changeCardinfo();
              ...
              service.member.memberinfo
                .memStorepack({ memid: row.memid || row.id })
                .then(res => {
                  if (res.resp_code == "200") {
                    this.memproducts = [];
                    this.memproducts = res.data;
                  }
                });
      ...
      service.member.memberinfo.getMemMoneyByMemid({ memid: this.usermemid, mtype: 1 }).then(res => {
          if (res.resp_code == 200) {
            if (res.data) {
              this.integral = res.data.money || 0;
              this.integral_all = res.data.moneycum || 0;
            }else{//没有积分后端返回null
              this.integral =  0;
              this.integral_all = 0;
            }
      ...
      this.getCardRelation(this.usermemid);
    },
    getCardRelation(val) {
      const params = {
        memid:val
      };
      let fetch = service.member.memberinfo.findRelevance;
      this.$refs.relevanceTable.reloadData({
        fetch,
        params
      });
      fetch = service.member.memberinfo.findRelieveRelevance;
      this.$refs.relieveRelevanceTable.reloadData({
        fetch,
        params
      });
    },
      ...
      if (this.cardinfo.id) {
        this.memberDesc.cards.forEach(card => {
          if (this.cardinfo.id == card.id) {
            ...
            this.cardinfo.status = card.status;
            this.cardinfo.shopid = card.shopid;

            this.cardinfo.dawncmcode = card.dawncmcode
            this.cardinfo.type = card.type
            service.member.memberinfo
              .memStorepack({
                memid: this.memberDesc.memid,
                cmid: this.cardinfo.id
              })
              .then(res => {
                if (res.resp_code == "200") {
                  this.cardproducts = [];
    ...
    updateCardmoney(row) {
      if (
        Number(row.nmoney) == Number(row.money) &&
        Number(row.ngmoney) == Number(row.gmoney)
      ) {
    ...
    queryCardRecords() {
      ...
      if (!this.cardrecordform.date || this.cardrecordform.date.length == 0) {
        yid.util.error("选择日期");
        return;
      }
    ...
    queryPackageList() {
      if (!this.packrecordform.date || this.packrecordform.date.length == 0) {
        yid.util.error("选择日期");
        return;
      }
    ...
    queryMemberRecords() {
      if (!this.memberfrom.date || this.memberfrom.date.length == 0) {
        yid.util.error("选择日期");
        return;
      }
    ...
    queryMemberPackages(page = 1) {
      ...
      service.member.memberinfo
        .queryMemberpagePackages(this.memberpackfrom)
        .then(res => {
          if (res.resp_code == "200") {
      ...
      service.member.memberinfo
        .queryPackageHj(this.memberpackfrom)
        .then(res => {
          if (res.resp_code == "200") {
            this.memberpackhj = res.data || {
              servsum: 0,
              packsum: 0,
              servje: 0,
              packje: 0
    ...
    queryIntegral() {
      if (this.integralForm.date && this.integralForm.date.length > 1) {
        this.integralForm.sdate = this.integralForm.date[0];
        this.integralForm.edate = this.integralForm.date[1];
      } else {
        this.integralForm.sdate = "";
        this.integralForm.edate = "";
      }
      let params = {
        memid: this.usermemid,
        mtype: "1",
        behavior: this.integralForm.behavior, //业务类型
        startTime: this.integralForm.sdate, //开始时间
        endTime: this.integralForm.edate, //结束时间
        page: "1",
        limit: "10"
      };
      const fetch = service.member.memberinfo.getMemMoneylogList;
      this.$refs.integral.reloadData({
        fetch,
        params
      });
    },
    importMembers(file) {
      ...
      if (
        file.name &&
        (file.name.indexOf("xls") > 0 || file.name.indexOf("xlsx") > 0)
      ) {
        this.memberImport.members = [];
        imporExecl(
          file,
          this.memberImport.members,
          this.memberImpdata[this.memberImport.type]
        );
      } else {
    ...
    downExcelTemplate1() {
      download(
        $yid.config.API.BASE + "api-member/memberinfo/excelTemplate1",
        {}
      );
    },
    downExcelTemplate2() {
      download(
        $yid.config.API.BASE + "api-member/memberinfo/excelTemplate2",
        {}
      );
    },
    downExcelTemplate3() {
      download(
        $yid.config.API.BASE + "api-member/memberinfo/excelTemplate3",
        {}
      );
    },
    ...
    getPackageRecordTypeName(row) {
      ...
      if (row.isHc == "1" && row.btype != "5") value += "(撤)";
      return value;
    },
    queryCouponlogs() {
    ...
    openChangeCard(cardno) {
      this.mobile = "";
      this.changeCardDialog.visible = true;
      this.changeCardDialog.cardno = cardno;
      this.changeCardDialog.omemid = this.memberDesc.memid;
      this.changeCardDialog.omemcode = this.memberDesc.memcode;
      this.changeCardDialog.memid = "";
      this.changeCardDialog.memcode = "";
      this.changeCardDialog.memname = "";
      this.changeCardDialog.nmemname = "";
      this.changeCardDialog.photo = "";
      this.changeCardDialog.mobile = "";
      this.changeCardDialog.vercode = "";
      this.changeCardDialog.count = 0;
    },
    searchMemberMobile(mobile) {
      if (!mobile) {
        yid.util.error("请输入手机号");
        return;
      }
      if (mobile.length != 11) {
        yid.util.error("手机号格式不对");
        return;
      }
      if (this.memberDesc.mobile == mobile) {
        yid.util.error("本人手机号不能转");
        return;
      }
      service.member.memberinfo
        .queryMemberMobile({ mobile: mobile })
        .then(res => {
          if (res.resp_code == 200) {
            if (null != res.data) {
              this.changeCardDialog.memid = res.data.id;
              this.changeCardDialog.memcode = res.data.memcode;
              this.changeCardDialog.memname = res.data.memname;
              this.changeCardDialog.mobile = res.data.mobile;
              this.changeCardDialog.photo = res.data.photo;
            } else {
              yid.util.error("未找到该手机号的会员");
            }
          } else {
            yid.util.error(res.resp_msg);
          }
    ...
    sendVercode(mobile) {
      ...
      service.member.memberinfo.sendVercode({ mobile: mobile }).then(res => {
        if (res.resp_code == 200) {
          this.changeCardDialog.count = 60;
          this.timer = setInterval(this.caseCount, 1000);
          yid.util.success(res.resp_msg);
        } else {
          yid.util.error(res.resp_msg);
        }
    ...
    caseCount() {
      this.changeCardDialog.count = this.changeCardDialog.count - 1;
      if (this.changeCardDialog.count <= 0) {
        clearInterval(this.timer);
        this.timer = "";
      }
    },
    async sureVercode(mobile, code) {
      if (!mobile) {
        yid.util.error("请输入手机号");
        return;
      }
      ...
      if (!code) {
        yid.util.error("请输入验证码");
        return;
      }
      if (code.length != 6) {
        yid.util.error("验证码格式不对");
        return;
      }
      await service.member.memberinfo
        .sureVercode({ mobile: mobile, code: code })
        .then(res => {
          if (res.resp_code != 200) {
            yid.util.error(res.resp_msg);
            return;
      ...
      await service.member.memberinfo
        .checkMember({ cardno: this.changeCardDialog.cardno })
        .then(res => {
          if (res.resp_code != 200) {
            yid.util.error(res.resp_msg);
            return;
      ...
      let param = {
        memid: this.changeCardDialog.memid,
        memcode: this.changeCardDialog.memcode,
        memname: this.changeCardDialog.memname,
        nmemname: this.changeCardDialog.nmemname,
        mobile: this.changeCardDialog.mobile,
        cardno: this.changeCardDialog.cardno,
        omemid: this.changeCardDialog.omemid,
        omemcode: this.changeCardDialog.omemcode
      };
      await service.member.memberinfo.sureChangeCard(param).then(res => {
        if (res.resp_code == 200) {
          this.changeCardDialog.visible = false;
          yid.util.success(res.resp_msg);
          this.getMember({ memid: this.changeCardDialog.omemid });
        } else {
          yid.util.error(res.resp_msg);
        }
      });
    ...
    //查看卡备注
    queryCardContext(row) {
      this.cardMemo.visible = true;
      this.cardMemo.title = "卡备注";
      service.member.memberinfo
        .queryCardShopMemo({
          cardno: row.cardno,
          shopid: row.shopid,
        ...
        .then(res => {
          console.log(res.data);
          this.cardMemo.data = res.data;
        });
    },
    //查询备注修改记录
    checkUpdateLog(row) {
      this.updateMemo.visible = true;
      this.updateMemo.title = "修改记录";
      service.member.memberinfo
        .queryCardMemo({ cardno: row.cardno, shopcode: row.shopcode })
        .then(res => {
          this.updateMemo.data = res.data;
        });
    },
    checkCardCancell(cardinfo) {
      const that = this;
      let flag = true;
      if (cardinfo.status != "1") {
        flag = false;
      }
      if (that.memberDesc.status != "1") {
        flag = false;
      }
      //if(that.brandCode != cardinfo.brandCode){
      //    flag = false;
      //}
      if (Number(cardinfo.money) > 0 || Number(cardinfo.gmoney) > 0) {
        flag = false;
      }
      if (that.cardpackages.length > 0) {
          that.cardpackages.forEach(pack=>{
              var today = new Date();
              var today_time =  this.formatDate(today);
              if(!yid.util.isEmpty(pack.expiryTime)){
                  var expiryTime = pack.expiryTime.substring(0,10);
                  if(today_time < expiryTime){
                      flag = false;
                  }
              }else{
                  flag = false;
      ...
      if (that.cardproducts.length > 0) {
        flag = false;
      }
        //如果是实体卡 卡号为14位而且结尾为0001 或 0002
        if(cardinfo.cardno && cardinfo.cardno.length == 14){
            if(cardinfo.cardno.substr(10) == '0001'){
                this.memberDesc.cards.forEach(card => {
                    if(card.cardno.substr(10) == '0002'){
                        if (card.status != '1') {
                            flag = false;
                        }
                        if (Number(card.money) > 0 || Number(card.gmoney) > 0) {
                            flag = false;
                        }
                    }

                    service.member.memberinfo.memStorepack({
                        memid: this.memberDesc.memid,
                        cmid: card.id
                    }).then(res => {
                        if (res.resp_code == "200") {
                    ...
                    service.member.memberinfo.queryCardPackages({
                        memid: this.memberDesc.memid,
                        cmid:  card.id
                    }).then(res => {
                        if (res.resp_code == "200") {
            ...
            if(cardinfo.cardno.substr(10) == '0002'){
                this.memberDesc.cards.forEach(card => {
                    if(card.cardno.substr(10) == '0001'){
                        if (card.status != '1') {
                            flag = false;
                        }
                        if (Number(card.money) > 0 || Number(card.gmoney) > 0) {
    ...
    cancellCard(cardinfo) {
      yid.util.confirm(
        "即将作废会员卡，并且不可恢复，是否确定继续操作？",
        "会员卡注销",
        "",
        () => {
          let memid = this.memberDesc.memid;
          let cardno = cardinfo.cardno;
          const params = { memid: memid, cardno: cardno };
          service.member.memberinfo.cancellCard(params).then(res => {
            if (res.resp_code == 200) {
              this.cardinfo.status = "4";
              yid.util.success("会员卡注销成功!");
            } else {
              yid.util.error(res.resp_msg);
            }
          });
      ...
      cancellMember(membercard) {
      let money = 0;
      let gmoney = 0;
      let flag = true;
      let msg = "";
      //判断会员是否存在卡金余额，赠送金余额、套餐余额、寄存产品、未使用的优惠券、红包、未使用的骁骁网团购订单、良品订单、积分
      console.log(this.memberDesc);
      if (this.memberDesc.cards != null && this.memberDesc.cards.length > 0) {
        this.memberDesc.cards.forEach(card => {
          money = Number(money) + Number(card.money);
          gmoney = Number(gmoney) + Number(card.gmoney);
        });
      }
      if (money > 0) {
        flag = false;
      ...
      if (this.memberDesc.packages != null && this.memberDesc.packages.length > 0) {
          let flag1 = true;
          this.memberDesc.packages.forEach(pack=>{
              var today = new Date();
              var today_time =  this.formatDate(today);
              if(!yid.util.isEmpty(pack.expiryTime)){
                  var expiryTime = pack.expiryTime.substring(0,10);
      ...
      if (this.memproducts != null && this.memproducts.length > 0) {
        flag = false;
        msg = msg + "寄存产品,";
      }
      if (this.memberDesc.couponInfos != null && this.memberDesc.couponInfos.length > 0) {
        flag = false;
        msg = msg + "未使用的优惠券,";
      }
      if (this.memberDesc.redenvelopeInfos != null && this.memberDesc.redenvelopeInfos.length > 0) {
        flag = false;
        msg = msg + "未使用的红包,";
      }
      //调用骁骁网接口查询骁骁网团购订单、良品订单、积分
           service.member.memberinfo
        .getOrderPageList({
          page: 1,
          limit: 10,
          memberId: this.usermemid,
          orderStatus: '2,3,4',
        })
        .then(res => {
          if (res.resp_code == 200) {
            if (res.data.length > 0) {
              ...
              msg = msg + "未使用的骁骁网团购订单,";
                ...
                const orderMap = {};
                orderMap.page = 1;
                orderMap.pageSize = 10;
                orderMap.memberId = this.usermemid;
                orderMap.orderStatusList = ['2','3','4'];
                //调用骁骁网接口查询骁骁网良品订单、良品订单、积分
                service.member.memberinfo
                    .getMemberOrderList(orderMap)
                    .then(res => {
                        if (res.resp_code == 200) {
                            if (res.data.length > 0) {
                                flag = false;
                            ...
                            if (!flag) {
                                ...
                                yid.util.confirm(
                                    ...
                                    "",
                                    () => {
                                        let memid = membercard.memid;
                                        const params = { memid: memid };
                                        service.member.memberinfo.cancellMember(params).then(res => {
                                            if (res.resp_code == 200) {
                                                this.memberDesc.status = "3";
                                                yid.util.success("会员资料注销成功!");
                                            } else {
                                                yid.util.error(res.resp_msg);
                                            }
                                        });
    ...
    restoreMem(row) {
      yid.util.confirm(
        ...
        () => {
          let memid = row.id;
          const params = { memid: memid };
          service.member.memberinfo.restoreMember(params).then(res => {
            if (res.resp_code == 200) {
              yid.util.success("会员资料恢复成功!");
              this.queryMemberCancells();
            } else {
              yid.util.error(res.resp_msg);
            }
          });
    ...
    relieveRelevance(row) {
      this.relieveDialog.visible = true;
      this.relieveDialog.memname = this.memberDesc.memname;
      this.relieveDialog.mobile = this.memberDesc.mobile;
      this.relieveDialog.dcardno = row.dcardno;
      this.relieveDialog.scardno = row.scardno;
      this.relieveDialog.memid = this.memberDesc.memid;
      this.relieveDialog.id = row.id;
    },
    recoverRelevance(row) {
      this.recoverDialog.visible = true;
      this.recoverDialog.memname = this.memberDesc.memname;
      this.recoverDialog.mobile = this.memberDesc.mobile;
      this.recoverDialog.dcardno = row.dcardno;
      this.recoverDialog.scardno = row.scardno;
      this.recoverDialog.memid = this.memberDesc.memid;
      this.relieveDialog.id = row.id;
    },
    sureRelieve() {
      service.member.memberinfo.sureRelieve(this.relieveDialog).then(res => {
        if (res.resp_code == 200) {
          this.relieveDialog.visible = false;
          this.getCardRelation(this.memberDesc.memid);
        }
      });
    },
    cancelRelieve() {
      this.relieveDialog.visible = false;
      this.recoverDialog.visible = false;
    },
    recoverRelieve(){
      this.sureCode(this.recoverDialog.mobile, this.recoverDialog.verifyCode)
    },
    async sureCode(mobile, code) {
      if (!code) {
        yid.util.error("请输入验证码");
        return;
      }
      ...
      service.member.memberinfo.sureVercode({mobile: mobile, code: code}).then(res => {
        if (res.resp_code != 200) {
          yid.util.error(res.resp_msg)
          return
        } else {
          service.member.memberinfo.recoverRelieve(this.recoverDialog).then(res => {
            if (res.resp_code == 200) {
              this.recoverDialog.visible = false;
              this.getCardRelation(this.memberDesc.memid);
            }
    ...
    //点击关联实体卡
    relationCard(cardinfo){
        if(cardinfo.dawncmcode == '' || cardinfo.dawncmcode == null){              //0代表未关联，只允许未关联的进行设置
    ...
    closedRelation(e){
        ...
        if(e !== '' || e !==null || e !== undefined){
            this.cardinfo.dawncmcode = e
            // this.cardinfo.type = 1
            this.$forceUpdate()
        }
    },
      formatDate(strTime) {
