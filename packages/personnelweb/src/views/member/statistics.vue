            ...
            <el-tab-pane label="会员数据汇总表" name="total">
                <el-form ref="searchForm" inline :model="searchForm" >
                    <el-form-item label="门店：" prop="shopid" :rules="[{ required: true, message: '必须选择一个门店'}]">
                        <el-select v-model.trim="searchForm.shopid" @clear="clearFormShop" @blur="clearFormShop" filterable :filter-method="filterShop" placeholder="请选择门店" style="width: 160px;">
            ...
            <el-tab-pane label="会员消费分析" name="consume">
                ...
                <yid-table pagination ref="servtable">
                    ...
                    <yid-table-column label="按月消费金额" min-width="150" prop="cmMoney">
                        <template slot-scope="scope">
                            {{scope.row.cmMoney == null ? scope.row.cmMoney : scope.row.cmMoney + '元'}}
                        </template>
                    </yid-table-column>
                    <yid-table-column label="按月消费次数" min-width="150" prop="cmNum">
                        <template slot-scope="scope">
                            {{scope.row.cmNum == null ? scope.row.cmNum : scope.row.cmNum + '次'}}
                        </template>
                    </yid-table-column>
                    <yid-table-column label="按月消费客单价" min-width="150" prop="cmUnivalent">
                        <template slot-scope="scope">
                            {{scope.row.cmUnivalent == null ? scope.row.cmUnivalent : scope.row.cmUnivalent + '元'}}
                        </template>
                    </yid-table-column>
                    <yid-table-column label="按年消费总额" min-width="150" prop="servTotal">
                        <template slot-scope="scope">
                            {{scope.row.servTotal == null ? scope.row.servTotal : scope.row.servTotal + '元'}}
                        </template>
                    </yid-table-column>
                    <yid-table-column label="按年消费总次数" min-width="150" prop="servNum">
                        <template slot-scope="scope">
                            {{scope.row.servNum == null ? scope.row.servNum : scope.row.servNum + '次'}}
                        </template>
                    </yid-table-column>
                    <yid-table-column label="按年消费客单价" min-width="150" prop="univalent">
                        <template slot-scope="scope">
                            {{scope.row.univalent == null ? scope.row.univalent : scope.row.univalent + '元'}}
                        </template>
                    </yid-table-column>
                </yid-table>
            </el-tab-pane>
    ...
    export default {
        ...
        methods:{
            ...
            exportMemServ(){
                ...
                download(yid.config.API.BASE + 'api-member/statisticsym/statisticsYmExportForChain', this.searchservForm)
            },
            handleClick(row){
                if(row.name == 'total'){
