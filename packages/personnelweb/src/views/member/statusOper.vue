<template>
  <div class="member-statusOper">
      ...
      <el-tab-pane label="冻结管理" name="freeze">
        ...
        <yid-table pagination ref="table">
          ...
          <yid-table-column
            label="冻结原因"
            min-width="150"
            prop="freezeMemo"></yid-table-column>
          <yid-table-column label="操作" min-width="150" fixed="right">
            <template slot-scope="scope">
              <el-link type="primary" @click="cancelFreezeMember(scope.row)"
                >解冻</el-link
              >
            </template>
          </yid-table-column>
    ...
    <yid-dialog
        ...
        <el-form inline>
          <el-form-item label="输入冻结原因：">
            <el-input
              ...
              :rows="2"
              clearable
              v-model="memDialog.freezeMemo"
              placeholder="请输入冻结原因，200字以内"
              style="width: 600px"></el-input>
          </el-form-item>
          <el-form-item label="添加会员资料：">
            <el-input
              clearable
              v-model.trim="memDialog.name"
              placeholder="请输入手机号或卡号"
              style="width: 240px"></el-input>
          </el-form-item>
          <el-form-item label="">
          ...
          <yid-table
            ...
            <yid-table-column label="操作" min-width="60" fixed="right">
              <template slot-scope="scope">
                <el-button
                  type="text"
                  @click="deleteMem(scope.$index, scope.row)"
      ...
      已添加<font color="red">{{ memdata.length }}</font
      >条会员
      <el-row justify="space-between" style="margin-top: 10px">
        <el-col>
          <el-button style="margin-left: 300px" @click="memCancel"
            >取消</el-button
          >&nbsp;&nbsp;&nbsp;&nbsp;
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      if (this.searchForm.code == '1') params.mobile = this.searchForm.name
      if (this.searchForm.code == '2') params.cardno = this.searchForm.name
      if (this.searchForm.code == '3') params.memname = this.searchForm.name
      if (this.searchForm.code == '4') params.memberid = this.searchForm.name
    ...
    cancelFreezeMember(row) {
      yid.util.confirm(
        '您确定要解冻[' + row.memname + ']的会员资料?',
        '',
        '',
        () => {
          service.member.memberinfo
            .cancelFreezeMember({ memid: row.id || row.memid })
            .then(res => {
    ...
    queryAddMem() {
      ...
      service.member.memberinfo
        .chainPageMembersNoPage({
          name: this.memDialog.name,
          status: this.memDialog.status
        })
        .then(res => {
          if (res.resp_code == '200') {
    ...
    freezeMembers() {
      ...
      service.member.memberinfo
        .freezeMember({
          memids: memids.substr(0, memids.length - 1),
          freezeMemo: this.memDialog.freezeMemo
        })
        .then(res => {
          if (res.resp_code == '200') {
