  ...
  <div class="dept">
    <el-form inline>
      <el-form-item label="">
        ...
        <el-button type="primary" @click="importSpecial" >导入员工价格</el-button>
      </el-form-item>
      <br/>
      ...
      <el-form-item label="员工：">
        <el-input clearable v-model="form.ee" placeholder="请输入工员或名称" style="width: 160px;"></el-input>
      </el-form-item>
      <el-form-item label="职务：" prop="pscode">
        <el-select clearable filterable placeholder="请选择" multiple v-model.trim="form.pscode">
          <el-option :key="item.pscode" :label="item.psname" :value="item.pscode"
    ...
    <yid-table ref="tableService" pagination style="width: 100%;" height="calc(100vh - 370px)"
               @selection-change="selectionData"
               :span-method="spanMethod">
      <el-table-column type="selection" width="55">
      </el-table-column>
      <yid-table-column label="员工工号" prop="eecode" width="100"></yid-table-column>
      <yid-table-column label="员工姓名" prop="eename" width="100"></yid-table-column>
      <yid-table-column label="职务" prop="psname" width="100"></yid-table-column>
      <yid-table-column label="职务级别" prop="pslname" width="140"></yid-table-column>
      <yid-table-column label="状态" prop="status" width="80">
        <template slot-scope="scope">
          <label>{{ scope.row.status == '1' ? '在职' : '离职' }}</label>
        </template>
      </yid-table-column>
      <yid-table-column label="剪发原价" prop="jfyjMoney" width="100"></yid-table-column>
      <yid-table-column label="剪发会员价" prop="jfhyjMoney" width="100"></yid-table-column>
      <yid-table-column label="店号" prop="shopcode" width="100"></yid-table-column>
      <yid-table-column label="所属门店" prop="shopname" width="140"></yid-table-column>
      <yid-table-column label="门店执行原价" prop="speprice" width="110">
      </yid-table-column>
      <yid-table-column label="门店执行会员价" prop="value" width="120">
        <template slot-scope="scope">
          <label v-if="scope.row.discountType == '2' && scope.row.dtype == '1'">{{ scope.row.value }}折</label>
          <label v-if="scope.row.discountType == '2' && scope.row.dtype == '2'">{{ scope.row.value }}</label>
          <label v-if="scope.row.discountType == '1' && scope.row.dtype == '1'">{{ scope.row.value }}折</label>
          <label v-if="scope.row.discountType == '1' && scope.row.dtype == '2'">{{ scope.row.value }}</label>
        </template>
      </yid-table-column>
      <yid-table-column label="价格类型" prop="istsj" width="100">
        <template slot-scope="scope">
          <label v-if="scope.row.istsj == '0'">{{ '个人价' }}</label>
          <label style="color: #ff0000" v-if="scope.row.istsj == '1'">{{ '门店价' }}</label>
          <label style="color: #ff0000" v-if="scope.row.istsj == '2'">{{ '特殊价' }}</label>
        </template>
      </yid-table-column>
      <yid-table-column label="入职日期" prop="inTime" width="160"></yid-table-column>
      <yid-table-column label="操作" min-width="120" prop="content" fixed="right">
        <template slot-scope="scope">
          <el-link type="primary" @click="setPrice(scope.row)">设置</el-link>
          <el-link type="primary" @click="selectLog(scope.row)" style="margin: 0 10px 0 10px;">查看</el-link>
        </template>
      </yid-table-column>
    ...
    <yid-dialog :title="price.title" :visible.sync="price.visible" width="400px">
      <el-form :model="price" label-width="140px">
        ...
        <el-checkbox v-model="price.isSynch">同步此价格到所有做业绩的门店</el-checkbox>
    ...
    <yid-dialog :title="updateLog.title" :visible.sync="updateLog.visible" width="650px">
      <yid-table :data="updateLog.data">
        <yid-table-column prop="createdTime" label="修改日期" min-width="100"></yid-table-column>
        <yid-table-column prop="jfyjMoney" label="剪发原价" min-width="100"></yid-table-column>
        <yid-table-column prop="jfhyjMoney" label="剪发会员价" min-width="100"></yid-table-column>
        <yid-table-column prop="createdBy" label="操作人" min-width="100"></yid-table-column>
      </yid-table>
    </yid-dialog>

    <yid-dialog :title="eeData.title" :visible.sync="eeData.visible" width="650px">
      <div style="margin-bottom: 10px">
        <el-checkbox v-model="eeData.isSynch">同步此价格到所有做业绩的门店</el-checkbox>
        <el-popover style="padding-left: 10px;"
                    placement="right"
                    title="提示"
                    width="380"
    ...
    <yid-dialog :title="importSpecialData.title" :visible.sync="importSpecialData.visible" @close="importCancel"
                width="700px">
      <el-form :model="importSpecialData.model" label-width="120px" ref="copyForm" :label-position="labelPosition">
        <p>批量导入员工价格方法</p>
        <p>1、点击链接:下载员工价格表模板</p>
        <p>2、下载完成并保存文件，然后打开文件并输入员工价格信息</p>
        <p>3、在下面导入修改完毕的模板文件</p>
        <el-form-item label="下载导入模板：" align="left">
          <el-link @click="downExcelTemplate" type="primary">员工价格表模板</el-link>
        </el-form-item>
        ...
        <el-form-item label="选择导入文件:">
          ...
          </el-upload><el-checkbox v-model="importSpecialData.isSynch">同步此价格到所有做业绩的门店</el-checkbox>
          <el-popover style="padding-left: 10px;"
                      placement="right"
                      title="提示"
                      width="380"
  ...
  methods: {
    spanMethod({row, column, rowIndex, columnIndex}) {
      if (columnIndex === 0 || columnIndex === 1 || columnIndex === 2 || columnIndex === 3 || columnIndex === 4 || columnIndex === 5
          || columnIndex === 6 || columnIndex === 7 || columnIndex === 13 || columnIndex === 14) {
        if (rowIndex === 0 || this.$refs.tableService.internalData[rowIndex - 1].eecode != row.eecode) {
          let rowspan = 0
        ...
        } else if (row.eecode == this.$refs.tableService.internalData[rowIndex - 1].eecode) {
          return [0, 0]
        } else {
          return [1, 1]
        }
    ...
    exportEeSpecial() {
      ...
      if (this.form.shopcode == '' && this.form.ee == '') {
        if (this.form.pscode.length == 0 && this.form.status == '' && this.form.shopcode == ''
    ...
    downExcelTemplate() {
      download(yid.config.API.BASE + 'api-base/serviceinfo/specialExcelTemplate', {})
    },
    selectionData(val) {
      var newArr= [];
    ...
    sureSelect() {
      if (this.eeObj.length <= 0) {
        ...
        for (let index in this.eeObj) {
          ...
          if (item.eename) {
            if (!item.jfhyjMoney || !item.jfyjMoney || item.jfhyjMoney == '' || item.jfyjMoney == '') {
    ...
    sure() {
      ...
      if (!numRe.test(this.price.jfyjMoney) || !numRe.test(this.price.jfhyjMoney)) {
