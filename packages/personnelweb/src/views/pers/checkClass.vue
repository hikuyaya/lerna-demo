    ...
    <div class="checkClass">
        ...
        <yid-table pagination ref="checkClassTable" style="margin-top: 15px;" :row-class-name="$yid.util.getPersCheckClass">
            ...
            <yid-table-column label="状态" min-width="70" prop="status">
                <template slot-scope="scope">
                    <el-switch disabled="disabled" v-model="scope.row.status" active-value="1" inactive-value="2" active-color="#339933" inactive-color="#DCDFE6">
                    </el-switch>
                </template>
            </yid-table-column>
...
<script>
    ...
    export default {
        ...
        methods : {
            ...
            getIsEdit(row){
                if(row.ccode == '-1' || row.ccode=='-2' || row.ccode=='学习' || row.ccode=='休息'){
                    return true;
                }else{
                    return false;
                }
            ...
            saveCheckClass(){
                this.$refs['checkClassForm'].validate((valid) => {
                    if(valid){
                        ...
                        if(this.checkClassForm.cname.indexOf("休息") != -1 || this.checkClassForm.cname.indexOf("学习") != -1){
