<template>
  <div class="checkClass">
    ...
    <yid-table
      ...
      :row-class-name="$yid.util.getPersCheckClass">
      <yid-table-column
        label="编码"
        min-width="80"
      ...
      <yid-table-column label="班次代表颜色" min-width="100" prop="color">
        <template slot-scope="scope">
          <div
            :style="[
              {
                background: getbgcolor(scope.row.color),
                width: '20px',
                height: '20px'
              }
            ]"></div>
      ...
      <!--<yid-table-column label="操作" min-width="100" prop="content">
                <template slot-scope="scope">
                    <el-link type="primary" :disabled="getIsEdit(scope.row)" @click="editCheckClass(scope.row)">编辑</el-link>
                </template>
            </yid-table-column>-->
    </yid-table>
    <yid-dialog
      :title="checkClassDialog.title"
      :visible.sync="checkClassDialog.visible"
      width="500px">
      <el-form ref="checkClassForm" :model="checkClassForm" label-width="160px">
        <el-form-item
          label="班次名称："
          prop="cname"
          :rules="[{ required: true, message: '班次名称为空' }]">
          <el-input
            v-model="checkClassForm.cname"
        ...
        <el-form-item
          label="班次代表颜色："
          prop="color"
          :rules="[{ required: true, message: '班次代表颜色为空' }]">
          <div
            :style="[
              {
                background: getbgcolor(this.checkClassForm.color),
                width: '20px',
                height: '20px'
              }
            ]"></div>
          <el-radio-group v-model="checkClassForm.color" @change="changeColor">
            <el-radio label="1#ff0000"
              ><div
                style="background-color: #ff0000; width: 20px; height: 20px"
            /></el-radio>
            <el-radio label="2#cb3939"
              ><div
                style="background-color: #cb3939; width: 20px; height: 20px"
            /></el-radio>
            <el-radio label="3#00b6ff"
              ><div
                style="background-color: #00b6ff; width: 20px; height: 20px"
            /></el-radio>
            <el-radio label="4#3373f3"
              ><div
                style="background-color: #3373f3; width: 20px; height: 20px"
            /></el-radio>
            <el-radio label="5#be00ee"
              ><div
                style="background-color: #be00ee; width: 20px; height: 20px"
            /></el-radio>
            <el-radio label="6#65007e"
              ><div
                style="background-color: #65007e; width: 20px; height: 20px"
            /></el-radio>
            <el-radio label="7#2407ff"
              ><div
                style="background-color: #2407ff; width: 20px; height: 20px"
            /></el-radio>
            <el-radio label="8#0c0064"
              ><div
                style="background-color: #0c0064; width: 20px; height: 20px"
            /></el-radio>
            <el-radio label="9#07f6ff"
              ><div
                style="background-color: #07f6ff; width: 20px; height: 20px"
            /></el-radio>
            <el-radio label="10#008a8f"
              ><div
                style="background-color: #008a8f; width: 20px; height: 20px"
            /></el-radio>
            <el-radio label="11#07ff4d"
              ><div
                style="background-color: #07ff4d; width: 20px; height: 20px"
            /></el-radio>
            <el-radio label="12#008425"
              ><div
                style="background-color: #008425; width: 20px; height: 20px"
            /></el-radio>
            <el-radio label="13#abf007"
              ><div
                style="background-color: #abf007; width: 20px; height: 20px"
            /></el-radio>
            <el-radio label="14#618900"
              ><div
                style="background-color: #618900; width: 20px; height: 20px"
            /></el-radio>
            <el-radio label="15#ffde00"
              ><div
                style="background-color: #ffde00; width: 20px; height: 20px"
            /></el-radio>
            <el-radio label="16#b29b00"
              ><div
                style="background-color: #b29b00; width: 20px; height: 20px"
            /></el-radio>
            <el-radio label="17#ff4e00"
              ><div
                style="background-color: #ff4e00; width: 20px; height: 20px"
            /></el-radio>
            <el-radio label="18#812700"
              ><div
                style="background-color: #812700; width: 20px; height: 20px"
            /></el-radio>
            <el-radio label="19#ff5353"
              ><div
                style="background-color: #ff5353; width: 20px; height: 20px"
            /></el-radio>
            <el-radio label="20#ff1e00"
              ><div
                style="background-color: #ff1e00; width: 20px; height: 20px"
            /></el-radio>
          </el-radio-group>
        </el-form-item>
    ...
    saveCheckClass() {
      this.$refs['checkClassForm'].validate(valid => {
        if (valid) {
          ...
          if (
            this.checkClassForm.cname.indexOf('休息') != -1 ||
            this.checkClassForm.cname.indexOf('学习') != -1
          ) {
            yid.util.info('不能定义休息或者学习名称的班次')
            return false
          }
          if (
            this.checkClassForm.status == null ||
            this.checkClassForm.status.length == 0
          ) {
            this.checkClassForm.status = '1'
          }
          if (
            this.checkClassForm.stime == null ||
            this.checkClassForm.stime.length == 0 ||
            this.checkClassForm.etime == null ||
            this.checkClassForm.etime.length == 0
          ) {
            yid.util.info('请选择上班时间')
            return false
          }
          if (
            this.checkClassForm.astime == null ||
            this.checkClassForm.astime.length == 0 ||
            this.checkClassForm.aetime == null ||
            this.checkClassForm.aetime.length == 0
          ) {
            yid.util.info('请选择上午上班时间')
            return false
          }
          if (
            this.checkClassForm.pstime == null ||
            this.checkClassForm.pstime.length == 0 ||
            this.checkClassForm.petime == null ||
            this.checkClassForm.petime.length == 0
          ) {
          ...
          service.pers.checkClass
            .getColorList({
              id: this.checkClassForm.id,
              color: String(this.checkClassForm.color)
            })
            .then(res => {
              if (res.resp_code == 200) {
                if (res.data.length > 0) {
                  yid.util.confirm(
                    ...
                    () => {
                      service.pers.checkClass
                        .saveCheckClass(this.checkClassForm)
                        .then(res => {
                          if (res.resp_code == 200) {
