<template>
    <div>
        <el-form ref="searchForm" inline :model="searchForm">
                <el-form-item label="门店：" @clear="clearFormShopList"  prop="shopid" :rules="[{ required: true, message: '请选择一个门店'}]">
        ...
        <el-form ref="form" :model="configForm" label-width="180px" color="#000" style="color: #000;padding-left: 60px">

            <label>1、迟到：</label>
            迟到超过 <el-input v-model="configForm.allowCd" onKeypress="return(/[\d]/.test(String.fromCharCode(event.keyCode)))" style="width: 80px"></el-input> 分钟后计为迟到；<br/><br/>
            <label>2、早退：</label>
            早退超过 <el-input v-model="configForm.allowZt" onKeypress="return(/[\d]/.test(String.fromCharCode(event.keyCode)))" style="width: 80px"></el-input> 分钟后计为早退；<br/><br/>
            <label>3、旷工设置：</label>
            ...
            <span style="padding-left: 30px; color: #999; display: block;margin: 10px 0 15px 0">提示：默认为扫码签到，如果勾选以下方式，则扫码签到时需同时满足勾选方式方可签到成功</span>
            <div style="padding-left: 60px">
                <el-checkbox v-model="configForm.isAddress" @change="checkAddress" disabled/>1、办公地点
                ...
                <el-checkbox v-model="configForm.isWifi" @change="checkWifi" disabled style="margin-top: 20px; margin-right: 30px" />2、Wi-Fi连接
    ...
    export default {
        ...
        methods: {
            ...
            getCheckRuleList(){
                ...
                service.pers.checkRule.checkruleList({isDel : '0','shopid':this.searchForm.shopid}).then(res=>{
                    ...
                    if(res.resp_code == 200) {
                        if(res.data.length > 0){
                            ...
                            if(res.data[0].isWifi === '1'){
                                ...
                                this.configForm.isWifi = false
                            )
                            if(res.data[0].isAddress === '1'){
                                this.configForm.isAddress = true
                            }else(
                                this.configForm.isAddress = false
                            )
                            this.wifiData = [];
                            this.addressData = [];
                            //给列表赋值
