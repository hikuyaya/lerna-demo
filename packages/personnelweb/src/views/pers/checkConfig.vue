<template>
  <div>
    ...
    <el-form
      ...
      label-width="180px"
      color="#000"
      style="color: #000; padding-left: 60px">
      <label>1、迟到：</label>
      迟到超过
      <el-input
        v-model="configForm.allowCd"
      ...
      早退超过
      <el-input
        v-model="configForm.allowZt"
        onKeypress="return(/[\d]/.test(String.fromCharCode(event.keyCode)))"
        style="width: 80px"></el-input>
      分钟后计为早退；<br /><br />
      <label>3、旷工设置：</label>
      <div style="max-height: 300px; overflow-y: scroll; margin-left: 60px">
        <div>
          <div style="margin: 10px 0 15px 0">
            <span style="margin-right: 20px">
              ...
              <el-input
                ...
                style="width: 80px"></el-input>
              分钟为旷工：
              <el-select v-model="ruleForm.kgDays" style="width: 90px">
                <el-option
                  v-for="item in kgdays"
                  :key="item.id"
                  :label="item.name"
                  :value="item.id"></el-option> </el-select
              >天</span
      ...
      <el-time-picker
        type="date"
        placeholder="选择日期"
        v-model="configForm.cstartTime"
        format="HH:mm"
        value-format="HH:mm"
        style="width: 100px"></el-time-picker>
      <yid-ask
        >例：开始时间为03:00，则03:00之前签到记录会作为前一天的签到</yid-ask
      ><br /><br />
      5、考勤方式：<label style="color: #999">（app端设置）</label><br />
      <span
        style="
          padding-left: 30px;
          color: #999;
        ...
        "
        >提示：默认为扫码签到，如果勾选以下方式，则扫码签到时需同时满足勾选方式方可签到成功</span
      >
      <div style="padding-left: 60px">
        <el-checkbox
          v-model="configForm.isAddress"
          @change="checkAddress"
          disabled />1、办公地点
        ...
        <el-table
          ...
          <el-table-column label="有效范围" min-width="80" prop="accuracy">
            <!-- <template slot-scope="scope">
          ...
          <!--<el-table-column label="操作" min-width="80" prop="content">
                        <template slot-scope="scope">
                            <el-link :disabled="!configForm.isAddress"  type="primary" @click="addressAlert(scope.row)">地图选址</el-link>
                        </template>
                    </el-table-column>-->
        </el-table>
        <el-checkbox
          v-model="configForm.isWifi"
          @change="checkWifi"
          disabled
          style="margin-top: 20px; margin-right: 30px" />2、Wi-Fi连接
        ...
        <el-table
          ...
          <!--<el-table-column label="操作" min-width="80" prop="content">
                        <template slot-scope="scope">
                            <el-link :disabled="!configForm.isWifi" type="primary" @click="wifiAlert(scope.row)">编辑</el-link>
                        </template>
                    </el-table-column>-->
        </el-table>
      </div>
    ...
    <yid-dialog
      :title="wifiDialog.title"
      :visible.sync="wifiDialog.visible"
      width="450px">
...
export default {
  ...
  methods: {
    ...
    getCheckRuleList() {
      service.pers.checkRule
        .checkruleList({ isDel: '0', shopid: this.searchForm.shopid })
        .then(res => {
      ...
      service.pers.shopConfig.shopconfigList({}).then(res => {
        if (res.resp_code == 200) {
          if (res.data.length > 0) {
            ...
            if (res.data[0].isWifi === '1') {
              this.configForm.isWifi = true
            } else this.configForm.isWifi = false
            if (res.data[0].isAddress === '1') {
              this.configForm.isAddress = true
            } else this.configForm.isAddress = false
            this.wifiData = []
            this.addressData = []
            //给列表赋值
            if (res.data[0].bssid != null && res.data[0].bssid.length > 0) {
              ...
              this.wifiData.push({
                bssid: this.wifiForm.bssid,
                mac: this.wifiForm.mac
              })
            } else {
    ...
    seachRecords() {
      ...
      this.$refs['searchForm'].validate(valid => {
        if (valid) {
          service.pers.shopConfig
            .shopconfigList({ shopid: this.searchForm.shopid })
            .then(res => {
              if (res.resp_code == 200) {
                if (res.data.length > 0) {
                  this.configForm = res.data[0]
                  ...
                  //给列表赋值
                  if (
                    res.data[0].bssid != null &&
                    res.data[0].bssid.length > 0
                  ) {
                    this.wifiForm.bssid = res.data[0].bssid
                    this.wifiForm.mac = res.data[0].mac
                    this.wifiData.push({
    ...
    delRule(ruleid) {
      service.pers.checkRule
        .saveCheckrule({ id: ruleid, isDel: '1' })
        .then(res => {
          if (res.resp_code == 200) {
            this.getCheckRuleList()
          }
    ...
    saveConfig() {
      if (
        this.configForm.allowCd == null ||
        this.configForm.allowCd.length == 0
      ) {
      ...
      if (
        this.configForm.allowZt == null ||
        this.configForm.allowZt.length == 0
      ) {
      ...
      if (this.configForm.isAddress) {
        if (
          this.addressData[0].address == null ||
          this.addressData[0].address.length == 0
        ) {
          yid.util.info('请编辑考勤地点')
          return
        }
        if (
          this.addressData[0].lat == null ||
          this.addressData[0].lat.length == 0
        ) {
          yid.util.info('请编辑经度')
          return
        }
        if (
          this.addressData[0].lng == null ||
          this.addressData[0].lng.length == 0
        ) {
          yid.util.info('请编辑纬度')
          return
        }
        if (
          this.addressData[0].accuracy == null ||
          this.addressData[0].accuracy.length == 0
        ) {
      ...
      if (this.configForm.isAddress) {
        this.configForm.isAddress = '1'
      } else {
        this.configForm.isAddress = '0'
      }

      if (this.configForm.isWifi) {
        this.configForm.isWifi = '1'
      } else {
        this.configForm.isWifi = '0'
      }
