<template>
  <div class="checkRecords">
      ...
      <el-tab-pane label="查看明细" name="detail">
        <el-form ref="searchForm" inline :model="searchForm">
          ...
          <el-form-item label="职务：" prop="pscode">
            <el-select
              ...
              filterable
              placeholder="请选择"
              v-model.trim="searchForm.pscode"
          ...
          <el-form-item label="级别：" prop="pslcode">
            <el-select
              ...
              filterable
              placeholder="请选择"
              v-model.trim="searchForm.pslcode">
              <el-option
                :key="item.pslcode"
                :label="item.pslname"
                :value="item.pslcode"
          ...
          <el-form-item label="人员：" prop="memname">
            <el-input
              ...
              filterable
              placeholder="姓名/工号"
              v-model="searchForm.memname"></el-input>
          </el-form-item>
          <el-form-item
            label="选择日期"
          ...
          <el-form-item style="margin-bottom: 0">
            <el-button @click="seachRecords('day')" type="primary"
              >查询</el-button
            >
          </el-form-item>
          <el-form-item style="margin-bottom: 0">
            <el-button @click="expRecords('day')" type="primary"
              >导出</el-button
            >
          </el-form-item>
        </el-form>
        <yid-table pagination ref="table" :data="tableData">
          ...
          <el-table-column label="上班打卡" align="center">
            <el-table-column prop="onWork" label="打卡时间" width="120">
              <template slot-scope="scope">
                ...
                <i
                  class="el-icon-edit"
                  @click="alertStatusDialog(scope.row, 'on', 0, 'day')"></i>
              </template>
            </el-table-column>
          ...
          <el-table-column label="下班打卡" align="center">
            <el-table-column prop="offWork" label="打卡时间" width="120">
              <template slot-scope="scope">
                ...
                <i
                  class="el-icon-edit"
                  @click="alertStatusDialog(scope.row, 'off', 0, 'day')"></i>
              </template>
            </el-table-column>
          ...
          <yid-table-column
            label="迟到时长（分）"
            min-width="100"
            prop="delay"></yid-table-column>
          <yid-table-column
            label="早退时长(分)"
            min-width="100"
            prop="zt"></yid-table-column>
          <yid-table-column label="休息(天)" min-width="80" prop="rest">
            <template slot-scope="scope">
              {{ scope.row.rest }}
              <i
                class="el-icon-edit"
                @click="alertkqDialog(scope.row, 'rest', 'day')"></i>
            </template>
          </yid-table-column>
          <yid-table-column label="学习(天)" min-width="80" prop="study">
            <template slot-scope="scope">
              ...
              <i
                class="el-icon-edit"
                @click="alertkqDialog(scope.row, 'study', 'day')"></i>
            </template>
          </yid-table-column>
          <yid-table-column label="旷工(天)" min-width="80" prop="kg">
            <template slot-scope="scope">
              ...
              <i
                class="el-icon-edit"
                @click="alertkqDialog(scope.row, 'kg', 'day')"></i>
            </template>
          </yid-table-column>
        </yid-table>
      </el-tab-pane>
      ...
      <el-tab-pane label="查看月表" name="months">
        <el-form ref="searchForm_month" inline :model="searchForm_month">
          <el-form-item
            ...
            :rules="[{ required: true, message: '门店不能为空' }]">
            <el-select
              @clear="clearFormShopList('month')"
              v-model.trim="searchForm_month.shopid"
              filterable
              :filter-method="filterShop"
              placeholder="请选择门店"
              style="width: 160px">
          ...
          <el-form-item label="职务：" prop="pscode">
            <el-select
              ...
              filterable
              placeholder="请选择"
              v-model.trim="searchForm_month.pscode"
          ...
          <el-form-item label="级别：" prop="pslcode">
            <el-select
              ...
              filterable
              placeholder="请选择"
              v-model.trim="searchForm_month.pslcode">
              <el-option
                :key="item.pslcode"
                :label="item.pslname"
                :value="item.pslcode"
          ...
          <el-form-item label="人员：" prop="memname">
            <el-input
              ...
              filterable
              placeholder="姓名/工号"
              v-model="searchForm_month.memname">
            </el-input>
          </el-form-item>
          <el-form-item
            label="选择日期"
          ...
          <el-form-item style="margin-bottom: 0">
            <el-button @click="seachRecords('month')" type="primary"
              >查询</el-button
            >
          </el-form-item>
          <el-form-item style="margin-bottom: 0">
        ...
        <yid-table
          ...
          <el-table-column
            :label="
              (month.substring(0, 1) == 0 ? month.substring(1) : month) +
              '月' +
              item +
              '号'
            "
            ...
            <el-table-column label="班次" min-width="70" align="center">
              <template slot-scope="scope">
                <span
                  class="classname"
                  :class="
                    getProps(scope.row, item, 'classname') == '未排班'
                      ? 'not'
                      : ''
                  "
                  >{{ getProps(scope.row, item, 'classname') }}</span
                >
              </template>
            </el-table-column>
            <el-table-column label="上班" min-width="70" align="center">
              <template slot-scope="scope">
                <span
                  class="kqBox"
                  @click="alertStatusDialog(scope.row, 'on', item, 'month')"
                  :class="
                    getProps(scope.row, item, 'classname') == '未排班'
                      ? ''
                      : getProps(scope.row, item, 'onStatus') == '正常'
                      ? ''
                      : getProps(scope.row, item, 'onStatus') == '迟到' ||
                        getProps(scope.row, item, 'onStatus') == '早退'
                      ? 'not1'
                      : getProps(scope.row, item, 'onStatus') == '旷工'
                      ? 'not2'
                      : ''
                  ">
                  <span class="kqzt"
                    >{{ getProps(scope.row, item, 'onStatus') }}
                    {{ getProps(scope.row, item, 'onBkStatus') }}</span
                  >
                  <span class="dktime">{{
                    getProps(scope.row, item, 'onWork')
                  }}</span>
                </span>
              </template>
            </el-table-column>
            <el-table-column label="下班" min-width="70" align="center">
              <template slot-scope="scope">
                <span
                  class="kqBox"
                  @click="alertStatusDialog(scope.row, 'off', item, 'month')"
                  :class="
                    getProps(scope.row, item, 'classname') == '未排班'
                      ? ''
                      : getProps(scope.row, item, 'offStatus') == '正常'
                      ? ''
                      : getProps(scope.row, item, 'offStatus') == '迟到' ||
                        getProps(scope.row, item, 'offStatus') == '早退'
                      ? 'not1'
                      : getProps(scope.row, item, 'offStatus') == '旷工'
                      ? 'not2'
                      : ''
                  ">
                  <span class="kqzt"
                    >{{ getProps(scope.row, item, 'offStatus') }}
                    {{ getProps(scope.row, item, 'offBkStatus') }}</span
                  >
                  <span class="dktime">{{
                    getProps(scope.row, item, 'offWork')
                  }}</span>
                </span>
              </template>
            </el-table-column>
    ...
    <yid-dialog
      :title="bcDialog.title"
      :visible.sync="bcDialog.visible"
      width="450px"
      @close="seachRecords">
    ...
    <yid-dialog
      :title="kqDialog.title"
      :visible.sync="kqDialog.visible"
      width="450px"
      @close="seachRecords('day')">
    ...
    <yid-dialog
      ...
      <el-form ref="kqForm" :model="statusForm">
        ...
        <el-form-item>
          <el-button class="editButton" @click="sureChangeStatus" type="primary"
            >确定</el-button
          >
        </el-form-item>
      </el-form>
      ...
      <el-form>
        <el-form-item>
          <div class="sureText">
            <span
              >调整打卡时间后, 考勤记录会标记为"补卡",
              并根据打卡时间重新核实考勤状态。 是否确定修改?</span
            >
            <!-- <span>是否确定修改?</span> -->
          </div>
        </el-form-item>
        <el-form-item>
          <el-button class="editButton" @click="changeStatus" type="primary"
            >确定</el-button
          >
          <el-button
            class="editButton"
...
export default {
  ...
  methods: {
    ...
    getData(reqParams, type) {
      ...
      if (type == 'day') {
        ...
        service.pers.checkRecords.getShopMonthRecords(params).then(res => {
          ...
          let month_temp =
            Number(date_temp.getMonth()) + 1 > 10
              ? Number(date_temp.getMonth()) + 1 + ''
              : '0' + (Number(date_temp.getMonth()) + 1)
          let days
          ...
          if (res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              if (month_tableData.length == 0) {
                month_tableData.push({
                  ...
                  shopcode:
                    res.data[i].shopcode == null
                      ? this.searchForm_month.shopcode
                      : res.data[i].shopcode,
                  data: []
                })
                  ...
                  month_tableData[0].data.push({
                    cdate:
                      n < 9
                        ...
                        : this.month + '-' + (n + 1),
                    id:
                      res.data[i].cdate != null
                        ? res.data[i].cdate.substring(8, 10) ==
                          (n < 9 ? '0' + (n + 1) : n + 1)
                          ? res.data[i].id
                          : null
                        : null,
                ...
                if (index == -1) {
                  ...
                  let index_temp = month_tableData[index].data.findIndex(
                    item => item.cdate == res.data[i].cdate.substring(5, 10)
                  )
                  month_tableData[index].data[index_temp] = {
                    cdate:
                      index_temp < 9
                        ? this.month + '-0' + (index_temp + 1)
                        : this.month + '-' + (index_temp + 1),
                    id:
                      res.data[i].cdate != null
                        ? res.data[i].cdate.substring(8, 10) ==
                          (index_temp < 9
            ...
            service.pers.checkRecords
              .getShopEeScheduleList(params1)
              .then(res => {
                for (let i = 0; i < month_tableData.length; i++) {
                  for (let m = 0; m < month_tableData[i].data.length; m++) {
                    if (
                      month_tableData[i].data[m].classname == null ||
                      month_tableData[i].data[m].classname == ''
                    ) {
                      if (
                        month_tableData[i].data[m].id == null ||
                        month_tableData[i].data[m].id == ''
                      ) {
                        let item = ''
                        item = res.data.filter(item => {
                          if (
                            month_tableData[i].eecode == item.eecode &&
                            month_tableData[i].data[m].cdate ==
                              item.day.substring(5, 10)
                          ) {
                            return item.classes
                          }
                        })[0]
    ...
    expRecords(type) {
      ...
      download(
        $yid.config.API.BASE + 'api-pers/checkrecords/expRecordsList',
        params
      )
    },
          ...
          if (j == 'eename') {
            ...
            if (j != null) {
              if (j.indexOf('+') == -1) {
                ...
                return v.data[index][prop] == null
                  ? '未排班'
                  : v.data[index][prop]
              } else {
                //一个单元格取考勤状态,打卡时间,(是否是补卡状态) 3个值需要拼接
                let arr = j.split('+')
                let index = arr[0].split('.')[0]
                let prop1
                if (v.data[index][arr[0].split('.')[1]] == 1) {
                  //打卡状态
                  prop1 = '正常'
                ...
                let bkStatus =
                  v.data[index][arr[2].split('.')[1]] == 1 ? '(补)' : '' //补卡状态

                let prop2 = arr[1].split('.')[1] //打卡时间
                let time =
                  v.data[index][prop2] == null
                    ? ''
                    : v.data[index][prop2].substring(11, 16)
    ...
    handleDownload() {
      ...
      let month =
        this.month.substring(0, 1) == '0' ? this.month.substring(1) : this.month
      for (let i = 0; i < this.days; i++) {
        multiHeader[0].push(month + '月' + (i + 1) + '号', '', '')
        tHeadr.push('班次', '上班', '下班')
        filterVal.push(
          i + '.classname',
          i + '.onStatus+' + i + '.onWork+' + i + '.onBkStatus',
          i + '.offStatus+' + i + '.offWork+' + i + '.offBkStatus'
        )
      }
    ...
    alertBcDialog(row) {
      ...
      ;(this.bcForm.id = row.id), (this.bcForm.bcid = '')
      this.bcForm.bccode = ''
    ...
    alertStatusDialog(row, type, index, tab) {
      ...
      if (tab == 'month') {
        ...
        if (type == 'on') {
          this.bkTime =
            row.data[index - 1].onWork != null
              ? row.data[index - 1].onWork.substring(11, 16)
              : ''
          this.oldTime =
            row.data[index - 1].onWork != null
              ? row.data[index - 1].onWork.substring(11, 16)
          ...
          this.bkTime =
            row.data[index - 1].offWork != null
              ? row.data[index - 1].offWork.substring(11, 16)
              : ''
          this.oldTime =
            row.data[index - 1].offWork != null
              ? row.data[index - 1].offWork.substring(11, 16)
        ...
        this.day =
          row.cdate.substring(8, 10).substring(0, 1) == 0
            ? row.cdate.substring(8, 10).substring(1)
            : row.cdate.substring(8, 10)
        if (type == 'on') {
          ...
          this.oldTime =
            row.offWork != null ? row.offWork.substring(11, 16) : ''
    ...
    getProps(evt, day, type) {
      if (type == 'classname') {
        return evt.data[day - 1].classname == null
          ? '未排班'
          : evt.data[day - 1].classname
      } else if (type == 'onWork') {
        return evt.data[day - 1].onWork == null
          ? ''
          : evt.data[day - 1].onWork.substring(11, 16)
        ...
        return evt.data[day - 1].offWork == null
          ? ''
          : evt.data[day - 1].offWork.substring(11, 16)
