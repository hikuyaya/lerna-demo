    ...
    <div class="employeeSchedule">
        <el-form :model="ruleForm" ref="ruleForm" inline>
            ...
            <el-form-item label="职务：" prop="pscode">
                <el-select  clearable filterable placeholder="请选择" v-model.trim="ruleForm.pscode" @change="getSearchFormPositionLevelList">
                    <el-option style="width: 100px;"  :key="item.id" :label="item.psname" :value="item.pscode" v-for="item in searchFormPositionList"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="级别：" prop="pslcode">
                <el-select  clearable filterable placeholder="请选择" v-model.trim="ruleForm.pslcode">
                    <el-option :key="item.id" :label="item.pslname" :value="item.pslcode" v-for="item in searchFormPositionLevelList"></el-option>
                </el-select>
            </el-form-item>
        ...
        <el-table ref="multipleTable" style="width: 100%;overflow-x: auto;overflow-y: auto;" height="470px" :data="tableData" @selection-change="handleSelectionChange">
            ...
            <el-table-column v-for="(item) in monthDayList"
                             align="center"
                             :prop="item.day"
                             :label="item.dayWeek">
                <template slot-scope="scope">
                    <div
                         :style="[{'background': scope.row[item.day]? getColor(scope.row[item.day].color) :'#FFFFFF','cursor':'pointer'}]"
                    >  {{scope.row[item.day]?scope.row[item.day].cname:"--"}}</div>
        ...
        <yid-dialog :title="pbDialog.title" :visible.sync="pbDialog.visible" width="450px">
            <el-form ref="pbForm" :model="pbForm"  label-width="140px">
                ...
                <el-form-item label="班次：" prop="pccode" :rules="[{ required: true, message: '请选择班次'}]">
                    <el-select value-key="id" placeholder="请选择班次" v-model.trim="pbForm.pccode">
                        <el-option :key="item.code" :value="item.ccode" :label="item.cname" v-for="item in checkClassAll"/>
                    </el-select>
                </el-form-item>
                <el-form-item>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            chageBc(row,item){
                //改变排班表信息
                const scheduleparam = {pccode:this.checkClassAll[classkey].ccode,dayStr:item.day,eeid:row.id,eecode:row.eecode,eename:row.eename};
