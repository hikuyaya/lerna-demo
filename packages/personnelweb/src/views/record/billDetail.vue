<template>
  <span class="billDetail">
      ...
      <el-row v-if="this.pageName !== 'onlineSale'" style="text-align: right; margin-top: 20px">
      ...
      <el-form ref="ruleForm" label-width="100px" >
          <el-form-item v-if="this.baseData.entryData.isHc == '1' && this.pageName !== 'onlineSale'" label="撤单时间：">
                {{this.baseData.entryData.hcTime}}
            </el-form-item>
            <el-form-item v-if="this.baseData.entryData.isHc == '1' && this.pageName !== 'onlineSale'" label="撤单人：">
                {{this.baseData.entryData.hcBy}}
            </el-form-item >
            <el-form-item v-if="this.baseData.entryData.isHc == '1' && this.pageName !== 'onlineSale'" label="撤单备注：">
                {{this.baseData.entryData.hcMemo}}
            </el-form-item>
          <el-form-item label="服务单号：" v-if="this.pageName !== 'onlineSale'">
              {{this.baseData.entryData.billcode}}
          </el-form-item>
          <el-form-item label="线上客装销售单号：" class="onlinedh" v-else style="width:300px;">
              {{this.baseData.entryData.billcode}}
          </el-form-item>
          <el-form-item label="客户信息：" v-if="this.pageName !== 'onlineSale'">
              {{this.baseData.entryData.custname ? this.baseData.entryData.custname : this.baseData.entryData.memName}}&nbsp;
              {{getSexName(this.baseData.entryData.custsex?this.baseData.entryData.custsex:this.baseData.entryData.sex)}}&nbsp;
              {{this.baseData.entryData.custtel?this.baseData.entryData.custtel:this.baseData.entryData.mobile}}&nbsp;
              {{this.baseData.entryData.cardInfo == null ? '' : '【'+this.baseData.entryData.cardInfo+' '+this.baseData.entryData.cmcode+'】'}}
              {{this.isgx == '1'? '(共享卡)':''}}
              {{this.baseData.entryData.dawncmcode? '关联实体卡:'+this.baseData.entryData.dawncmcode:''}}
          </el-form-item>
          ...
          <el-form-item label="提货方式：" v-if="this.pageName === 'onlineSale'">
              {{this.baseData.entryData.pickWay == 0 ? '物流部邮寄' : this.baseData.entryData.pickWay == 1 ? '到店自提' : ''}}
          </el-form-item>
          ...
          <el-form-item v-show="this.baseData.entryData.memid && this.pageName !== 'onlineSale'" label="开单方式：">
          ...
          <el-form-item v-show="this.baseData.entryData.yycode && this.pageName !== 'onlineSale'" label="预约单据：">
               {{this.baseData.entryData.yycode}}&nbsp;&nbsp;&nbsp;&nbsp;{{this.baseData.entryData.yydcname}}
          </el-form-item>
            <el-form-item v-if="this.pageName === 'onlineSale'" class="onlinetime" label="提货时间/核销时间: " style="width: 300px;">
              ...
              <el-table
                  ...
                  <el-table-column
                    ...
                    <template slot-scope="scope">
                          {{scope.row.sname}}{{(scope.row.norms == undefined || scope.row.norms == '')?'':'('+scope.row.norms+')'}}{{(scope.row.isgx&&scope.row.isgx == '1')?"(共享)":''}}
                    </template>
                  </el-table-column>
                  <el-table-column
                          prop="stype"
                          ...
                          width="180">
                      <template slot-scope="scope">
                          {{scope.row.stype == '1' ? "项目" : "客装"}} {{scope.row.isKeep == '0' ? '' : '(寄存)'}}
                      </template>
                  </el-table-column>
                  <el-table-column
                          prop="num"
                  ...
                  <el-table-column
                      ...
                      <template slot-scope="scope">
                          ...
                          <span v-if="scope.row.additional != 0">
                              <span v-if="scope.row.additionaltype == 1">+{{scope.row.additional}}</span>
                              <span v-if="scope.row.additionaltype == 2">-{{scope.row.additional}}</span>
                          </span>
                      </template>
                  </el-table-column>
                  <el-table-column
          ...
          <el-form label="" v-else>
              <el-table :data="this.tablelist" border style="width: 80%;margin-left:30px;">
          ...
          <el-form-item label="支付方式：" v-if="this.pageName !== 'onlineSale'">
              ...
              <span v-if="Number(this.baseData.entryData.zkePop) > 0">活动折扣 -￥{{this.baseData.entryData.zkePop}}<br></span>
              <span v-if="Number(this.baseData.entryData.zkeHyq) > 0">优惠券 -￥{{this.baseData.entryData.zkeHyq}}
                  {{coupons}}<br>
              </span >
              <span v-for="pays in this.baseData.entryData.payList">{{pays.payname}} ￥{{pays.money}}{{pays.isdf=='1' || pays.isgx == '1'?'(代付)':''}}<br></span>
              <div style="border-top: 1px solid #333;width: 250px" />
              <span>应收金额：￥{{this.baseData.entryData.hjbillje}}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;消费金额：￥{{this.baseData.entryData.hjbillje}}</span>
          </el-form-item>
            <el-form-item label="支付方式：" class="onlinezf" style="display:flex;margin-top: 2vh;" v-else>
                <span>单据金额：￥{{this.baseData.entryData.totalMoney}}</span><br>
                <span>骁骁网: ￥{{this.baseData.entryData.xxwbusinesslog.paymoney}}</span>
            </el-form-item>
            <el-form-item label="营销活动：" v-if="this.pageName !== 'onlineSale'">
                 <div v-for="activitythis in this.baseData.entryData.serviceActivity">
                    <span v-show="activitythis.popname">
                      {{activitythis.popname}}(合计折扣: {{activitythis.zkePop}} 赠送内容: {{activitythis.zsdescription}})
                    </span>
                </div>
            </el-form-item>
          ...
          <el-form-item label="核销记录：" v-else>
              <span>骁骁网:  {{this.baseData.entryData.xxwbusinesslog.billcode}}</span>
          </el-form-item>
          ...
          <el-form-item v-if="this.baseData.entryData.shopcode.startsWith('MF') && this.pageName !== 'onlineSale'" label="顾客类型：">
            <span v-show="this.baseData.entryData.customerType == '0'">储值客</span>
            <span v-show="this.baseData.entryData.customerType == '1'">团购客</span>
            <span v-show="this.baseData.entryData.customerType == '2'">非储值客</span>
          </el-form-item>
           ...
           <el-form-item label="温馨提示: " v-if="this.pageName === 'onlineSale'">
                <br> 
                <span>请门店店员与顾客确认后，操作提货；提货成功后，不提供退换货服务!</span>
           </el-form-item>

      </el-form>

      <yid-dialog :title="editDialog.title" :visible.sync="editDialog.visible" width="550px">
          <span>项目名称:{{editForm.sname}}&nbsp;&nbsp;&nbsp;&nbsp;消费金额:￥{{editForm.money}}</span><br><br>

          <template v-for="station in stations">
                <div>
                    <div>第{{station.postion}}工位：&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;设置业绩(输入的金额必须大于0)  是否指定:</div>
                    <template :index="index" v-for="(ees,index) in editRow.royaltyList">
                        <div v-if="station.postion === ees.postion" style="margin: 5px 0 5px 0">
                           <el-select placeholder="请选择" v-model="editForm.eeyjList[index].eeid" style="width: 120px">
                            ...
                            <el-select placeholder="请选择" v-model="editForm.eeyjList[index].isApppoint" style="width: 100px">
                                <el-option v-for="item in apppoints" :key="item.id" :label="item.name" :value="item.id"></el-option>
                            </el-select>&nbsp;&nbsp;
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getYjs(saleTime,id){
                //查询业绩列表
                service.cashier.serviceRecords.servicrecordyj({saleTime:saleTime,id:id}).then(res=>{
            ...
            editBill(row){//编辑单据
                ...
                this.editForm.money = this.keepTwoDecimal(Number(row.num) * Number(row.price));
