    ...
    <div class="packagebillDetail">
        ...
        <el-form ref="ruleForm" label-width="100px" >
            ...
            <el-form-item label="客户信息：">
                {{this.baseData.entryData.memName}} {{getSexName(this.baseData.entryData.sex)}} {{this.baseData.entryData.mobile}} {{this.baseData.entryData.cardname == null ? '' : '【'+this.baseData.entryData.cardname+' '+this.baseData.entryData.cardno+'】'}}
            ...
            <el-form-item label="消费详情：">
                <el-table
                    ...
                    <el-table-column
                            ...
                            width="180">
                        <template slot-scope="scope">
                             <span v-show="scope.row.gmoney && baseData.entryData.isHcMkt != '1'">
                    ...
                    <el-table-column
                            ...
                            label="服务人员">
                        <template slot-scope="scope">
                            <span v-for="royaltys in scope.row.yjsList">销售：{{royaltys.eecode}} {{royaltys.eename}}<br></span>
                    ...
                    <el-table-column
                            ...
                            label="业绩">
                        <template slot-scope="scope">
                            <span v-for="royaltys in scope.row.yjsList">￥{{royaltys.yjje}}<br></span>
                        </template>
                    </el-table-column>
                    <el-table-column
                            prop="royalty"
                            label="提成">
                        <template slot-scope="scope">
                            <span v-for="royaltys in scope.row.yjsList">￥{{royaltys.royalty == null ? 0 :royaltys.royalty}}<br></span>
                        </template>
                    </el-table-column>
                </el-table>
            </el-form-item>
            <el-form-item label="支付方式：">
                ...
                <span v-for="pays in this.baseData.entryData.paysList">{{pays.payname}} ￥{{pays.payje}}<br></span>
                <div style="border-top: 1px solid #333;width: 250px" />
                <span>应收金额：￥{{this.baseData.entryData.ysje}}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;消费金额：￥{{this.baseData.entryData.ysje}}</span>
            </el-form-item>
            <el-form-item label="核销记录：">
                <span v-for="pays in this.baseData.entryData.xmdList">{{pays.paycode=='27'?'新美大':'骁骁网'}}：{{pays.paymentNo}}&nbsp;&nbsp;&nbsp;&nbsp;</span>
            </el-form-item>
            <el-form-item label="活动详情：">
                ...
                <div class="zengs" v-show="this.baseData.entryData.isHcMkt !='1' && (Object.keys(this.baseData.entryData.zsinfo).length != 0 || this.baseData.entryData.gmoney)">
                    <span class="item" v-show="this.baseData.entryData.mktBillmemo">充值累计单据：
                        <label>【{{this.baseData.entryData.mktBillmemo}}】</label>&nbsp;&nbsp;</span>
                    <span class="item">活动名称：<label>【{{this.baseData.entryData.mktname}}】</label></span>
                    <span class="item" >赠送内容： /
                                <label v-show="this.baseData.entryData.gmoney">赠送金：{{'￥'+this.baseData.entryData.gmoney}}  /  </label>
                                <label v-show="this.baseData.entryData.zsinfo.walletname">赠送红包：{{this.baseData.entryData.zsinfo.walletname}}  /  </label>
                                <label v-show="this.baseData.entryData.zsinfo.sername">赠送服务项目：{{this.baseData.entryData.zsinfo.sername}}  /  </label>
                                <label v-show="this.baseData.entryData.zsinfo.productname">赠送产品：{{this.baseData.entryData.zsinfo.productname}}  /</label>
                                <label v-show="this.baseData.entryData.zsinfo.zqname">赠送优惠券：{{this.baseData.entryData.zsinfo.zqname}}  /</label>
                            </span>
                </div>
            </el-form-item>
        ...
        <yid-dialog :title="editDialog.title" :visible.sync="editDialog.visible" width="500px">
            <span>消费内容:{{editForm.content}}&nbsp;&nbsp;&nbsp;&nbsp;消费金额:￥{{editForm.money}}</span><br><br>

            <template v-for="station in stations">
                <div>
                    <div>销售：&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;设置业绩(输入的金额必须大于0)</div>
                    <template :index="index" v-for="(ees,index) in editRow.yjsList">
                        <div v-if="station.postion === ees.postion" style="margin: 5px 0 5px 0">
                            ...
                            <el-input v-model="editForm.eeyjList[index].yjje" style="width: 150px"></el-input>&nbsp;&nbsp;
    ...
    export default {
        ...
        methods: {
            async initData(obj){
                ...
                if(this.baseData.entryData.cardZsInfoVOS && this.baseData.entryData.cardZsInfoVOS.length>0){
                    let walletsArr = []
                    let zqArr = []
                    let sersArr = []
                    let productsArr = []
                    ...
                    this.$set(this.baseData.entryData.zsinfo,'name',this.baseData.entryData.cardZsInfoVOS[0].maname)
                    this.$set(this.baseData.entryData.zsinfo,'walletname',walletsArr.join(','))
                    this.$set(this.baseData.entryData.zsinfo,'zqname',zqArr.join(','))
                    this.$set(this.baseData.entryData.zsinfo,'sername',sersArr.join(','))
                    this.$set(this.baseData.entryData.zsinfo,'productname',productsArr.join(','))
                }
            ...
            getYjs(saleTime,id){
                //查询业绩列表
                service.cashier.cardRecords.cardrecordyj({saleTime:saleTime,id:id}).then(res=>{
