            ...
            <div v-show="showList">
                <el-form ref="searchForm" inline :model="searchForm">
                    ...
                    <el-form-item label="充值单号：" prop="refbillcode">
                        <el-input v-model="searchForm.refbillcode" clearable  style="width: 180px;"></el-input>
                    </el-form-item>
                ...
                <yid-table  ref="table" pagination style="width: 100%;margin-top: 20px;">
                    <yid-table-column prop="billcode" label="调整单单号" width="150"></yid-table-column>
                    <yid-table-column prop="shopname" label="门店" width="100"></yid-table-column>
                    <yid-table-column prop="refbillcode" label="充值单号" width="150"></yid-table-column>
                    <yid-table-column prop="createdBy" label="创建人" width="100"></yid-table-column>
                    <yid-table-column prop="createdTime" label="创建时间" width="150">
                        <template slot-scope="scope">
                            {{$yid.util.formatTime(scope.row.createdTime)}}
                        </template>
                    </yid-table-column>
                    <yid-table-column prop="auditedBy" label="审核人" width="100"></yid-table-column>
                    ...
                    <yid-table-column
                        ...
                        <template slot-scope="scope">
                            <el-button type="text" @click="showBill(scope.row)" style="margin-left: 6px">明细</el-button>
                        </template>
                    </yid-table-column>
                </yid-table>
            </div>
        ...
        <el-collapse-transition>
            <div v-show="!showList">
                ...
                <el-form ref="billForm" :model="billForm" style="margin-top: 20px" label-width="100px" label-position="right">
                    <div id="container">
                        <div style="width: 200px;display: inline-block;">门店：{{billForm.shopcode}}&nbsp;&nbsp;{{billForm.shopname}}</div>
                        <div style="width: 210px;display: inline-block;">调整单号：{{billForm.billcode}}</div>
                ...
                <div>
                    <el-divider/>
                    <div style="width: 40%;height:20px;display: inline-block;">原单据信息：</div>
                    <div style="width: 10%;height:20px;display: inline-block;"></div>
                    <div style="width: 40%;height:20px;display: inline-block;">调整单据：</div>
                </div>
                <div id="billdetail">
                    <div style="width: 40%;height:380px;display: inline-block;border:1px solid rgba(0,0,0,0.25);">
                        <el-form ref="oldDetailForm" :model="oldDetailForm" style="margin-top: 5px;margin-left: 10px" label-width="100px" label-position="right">
                            <div>
                                <div style="width: 350px;">原单据编码：<el-input v-model="oldDetailForm.refbillcode" :disabled="refbillcodediv" style="width: 180px;"></el-input>
                                    &nbsp;&nbsp;<el-link :disabled="refbillcodediv" type="primary" @click="queryCardBill">查询</el-link></div>
                                <div style="width: 300px;">会员姓名：{{oldDetailForm.memName}}</div>
                                <div style="width: 300px;">会员卡号：{{oldDetailForm.cardno}}</div>
                                <div style="width: 300px;">付款金额：{{oldDetailForm.ysje}}</div>
                                <div style="width: 300px;">充值门店：{{oldDetailForm.refshopcode}}{{oldDetailForm.refshopname}}</div>
                                <div style="width: 300px;">付款方式：
                                    <span v-for="pays in oldDetailForm.csadjPayolds">{{pays.payname}}|{{pays.payje}}<br></span>
                                </div>
                            </div>
                            <div>
                                <el-divider/>
                            ...
                            <div>
                                ...
                                <template v-for="station in stations">
                                    <div>
                                        {{station.name}}<br>
                                        <template :index="index" v-for="(ees,index) in oldDetailForm.csadjYjolds">
                                            <div v-if="station.postion === ees.postion" style="margin: 5px 0 5px 0">
                                                <el-select placeholder="请选择" disabled="disabled" v-model="oldDetailForm.csadjYjolds[index].eeid" style="width: 120px">
                                                    ...
                                                    <el-option label="公单" value="-1"></el-option>
                                                </el-select>&nbsp;
                                                <el-input disabled="disabled" v-model="oldDetailForm.csadjYjolds[index].je" style="width: 150px"></el-input>&nbsp;
                                            </div>
                                        </template>
                                    </div>
                                </template>
                    ...
                    <div style="width: 40%;height:380px;display: inline-block;border:1px solid rgba(0,0,0,0.25);">
                        <template v-for="station in stations">
                            <div style="margin-top: 5px;margin-left: 10px">
                                {{station.name}}<br>
                                <template :index="index" v-for="(ees,index) in oldDetailForm.csadjYjs">
                                    <div v-if="station.postion === ees.postion" style="margin: 5px 0 5px 0">
                                        <el-select placeholder="请选择" :disabled="showNewDetail" v-model="oldDetailForm.csadjYjs[index].eeid" style="width: 120px">
                                            <template v-for="psls in station.pslist">
                                                <template v-for="emp in employeeList">
                                                    <div v-if="emp.pscode === psls.pscode">
                                                        <el-option  :key="emp.id" :label="emp.eename" :value="emp.id"></el-option>
                                        ...
                                        <el-input :disabled="showNewDetail" v-model="oldDetailForm.csadjYjs[index].je" style="width: 150px"></el-input>&nbsp;&nbsp;
                                        <el-link type="primary" :disabled="showNewDetail" @click="delEmployee(station,ees,index)">删除</el-link>
                                    </div>
                                </template>
                                <div>
                                    <el-link type="primary" :disabled="showNewDetail" @click="addEmployee(station)">添加</el-link>
                                </div>
                            </div>
                        </template>
                    </div>
