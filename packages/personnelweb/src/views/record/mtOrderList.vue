    ...
    <yid-table pagination ref="table" style="margin-top: 15px">
      <yid-table-column label="来源" min-width="120" prop="channel">
        <template slot-scope="scope">{{
          scope.row.channel == 'DY'
            ? '抖音'
            : scope.row.channel == 'MT'
            ? '美团'
            : ''
        }}</template>
      </yid-table-column>
      <yid-table-column label="券号" min-width="120" prop="code">
        <template slot-scope="scope">{{
          scope.row.realCode ? scope.row.realCode : scope.row.code
        }}</template>
      </yid-table-column>
      <yid-table-column
        label="商品名称"
        min-width="250"
        prop="product"></yid-table-column>
      <yid-table-column
        label="券金额"
        min-width="100"
      ...
      <yid-table-column
        label="核销时间"
        min-width="150"
        prop="paySuccTime"></yid-table-column>
      ...
      <yid-table-column
        ...
        <template slot-scope="scope">
          <span v-if="scope.row.status == 1"
            ><el-link type="primary" @click="tkBill(scope.row)"
              >标记退款</el-link
            ></span
          >
          <span v-if="scope.row.status == 4"
            ><el-link type="primary" @click="cancelTkBill(scope.row)"
              >取消标记</el-link
            ></span
          >
        </template>
