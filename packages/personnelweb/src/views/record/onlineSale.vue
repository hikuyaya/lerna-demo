<template>
    <div class="member-relation">
        <el-collapse-transition>
            <div v-show="!detailShow">
                <el-form ref="searchForm" inline :model="searchForm">
                    ...
                    <el-form-item label="业务类型：">
                        <el-select clearable v-model="searchForm.brandCode" @change="changeShop" style="width: 100px">
                ...
                <!-- 线上客装销售记录 -->
                <yid-table class="onlinetable" pagination ref="table" style="margin-top: 15px;">
                    <yid-table-column label="线上客装销售单号" min-width="200" prop="billcode"></yid-table-column>
                    <yid-table-column label="店名" min-width="150" prop="shopname"></yid-table-column>
                    <yid-table-column label="提货时间" min-width="200" prop="updatedTime"></yid-table-column>
                    <yid-table-column label="顾客" min-width="200" prop="memname">
                        <template slot-scope="scope">
                            <!-- {{scope.row.memname + ' '+scope.row.sex + ' ' + scope.row.mobile}} -->
                            {{scope.row.memname}} {{scope.row.sex == null ? '' :scope.row.sex == 1 ? '男' : '女'}} {{scope.row.mobile}}
                        </template>
                    </yid-table-column>
                    ...
                    <yid-table-column label="单据金额" min-width="200" prop="hjbillje"></yid-table-column>
                    <yid-table-column label="提货方式" min-width="200" prop="pickWay">
                        <template slot-scope="scope">
                            {{scope.row.pickWay == 0 ? '物流部邮寄' : scope.row.pickWay == 1 ? '到店自提' : ''}}
                        </template>
                    </yid-table-column>
                    <yid-table-column label="操作人" min-width="150" prop="createdBy"></yid-table-column>
                    <yid-table-column label="详情" min-width="150" prop="content" fixed="right">
                        <template slot-scope="scope">
                            <el-link type="primary" @click="checkDetail(scope.row)">详情</el-link>
                        </template>
                    </yid-table-column>
                </yid-table>
            </div>
         ...
         <el-collapse-transition>
            <div v-show="detailShow">
                <billDetail ref="billDetailPage" :pageName="pageName" @onClose="closeDetailPage"></billDetail>
            </div>
        </el-collapse-transition>
