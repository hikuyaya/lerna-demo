    ...
    <el-form ref="ruleForm" label-width="100px">
      ...
      <el-form-item label="消费详情：">
        <el-table :data="tableData" border style="width: 100%">
          <el-table-column prop="spname" label="消费内容" width="180">
          </el-table-column>
          <el-table-column prop="sname" label="套餐内容" width="180">
            <template slot-scope="scope">
              <span v-for="serv in scope.row.servList"
                >【{{ serv.stype == '2' ? '产品' : '项目' }}】
                {{ serv.sname }}*{{ serv.num }}<br
              /></span>
            </template>
          </el-table-column>
          <el-table-column prop="royaltyNum" label="数量" width="100">
          </el-table-column>
          <el-table-column prop="ysje" label="总价">
            <template slot-scope="scope">
      ...
      <el-form-item label="活动详情：">
        ...
        <div class="zengs" v-show="zsData.length > 0" v-for="zs in zsData">
          <span class="item"
            >活动名称：<label>【{{ zs.name }}】</label></span
          >
          <span class="item"
            >赠送内容： /
            <label v-show="zs.walletname"
              >赠送红包：{{ zs.walletname }} /
            </label>
            <label v-show="zs.ticketname"
              >赠送优惠券：{{ zs.ticketname }} /
            </label>
          </span>
        </div>
  ...
  methods: {
    ...
    getYjs(saleTime, id) {
      ...
      service.cashier.packageRecords
        .packagerecordyj({ saleTime: saleTime, id: id })
        .then(res => {
    ...
    getZss(saleTime, id) {
      service.cashier.packageRecords
        .packagerecordzs({ saleTime: saleTime, id: id })
        .then(res => {
          if (res.resp_code == 200) {
