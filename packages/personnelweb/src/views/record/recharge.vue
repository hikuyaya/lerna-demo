  ...
  <div class="member-relation">
      ...
      <div v-show="!detailShow">
        ...
        <yid-table pagination ref="table" style="margin-top: 15px">
          ...
          <yid-table-column
            label="充值累计单据"
            min-width="180"
            prop="mktBillmemo"></yid-table-column>
          ...
          <yid-table-column
            ...
            <template slot-scope="scope">
              ...
              >
              <el-link
                v-if="scope.row.mktname != null && scope.row.isHcMkt != '1'"
                type="primary"
                @click="revokeMktBill(scope.row)"
    ...
    <el-collapse-transition>
      <div v-show="detailShow">
        <cardbillDetail
          ref="billDetailPage"
          @onClose="closeDetailPage"></cardbillDetail>
      </div>
    </el-collapse-transition>
      ...
      <el-form label-width="140px" ref="copyForm">
        ...
        <el-form-item label="开卡充值活动：" prop="ismkt">
          <el-select
            ...
            filterable
            placeholder="请选择"
            v-model.trim="searchForm.ismkt"
            @change="clearMktBillcode">
            <el-option
              :key="item.id"
              :label="item.name"
              :value="item.id"
              v-for="item in isMktList"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="" prop="mktname" v-if="searchForm.ismkt != '0'">
          ...
          <el-select
            ...
            filterable
            placeholder="请选择"
            v-model.trim="searchForm.mktbillcode">
            <el-option
              :key="item.billcode"
              :label="item.name"
              :value="item.billcode"
              v-for="item in mktInfos"></el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    importBill() {
      ...
      if (
        $yid.util.isEmpty(updatedTimeStart) ||
        $yid.util.isEmpty(updatedTimeEnd)
      ) {
      ...
      download(
        $yid.config.API.BASE +
          'api-cashier/cardrecords/cardrecordsExportForChain',
        this.searchFormReq
      )
    },
