<template>
    <div class="member-relation">
            ...
            <div v-show="!detailShow">
                ...
                <yid-table pagination ref="table" style="margin-top: 15px;">
                    ...
                    <yid-table-column label="消费内容" min-width="200" prop="snames">
                        <template slot-scope="scope">
                            <span v-for="infos in scope.row.infoVOList">{{infos.spname}}*{{1}}<br></span>
                        </template>
                    </yid-table-column>
                    ...
                    <el-table-column
                            ...
                            label="服务人员">
                        <template slot-scope="scope">
                            <span v-for="royaltys in scope.row.royaltyList">{{royaltys.eename}}|{{royaltys.yj}}<br></span>
                        </template>
                    </el-table-column>
        ...
        <el-collapse-transition>
            <div v-show="detailShow">
                <packagebillDetail ref="billDetailPage" @onClose="closeDetailPage"></packagebillDetail>
            </div>
        </el-collapse-transition>
        ...
        <yid-dialog :title="searchDialog.title" :visible.sync="searchDialog.visible" width="450px">
            <el-form label-width="140px" ref="copyForm">
                ...
                <el-form-item label="项目/套餐名称：" prop="isOpen">
                    <el-select clearable value-key="id" filterable placeholder="请选择" v-model.trim="searchForm.spcode">
                        <el-option :key="item.spcode" :label="item.spname" :value="item.spcode" v-for="item in packages"></el-option>
                    </el-select>
                </el-form-item>
        ...
        methods: {
           ...
           /* getCraftsmans(){
            ...
            importBill(){
                ...
                download($yid.config.API.BASE + 'api-cashier/packagerecords/packagerecordsExportForChain', this.searchFormReq)
            },
