<template>
    <div class="member-relation">
        <el-tabs v-model="activeName" @tab-click="handleClick" v-show="!detailShow">
                ...
                <yid-table pagination ref="table">
                    ...
                    <yid-table-column label="卡号" min-width="150" prop="cmcode">
                        <template slot-scope="scope">
                            ...
                            <span>{{scope.row.cmcode}}<br>{{scope.row.isgx == '1'?"(共享卡)":""}}</span>
                        </template>
                    </yid-table-column>
                    <yid-table-column label="顾客" min-width="180" prop="cardInfo" class="titlelabel">
                        <template slot-scope="scope">
                            <span>{{scope.row.custname ? scope.row.custname : scope.row.memName}} |{{getSexName(scope.row.custsex ? scope.row.custsex : scope.row.sex)}}</span><br>
                            <span>计客次：{{scope.row.isCompute == '1' ? "是" : "否"}}</span><br/>
                            <span>{{scope.row.custtel ? scope.row.custtel : scope.row.mobile}}</span><br>
                        </template>
                    </yid-table-column>
                    <yid-table-column label="项目/客装" min-width="200" prop="servs" align="left">
                        <template slot-scope="scope">
                            <span v-for="servs in scope.row.servVOList">{{servs.sname}}*{{servs.num}} [{{getServType(servs.stype)}}]<br></span>
                        </template>
                    </yid-table-column>
                    <yid-table-column label="第一工位" min-width="180" prop="royaltys" align="left">
                        <template slot-scope="scope">
                            <span v-for="royaltys in scope.row.royaltyList">
                                <span v-if="Number(royaltys.postion) == 1">
                                    {{royaltys.eename}}|{{royaltys.yj}}|{{Number(royaltys.isApppoint) == 1 ? '指定':'不指定'}}<br>
                                </span>
                            </span>
                        </template>
                    </yid-table-column>
                    <yid-table-column label="第二工位" min-width="180" prop="royaltys" align="left">
                        <template slot-scope="scope">
                             <span v-for="royaltys in scope.row.royaltyList">
                    ...
                    <yid-table-column label="营销活动" min-width="200" prop="serviceActivity" align="left">
                        <template slot-scope="scope">
            ...
            <el-tab-pane label="已过期" name="expired">
                ...
                <yid-table pagination ref="tableOver" style="margin-top: 15px;">
                    ...
                    <yid-table-column label="店名" min-width="150" prop="shopname"></yid-table-column>
                    ...
                    <yid-table-column label="操作" min-width="120" prop="content" fixed="right">
                        <template slot-scope="scope">
                            <el-link type="primary" @click="cancelBill(scope.row)">作废</el-link>
                        </template>
                    </yid-table-column>
                </yid-table>
            </el-tab-pane>
            <el-tab-pane label="已作废" name="invalid">
                ...
                <yid-table pagination ref="tableZf" style="margin-top: 15px;">
                    ...
                    <yid-table-column label="服务单号" min-width="200" prop="billcode"></yid-table-column>
                    <yid-table-column label="店名" min-width="150" prop="shopname"></yid-table-column>
                    <yid-table-column label="开单时间" min-width="200" prop="createdTime"></yid-table-column>
                    <yid-table-column label="项目" min-width="200" prop="servs">
                        <template slot-scope="scope">
                            <span v-for="servs in scope.row.servVOList">￥{{servs.sname}}*{{servs.num}} [{{getServType(servs.stype)}}]<br></span>
                        </template>
                    ...
                    <yid-table-column label="作废时间" min-width="200" prop="cancelTime"></yid-table-column>
                    <yid-table-column label="作废理由" min-width="200" prop="cancelReason"></yid-table-column>
                    <yid-table-column label="操作人" min-width="150" prop="cancelOper"></yid-table-column>
                    <yid-table-column label="操作" min-width="120" prop="content" fixed="right">
                        <template slot-scope="scope">
                            <el-link type="primary" @click="deleteZfBill(scope.row)">删除</el-link>
                        </template>
                    </yid-table-column>
                </yid-table>
            </el-tab-pane>
        ...
        <yid-dialog :title="searchDialog.title" :visible.sync="searchDialog.visible" width="450px">
            <el-form label-width="140px" ref="copyForm">
                ...
                <el-form-item label="开单方式：" prop="isread">
                    <el-select clearable value-key="id" filterable placeholder="请选择" v-model.trim="searchForm.isread">
                        <el-option :key="item.id" :label="item.name" :value="item.id" v-for="item in isreadList"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="客装：" prop="tid">
                    <el-select v-model="searchForm.tid" filterable  placeholder="请选择" clearable>
                        <el-option
                                v-for="item in prods"
                                :key="item.pcode"
                                :label="item.pname"
        ...
        <yid-dialog :title="cancelDialog.title" :visible.sync="cancelDialog.visible" width="450px">
            <el-form label-width="140px" ref="cancelForm">
                <el-form-item label="请输入作废原因：" prop="cancelReason">
                    <el-input v-model="cancelForm.cancelReason" type="textarea" :rows="4" style="width: 80%;" placeholder="请输入0-30个字符"/>
                </el-form-item>
                <el-form-item>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getProducts(){
                service.member.memberinfo.selectProductschain({status:'1'}).then(res=>{
                    if(res.resp_code == 200) {
                        this.prods = res.data;
                    }
                })
            ...
            getServs(){
                service.member.memberinfo.selectServiceschain({isDel:'0',status:'1'}).then(res=>{
                    if(res.resp_code == 200) {
                        this.sers = res.data;
                    }
                })
            ...
            search(){
                this.$refs['searchForm'].validate((valid) => {
                    if (valid) {
                        ...
                        if($yid.util.isEmpty(this.searchFormReq.saleTimeStart) || $yid.util.isEmpty(this.searchFormReq.saleTimeEnd)){
                            $yid.util.alert("请选择日期再进行查询!");
                            return;
                        }
                        let arg = this.DateMinus(this.searchFormReq.saleTimeStart, this.searchFormReq.saleTimeEnd);
                        if(Number(arg) > 30){
            ...
            searchOver(){
                ...
                if($yid.util.isEmpty(this.searchFormReqOver.updatedTimeStart) || $yid.util.isEmpty(this.searchFormReqOver.updatedTimeEnd)){
                    $yid.util.alert("请选择日期再进行查询!");
                    return;
                }
                let arg = this.DateMinus(this.searchFormReqOver.updatedTimeStart, this.searchFormReqOver.updatedTimeEnd);
                if(Number(arg) > 30){
                    $yid.util.error("时间间隔请选择30天之内!");
                    return;
                }
            ...
            searchZf(){
                ...
                if($yid.util.isEmpty(this.searchFormReqZf.cancelTimeStart)|| $yid.util.isEmpty(this.searchFormReqZf.cancelTimeEnd)){
                    $yid.util.alert("请选择日期再进行查询!");
                    return;
                }
                let arg = this.DateMinus(this.searchFormReqZf.cancelTimeStart, this.searchFormReqZf.cancelTimeEnd);
                if(Number(arg) > 30){
                    $yid.util.error("时间间隔请选择30天之内!");
                    return;
                }
            ...
            cancelSale(row){
                ...
                if(this.cancelForm.cancelReason != null && this.cancelForm.cancelReason.length > 0){
            ...
            importBill(){
                ...
                download($yid.config.API.BASE + 'api-cashier/servicerecords/servicerecordsExportForChain', this.searchFormReq)
            },
