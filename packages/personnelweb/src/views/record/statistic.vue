  ...
  <div class="member-relation">
    ...
    <yid-table pagination ref="table" style="margin-top: 15px">
      ...
      <yid-table-column label="剩余金额" min-width="80" prop="verMoney">
        <template slot-scope="scope">
          <template v-if="scope.row.verMoney"
            >￥{{ scope.row.verMoney }}</template
          >
      ...
      <yid-table-column label="核销记录" min-width="180" prop="recordList">
        <template slot-scope="scope">
          <template v-if="scope.row.recordList.length > 0">
            <span v-for="record in scope.row.recordList"
              >{{ record.refbillcode }}&nbsp;&nbsp;￥{{ record.money }}<br
            /></span>
          </template>
      ...
      this.$refs['searchForm'].validate(valid => {
        if (valid) {
          ...
          let arg = this.DateMinus(
            this.searchFormReq.payTimeStart,
            this.searchFormReq.payTimeEnd
          )
          if (Number(arg) > 30) {
