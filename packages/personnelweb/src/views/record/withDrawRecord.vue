  ...
  <div class="member-relation">
      ...
      <el-tab-pane label="服务单" name="analyze">
        ...
        <el-form ref="searchForm" inline :model="searchForm">
          ...
          <el-form-item style="margin-bottom: 0">
            ...
            <el-button @click="seniorSearch1" type="primary"
              >高级查询</el-button
            >
          </el-form-item>
        </el-form>
        ...
        <yid-table pagination ref="table">
          ...
          <yid-table-column label="客户" min-width="180" prop="cardInfo">
            <template slot-scope="scope">
              ...
              ><br />
              <span>{{
                scope.row.cardInfo == null
                  ? ''
                  : '【' + scope.row.cardInfo + ' ' + scope.row.cmcode + '】'
              }}</span>
            </template>
          </yid-table-column>
          ...
          <yid-table-column label="支付方式" min-width="150" prop="pays">
            <template slot-scope="scope">
              <span v-for="pays in scope.row.payList"
                >￥{{ pays.money }} {{ pays.payname }}<br
              /></span>
            </template>
          </yid-table-column>
          ...
          <yid-table-column label="员工业绩" min-width="150" prop="yjs">
            <template slot-scope="scope">
              <span v-for="royaltys in scope.row.royaltyList"
                >业绩：￥{{ royaltys.yj }} {{ getisAppoint(royaltys.isApppoint)
                }}<br
              /></span>
            </template>
          </yid-table-column>
      ...
      <el-tab-pane label="开卡充值" name="assignment">
        ...
        <el-form ref="czSearchForm" inline :model="czSearchForm">
          <el-form-item
            ...
            <el-select
              clearable
              v-model.trim="czSearchForm.shopid"
              filterable
          ...
          <el-form-item style="margin-bottom: 0">
            ...
            <el-button @click="seniorSearch2" type="primary"
              >高级查询</el-button
            >
          </el-form-item>
        </el-form>
        <yid-table pagination ref="tableCz">
          ...
          <yid-table-column label="顾客信息" min-width="180" prop="cardInfo">
            <template slot-scope="scope">
              ...
              ><br />
              <span>{{
                scope.row.cardname == null
                  ? ''
                  : '【' + scope.row.cardname + ' ' + scope.row.cardno + '】'
              }}</span>
            </template>
          </yid-table-column>
          <yid-table-column label="金额" min-width="150" prop="payments">
            <template slot-scope="scope">
              <span v-for="pays in scope.row.paysList"
                >￥{{ pays.payje }} {{ pays.payname }}<br
              /></span>
            </template>
          </yid-table-column>
          ...
          <yid-table-column label="员工" min-width="180" prop="members">
            <template slot-scope="scope">
              <span v-for="yjs in scope.row.yjsList"
                >{{ yjs.eecode }} {{ yjs.eename }}<br
              /></span>
            </template>
          </yid-table-column>
          ...
          <yid-table-column label="提成" min-width="150" prop="royaltys">
            <template slot-scope="scope">
              <span v-for="yjs in scope.row.yjsList"
                >￥{{ yjs.royalty == null ? 0 : yjs.royalty }}<br
              /></span>
            </template>
          </yid-table-column>
          <yid-table-column
          ...
          <yid-table-column
            ...
            fixed="right">
            <template slot-scope="scope">
              <el-link type="primary" @click="checkDetailCz(scope.row)"
                >查看详情</el-link
              >
            </template>
          </yid-table-column>
      ...
      <el-tab-pane label="套餐购买" name="logs">
        ...
        <el-form ref="packSearchForm" inline :model="packSearchForm">
          <el-form-item
            ...
            <el-select
              clearable
              v-model.trim="packSearchForm.shopid"
              filterable
          ...
          <el-form-item style="margin-bottom: 0">
            ...
            <el-button @click="seniorSearch3" type="primary"
              >高级查询</el-button
            >
          </el-form-item>
        </el-form>
        ...
        <yid-table pagination ref="tablePack">
          ...
          <yid-table-column label="套餐内容" min-width="150" prop="snames">
            <template slot-scope="scope">
              <span v-for="serv in scope.row.infoVOList"
                >{{ serv.spname }}*1 ￥{{ serv.totalprice }}<br
              /></span>
            </template>
          </yid-table-column>
          <yid-table-column label="员工" min-width="150" prop="members">
            <template slot-scope="scope">
              <span v-for="royaltys in scope.row.royaltyList"
                >{{ royaltys.eecode }} {{ royaltys.eename }}<br
              /></span>
            </template>
          </yid-table-column>
          <yid-table-column label="业绩" min-width="150" prop="eeyjs">
          ...
          <yid-table-column
            ...
            fixed="right">
            <template slot-scope="scope">
              <el-link type="primary" @click="checkDetailPack(scope.row)"
                >查看详情</el-link
              >
            </template>
          </yid-table-column>
    ...
    <el-collapse-transition>
      <div v-show="detailShowCz">
        <cardbillDetail
          ref="billDetailPageCz"
          @onClose="closeDetailPageCz"></cardbillDetail>
      </div>
    </el-collapse-transition>
    ...
    <el-collapse-transition>
      <div v-show="detailShowPack">
        <packagebillDetail
          ref="billDetailPagePack"
          @onClose="closeDetailPagePack"></packagebillDetail>
      </div>
    </el-collapse-transition>

    <yid-dialog
      :title="searchDialog1.title"
      :visible.sync="searchDialog1.visible"
      width="450px">
    ...
    <yid-dialog
      :title="searchDialog2.title"
      :visible.sync="searchDialog2.visible"
      width="450px">
      <el-form label-width="140px" ref="copyForm">
        <el-form-item label="消费日期：" prop="billcode">
          <el-date-picker
        ...
        <el-form-item label="充值单号：" prop="billcode">
          <el-input
            clearable
            v-model="czSearchForm.billcode"
            style="width: 180px"></el-input>
        </el-form-item>
        <el-form-item label="电话号码：" prop="mobile">
          <el-input
            clearable
            v-model="czSearchForm.mobile"
            style="width: 180px"></el-input>
        </el-form-item>
        <el-form-item label="支付方式：" prop="paycode">
          <el-select
            ...
            filterable
            placeholder="请选择"
            v-model.trim="czSearchForm.paycode">
            <el-option
              :key="item.paycode"
              :label="item.payname"
              :value="item.paycode"
        ...
        <el-form-item label="顾客姓名：" prop="memName">
          <el-input
            clearable
            v-model="czSearchForm.memName"
            style="width: 180px"></el-input>
        </el-form-item>
        <el-form-item label="会员卡号：" prop="channel">
          <el-input
            clearable
            v-model="czSearchForm.cmcode"
            style="width: 180px"></el-input>
        </el-form-item>
        <el-form-item label="充值类型：" prop="isOpen">
          <el-select
            ...
            filterable
            placeholder="请选择"
            v-model.trim="czSearchForm.isOpen">
            <el-option
              :key="item.id"
              :label="item.name"
              :value="item.id"
              v-for="item in opens"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="服务员工：" prop="eeid">
    ...
    <yid-dialog
      :title="searchDialog3.title"
      :visible.sync="searchDialog3.visible"
      width="450px">
      <el-form label-width="140px" ref="copyForm">
        <el-form-item label="消费日期：" prop="billcode">
          <el-date-picker
        ...
        <el-form-item label="套餐单号：" prop="billcode">
          <el-input
            clearable
            v-model="packSearchForm.billcode"
            style="width: 180px"></el-input>
        </el-form-item>
        <el-form-item label="电话号码：" prop="mobile">
          <el-input
            clearable
            v-model="packSearchForm.mobile"
            style="width: 180px"></el-input>
        </el-form-item>
        <el-form-item label="顾客姓名：" prop="memName">
          <el-input
            clearable
            v-model="packSearchForm.memName"
            style="width: 180px"></el-input>
        </el-form-item>
        <el-form-item label="项目/套餐名称：" prop="isOpen">
          <el-select
            ...
            filterable
            placeholder="请选择"
            v-model.trim="packSearchForm.sid">
            <el-option
              :key="item.id"
              :label="item.spname"
              :value="item.id"
        ...
        <el-form-item label="支付方式：" prop="paycode">
          <el-select
            ...
            filterable
            placeholder="请选择"
            v-model.trim="packSearchForm.paycode">
            <el-option
              :key="item.paycode"
              :label="item.payname"
              :value="item.paycode"
    ...
    <yid-dialog title="新增撤单" :visible.sync="addRecordVisible" width="700px">
      <el-form :model="addRecordForm" label-width="300px">
        <el-form-item label="单据编码:" label-width="120px">
          <el-input
            v-model="addRecordForm.billcode"
            style="width: 180px"></el-input>
        </el-form-item>
        <el-form-item label="撤单备注:" label-width="120px">
          <el-input
            type="textarea"
            v-model="addRecordForm.hcMemo"
        ...
        <el-form-item>
          ...
          <el-button type="primary" @click="addRecord(addRecordType)"
            >确 定</el-button
          >
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    searchCz() {
      this.$refs['czSearchForm'].validate(valid => {
        if (valid) {
          ...
          let arg = this.DateMinus(
            this.searchFormReqCz.updatedTimeStart,
            this.searchFormReqCz.updatedTimeEnd
          )
          if (Number(arg) > 30) {
            $yid.util.error('撤销时间间隔请选择30天之内!')
            return
    ...
    searchPack() {
      this.$refs['packSearchForm'].validate(valid => {
        if (valid) {
          ...
          let arg = this.DateMinus(
            this.searchFormReqPack.saleTimeStart,
            this.searchFormReqPack.saleTimeEnd
          )
          if (Number(arg) > 30) {
            $yid.util.error('撤销时间间隔请选择30天之内!')
            return
    ...
    addRecord(type) {
      ...
      if (type == 1) {
        ...
        service.cashier.packageRecords
          .revokeBill(this.addRecordForm)
          .then(res => {
            if (res.resp_code == 200) {
              $yid.util.success('操作成功')
