    ...
    <div class="member-relation">
        <el-form ref="searchForm" inline :model="searchForm">
            <el-form-item label="日期：">
                ...
                <el-form-item label="门店：" prop="shopid1" :rules="[{ required: true, message: '请选择一个门店'}]">
                    <el-select clearable v-model.trim="searchForm.shopid1"  filterable :filter-method="filterShop1" placeholder="请选择门店" style="width: 160px;">
                        <el-option :key="item.id" :label="item.shopname" :value="item.id" v-for="item in filterShopList1">
        ...
        <yid-table pagination ref="table" style="margin-top: 15px;">
            <yid-table-column label="类型" min-width="90" prop="type">
                <template slot-scope="scope">
                    {{scope.row.type == '2' ? '团购': scope.row.type == '3' ? '外卖-邮寄' : scope.row.type == '4' ? '外卖-自提' : ''}}
                </template>
            </yid-table-column>
            <yid-table-column label="骁骁网订单号" min-width="170" prop="billcode"></yid-table-column>
            <yid-table-column label="券号" min-width="150" prop="sgcode"></yid-table-column>
            <yid-table-column label="商品名称" min-width="180" prop="sgname"></yid-table-column>
            <yid-table-column label="券金额" min-width="80" prop="paymoney"></yid-table-column>
            <yid-table-column label="使用门店" min-width="100" prop="shopname"></yid-table-column>
            <yid-table-column label="核销时间" min-width="150" prop="rqsj"></yid-table-column>
            <yid-table-column label="顾客" min-width="90" prop="name"></yid-table-column>
            ...
            <yid-table-column label="操作人" min-width="90" prop="updatedBy"></yid-table-column>
        </yid-table>
        ...
        <yid-dialog :title="searchDialog.title" :visible.sync="searchDialog.visible" width="450px">
            <el-form label-width="140px" ref="copyForm">
                <el-form-item label="使用门店：">
                    <el-select clearable v-model.trim="searchForm.shopid2"  filterable :filter-method="filterShop2" placeholder="请选择门店" style="width: 160px;">
                ...
                <el-form-item label="关联订单：" prop="refbillcode">
                    <el-input  v-model="searchForm.refbillcode" style="width: 200px;"></el-input>
                </el-form-item>
    ...
    export default {
        ...
        computed:{
            searchFormReq: function () {
                ...
                reqObj.rqsjStart = ''
                reqObj.rqsjEnd = ''
                if(this.searchForm.model=="pt" && saleTimeRange && saleTimeRange.length>0){
        ...
        methods: {
            ...
            search(){
                this.$refs['searchForm'].validate((valid) => {
                    if (valid) {
                        let arg = this.DateMinus(this.searchFormReq.rqsjStart, this.searchFormReq.rqsjEnd);
                        if(Number(arg) > 30){
