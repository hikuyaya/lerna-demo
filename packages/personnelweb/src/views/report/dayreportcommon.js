...
export const common = {
  ...
  methods: {
    formatMoney(row, column) {
      ...
      if (moneystr == null || moneystr == '' || moneystr == '0') return '0.00'
    ...
    jsData() {
      ...
      const servList = [],
        kzList = []
      const bxm = { name: '大项目合计', sl: 0, je: 0, yj: 0 },
        sxm = { name: '小项目合计', sl: 0, je: 0, yj: 0 }
      const kzhj = { name: '客装合计', sl: 0, je: 0, yj: 0 },
        nghj = { name: '内购合计', sl: 0, je: 0, yj: 0 }
        ...
        if (head.payList && head.payList.length > 0) {
          ...
          head.payList.forEach(pay => {
            ...
            if (
              ...
              fczks = 0
            if (pay.paycode == '01')
              salepays.pay1 = add(salepays.pay1, pay.money)
            ...
            } else if (pay.paycode == '27')
              salepays.pay6 = add(salepays.pay6, pay.money)
            else if (pay.paycode == '02')
              salepays.pay7 = add(salepays.pay7, pay.money)
            else if (pay.paycode == '03')
              salepays.pay8 = add(salepays.pay8, pay.money)
            else if (pay.paycode == '11')
              salepays.pay9 = add(salepays.pay9, pay.money)
            else if (pay.paycode == '29')
              salepays.pay10 = add(salepays.pay10, pay.money)
            ...
            // 06 套餐, 50 套餐支付
            if (pay.paycode == '06' || pay.paycode == '50')
              sumdata.xdc = add(sumdata.xdc, pay.money)
            // ptcode='02' 赠送
      ...
      this.packagebillhead.forEach((head, index) => {
        ...
        if (head.paysList && head.paysList.length > 0) {
          ...
          head.paysList.forEach(pay => {
            pays += pay.payname + ':' + pay.money + '\n'
            if (pay.paycode == '01')
              packpays.pay1 = add(packpays.pay1, pay.money)
            else if (pay.paycode == '07') {
              ...
              packpays.pay6 = add(packpays.pay6, pay.money)
            else if (pay.paycode == '02')
              packpays.pay7 = add(packpays.pay7, pay.money)
            else if (pay.paycode == '03')
              packpays.pay8 = add(packpays.pay8, pay.money)
            else if (pay.paycode == '11')
              packpays.pay9 = add(packpays.pay9, pay.money)
            else if (pay.paycode == '29')
              packpays.pay10 = add(packpays.pay10, pay.money)
      ...
      this.cardbillhad.forEach((head, index) => {
        ...
        if (head.paysList && head.paysList.length > 0) {
          ...
          head.paysList.forEach(pay => {
            ...
            if (pay.paycode == '01')
              cardpays.pay1 = add(cardpays.pay1, pay.payje)
            else if (pay.paycode == '07') {
              ...
              cardpays.pay6 = add(cardpays.pay6, pay.payje)
            else if (pay.paycode == '02')
              cardpays.pay7 = add(cardpays.pay7, pay.payje)
            else if (pay.paycode == '03')
              cardpays.pay8 = add(cardpays.pay8, pay.payje)
            else if (pay.paycode == '11')
              cardpays.pay9 = add(cardpays.pay9, pay.payje)
            else if (pay.paycode == '29')
              cardpays.pay10 = add(cardpays.pay10, pay.payje)
      ...
      const xmhj = {
        name: '合计',
        sl: add(bxm.sl, sxm.sl),
        je: add(bxm.je, sxm.je),
        yj: add(bxm.yj, sxm.yj)
      }
