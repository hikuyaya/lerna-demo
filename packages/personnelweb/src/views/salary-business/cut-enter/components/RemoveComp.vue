<!--
 * @Author: wqy
 * @Date: 2022-08-11 15:41:33
 * @LastEditors: wqy
 * @LastEditTime: 2022-08-11 15:45:23
 * @FilePath: \personnelweb\src\views\salary-business\cut-enter\components\RemoveComp.vue
 * @Description: 
-->
...
export default {
  ...
  methods: {
    async onCheck() {
      ...
      const { data } = await service.salaryBusiness.cutEnter.getBillsByBatchNo({
    ...
    handleOperate() {
      if (this.type === 'approve') {
        this.$confirm(
          ...
          .then(async () => {
            await service.salaryBusiness.cutEnter.censorBillByBatchNo({
              batchNo: this.batchNo
            })
        ...
        this.$confirm(`您确认要移除此批次中所有数据吗？`, `移除确认`, {
          ...
          .then(async () => {
            await service.salaryBusiness.cutEnter.deleteBillByBatchNo({
              batchNo: this.batchNo
            })
            this.$message.success('操作成功')
            this.$emit('refresh')
