<template>
  <div>
    <el-collapse-transition>
      <div v-show="showList">
        <el-button @click="showCardSystemSettingPage()" type="primary">新增</el-button>
        <el-form inline :model="searchForm" ref="searchForm" style="margin-top: 8px">
          <el-form-item label="会员卡体系：">
            <el-input clearable v-model="searchForm.pcodename" placeholder="编码/名称" style="width: 160px;"></el-input>
          </el-form-item>
          <el-form-item label="品牌：">
            <el-select clearable v-model="searchForm.brandId" @change="changeBrand">
        ...
        <yid-table ref="table" style="margin-top: 10px;">
          <yid-table-column label="编码" min-width="80" prop="code"></yid-table-column>
          <yid-table-column label="名称" min-width="80" prop="name"></yid-table-column>
          ...
          <yid-table-column label="状态" min-width="80" prop="status">
            <template slot-scope="scope">
              <el-switch v-model="scope.row.status" active-text="启用"
                         inactive-text="停用" active-value="1" inactive-value="2" active-color="#339933"
                         inactive-color="#DCDFE6" @change="updateStatus(scope.row)">
              </el-switch>
            </template>
          ...
          <yid-table-column label="操作" min-width="80" prop="shopNum">
            <template slot-scope="scope" fixed="right">
              <el-link type="primary" @click="showCardSystemSettingPage(scope.row)">编辑</el-link>
              <el-link type="primary" style="margin-left: 6px" @click="showCardSettingPage(scope.row)">设置会员卡</el-link>
            </template>
          </yid-table-column>
        </yid-table>
      </div>
    ...
    <el-collapse-transition>
      <div v-show="showCardSystemSetting">
        <CardSystemSetting ref="cardSystemSetting" @onClose="closeCardSystemSettingPage"/>
      </div>
    </el-collapse-transition>
...
export default {
  ...
  methods: {
    ...
    async showShopDialog(row) {
      ...
      this.shopDialog.orgSelectShopList = JSON.parse(JSON.stringify(row.shopList));

      if (this.brandList.length == 0 || this.btypeList.length == 0 || this.regionData.length == 0) {
    ...
    updateStatus(row) {
      ...
      if (row.status == "2") {
        let tips = "禁用后，【" + row.name + "】内的会员卡同时禁用！确定要禁用吗？";
        yid.util.confirm(tips, '', '', () => {
