  ...
  <div class="dept">
    <el-form
      ...
      <el-form-item label="卡名称:" prop="cpname">
        <el-input
          clearable
          v-model.trim="searchForm.cpname"
          placeholder="请输入卡名称"
      ...
      <yid-table-column label="充值赠送规则" min-width="140" prop="ruleType">
        <template slot-scope="scope">
          <el-link
            ...
            >{{ scope.row.ruleType == '0' ? '未设置' : '查看规则' }}</el-link
          >
        </template>
      </yid-table-column>
      <yid-table-column label="异店划卡规则" min-width="140" prop="ruleType">
        <template slot-scope="scope">
          <el-link type="primary" @click="showCardUseDialog(scope.row)"
            >查看异店划卡规则</el-link
          >
        </template>
      </yid-table-column>
    ...
    <yid-dialog
      :title="cardUseDialog.title"
      :visible.sync="cardUseDialog.visible"
      width="800px">
      <el-row type="flex" justify="space-between" style="width: 100%">
      ...
      <el-row
        ...
        <el-col>
          <el-row>{{
            cardUseDialog.isAcrossShopPay ? '允许' : '不允许'
          }}</el-row>
          <el-row
            v-if="
              cardUseDialog.isAcrossShopPay && cardUseDialog.cardType == '1'
            "
            >卡金类型：{{ cardUseDialog.moneyArrStr }}</el-row
          >
        </el-col>
      </el-row>
      <el-row
        ...
        <el-col>{{
          cardUseDialog.isAcrossShopRecharge ? '允许' : '不允许'
        }}</el-col>
      </el-row>
      <el-row
        type="flex"
        ...
        <el-col>
          <el-row>{{
            cardUseDialog.isAcrossBrandPay ? '允许' : '不允许'
          }}</el-row>
          <el-row
            v-if="
              cardUseDialog.isAcrossBrandPay && cardUseDialog.cardType != '1'
            "
            >门店品牌：{{ cardUseDialog.brandArrStr }}</el-row
          >
        </el-col>
      </el-row>
    </yid-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      searchRules: {
        shopid: [
          { required: true, message: '必须选择一个门店', trigger: 'change' }
        ]
      },
      allShopList: [],
      filterShopList: [],
