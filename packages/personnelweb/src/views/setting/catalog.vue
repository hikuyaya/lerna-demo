  ...
  <div class="dept">
    <el-tabs v-model="activeName" @tab-click="handleClick">
      <el-tab-pane label="服务项目分类" name="serve">
        <el-button @click="serveAlert(false, '')" type="primary"
          >添加</el-button
        >
        <el-button @click="lookup()"
          >查看{{ status == '2' ? '禁用' : '正常' }}的服务项目分类</el-button
        >
        <yid-table
          pagination
          ref="servetable"
          ...
          <yid-table-column label="操作" min-width="250" prop="content">
            <template slot-scope="scope">
              ...
              <el-link
                ...
                @click="serveDelete(scope.row)"
                >{{ scope.row.status == '1' ? '禁用' : '恢复' }}</el-link
              >
            </template>
          </yid-table-column>
        </yid-table>
      ...
      <el-tab-pane label="商品类别" name="catalog">
        <el-button @click="goodsAlert(false, row)" type="primary"
          >添加</el-button
        >
        <el-button @click="lookup1()"
          >查看{{ status1 == '2' ? '禁用' : '正常' }}的商品类别</el-button
        >
        <yid-table
          pagination
          ref="goodstable"
          ...
          <yid-table-column
            label="商品类别"
            min-width="150"
            prop="cname"></yid-table-column>
          <yid-table-column label="商品子类" min-width="200" prop="son">
            <template slot-scope="scope">
              {{ scope.row.son }}
              <el-link type="primary" @click="subAlert(scope.row)"
                >编辑</el-link
              >
            </template>
          </yid-table-column>
          <yid-table-column label="操作" min-width="250" prop="content">
            <template slot-scope="scope">
              <el-link type="primary" @click="goodsAlert(true, scope.row)"
                >编辑</el-link
              >
              <el-link
                type="primary"
      ...
      <el-tab-pane label="套餐类别" name="packageType">
        <el-button @click="packageAlert(false, '')" type="primary"
          >添加</el-button
        >
        <yid-table
          pagination
          ...
          <yid-table-column label="操作" min-width="250" prop="content">
            <template slot-scope="scope">
              <el-link type="primary" @click="packageAlert(true, scope.row)"
                >编辑</el-link
              >
              <el-link
                type="primary"
      ...
      <el-tab-pane label="会员来源配置" name="origin">
        <el-button @click="originAlert(false, '')" type="primary"
          >添加</el-button
        >
        <el-button @click="lookup2()"
          >查看{{ status2 == '2' ? '禁用' : '正常' }}的会员来源</el-button
        >
        <yid-table
          pagination
          ref="origintable"
          ...
          <yid-table-column
            label="来源编号"
            min-width="100"
            prop="dccode"></yid-table-column>
          <yid-table-column
            label="来源名称"
            min-width="150"
            prop="dcname"></yid-table-column>
          <yid-table-column label="操作" min-width="250" prop="content">
            <template slot-scope="scope">
              <el-link
                type="primary"
                :disabled="getIsEdit(scope.row)"
                @click="originAlert(true, scope.row)"
                >编辑</el-link
              >
              <el-link
                type="primary"
      ...
      <el-tab-pane label="会员状态" name="status">
        <el-table
          ...
          <yid-table-column
            label="状态编码"
            min-width="100"
            prop="mdscode"></yid-table-column>
          <yid-table-column
            label="状态名称"
            min-width="150"
            prop="mdsname"
            :render-header="renderHeaderMethods"></yid-table-column>
          <yid-table-column
            label="未来店（天数）"
            min-width="150"
            prop="missDay"></yid-table-column>
          <yid-table-column label="操作" min-width="250" prop="content">
            <template slot-scope="scope">
              <el-link type="primary" @click="statusAlert(true, scope.row)"
                >编辑</el-link
              >
            </template>
          </yid-table-column>
      ...
      <el-tab-pane label="会员等级" name="level">
        <el-button @click="levelAlert(false, '')" type="primary"
          >添加</el-button
        >
        <el-button @click="lookup3()"
          >查看{{ status3 == '2' ? '禁用' : '正常' }}的会员等级</el-button
        >
        ...
        <yid-table
          ...
          <yid-table-column
            label="等级名称"
            min-width="100"
            prop="mdgname"></yid-table-column>
          <yid-table-column label="等级图标" min-width="150" prop="photopath">
            <!-- 图片的显示 -->
            <template slot-scope="scope">
              <img :src="scope.row.photopath" min-width="25" height="25" />
            </template>
          </yid-table-column>
          <yid-table-column
            label="单店近一年消费金额≥（元）"
          ...
          <yid-table-column label="操作" min-width="250" prop="content">
            <template slot-scope="scope">
              <el-link type="primary" @click="levelAlert(true, scope.row)"
                >编辑</el-link
              >
              <el-link
                type="primary"
                style="margin: 0 10px 0 10px"
                @click="levelDelete(scope.$index, scope.row)"
                >{{ scope.row.status == '1' ? '禁用' : '恢复' }}</el-link
              >
            </template>
          </yid-table-column>
    ...
    <yid-dialog
      :title="serveDialog.title"
      :visible.sync="serveDialog.visible"
      width="450px">
      <el-form ref="serviceForm" :model="serviceForm" label-width="140px">
        <el-form-item
          label="项目类型编号："
          prop="scode"
          :rules="[{ required: true, message: '项目类型编号为空' }]">
          {{ serviceForm.scode }}
        </el-form-item>
        <el-form-item
          label="项目类型名称："
          prop="sname"
          :rules="[{ required: true, message: '项目类型名称为空' }]">
        ...
        <el-form-item
          ...
          <el-select
            placeholder="请选择所属部门"
            v-model.trim="serviceForm.deptObj">
            <el-option
    ...
    <yid-dialog
      ...
      <el-form ref="goodsForm" :model="goodsForm" label-width="140px">
        <el-form-item
          label="商品类型编号："
          prop="ccode"
          :rules="[{ required: true, message: '商品类型编号为空' }]">
          {{ goodsForm.ccode }}
        </el-form-item>
        <el-form-item
          label="商品类型名称："
          prop="cname"
          :rules="[{ required: true, message: '商品类型名称为空' }]">
        ...
        <el-form-item
          ...
          <el-select
            placeholder="请选择所属部门"
            v-model.trim="goodsForm.branchObj">
            <el-option
              v-for="val in deptData"
              :key="val.id"
              :value="val.id"
    ...
    <yid-dialog
      ...
      <el-button @click="lookupz()"
        >查看{{ statusz == '2' ? '禁用' : '正常' }}的商品子类</el-button
      >
      ...
      <yid-table
        ...
        <yid-table-column label="操作" min-width="250" prop="content">
          <template slot-scope="scope">
            <el-link type="primary" @click="subAdd(true, scope.row)"
              >编辑</el-link
            >
            <el-link
              type="primary"
              @click="goodsDeletez(scope.row)"
              style="margin: 0 10px 0 10px"
              >{{ scope.row.status == '1' ? '禁用' : '恢复' }}</el-link
            >
          </template>
    ...
    <yid-dialog
      :title="subAddDialog.title"
      :visible.sync="subAddDialog.visible"
      width="auto">
      <el-form ref="subForm" :model="subForm" label-width="140px">
        <el-form-item
          label="商品子类编号："
          prop="ccode"
          :rules="[{ required: true, message: '商品子类编号为空' }]">
          {{ subForm.ccode }}
        </el-form-item>
        <el-form-item
          label="所属父类："
          prop="parentId"
          :rules="[{ required: true, message: '请选择所属父类' }]">
          <el-select
            placeholder="请选择所属父类"
            v-model.trim="subForm.parentId">
            <el-option
              v-for="val in categoryfs"
              :disabled="val.status == '1' ? false : true"
              :key="val.id"
              :value="val.id"
              :label="val.cname" />
          </el-select>
        ...
        <el-form-item
          label="商品子类型名称："
          prop="cname"
          :rules="[{ required: true, message: '商品子类型名称为空' }]">
    ...
    <yid-dialog
      :title="originDialog.title"
      :visible.sync="originDialog.visible"
      width="auto">
      <el-form :model="channelForm" label-width="140px" ref="channelForm">
        <el-form-item
          label="来源编号："
          prop="dccode"
          :rules="[{ required: true, message: '来源编号为空' }]">
          {{ channelForm.dccode }}
        </el-form-item>
        <el-form-item
          label="来源名称："
          prop="dcname"
          :rules="[{ required: true, message: '来源名称为空' }]">
    ...
    <yid-dialog
      ...
      <el-form :model="statusForm" label-width="140px" ref="statusForm">
        <el-form-item
          label="状态编号："
          prop="mdscode"
          :rules="[{ required: true, message: '状态编号为空' }]">
          {{ statusForm.mdscode }}
        </el-form-item>
        <el-form-item
          label="状态名称："
          prop="mdsname"
          :rules="[{ required: true, message: '状态名称为空' }]">
        ...
        <el-form-item
          label="未来店（天数）："
          prop="missDay"
          :rules="[{ required: true, message: '未来店（天数）为空' }]">
          <el-input
            onkeyup="this.value=this.value.replace(/[^\d.]/g,'');"
            maxlength="4"
            v-model="statusForm.missDay" />
        </el-form-item>
        <el-form-item>
    ...
    <yid-dialog
      ...
      <el-form :model="levelForm" label-width="160px" ref="levelForm">
        <el-form-item
          label="等级名称："
          prop="mdgname"
          :rules="[{ required: true, message: '等级名称为空' }]">
          <el-input v-model="levelForm.mdgname" />
        </el-form-item>
        <el-form-item
          label="最近一年消费金额≥："
          prop="smoney"
          :rules="[{ required: true, message: '最近一年消费金额≥为空' }]">
          <el-input type="number" v-model="levelForm.smoney" />
        </el-form-item>
        <el-form-item label="等级图标：" prop="photoindex">
          <div class="levelWrap">
            <div
              :class="[isInArray(0) ? 'xiangm active' : 'xiangm']"
              :key="'index' + 0"
              @click="clickServer(0)">
              <span></span>
              <ul class="gou">
            ...
            <div
              :class="[isInArray(1) ? 'xiangm active' : 'xiangm']"
              :key="'index' + 1"
              @click="clickServer(1)">
              <span></span>
              <ul class="gou">
              ...
              <label class="xmname"
                ><img
                  width="50px"
                  height="50px"
                  src="http://120.119.110.251:8888/group1/M00/00/01/eHdu-175f6-ARoYHAAANb29Tguw672.png"
              /></label>
            </div>
            <div
              :class="[isInArray(2) ? 'xiangm active' : 'xiangm']"
              :key="'index' + 2"
              @click="clickServer(2)">
              <span></span>
              <ul class="gou">
              ...
              <label class="xmname"
                ><img
                  width="50px"
                  height="50px"
                  src="http://120.119.110.251:8888/group1/M00/00/01/eHdu-175g-GAW9jzAAAQco5nH_E710.png"
              /></label>
            </div>
            <div
              :class="[isInArray(3) ? 'xiangm active' : 'xiangm']"
              :key="'index' + 3"
              @click="clickServer(3)">
              <span></span>
              <ul class="gou">
              ...
              <label class="xmname"
                ><img
                  width="50px"
                  height="50px"
                  src="http://120.119.110.251:8888/group1/M00/00/01/eHdu-175g_CAeMxRAAARWUOeZSs880.png"
              /></label>
            </div>
            <div
              :class="[isInArray(4) ? 'xiangm active' : 'xiangm']"
              :key="'index' + 4"
              @click="clickServer(4)">
              <span></span>
              <ul class="gou">
              ...
              <label class="xmname"
                ><img
                  width="50px"
                  height="50px"
                  src="http://120.119.110.251:8888/group1/M00/00/01/eHdu-175hAaARfSAAAAPZHkUPaU498.png"
              /></label>
            </div>
            <div
              :class="[isInArray(5) ? 'xiangm active' : 'xiangm']"
              :key="'index' + 5"
              @click="clickServer(5)">
              <span></span>
              <ul class="gou">
              ...
              <label class="xmname"
                ><img
                  width="50px"
                  height="50px"
                  src="http://120.119.110.251:8888/group1/M00/00/01/eHdu-175hKKAah18AAAPj0ot3XM129.png"
              /></label>
            </div>
            <div
              :class="[isInArray(6) ? 'xiangm active' : 'xiangm']"
              :key="'index' + 6"
              @click="clickServer(6)">
              <span></span>
              <ul class="gou">
              ...
              <label class="xmname"
                ><img
                  width="50px"
                  height="50px"
                  src="http://120.119.110.251:8888/group1/M00/00/01/eHdu-175hLKAWGI8AAASSAhWVXc491.png"
              /></label>
            </div>
            <div
              :class="[isInArray(7) ? 'xiangm active' : 'xiangm']"
              :key="'index' + 7"
              @click="clickServer(7)">
              <span></span>
              <ul class="gou">
              ...
              <label class="xmname"
                ><img
                  width="50px"
                  height="50px"
                  src="http://120.119.110.251:8888/group1/M00/00/01/eHdu-175hPGAX4y2AAAPHrPn_eA904.png"
              /></label>
            </div>
            <div
              :class="[isInArray(8) ? 'xiangm active' : 'xiangm']"
              :key="'index' + 8"
              @click="clickServer(8)">
              <span></span>
              <ul class="gou">
              ...
              <label class="xmname"
                ><img
                  width="50px"
                  height="50px"
                  src="http://120.119.110.251:8888/group1/M00/00/01/eHdu-175hPuATLD1AAAW3bzDY6w499.png"
              /></label>
            </div>
          </div>
        </el-form-item>
  ...
  methods: {
    ...
    levelAlert(tag = false, row) {
      ...
      if (tag == false) {
        ...
        this.chosenIndex = []
        this.chosenIndex.push(Number(row.photoindex))
        this.levelForm.photopath = row.photopath
    ...
    savememLevel() {
      this.$refs['levelForm'].validate(valid => {
        if (valid) {
          this.levelForm.photoindex = this.chosenIndex[0]
          this.levelForm.photopath = this.getGradePng(this.chosenIndex[0])
