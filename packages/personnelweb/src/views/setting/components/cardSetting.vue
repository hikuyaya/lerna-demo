  ...
  <div class="dept">
    ...
    <el-form inline style="margin-top: 8px">
      ...
      <el-form-item label="">
        <el-button @click="lookup()"
          >{{ status == '1' ? '查看停办' : '查看正常' }}会员卡</el-button
        >
      </el-form-item>
    </el-form>
    <yid-table ref="table" :data="cardData">
      ...
      <yid-table-column
        label="项目折扣"
        min-width="100"
        prop="serDis"></yid-table-column>
      <yid-table-column
        label="商品折扣"
        min-width="100"
        prop="proDis"></yid-table-column>
      ...
      <yid-table-column label="充值赠送规则" min-width="140" prop="ruleType">
        <template slot-scope="scope">
          <el-link
            ...
            v-if="scope.row.cardType == '1'"
            >{{ scope.row.ruleType == '0' ? '新建规则' : '编辑规则' }}</el-link
          >
        </template>
      </yid-table-column>
      ...
      <yid-table-column label="门店会员卡状态" min-width="110" prop="content">
        <template slot-scope="scope">
          <el-link type="primary" @click="showShopCardStatusDialog(scope.row)"
            >设置</el-link
          >
        </template>
      </yid-table-column>
    ...
    <yid-dialog
      ...
      <el-form
        ...
        <el-form-item label="卡编号：">
          <el-input v-model="addDialog.cpcode" disabled style="width: 80px" />
          ...
          <el-form
            ...
            <el-form-item label="充值提醒金额：">
              <el-input
                type="number"
                clearable
                v-model="addDialog.reminderMoney"
                min="0"
                placeholder="充值提醒金额"
                style="width: 220px" />
            </el-form-item>
            <el-form-item label="有效期：">
              <el-input
                type="number"
                clearable
                v-model="addDialog.expireMonth"
                min="0"
                placeholder="不填为永久有效"
                style="width: 220px" />个月
            </el-form-item>
          </el-form>
          <el-form
            v-if="addDialog.cardType == 2"
            label-position="left"
            class="form-type"
            label-width="100px">
            <el-form-item label="开卡金额：">
              <el-input
                type="number"
                v-model="addDialog.money2"
                placeholder="开卡金额"
                style="width: 220px" />
            </el-form-item>
            <el-form-item label="有效期：">
              <el-select
                v-model="addDialog.expireMonth2"
                filterable
                placeholder="有效月数"
                style="width: 160px">
            ...
            <el-form-item label="续卡配置：">
              <div style="width: 100%">
                <el-row
                  :key="index"
                  v-for="(card, index) in addDialog.cardRules"
                  style="width: 100%">
                  <el-col :span="8">
                    <el-input
                      type="number"
        ...
        <el-form-item label="项目折扣：" prop="serDis">
          <el-input
            type="number"
            clearable
            v-model="addDialog.serDis"
            style="width: 500px"
            min="0"
            max="9.99"
            placeholder="折扣请输入一个数字，如9表示9折，8.5表示八五折，0表示没有折扣" />
        </el-form-item>
        <el-form-item label="商品折扣：" prop="proDis">
          <el-input
            type="number"
            clearable
            v-model="addDialog.proDis"
            style="width: 500px"
            min="0"
            max="9.99"
            placeholder="折扣请输入一个数字，如9表示9折，8.5表示八五折，0表示没有折扣" />
        ...
        <el-form-item label="开卡备注：">
          ...
          <explain style="color: silver"
            >"添加会员(开卡）”操作时，新开卡会员的备注栏将导入此处输入的内容，可以用于对某种卡的使用或操作方法做特别说明</explain
          >
        </el-form-item>
        <el-form-item label="会员卡面图片：">
          <el-upload
            ...
            <img
              v-if="addDialog.photopath"
              :src="addDialog.photopath"
    ...
    <yid-dialog
      ...
      <yid-table
        ...
        <yid-table-column label="操作" min-width="150">
          <template slot-scope="scope">
            <el-link type="primary" @click="editRuleDialog(scope.row)"
              >编辑</el-link
            >
            <label> / </label>
            <el-link type="primary" @click="delRuleDialog(scope.row)"
              >删除</el-link
            >
          </template>
        </yid-table-column>
      ...
      <el-row
        ...
        <el-col span="6"></el-col>
        <el-col span="6"
          ><el-button @click="levelCancel(2)">确定</el-button></el-col
        >
        <el-col span="6"></el-col>
      </el-row>
    </yid-dialog>
    ...
    <yid-dialog
      :title="ruleAddDialog.title"
      :visible.sync="ruleAddDialog.visible"
      @close="levelCancel(3)"
      width="800px">
      <el-form
        :model="ruleAddDialog"
        ...
        <el-form-item label="开卡/充值金额：" prop="money">
          <el-input
            ...
            min="0"
            placeholder="金额"
            v-model="ruleAddDialog.money" /><br />
        </el-form-item>
        ...
        <el-form-item label="赠送：" prop="giftMoney">
          <el-input type="number" min="0" v-model="ruleAddDialog.giftMoney" />{{
            ruleAddDialog.giftType == '1' ? '元' : '%'
          }}
        </el-form-item>
        <el-form-item>
    ...
    <yid-dialog
      ...
      width="800px"
      @close="closeShopCardStatusDialog">
      <row
        >{{ shopDialog.selectCard ? shopDialog.selectCard.cpname : '' }}
      </row>
      <el-form
        ref="searchShopForm"
        inline
        :model="searchShopForm"
        style="margin-top: 8px">
        <el-form-item label="门店：">
          <el-select
            clearable
            v-model.trim="searchShopForm.shopid"
            @clear="clearShopForm"
            @change="changeSelectShopForm"
            filterable
            ...
            <el-option
              ...
              :label="item.shopname"
              :value="item.shopid"
              v-for="item in shopDialog.filterShopList">
        ...
        <el-form-item>
          <row>
            <el-button type="primary" @click="batchUpdateCardShopStatus(1)"
              >启用</el-button
            >
            <el-button @click="batchUpdateCardShopStatus(2)"
              >停办</el-button
            ></row
          >
        </el-form-item>
      ...
      <yid-table
        ref="cardShopList"
        :data="shopDialog.filterShopList"
        style="margin-top: 8px"
        ...
        <yid-table-column label="门店名称" min-width="100" prop="status">
          <template slot-scope="scope">
            <el-switch
              ...
              @change="updateCardShopStatus(scope.row)">
            </el-switch>
          </template>
        </yid-table-column>
      </yid-table>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        cpname: [
          { required: true, message: '请填写卡名称' },
          { min: 0, max: 30, message: '卡名称在30个字符以内' }
        ],
  ...
  methods: {
    initData(data) {
      this.status = '1'
      this.ctid = data.id
      this.isSystemActive = data.status == '1'
    ...
    changeSelectShopForm(shopid) {
      this.shopDialog.filterShopList = this.shopDialog.allShopList.filter(
        item => {
          // 如果直接包含输入值直接返回true
    ...
    clearShopForm() {
      this.shopDialog.filterShopList = this.shopDialog.allShopList
    },
    showShopCardStatusDialog(row) {
      this.shopDialog.allShopList = Object.assign(row.cardIshopList)
      this.shopDialog.filterShopList = Object.assign(row.cardIshopList)
      this.shopDialog.selectShopList = []
      this.shopDialog.selectCard = row
      if (!this.shopDialog.allShopList) {
        this.shopDialog.allShopList = []
        this.shopDialog.filterShopList = []
      }
      this.shopDialog.visible = true
      if (this.$refs.cardShopList != null) {
        let t = this.$refs.cardShopList
        //todo  需要增加清除select方法
      }
    },
    closeShopCardStatusDialog() {
      this.shopDialog.selectShopList = []
      this.shopDialog.selectCard = null
      this.shopDialog.visible = false
    },
    handleSelectionShopChange(rows) {
      this.shopDialog.selectShopList = rows
    },
    batchUpdateCardShopStatus(type) {
      if (
        !this.shopDialog.selectShopList ||
        this.shopDialog.selectShopList.length <= 0
      ) {
        $yid.util.error('请勾选门店~')
        return
      }

      if (type == '1') {
        if (this.shopDialog.selectCard.status == '2') {
          $yid.util.error('该会员卡已经是停用状态，不能操作~')
          return
        }
      }
      for (let i = 0; i < this.shopDialog.selectShopList.length; i++) {
        this.shopDialog.selectShopList[i].status = type
      }
      service.chain.cardIshop
        .batchSave(this.shopDialog.selectShopList)
        .then(res => {
          if (res.resp_code == '200') {
            this.queryCardInfo(this.status)
            this.closeShopCardStatusDialog()
          } else {
            yid.util.error(res.resp_msg)
          }
        })
    },
    updateCardShopStatus(row) {
      ...
      service.chain.cardIshop.save(row).then(res => {
        if (res.resp_code == '200') {
          this.queryCardInfo(this.status)
        } else {
          yid.util.error(res.resp_msg)
    ...
    add() {
      ...
      this.addDialog.isTransfer = '1'
      this.$refs.name.focus()
    },
    edit(row) {
      this.addDialog.title = '编辑会员卡'
      const that = this
      service.chain.cardInfo.cardInfoByID(row.id).then(res => {
    ...
    stop(row) {
      if (!this.isSystemActive) {
        $yid.util.error('该会员卡体系已经是停用状态，不能操作~')
        row.status = '2' //这种体系停用状态下  卡会员卡都是停用状态
        return
      }
      ...
      service.chain.cardInfo.editCardStatus(params).then(res => {
        if (res.resp_code == '200') {
    ...
    saveCardInfo() {
      ...
      this.$refs['copyForm'].validate(valid => {
        if (valid) {
          ...
          that.addDialog.ctid = this.ctid
          service.chain.cardInfo.saveCardInfo(that.addDialog).then(res => {
            if (res.resp_code == '200') {
              that.levelCancel(1)
    ...
    editRuleDialog(row) {
      ...
      this.ruleAddDialog.ctid = row.ctid
    },
    delRuleDialog(row) {
      const that = this
      yid.util.confirm('确定删除充卡/充值规则？', '', '', () => {
        that.ruleDialog.cardRules.forEach(item => {
          if (row.id == item.id) {
            service.chain.cardInfo.delCardRule(item.id).then(res => {
              if (res.resp_code == '200') {
    ...
    saveRuleDialog() {
      ...
      this.$refs['cardForm'].validate(valid => {
        if (valid) {
          ...
          if (
            that.ruleAddDialog.giftType == '1' &&
            that.ruleAddDialog.giftMoney
          ) {
            if (
              Number(that.ruleAddDialog.giftMoney) >
              Number(that.ruleAddDialog.money)
            ) {
          ...
          if (
            that.ruleAddDialog.giftType == '2' &&
            that.ruleAddDialog.giftMoney
          ) {
            if (
              Number(that.ruleAddDialog.giftMoney) < 0 ||
              Number(that.ruleAddDialog.giftMoney) > 100
            ) {
          ...
          that.ruleAddDialog.ctid = this.ctid
          service.chain.cardInfo.saveCardRules(that.ruleAddDialog).then(res => {
            if (res.resp_code == '200') {
              that.levelCancel(3)
              that.ruleDialog.cardRules = res.data
              yid.util.success(res.resp_msg)
    ...
    queryCardInfo(status) {
      service.chain.cardInfo
        .searchListAll({ status: status, ctid: this.ctid })
        .then(res => {
