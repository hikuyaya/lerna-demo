<template>
    <div class="dept">
        ...
        <el-form inline style="margin-top: 8px">
            ...
            <el-form-item label="">
                <el-button  @click="lookup()">{{status =="1" ? '查看停办':'查看正常'}}会员卡</el-button>
            </el-form-item>
        </el-form>
        <yid-table ref="table" :data="cardData">
            ...
            <yid-table-column label="门店会员卡状态" min-width="110" prop="content">
                <template slot-scope="scope">
                    <el-link type="primary" @click="showShopCardStatusDialog(scope.row)">设置</el-link>
                </template>
            </yid-table-column>
        </yid-table>
        ...
        <yid-dialog :title="shopDialog.title" :visible.sync="shopDialog.visible" width="800px" @close="closeShopCardStatusDialog">
            <row>{{shopDialog.selectCard ? shopDialog.selectCard.cpname : ""}} </row>
            <el-form ref="searchShopForm" inline :model="searchShopForm"  style="margin-top: 8px">
                <el-form-item label="门店：">
                    <el-select clearable v-model.trim="searchShopForm.shopid" @clear="clearShopForm"   @change="changeSelectShopForm" filterable :filter-method="filterShop" placeholder="请选择门店" style="width: 160px;">
                        <el-option :key="item.id" :label="item.shopname" :value="item.shopid" v-for="item in shopDialog.filterShopList">
                ...
                <el-form-item>
                    <row>  <el-button type="primary" @click="batchUpdateCardShopStatus(1)">启用</el-button>
                        <el-button  @click="batchUpdateCardShopStatus(2)">停办</el-button></row>
                </el-form-item>
            </el-form>
            <yid-table ref="cardShopList" :data="shopDialog.filterShopList" style="margin-top: 8px" @selection-change="handleSelectionShopChange">
                ...
                <yid-table-column label="门店名称" min-width="100" prop="status">
                    <template slot-scope="scope">
                        <el-switch v-model="scope.row.status"  active-text="启用"
                                   inactive-text="停用" active-value="1" inactive-value="2" active-color="#339933" inactive-color="#DCDFE6" @change="updateCardShopStatus(scope.row)">
                        </el-switch>
                    </template>
                </yid-table-column>
            </yid-table>
    ...
    export default {
        ...
        data() {
            return {
                ...
                rules:{
                    cpname:[{required: true, message:'请填写卡名称'}, {min: 0, max: 30, message: '卡名称在30个字符以内'}],
        ...
        methods: {
            ...
            changeSelectShopForm(shopid){
                this.shopDialog.filterShopList = this.shopDialog.allShopList.filter((item) => {
                    // 如果直接包含输入值直接返回true
            ...
            batchUpdateCardShopStatus(type){
                if(!this.shopDialog.selectShopList || this.shopDialog.selectShopList.length <= 0){
                ...
                service.chain.cardIshop.batchSave(this.shopDialog.selectShopList).then(res =>{
            ...
            queryCardInfo(status){
                service.chain.cardInfo.searchListAll({status:status,ctid:this.ctid}).then(res =>{
