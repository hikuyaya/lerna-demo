<template>
  <div>
    ...
    <el-form :model="settingForm" :rules="settingFormRules" label-width="200px" ref="settingForm"
             style="margin-top: 8px">
      <el-form-item label="编码：" prop="code">
        {{ settingForm.code }}
        <explain style="color:silver">编号限定从001开始的整数,自动叠加</explain>
      </el-form-item>
      <el-form-item label="门店品牌：" prop="brandId">
        <el-select clearable v-model="settingForm.brandId" :disabled="!isAddNew" @change="handleChangeBrand">
          <el-option v-for="item in brandList" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label=" 会员卡体系名称：" prop="name">
        <el-input ref="name" v-model="settingForm.name" placeholder="卡名称" style="width: 150px"/>
      </el-form-item>
      <el-form-item label="适用门店:" prop="shopList">
        <el-button type="primary" @click="selectShop()">选择门店</el-button>
        {{ settingForm.shopList && settingForm.shopList.length > 0 ? '共选中' + settingForm.shopList.length + '家门店' : '' }}
      </el-form-item>
      <row>储值卡异店划卡设置：</row>
      <el-form-item label="是否允许跨店消费/充值" style="margin-top: 8px" prop="isSdiffshop">
        <el-radio-group v-model="settingForm.isSdiffshop" @change="handleChangeSdiffshop">
          <el-radio label="1">允许</el-radio>
          <el-radio label="0">不允许</el-radio>
        </el-radio-group>
        <yid-ask>适用门店内的跨店消费，不允许则只能在开卡充值门店消费，非适用门店内跨店可设置跨品牌消费</yid-ask>
      </el-form-item>
      <el-form-item label="卡金类型" prop="sShopCardMoneyTypeArr" v-if="settingForm.isSdiffshop == '1'">
        <el-checkbox-group v-model="settingForm.sShopCardMoneyTypeArr">
          <el-checkbox v-for="item in cardMoneyTypeArr" :label="item.value" :key="item.value">{{ item.name }}
          </el-checkbox>
        </el-checkbox-group>
      </el-form-item>
      ...
      <el-form-item label="是否允许跨品牌消费" style="margin-top: 8px" prop="isSdiffbrand">
        <el-radio-group v-model="settingForm.isSdiffbrand" @change="handleChangeSdiffbrand">
          <el-radio label="1">允许</el-radio>
          <el-radio label="0">不允许</el-radio>
        </el-radio-group>
        <yid-ask>跨品牌消费按照消费门店最低会员卡折扣的会员卡规则，最低折扣会员卡判定规则：项目折扣+产品折扣之和数字最大的会员卡</yid-ask>
      </el-form-item>
      ...
      <el-form-item label="门店品牌" v-if="settingForm.isSdiffbrand == '1'" prop="czkBrandTypeArr">
        <el-checkbox :indeterminate="sBrandIndeterminates" v-model="isCheckSBrandTypeAll"
                     @change="handlesBrandTypeCheckAllChange">全选
        </el-checkbox>
        <div style="margin: 15px 0;"></div>
        <el-checkbox-group v-model="settingForm.czkBrandTypeArr" @change="handleCheckedSBrandTypeChange">
      ...
      <el-form-item label="是否允许跨店消费" style="margin-top: 8px" prop="isMdiffshop">
        ...
        <yid-ask>适用门店内的跨店消费，不允许则只能在开卡门店消费，非适用门店内跨店可设置跨品牌消费</yid-ask>
      </el-form-item>
      ...
      <el-form-item label="是否允许跨品牌消费" style="margin-top: 8px" prop="isMdiffbrand">
        <el-radio-group v-model="settingForm.isMdiffbrand" @change="handleChangeMdiffbrand">
          <el-radio label="1">允许</el-radio>
          <el-radio label="0">不允许</el-radio>
        </el-radio-group>
        <yid-ask>跨品牌消费按照消费门店最低会员卡折扣的会员卡规则，最低折扣会员卡判定规则：项目折扣+产品折扣之和数字最大的会员卡</yid-ask>
      </el-form-item>
      <el-form-item label="门店品牌" v-if="settingForm.isMdiffbrand == '1'" prop="zgkBrandTypeArr">
        <el-checkbox :indeterminate="mBrandIndeterminates" v-model="isCheckMBrandTypeAll"
                     @change="handlemBrandTypeCheckAllChange">全选
        </el-checkbox>
        <div style="margin: 15px 0;"></div>
        <el-checkbox-group v-model="settingForm.zgkBrandTypeArr" @change="handleCheckedMBrandTypeChange">
          <el-checkbox v-for="item in brandList" :label="item" :key="item.id">{{ item.name }}</el-checkbox>
        </el-checkbox-group>
      </el-form-item>
    ...
    <yid-dialog :title="shopDialog.title" :visible.sync="shopDialog.visible" @close="cancelSaveShop" width="1100px">
      <el-row type="flex" justify="space-between">
        <el-form inline :model="shopSearchForm" ref="shopSearchForm">
          ...
          <el-form-item label="状态：">
            <el-select clearable v-model="shopSearchForm.status" placeholder="请选择状态" style="width: 120px;">
...
export default {
  ...
  data() {
    return {
      ...
      settingFormRules: {
        ...
        zgkBrandTypeArr: [{type: 'array', required: true, message: '请至少选择一个可跨品牌'}],
        czkBrandTypeArr: [{type: 'array', required: true, message: '请至少选择一个可跨品牌'}],
        sBrandCardMoneyTypeArr: [{type: 'array', required: true, message: '请至少选择一个卡金类型'}],
        sShopCardMoneyTypeArr: [{type: 'array', required: true, message: '请至少选择一个卡金类型'}],
      },
      shopList: [],
      brandList: [],
      btypeList: [],
  ...
  methods: {
    ...
    initEditSetting(data) {
      ...
      this.shopDialog.orgSelectShopList = JSON.parse(JSON.stringify(data.shopList));
      ...
      if (data.isMdiffbrand == "1") {
        ...
        this.mBrandIndeterminates = checkedCount > 0 && checkedCount < this.brandList.length;
      }
    ...
    deleteShop(row) {
      ...
      let tips = "删除门店,【" + cardName + "】下的会员卡将不在【" + shopname + "】中显示，若会员卡在本店还存有卡金将不能使用，确定要删除吗？"
    ...
    save() {
      this.$refs["settingForm"].validate((valid) => {
        ...
        let isAllGive = false
        for (let i = 0; i < this.settingForm.sShopCardMoneyTypeArr.length; i++) {
        ...
        isAllowPay = false;
        isAllGive = false
        for (let i = 0; i < this.settingForm.sBrandCardMoneyTypeArr.length; i++) {
