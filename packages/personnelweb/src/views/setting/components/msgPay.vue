<template>
    <div>
        <el-alert
            ...
            <el-button type="primary" @click="showMsgPriceDialog" style="margin-left: 16px">充值</el-button>
        </el-alert>
        ...
        <yid-table pagination ref="table">
            <yid-table-column label="订单号" min-width="100" prop="billcode"></yid-table-column>
            <yid-table-column label="短信条数" min-width="100" prop="rechargenum"></yid-table-column>
            <yid-table-column label="订单金额" min-width="100" prop="payje"></yid-table-column>
            <yid-table-column label="支付方式" min-width="100" prop="ptype">
                <template slot-scope="scope">
                    {{scope.row.ptype == '1' ? "微信" : "支付宝"}}
                </template>
            </yid-table-column>
            <yid-table-column label="支付时间" min-width="100" prop="payTime"></yid-table-column>
        </yid-table>

        <yid-dialog :title="msgPriceDialogInfo.title" :visible.sync="msgPriceDialogInfo.visible" @close="" width="750px">
            <el-row :gutter="12">
        ...
        <yid-dialog :title="payDialogInfo.title" :visible.sync="payDialogInfo.visible" @close="closePayDialog" width="750px">
            <el-row gutter="12">
            ...
            <el-row gutter="12" >
                <!--//1微信2支付宝-->
                <el-col :span="6">
                    <div  @click="showQrCodeDialog('1')" style="background-color: #00AAEE"> 微信支付 </div>
                </el-col>
                <el-col :span="6" >
                    <div @click="showQrCodeDialog('2')" style="background-color: #55a532">支付宝支付</div></el-col>
            </el-row>
        </yid-dialog>

        <yid-dialog :title="qrCodeDialogInfo.title" :visible.sync="qrCodeDialogInfo.visible" @close="closePayDialog" width="500">
            <el-row gutter="8">
    ...
    export default {
        ...
        watch: {
            payOrder() {
                if (this.orderInfo.billcode == this.payOrder.billcode) {
                    if (this.payOrder.resp_code == 200 && this.payOrder.data.status == "2") {
                        this.getMsgAllowance();
                    } else{
        ...
        methods: {
            ...
            showQrCodeDialog(payType){
                ...
                that.qrCodeDialogInfo.title = "1" == payType ? "请使用微信扫码":"请使用支付宝扫码";
