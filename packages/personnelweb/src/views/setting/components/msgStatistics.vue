  ...
  <div>
    <el-form ref="searchForm" inline :model="searchForm" :rules="searchRules">
      ...
      <el-form-item label="日期模式:" prop="searchDateType">
        <el-select
          clearable
          v-model.trim="searchForm.searchDateType"
          placeholder="时间"
          style="width: 160px"
          @change="changeSearchDateType">
          <el-option
            :key="item.value"
            :label="item.name"
            :value="item.value"
    ...
    <yid-table pagination ref="table">
      ...
      <yid-table-column
        label="验证码短信"
        min-width="100"
        prop="verifyNum"></yid-table-column>
      <yid-table-column
        label="群发短信"
        min-width="100"
        prop="groupNum"></yid-table-column>
      <yid-table-column
        label="服务回访"
        min-width="100"
      ...
      <yid-table-column
        label="生日短信"
        min-width="100"
        prop="brithdayNum"></yid-table-column>
      <yid-table-column
        label="短信充值"
        min-width="100"
      ...
      <yid-table-column
        label="短信分配"
        min-width="100"
        prop="assignNum"></yid-table-column>
      <yid-table-column
        label="剩余短信"
        min-width="100"
        prop="shopname"></yid-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      searchRules: {
        searchDateType: [
          { required: true, message: '请选择查询日期模式', trigger: 'blur' }
        ],
  ...
  methods: {
    ...
    getStatisticsList(reqParams) {
      let fetch =
        reqParams.searchDateType == 'm'
          ? service.push.smsMstatis.listChain
          : service.push.smsDstatis.listChain
