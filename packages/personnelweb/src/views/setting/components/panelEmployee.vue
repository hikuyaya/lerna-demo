  ...
  <div>
    ...
    <el-row>
      ...
      <el-col :span="1" style="padding: 60px 0px">
        <div>
          <el-button @click="transfer()" type="primary" size="mini"
          ...
          <yid-table-column label="操作" min-width="80">
            <template slot-scope="scope">
              <el-link
                ...
                @click="deleteDataRow(scope.$index)">
                删除
              </el-link>
            </template>
          </yid-table-column>
...
export default {
  ...
  watch: {
    'queryModel.psid'() {
      this.pslList = this.dt.pslList.filter(
        psl => psl.psid == this.queryModel.psid
      )
    },
    defaultRightData(v) {
      this.rightData = v
  ...
  methods: {
    ...
    doConfirm() {
      ...
      if (this.confirmFunc) this.confirmFunc(this.rightData)
