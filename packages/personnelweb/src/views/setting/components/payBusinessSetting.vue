        ...
        <el-tabs v-model="activeName">
            <el-tab-pane label="项目" name="1">
                ...
                <el-table ref="serviceTable" border :data="paymentData" style="margin-top: 10px;"  class="class1">
                    ...
                    <el-table-column label="设置为混合支付" prop="isMixed" min-width="120">
                        <template slot-scope="scope">
                            <el-checkbox v-model="scope.row.isMixed">混合支付</el-checkbox>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="费率" prop="freeRate" min-width="150"></el-table-column>
                </el-table>
            </el-tab-pane>
        ...
        methods:{
                    ...
                    onEnd ({ newIndex, oldIndex }) {
                        ...
                        _this.paymentData.forEach((pay,index) =>{
                            params.payments.push({
                                ...
                                isOften:pay.isOften ? "1" : "0",
                                enable:pay.enable ? "1" : "0",
                                isMixed:pay.isMixed ? "1" : "0",
                                payid:pay.payid
                            })
                        })
            ...
            getPaymentConfig(){
                ...
                let param={
                    ...
                    status:'1',
                    phid:this.payTheadId
                }
                if(this.activeName == '3' || this.activeName == '4'){
                    param.paycodes='02,03,06,16'
                }
                if(this.activeName == '5'){
                    param.paycodes='06,16'
                }
                if(this.activeName == '6'){
                    param.pays='52,22,23,53'
                }
                service.chain.payTemplate.listAll(param).then(res =>{
                    if(res.resp_code=="200"){
                        res.data.forEach(each =>{
                            each.isOften=each.isOften=='1'?true:false
                            each.enable=each.enable=='1'?true:false
                            each.isMixed=each.isMixed=='1'?true:false
...
//                    }
                ...
                params.payments.forEach(pay =>{
                    ...
                    pay.isMixed = pay.isMixed && pay.isMixed != "" ? "1" : "0"
                })
                service.chain.payTemplate.batchSave(params).then(res =>{
                    this.result(res)
                })
