  ...
  <div class="mtkeysConfig">
      ...
      <div v-show="showList">
        <el-form :model="searchForm" ref="searchForm" inline>
          <el-form-item label="抖音店号：">
            <el-input clearable placeholder="请输入" v-model.trim="searchForm.poiName" style="width: 200px;"></el-input>
          </el-form-item>
          <el-form-item label="抖音poi_id：">
            <el-input clearable placeholder="请输入" v-model.trim="searchForm.poiId" style="width: 200px;"></el-input>
          </el-form-item>

          <el-form-item label="系统店号：">
            <el-input clearable placeholder="请输入" v-model.trim="searchForm.shopcode" style="width: 200px;"></el-input>
          </el-form-item>

          <el-form-item label="系统店名：">
            <el-input clearable placeholder="请输入" v-model.trim="searchForm.shopname" style="width: 200px;"></el-input>
          </el-form-item>
        ...
        <el-row style="color:#e6a23c; margin: 15px 0">操作提示：开通抖音账号映射权限请联系平台客服办理</el-row>
        <yid-table pagination ref="mtkeystable" :data="mtkeysData" style="margin-top: 15px;">
          ...
          <yid-table-column label="抖音id" min-width="100" prop="poiId"></yid-table-column>
          <yid-table-column label="抖音店名" min-width="100" prop="poiName"></yid-table-column>
          <yid-table-column label="操作" min-width="150" prop="id">
            <template slot-scope="scope">
              <el-link type="primary" style="margin: 0 10px 0 10px;" @click="deleteDyShop(scope.row)">删除</el-link>
            </template>
          </yid-table-column>
        </yid-table>
      </div>
    ...
    <el-collapse-transition>
      <div v-show="!showList">
        ...
        <yid-table ref="douyinShoptable"
                   :data="douYinShopDatatmp"
                   style="margin-top: 15px;">
          <yid-table-column label="抖音poi_id" min-width="100" prop="poi_id"></yid-table-column>
          <yid-table-column label="抖音店名" min-width="100" prop="poi_name">

          </yid-table-column>
          <yid-table-column label="抖音地址" min-width="100" prop="address"></yid-table-column>
          <yid-table-column label="操作" min-width="150" prop="longitude">
            <template slot-scope="scope">
              <el-link type="primary" style="margin: 0 10px 0 10px;" @click="bindShop(scope.row)">绑定门店</el-link>
            </template>
          </yid-table-column>
        </yid-table>
...
export default {
  ...
  methods: {
    searchName() {
      this.douYinShopDatatmp = this.douYinShopData.filter(data => !this.search || data.poi_name.toLowerCase().includes(this.search.toLowerCase()))
