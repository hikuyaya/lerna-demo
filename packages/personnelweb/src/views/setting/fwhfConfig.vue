<template>
  <div class="fwhfConfig">
    <el-form
      ...
      <el-form-item label=""
        ><el-button type="primary" @click="getFwhfConfig()"
          >查询</el-button
        ></el-form-item
      ><br />
    </el-form>
    <div class="con">
                ...
                <div class="one">
                  <div class="xmtit">
                    ...
                    <el-link
                      type="primary"
                      :underline="false"
                      @click="showEditDialogType(wxitem.id)"
                      >编辑项目</el-link
                    >
                    &nbsp;&nbsp;
                    <el-link
                      type="primary"
                      :underline="false"
                      @click="deleteWxReturn(wxitem.id, wxitem.servtid)"
                      >删除</el-link
                    >
                  </div>
      ...
      <div class="conright" v-if="showtemp">
        <div style="text-align: right">
          <el-link type="primary" :underline="false" @click="getMbConfig"
            >消息模板管理</el-link
          >
        </div>
        ...
        <el-select
          v-model="postion"
          :disabled="wxtask.length > 0 ? true : false"
          v-if="showPos"
          placeholder="请选择"
          style="width: 300px">
          <el-option
      ...
      <div style="padding: 20px">
        ...
        <div
          ...
          <template v-for="(item, index) in serverInfos">
            ...
            <div
              v-if="isInArray(item.id) == true && item.disp == 'open'"
              class="xiangm active"
              @click="clickServer(item.id)">
              <span></span>
              <ul class="gou">
            ...
            <div
              v-if="isInArray(item.id) == false && item.disp == 'open'"
              class="xiangm"
              @click="clickServer(item.id)">
              <span></span>
              <ul class="gou">
...
export default {
  ...
  methods: {
    ...
    savetemp() {
      ...
      let addobj = {
        afterDay: this.afterDay,
        pstid: this.tempid,
        pstname: this.tempname,
        wrid: this.wrid,
        type: this.type
      }
      ...
      yid.service.fwbackConfig.addWxtask(addobj).then(res => {
        ...
        yid.service.fwbackConfig.getWxtaskBywrIdAndtype(par).then(restask => {
          ...
          yid.service.fwbackConfig
            .saveServiceWxReturn(this.selectReobj)
            .then(reschanl => {})
        })
      })
    },
    ...
    addserver() {
      ...
      servids =
        servids.substring(servids.length - 1) == ','
          ? servids.substring(0, servids.length - 1)
          : servids
      servnames =
        servnames.substring(servnames.length - 1) == ','
          ? servnames.substring(0, servnames.length - 1)
          : servnames
      let pm = {}
      if (this.id == -1) {
        pm = {
    ...
    clickServer(index) {
      if (!this.chosenIndex.includes(index)) {
        ...
        this.chosenIndex.splice(
          this.chosenIndex.findIndex(item => item === index),
          1
        )
      }
    },
    ...
    showAddDialogType() {
      ...
      yid.service.fwbackConfig
        .getServiceInfoByTypeId(typeParm)
        .then(res => {
          let parm = {
            servtid: this.editableTabsValue,
            isDel: '0',
