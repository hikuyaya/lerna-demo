  ...
  <div class="messMbConfig">
    ...
    <yid-table pagination ref="table" :data="messData" style="margin-top: 15px">
      ...
      <yid-table-column label="操作" width="150" prop="content">
        <template slot-scope="scope">
          <el-link type="primary" @click="showDialogType(scope.row.id)"
            >编辑</el-link
          >
          <el-link
            type="primary"
            style="margin: 0 20px"
            @click="serveDelete(scope.row.id)"
            >删除</el-link
          >
        </template>
      </yid-table-column>
        ...
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm">
          ...
          <div>
            ...
            <el-form-item label="头消息模板" v-if="stype == 2" prop="first">
              ...
              <el-row>可选通配符：</el-row>
              <div class="tongpf">
                <ul @click="selmodel('[顾客昵称]')">
                  [顾客昵称]
                </ul>
                <ul @click="selmodel('[顾客手机号]')">
                  [顾客手机号]
                </ul>
              </div>
            </el-form-item>
            <el-form-item label="尾消息模板" prop="remask">
              <div style="margin: 15px 0 15px 70px">
                <el-input
                  ...
                  placeholder="请输入内容"
                  v-model="ruleForm.remask"
                  style="width: 400px; font-size: 12px">
                </el-input>
              </div>
              <el-row>可选通配符：</el-row>
              <div class="tongpf">
                <ul @click="selmodelremask('[顾客昵称]')">
                  [顾客昵称]
                </ul>
                <ul @click="selmodelremask('[顾客手机号]')">
                  [顾客手机号]
                </ul>
                <ul @click="selmodelremask('[顾客称谓]')">
                  [顾客称谓]
                </ul>
                <ul @click="selmodelremask('[顾客昵称]')">
                  [顾客昵称]
                </ul>
                <ul @click="selmodelremask('[门店名称]')">
                  [门店名称]
                </ul>
                <ul @click="selmodelremask('[提醒期]')">
                  [提醒期]
                </ul>
                <ul @click="selmodelremask('[项目名称]')">
                  [项目名称]
                </ul>
              </div>
            </el-form-item>
