<template>
  <div class="mtkeysConfig">
        <el-collapse-transition>
          <div v-show="showList">
            <el-button type="primary" @click="dialogFormMt=true">新增账号</el-button>
            <el-row style="color:#e6a23c; margin: 15px 0">操作提示：开通美团账号映射权限请联系平台客服办理</el-row>
            <yid-table pagination ref="mtkeystable" :data="mtkeysData" style="margin-top: 15px;">
              <yid-table-column label="美团帐号" min-width="100" prop="account"></yid-table-column>
              ...
              <yid-table-column label="授权状态" min-width="120">
                <template slot-scope="scope">
                            <span v-if="!$yid.util.isEmpty(scope.row.accessToken)">
                                ...
                                <span v-if="scope.row.needAuth == '1'"><font color="red">即将过期</font></span>
                            </span>
                  <span v-if="$yid.util.isEmpty(scope.row.accessToken)">未授权</span>
                </template>
              </yid-table-column>
              <yid-table-column label="授权到期时间" min-width="120" prop="expiresDate">
                <template slot-scope="scope">
                            <span v-if="!$yid.util.isEmpty(scope.row.accessToken)">
                                <span
                                    v-if="scope.row.needAuth == '0'">{{ $yid.util.formatDate(scope.row.expiresDate) }}</span>
                                <span v-if="scope.row.needAuth == '1'"><font
                                    color="red">{{ $yid.util.formatDate(scope.row.expiresDate) }}</font></span>
                            </span>
              ...
              <yid-table-column label="门店数量" min-width="120">
                <template slot-scope="scope">
                  <el-link type="primary" @click="showShopConfig(scope.row)">共{{ scope.row.mtShopsList.length }}家门店
                  </el-link>
                </template>
              </yid-table-column>
              <yid-table-column label="剩余刷新次数" min-width="120" prop="remainRefreshCount"></yid-table-column>
              <yid-table-column label="备注" min-width="200" prop="name"></yid-table-column>
              <yid-table-column label="操作" min-width="150" prop="content">
                <template slot-scope="scope">
                  <el-link type="primary" @click="getUrl(scope.row)">去授权</el-link>
                  <!--<a type="_blank"  :href="getUrl(scope.row)">去授权</a>-->
                  <el-link type="primary" style="margin: 0 10px 0 10px;" @click="editMtsqDialog(scope.row)">编辑</el-link>
                </template>
              </yid-table-column>
            </yid-table>
          </div>
        ...
        <el-collapse-transition>
          <div v-show="!showList">
            <el-button @click="back" type="primary">返回</el-button>&nbsp;&nbsp;&nbsp;门店列表页
            <div style="margin-top: -10px;">
              <el-divider/>
            </div>
            <el-button @click="showAddShopDialog" type="primary">添加门店</el-button>
            <yid-table ref="shoptable" :data="shopData" style="margin-top: 15px;">
              ...
              <yid-table-column label="美团门店ID" min-width="150" prop="openShopUuid">
                <template slot-scope="scope">
                  <el-input placeholder="请输入对应美团门店ID" v-model="scope.row.openShopUuid"
                            @change="saveMtShop(scope.row)"></el-input>
                </template>
              </yid-table-column>
              <yid-table-column label="操作" min-width="120" prop="content">
                <template slot-scope="scope">
                  <el-link type="primary" @click="deleteMtShop(scope.$index,scope.row)">删除</el-link>
                </template>
              </yid-table-column>
            </yid-table>
          </div>
    ...
    <yid-dialog :title="mtsqDialog.title" :visible.sync="mtsqDialog.visible" width="800px">
    ...
    <yid-dialog :title="editDialog.title" :visible.sync="editDialog.visible" width="500px">
      <el-form ref="editForm" :model="editForm" label-width="140px">
        <el-form-item label="账号管理员：" prop="accountName">
          <el-input v-model="editForm.accountName" label="姓名" style="width: 120px"/>&nbsp;&nbsp;<el-input
            v-model="editForm.tel" label="联系电话" style="width: 150px"/>
        </el-form-item>
        <el-form-item label="帐号备注：" prop="name">
          <el-input type="textarea" v-model="editForm.name" :rows="3" style="width: 280px"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <yid-dialog :title="addShopDialog.title" :visible.sync="addShopDialog.visible" width="800px">
      <el-form ref="searchShopForm" :model="searchShopForm" label-width="20px">
        <el-form-item label="">
          <el-select value-key="id" v-model="searchShopForm.regionIds" multiple placeholder="请选择" style="width: 400px">
            <el-option v-for="item in dictRegions" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>&nbsp;&nbsp;
        ...
        <el-checkbox-group v-model="checkShop" @change="handleCheckedCitiesChange">
          <el-checkbox v-for="item in checkShopList" :key="item.id" :label="item.id" style="width: 150px">
...
export default {
  ...
  methods: {
    ...
    getUrl(item) {
      window.open("https://e.dianping.com/dz-open/merchant/auth?app_key=" + item.appkey + "&redirect_url=" + encodeURIComponent(yid.config.API.BASE + "api-third/auth/callback") + "&state=" + item.id);
    }
    ,
    ...
    deleteMtShop(index, row) {
      ...
      if (yid.util.isEmpty(row.id)) {
        ...
        yid.util.confirm('你确定要删除' + row.shopname + '的授权吗？', '', '', () => {
