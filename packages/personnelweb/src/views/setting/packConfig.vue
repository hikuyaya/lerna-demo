  ...
  <div class="appoint-list">
    ...
    <el-form inline>
      <el-form-item>
        ...
        <el-button type="primary" @click="duoServPack()" v-if="false">新增多选套餐</el-button>
      </el-form-item>
      <br/>
      <el-form-item label="套餐：">
        <el-input clearable v-model.trim="form.package" placeholder="套餐编码/名称" style="width: 180px;"></el-input>
      </el-form-item>
      <el-form-item label="套餐分类：">
        <el-select clearable filterable v-model="form.branch" @change="changeBranchType" placeholder="请选择组"
                   style="width: 160px;">
          <el-option
      ...
      <el-form-item label="">
        <el-select clearable filterable v-model="form.ptid" placeholder="请选择分类" style="width: 160px;">
          <el-option
      ...
      <el-form-item label="品牌：">
        <el-select clearable v-model="form.brand" @change="filterBrand(form.brand)" placeholder="请选择品牌"
                   style="width: 120px;">
          <el-option v-for="item in brands" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="门店：">
        <el-select @clear="clearFormShop" @blur="clearFormShop" clearable v-model.trim="form.shop" filterable
                   :filter-method="filterShop" placeholder="请选择门店"
                   style="width: 160px;">
    ...
    <yid-table pagination ref="tableData" @selection-change="handleServicesChange">
      <yid-table-column label="编号" min-width="60" prop="spcode"></yid-table-column>
      <yid-table-column label="套餐名称" min-width="140" prop="spname"></yid-table-column>
      <yid-table-column label="可售时间" min-width="180" prop="date" :formatter="dateFormat"></yid-table-column>
      <yid-table-column label="有效期" min-width="80" prop="expiryType" :formatter="dateFormat1"></yid-table-column>
      <yid-table-column label="套餐价" min-width="70" prop="totalprice"></yid-table-column>
      <yid-table-column label="成本价" min-width="70" prop="cost"></yid-table-column>
      <yid-table-column label="套餐内容" min-width="220" prop="content" align="left">
        <template slot-scope="scope">
          <el-row type="flex" :key="item.id" v-for="item in scope.row.servs" justify="space-between">
            <el-col :span="22">
              <div v-if="scope.row.isMultiple=='0'">{{ item.typename }}-{{ item.sdname }}, {{ item.num }}次,
                总价:{{ item.pprice }} {{ item.isGift == '1' ? '赠' : '' }}
              </div>
              <div v-if="scope.row.isMultiple=='1'">
                {{ item.sdname }}, {{ item.num }}次, 总价:{{ item.pprice }} {{ item.isGift == '1' ? '赠' : '' }}
              </div>
            </el-col>
          </el-row>
      ...
      <yid-table-column label="适用门店" prop="shopNum" width="140" align="left">
        <template slot-scope="scope">
          <label>{{ scope.row.brandname ? scope.row.brandname + '' : '' }} <br/>
            共{{ scope.row.shopNum ? scope.row.shopNum : '0' }}家门店</label><br/>
          <el-link v-if="scope.row.brandname" @click="lookupShop(scope.row)" type="primary">查看修改</el-link>
        </template>
      </yid-table-column>
    ...
    <yid-dialog :title="packAddDialog.title" :visible.sync="packAddDialog.visible" width="740px">
      <el-form :model="packAddDialog" :rules="formrules" label-width="145px" ref="copyForm1"
        ...
        <el-form-item label="可售时间:">
          <el-date-picker style="width:320px;"
                          ...
                          type="daterange" @change="changDates1"
                          start-placeholder="开始日期" range-separator="至" end-placeholder="结束日期">
          </el-date-picker>
          <el-checkbox v-model="packAddDialog.type" @change="changDate1" style="margin-left: 5px;">长期
          </el-checkbox>
        </el-form-item>
        <el-form-item label="有效期:">
          <el-radio-group v-model="packAddDialog.expiryType">
            ...
            <div style="margin-top: 3px;">
              ...
              <explain style="color:silver">从购买之日起延迟多少个月到期</explain>
            </div>
        ...
        <el-form-item label="每人仅限购买:">
          <el-input type="number" min="1" v-model="packAddDialog.bugcs"
        ...
        <el-form-item label="添加服务项目:">
          ...
          <el-row :gutter="10" :key="item.servid" v-for="item in packAddDialog.servs" type="flex"
                  style="margin-top: 12px;">
            <el-col :span="4">{{ item.sdname }}</el-col>
            <el-col :span="7">
              <el-input type="number" min="1" v-model="item.num" style="width:100px;margin: 0 10px 0 10px"
                        placeholder="次数" clearable/>
              次
            </el-col>
            <el-col :span="7">
              <el-input type="number" min="0" v-model="item.pprice" style="width:120px; margin-right:5px"
                        placeholder="套餐总价" clearable/>
              元
            </el-col>
            <el-col :span="3">
            ...
            <el-col :span="3">
              <el-link type="primary" @click="deleteServ(item.servid)">删除</el-link>
            </el-col>
          </el-row>
        ...
        <el-form-item label="添加产品:">
          ...
          <el-row :gutter="10" :key="item.pid" v-for="item in packAddDialog.products" type="flex"
                  style="margin-top: 12px;">
            <el-col :span="4">{{ item.sdname }}</el-col>
            <el-col :span="7">
              <el-input type="number" min="1" v-model="item.num" style="width:100px;margin: 0 10px 0 10px"
            ...
            <el-col :span="3">
              <el-link type="primary" @click="deleteProd(item.pid)">删除</el-link>
            </el-col>
          </el-row>
        ...
        <el-form-item label="套餐销售:">
          <el-checkbox v-model="packAddDialog.issale" style="margin-left: 5px;">允许线上销售</el-checkbox>
        </el-form-item>
        ...
        <el-form-item label="品牌：" prop="brand">
          <el-select v-if="packAddDialog.brandname?false:true" clearable v-model="packAddDialog.brand"
                     @change="changBrnad('1')">
            <el-option v-for="item in brands" :label="item.name" :value="item.id"></el-option>
          </el-select>
          <el-label v-if="packAddDialog.brandname?true:false">{{ packAddDialog.brandname }}</el-label>
        </el-form-item>
        <el-form-item label="适用门店：" prop="shop">
          <el-button type="primary" @click="selectShop()">选择门店</el-button>
          {{ packAddDialog.shoplist ? '共选中' + packAddDialog.shoplist.length + '家门店' : '' }}
        </el-form-item>
        <el-form-item label="套餐分类：" prop="branch" :rules="[{required: true,message:'请选择套餐分类'}]">
          <el-select v-model="packAddDialog.branch" @change="changePackageBranch" placeholder="请选择部门"
                     style="width: 160px;">
            <el-option
                v-for="item in branchs"
                :key="item.id"
          ...
          <el-select v-model="packAddDialog.ptid" @change="changePackageType" placeholder="请选择类别"
        ...
        <el-form-item label="套餐图片：">
          <el-upload
            ...
            <img v-if="packAddDialog.photopath" :src="packAddDialog.photopath" class="avatar el-upload">
        ...
        <el-form-item label="是否允许跨店消费:">
          <el-radio :label="1" v-model="packAddDialog.isDiffshop" style="margin-left: 5px;">允许</el-radio>
          <el-radio :label="0" v-model="packAddDialog.isDiffshop" style="margin-left: 5px;">不允许</el-radio>
          <yid-ask>适用门店内的跨店消费，赠送项目/产品仅能在赠送门店消费，不允许则只能在购买套餐门店消费；非适用门店内消费可设置跨品牌消费</yid-ask>
          <el-form-item label="套餐内容:" label-width="80px" v-if="packAddDialog.isDiffshop=='1'">
            <el-checkbox v-model="packAddDialog.isAllowPackage" style="margin-left: 5px;">服务项目</el-checkbox>
            <el-checkbox v-model="packAddDialog.isAllowProduct" style="margin-left: 5px;">产品</el-checkbox>
          </el-form-item>
        </el-form-item>
        <el-form-item label="是否允许跨品牌消费:">
          <el-radio :label="1" v-model="packAddDialog.isDiffbrand" style="margin-left: 5px;">允许</el-radio>
          <el-radio :label="0" v-model="packAddDialog.isDiffbrand" style="margin-left: 5px;">不允许</el-radio>
          <el-form-item label="套餐内容:" label-width="80px" style="margin-bottom:5px;"
                        v-if="packAddDialog.isDiffbrand=='1'">
            <el-checkbox v-model="packAddDialog.ibAllowPackage" style="margin-left: 5px;">服务项目</el-checkbox>
            <el-checkbox v-model="packAddDialog.ibAllowProduct" style="margin-left: 5px;">产品</el-checkbox>
          </el-form-item>
          <el-form-item label="门店品牌:" label-width="80px" style="margin-bottom:5px;"
                        v-if="packAddDialog.isDiffbrand=='1'">
            <el-checkbox v-model="packAddDialog.isBrandAll" @change="changBrand1" style="margin-left: 5px;">
              全部
            </el-checkbox>
            <el-checkbox v-for="item in packAddDialog.brands" @change="changBrand3" v-model="item.flag">
              {{ item.name }}
            </el-checkbox>
          </el-form-item>
        </el-form-item>
    ...
    <yid-dialog :title="packDuoDialog.title" :visible.sync="packDuoDialog.visible" width="740px">
      <el-form ref="copyForm4" :model="packDuoDialog" :rules="formrules" label-width="130px"
               :label-position="labelPosition">
        <el-form-item label="套餐编号:">
        ...
        <el-form-item label="可售时间:">
          <el-date-picker style="width:320px;" @change="changDates2"
          ...
          <el-checkbox v-model="packDuoDialog.type" @change="changDate2" style="margin-left: 5px;">长期
          </el-checkbox>
        </el-form-item>
        <el-form-item label="有效期:">
          <el-radio-group v-model="packDuoDialog.expiryType">
            ...
            <div style="margin-top: 3px;">
              ...
              <el-input type="number" min="1" v-model="packDuoDialog.expirtyMonth"
                        :disabled="packDuoDialog.expiryType!='1'"
                        style="width:100px;" clearable></el-input>
              <el-tag style="margin-left: 5px;">个月</el-tag>
              <explain style="color:silver">从购买之日起延迟多少个月到期</explain>
            </div>
            <div style="margin-top: 4px;">
              <el-radio :label="2" @change="changeDuoCheck">指定到固定日期</el-radio>
              <el-date-picker
        ...
        <el-form-item label="每人仅限购买:">
          <el-input type="number" min="1" v-model="packDuoDialog.bugcs"
                    style="width:120px;" clearable/>
          <el-tag style="margin-left: 5px;">次</el-tag>
        </el-form-item>
        <el-form-item label="添加服务项目:">
          ...
          <div :key="index" v-for="(serv,index) in packDuoDialog.servs">
            <el-row :gutter="24" type="flex" style="margin-top: 12px;">
              <el-col :span="15">项目组标题
                <el-input v-model="serv.sdname" placeholder="项目组标题"
                          style="width:240px;margin: 0 10px 0 10px" clearable/>
              </el-col>
              <el-col :span="3">
                <el-link type="primary" @click="deleteDuoServ(index)">删除</el-link>
              </el-col>
            </el-row>
            <el-row :gutter="24" type="flex" style="margin-top: 12px;">
              <el-col :span="18">
                <el-tag :key="i" v-for="(item,i) in serv.items" @close="delDuoServ(index,i)"
                        style="margin-right: 5px" closable>{{ item.servname }}
                </el-tag>
              </el-col>
              <el-col :span="6">
                <el-link type="primary" @click="addDuoServ(index,serv.items)">添加</el-link>
              </el-col>
            </el-row>
            <el-row :gutter="24" type="flex" style="margin-top: 12px;">
              <el-col :span="12">每次可选项目次数
                <el-input type="number" min="1" v-model="serv.selNum"
                          style="width:100px;margin: 0 10px 0 10px" placeholder="次数" clearable/>
                次
              </el-col>
              <el-col :span="6">
            ...
            <el-row :gutter="24" type="flex" style="margin-top: 12px;">
              <el-col :span="12">套餐总次数
                <el-input type="number" min="1" v-model="serv.totalNum"
                          style="width:140px;margin: 0 10px 0 10px" placeholder="总次数" clearable/>
                次
              </el-col>
            ...
            <el-row :gutter="24" type="flex" style="margin-top: 12px;">
              <el-col :span="12">套餐总价
                <el-input type="number" min="0" v-model="serv.pprice"
                          style="width:152px;margin: 0 10px 0 10px" placeholder="套餐总价" clearable/>
                元
              </el-col>
              <el-col :span="12"></el-col>
            </el-row>
        ...
        <el-form-item label="添加产品:">
          ...
          <div :key="index" v-for="(product,index) in packDuoDialog.products">
            <el-row :gutter="24" type="flex" style="margin-top: 12px;">
              <el-col :span="15">商品组标题
                <el-input v-model="product.sdname"
                          style="width:240px;margin: 0 10px 0 10px" placeholder="项目组标题" clearable/>
              </el-col>
              <el-col :span="3">
                <el-link type="primary" @click="deleteDuoGood(index)">删除</el-link>
              </el-col>
            </el-row>
            <el-row :gutter="24" type="flex" style="margin-top: 12px;">
              <el-col :span="18">
                <el-tag :key="i" v-for="(item,i) in product.items"
                        @close="delDuoGood(index,i)" style="margin-right: 5px" closable>{{ item.pname }}
                </el-tag>
              </el-col>
              <el-col :span="6">
                <el-link type="primary" @click="addDuoGood(index,product.items)">添加</el-link>
              </el-col>
            </el-row>
            <el-row :gutter="24" type="flex" style="margin-top: 12px;">
              <el-col :span="12">每次可选产品次数
                <el-input type="number" min="1" v-model="product.selNum"
                          style="width:100px;margin: 0 10px 0 10px" placeholder="次数" clearable/>
                次
              </el-col>
              <el-col :span="6">
            ...
            <el-row :gutter="24" type="flex" style="margin-top: 12px;">
              <el-col :span="12">套餐总次数
                <el-input type="number" min="1" v-model="product.totalNum"
                          style="width:140px;margin: 0 10px 0 10px" placeholder="次数" clearable/>
                次
              </el-col>
            ...
            <el-row :gutter="24" type="flex" style="margin-top: 12px;">
              <el-col :span="12">套餐总价
                <el-input type="number" min="0" v-model="product.pprice"
                          style="width:152px;margin: 0 10px 0 10px" placeholder="套餐总价" clearable/>
                元
              </el-col>
              <el-row :span="12"></el-row>
        ...
        <el-form-item label="套餐销售:">
          <el-checkbox v-model="packDuoDialog.issale" style="margin-left: 5px;">允许线上销售</el-checkbox>
        </el-form-item>
        <el-form-item label="是否允许转赠：">
        ...
        <el-form-item label="品牌：" prop="brand">
          <el-select v-if="packDuoDialog.brandname?false:true" clearable v-model="packDuoDialog.brand"
                     @change="changBrnad('2')">
            <el-option v-for="item in brands" :label="item.name" :value="item.id"></el-option>
          </el-select>
          <el-label v-if="packDuoDialog.brandname?true:false">{{ packDuoDialog.brandname }}</el-label>
        </el-form-item>
        <el-form-item label="适用门店：" prop="shop">
          <el-button type="primary" @click="selectShop()">选择门店</el-button>
          {{ packDuoDialog.shoplist ? '共选中' + packDuoDialog.shoplist.length + '家门店' : '' }}
        </el-form-item>
        <el-form-item label="套餐分类：" prop="branch" :rules="[{required: true,message:'请选择套餐分类'}]">
          <el-select v-model="packDuoDialog.branch" @change="changeDuoPackageBranch" placeholder="请选择部门"
                     style="width: 160px;">
            <el-option
                v-for="item in branchs"
                :key="item.id"
          ...
          <el-select v-model="packDuoDialog.ptid" @change="changeDuoPackageType" placeholder="请选择类别"
                     style="width: 160px;margin-left: 20px;">
            <el-option
        ...
        <el-divider>异店划套餐设置</el-divider>
        <el-form-item label="是否允许跨店消费:">
          <el-radio :label="1" v-model="packDuoDialog.isDiffshop" style="margin-left: 5px;">允许</el-radio>
          <el-radio :label="0" v-model="packDuoDialog.isDiffshop" style="margin-left: 5px;">不允许</el-radio>
          <yid-ask>适用门店内的跨店消费，赠送项目/产品仅能在赠送门店消费，不允许则只能在购买套餐门店消费；非适用门店内消费可设置跨品牌消费</yid-ask>
          <el-form-item label="套餐内容:" label-width="80px" v-if="packDuoDialog.isDiffshop=='1'">
            <el-checkbox v-model="packDuoDialog.isAllowPackage" style="margin-left: 5px;">服务项目</el-checkbox>
            <el-checkbox v-model="packDuoDialog.isAllowProduct" style="margin-left: 5px;">产品</el-checkbox>
          </el-form-item>
        </el-form-item>
        <el-form-item label="是否允许跨品牌消费:">
          <el-radio :label="1" v-model="packDuoDialog.isDiffbrand" style="margin-left: 5px;">允许</el-radio>
          <el-radio :label="0" v-model="packDuoDialog.isDiffbrand" style="margin-left: 5px;">不允许</el-radio>
          <el-form-item label="套餐内容:" label-width="80px" style="margin-bottom:5px;"
                        v-if="packDuoDialog.isDiffbrand=='1'">
            <el-checkbox v-model="packDuoDialog.ibAllowPackage" style="margin-left: 5px;">服务项目</el-checkbox>
            <el-checkbox v-model="packDuoDialog.ibAllowProduct" style="margin-left: 5px;">产品</el-checkbox>
          </el-form-item>
          <el-form-item label="门店品牌:" label-width="80px" style="margin-bottom:5px;"
                        v-if="packDuoDialog.isDiffbrand=='1'">
            <el-checkbox v-model="packDuoDialog.isBrandAll" @change="changBrand2" style="margin-left: 5px;">
              全部
            </el-checkbox>
            <el-checkbox v-for="item in packDuoDialog.brands" @change="changBrand3" v-model="item.flag">
              {{ item.name }}
            </el-checkbox>
          </el-form-item>
        </el-form-item>
    ...
    <yid-dialog :title="servAddDialog.title" :visible.sync="servAddDialog.visible" width="800px">
      <el-form label-width="100px" ref="copyForm2">
        <el-form-item label="筛选：">
          <el-select clearable v-model="servAddDialog.branch"
                     @change="changeBranch(servAddDialog.branch,'1')" placeholder="选择部门"
                     style="width: 140px;">
          ...
          <el-select clearable v-model="servAddDialog.serviceType" placeholder="请选择分类"
          ...
          <el-input clearable v-model="servAddDialog.sername" placeholder="项目名称/编码"
                    style="margin-left: 5px;width: 160px;"></el-input>
          <el-button type="primary" @click="addServs()" v-if="!duoflag" style="margin-left: 6px">确认添加
          </el-button>
          <el-button type="primary" @click="addDuoServs(servAddDialog.index)" v-if="duoflag"
                     style="margin-left: 6px">确认添加
          </el-button>
        ...
        <el-form-item label="">
          <table border="1" style="width: 100%" class="serve-table">
            <tr>
              ...
              <th width="25%">分类</th>
              <th width="55%">
                <el-checkbox v-model="servAddDialog.checkbox" @change="checkServ"></el-checkbox>
                (编号）项目名称
              </th>
            </tr>
            <tr :key="item.servid"
              ...
              <td>{{ item.typename }}</td>
              <td>
                <el-checkbox v-model="item.type" :label="item.servid"></el-checkbox>
    ...
    <yid-dialog :title="goodAddDialog.title" :visible.sync="goodAddDialog.visible" width="800px">
      <el-form label-width="120px" ref="copyForm3">
        <el-form-item label="筛选：">
          <el-select clearable v-model="goodAddDialog.branch"
                     @change="changeBranch(goodAddDialog.branch,'2')" placeholder="所属部门"
                     style="width: 140px;">
          ...
          <el-input clearable v-model="goodAddDialog.goodname" placeholder="产品名称/编码"
                    style="margin-left: 5px;width: 160px;"></el-input>
          <el-button type="primary" @click="addGoods()" v-if="!duoflag" style="margin-left: 6px">确认添加
          </el-button>
          <el-button type="primary" @click="addDuoGoods(goodAddDialog.index)" v-if="duoflag"
                     tyle="margin-left: 6px">确认添加多
          </el-button>
        ...
        <el-form inline>
          ...
          <el-form-item label="区域：">
            <el-select ref="selectRegionRef" @clear="clearSelectRegion" clearable
                       v-model="shopDialog.region.name" style="width: 160px;">
    ...
    <yid-dialog :title="importDialog.title" :visible.sync="importDialog.visible" @close="importCancel"
                width="700px">
...
export default {
  ...
  data() {
    return {
      ...
      formrules: {
        spname: [{required: true, message: '请填写项套餐名称'}, {min: 0, max: 16, message: '套餐名称在16个汉字以内'}],
    ...
    screenService() {
      return function (item) {
        ...
        if (this.servAddDialog.sername) {
          if (item.sdname.indexOf(this.servAddDialog.sername) !== -1 ||
  ...
  methods: {
    ...
    editServPack(data) {
      ...
      if (row.isMultiple == "0") {
        ...
        this.packAddDialog.isAllowPackage = row.dsAllowPackage == '1' ? true : false;
        this.packAddDialog.isAllowProduct = row.dsAllowProduct == '1' ? true : false;
        this.packAddDialog.isDiffbrand = Number(row.isDiffbrand);
        this.packAddDialog.ibAllowPackage = row.dbAllowPackage == '1' ? true : false;
        this.packAddDialog.ibAllowProduct = row.dbAllowProduct == '1' ? true : false;
        if (row.dbBrandnames) {
      ...
      if (row.isMultiple == "1") {
        ...
        this.packDuoDialog.isAllowPackage = row.dsAllowPackage == '1' ? true : false;
        this.packDuoDialog.isAllowProduct = row.dsAllowProduct == '1' ? true : false;
        this.packDuoDialog.isDiffbrand = Number(row.isDiffbrand);
        this.packDuoDialog.ibAllowPackage = row.dbAllowPackage == '1' ? true : false;
        this.packDuoDialog.ibAllowProduct = row.dbAllowProduct == '1' ? true : false;
        if (row.dbBrandnames) {
        ...
        if (valid) {
          ...
          that.packDuoDialog.servs.forEach((item, index) => {
            ...
            if (item.totalNum && item.pprice) {
              item.price = Number(item.pprice) / Number(item.totalNum).toFixed(1)
            }
          })
          ...
          if (that.packDuoDialog.cost && Number(that.packDuoDialog.cost) > Number(pprice)) {
          ...
          if (that.packDuoDialog.isDiffshop == '1') {
            ...
            if (!that.packDuoDialog.isAllowPackage && !that.packDuoDialog.isAllowProduct) {
          ...
          if (that.packDuoDialog.isDiffbrand == '1') {
            ...
            if (!that.packDuoDialog.ibAllowPackage && !that.packDuoDialog.ibAllowProduct) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (that.packAddDialog.isDiffshop == '1') {
            ...
            if (!that.packAddDialog.isAllowPackage && !that.packAddDialog.isAllowProduct) {
              yid.util.error("设置可以跨店消费，套餐内容必需勾选一项")
              return
            }
          } else {
          ...
          if (that.packAddDialog.isDiffbrand == '1') {
            ...
            if (!that.packAddDialog.ibAllowPackage && !that.packAddDialog.ibAllowProduct) {
              yid.util.error("设置可以跨品牌消费，套餐内容必需勾选一项")
              return
            }
          } else {
    ...
    init() {
      this.excelConvertUrl = $yid.config.API.BASE + "api-chain/servicepackage/convertSystem";
    ...
    deleteShop(row, spname) {
      yid.util.confirm('删除后【' + spname + '】不能在【' + row.shopname + '】售卖，确定要删除吗？？', '', '', () => {
        let i = -1;
    ...
    saveImportPackage() {
      if (this.importDialog.services.length == 0) {
        ...
        service.servicepackage.saveSystem(this.importDialog.services).then(res => {
          if (res.resp_code == "200") {
            this.querylist();
