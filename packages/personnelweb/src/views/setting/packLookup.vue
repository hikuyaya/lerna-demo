<template>
    <div class="appoint-list">
        ...
        <el-form inline v-model="form">
            <el-form-item label="门店：" prop="shop" :rules="[{ required: true, message: ' '}]">
                <el-select clearable v-model="form.shop"  filterable :filter-method="filterShop" placeholder="请选择门店" style="width: 160px;">
        ...
        <yid-table-column label="套餐名称" min-width="160" prop="spname"></yid-table-column>
        <yid-table-column label="可售时间" min-width="180" prop="date" :formatter="dateFormat"></yid-table-column>
        <yid-table-column label="有效期" min-width="100" prop="expiryType" :formatter="dateFormat1"></yid-table-column>
        ...
        <yid-table-column label="套餐内容" min-width="240" prop="content" align="left">
            <template slot-scope="scope">
                <el-row type="flex" :key="item.id" v-for="item in scope.row.servs" justify="space-between">
                    <el-col :span="22">
                        <div v-if="scope.row.isMultiple=='0'">{{item.typename}}-{{item.sdname}}, {{item.num}}次, 总价:{{item.pprice}} {{item.isGift=='1'?'赠':''}}</div>
                        <div v-if="scope.row.isMultiple=='1'">{{item.sdname}}, {{item.num}}次, 总价:{{item.pprice}} {{item.isGift=='1'?'赠':''}}</div>
                    </el-col>
                </el-row>
        ...
        <yid-table-column label="异店划套餐规则"  prop="shopNum" width="120">
            <template slot-scope="scope">
                <el-link @click="lookupRule(scope.row)" type="primary">查看规则</el-link>
            </template>
        </yid-table-column>
        ...
        <yid-dialog :title="lookupDialog.title" :visible.sync="lookupDialog.visible" @close="lookupDialog.visible=false" width="500px">
            <el-form :model="lookupDialog"  label-width="150px" ref="copyForm">
                ...
                <el-form-item label="是否允许跨店消费: ">
                    ...
                    <div v-show="lookupDialog.isDiffshop == '1'">
                        套餐内容：{{lookupDialog.dsAllowPackage == '1' ? '项目':''}}  {{lookupDialog.dsAllowProduct == '1' ? '产品':''}}
                    </div>
                </el-form-item>
                <el-form-item label="是否允许跨品牌消费: ">
                    {{lookupDialog.isDiffbrand == '1' ? '允许':'不允许'}}
                    <div v-show="lookupDialog.isDiffbrand == '1'">
                        套餐内容：{{lookupDialog.dbAllowPackage == '1' ? '项目':''}}  {{lookupDialog.dbAllowProduct == '1' ? '产品':''}}<br/>
                        门店品牌：<label v-for="brand in lookupDialog.dbBrands">{{brand}};  </label>
                    </div>
                </el-form-item>
            </el-form>
        </yid-dialog>
