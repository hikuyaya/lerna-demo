    ...
    <div>
        <el-form ref="searchForm" inline :model="searchForm">
            <el-form-item label="门店：" prop="shopid" :rules="[{ required: true, message: '必须选择一个门店'}]">
                <el-select @clear="clearFormShop" @blur="clearFormShop" clearable v-model.trim="searchForm.shopid" @change="changeShop"
                           filterable
            ...
            <el-form-item label="部门">
                <el-select clearable filterable v-model="searchForm.branch" @change="changeBranch" placeholder="请选择部门"
                           style="width: 160px;">
                    <el-option
            ...
            <el-form-item label="分类">
                <el-select clearable filterable v-model="searchForm.servtid" placeholder="请选择分类" style="width: 160px;">
                    <el-option
            ...
            <el-form-item label="项目">
                <el-input clearable filterable v-model="searchForm.servname" placeholder="项目编码/项目名称"
                          style="width: 150px;"></el-input>
            </el-form-item>
            <el-form-item label="状态">
                <el-select clearable filterable v-model="searchForm.status" placeholder="请选择" style="width: 100px;">
                    <el-option
        ...
        <yid-table pagination ref="table" style="width: 100%;margin-top: 20px;" :data="tableData">
            ...
            <yid-table-column label="会员卡折扣价设定" prop="discount" width="180">
                <template slot-scope="scope">
                    <el-row type="flex" justify="space-between">
                        <el-col :span="22">
                            <div :key="item.id" v-for="item in scope.row.servcards">{{ item.cardname }}
            ...
            <yid-table-column label="员工级别价格设置" prop="special" width="180">
                <template slot-scope="scope">
                    <el-row type="flex" justify="space-between">
                        ...
                        <el-col :span="2"><i class="el-icon-document" @click="showSpecialDialog(scope.row)"></i>
                        </el-col>
                    </el-row>
                </template>
            </yid-table-column>
        ...
        <yid-dialog :title="speicalDialog.title" :visible.sync="speicalDialog.visible" @close="levelCancel"
            ...
            <el-row type="flex" justify="space-between">
                ...
                <el-col :span="6">
                    <el-select v-model="speicalDialog.id" @change="changSpeicalData">
                        <el-option v-show="item.eecode" :key="item.id" v-for="item in speicalDialog.speicalData" :label="item.eecode + '-' + item.name "
                                   :value="item.id">
                        </el-option>
                        <el-option v-show="!item.eecode" :key="item.id" v-for="item in speicalDialog.speicalData" :label="item.name "
                                   :value="item.id">
                        </el-option>
                    </el-select>
                </el-col>
            ...
            <yid-table ref="tab" style="margin-top: 15px;" :data="speicalDialog.speicalRows">
                <yid-table-column label="职位" min-width="150" prop="btype">
                    <template slot-scope="scope">
                        <div>
                            [{{ scope.row.btype == "1" ? "职务级别" : "个人"
                            }}]{{ scope.row.btype == "1" ? scope.row.pslname : scope.row.eecode + '-' + scope.row.eename}}
                        </div>
                    </template>
                </yid-table-column>
                ...
                <yid-table-column label="最低价格" min-width="130" prop="minPrice">
                    <template slot-scope="scope">
                        <el-input clearable type="number" min="1" :max="scope.row.specialPrice"
                                  ...
                                  placeholder="不必填" v-model="scope.row.minPrice"/>
                    </template>
                </yid-table-column>
    ...
    export default {
        ...
        methods: {
            ...
            init() {
                ...
                service.base.servicebaseInfo.findCardinfos({"shopid": this.searchForm.shopid}).then(res => {
                    if (res.data) {
                ...
                service.base.servicebaseInfo.findEmployees({
                    "status": "1",
                ...
                service.base.servicebaseInfo.findPositions({
                    "status": "1",
                    "shopid": this.searchForm.shopid
                }).then(res => {
            ...
            saveSpeical() {
                ...
                this.speicalDialog.speicalRows.forEach(item => {
                    ...
                    if (item.specialPrice && item.discountType == "0") {
                        message += (item.dtype == '1' ? item.pslname : item.eename) + "会员卡折扣必须设置";
                    }
                })
                if (message) {
