<template>
  <div class="dept">
    <el-form inline>
      ...
      <el-form-item label="">
        <el-button type="primary" @click="addPerfratioRule()"
          >添加业绩比率规则</el-button
        >
      </el-form-item>
    </el-form>
    ...
    <yid-table ref="table" :data="perfratioData" style="margin-top: 12px">
      ...
      <yid-table-column
        label="支付方式"
        min-width="100"
        prop="payname"></yid-table-column>
      <yid-table-column
        label="业绩比率"
        min-width="100"
        prop="perfratio"></yid-table-column>
      <yid-table-column label="操作" min-width="100" prop="content">
        <template slot-scope="scope">
          <el-button type="text" @click="editPerfratio(scope.row)"
            ><i class="edit"></i>编辑</el-button
          >
          <el-button type="text" @click="deletePerfratio(scope.row)"
    ...
    <yid-dialog
      :title="addPerfratio.title"
      :visible.sync="addPerfratio.visible"
      width="600px">
      <el-form label-width="100px" ref="selectPro">
        <el-form-item label="选择项目类别:">
          <el-select
            clearable
            v-model="addPerfratio.branch"
            @change="changeBranchType(addPerfratio.branch)"
            placeholder="请选择项目"
            style="width: 140px">
            <el-option
          ...
          <el-select
            clearable
            v-model="addPerfratio.serviceType"
            placeholder="请选择分类"
      ...
      <el-form label-width="100px" ref="selectPay">
        <el-form-item
          ...
          <el-input
            ...
            max="100"
            placeholder="业绩比率"
            v-model="addPerfratio.perfratio" /><br />
        </el-form-item>
      </el-form>
      <div style="text-align: center">
        <el-button type="primary" @click="savePerfratio(addPerfratio.id)"
          >确认添加</el-button
        >
        <el-button type="primary" @click="addPerfratio.visible = false"
          >取消</el-button
        >
      </div>
    </yid-dialog>
  ...
  methods: {
    ...
    savePerfratio() {
      ...
      if (this.addPerfratio.perfratio == null) {
        ...
        if (
          this.addPerfratio.perfratio < 0 ||
          this.addPerfratio.perfratio > 100
        ) {
          yid.util.error('请输入0到100整数!')
          return
        }
      ...
      this.addPerfratio.serviceTypes.forEach(item => {
        if (
          item.branch == this.addPerfratio.branch &&
          item.scode == this.addPerfratio.serviceType
        ) {
