<template>
    <div class="dept">
        <el-form inline>
            ...
            <el-form-item label="">
                <el-button type="primary" @click="openAddServ()">选择添加项目</el-button>
            </el-form-item>
            <el-form-item label="">
                <el-button type="primary" @click="batchChangPrice()">批量修改基础定价</el-button>
            </el-form-item>
            <el-form-item label="">
                <el-button type="primary" @click="batchChangeCard()">批量修改会员卡折扣价</el-button>
            </el-form-item>
            <el-form-item label="">
                <el-button type="primary" @click="batchChangeSort()">分组和排序</el-button>
            </el-form-item>
            <br/>
            <el-form-item label="筛选">
                <el-select clearable filterable v-model="form.branch" @change="changeBranch" placeholder="请选择部门"
                           style="width: 160px;">
                    <el-option
        ...
        <yid-table ref="tableService" pagination style="width: 100%;" @selection-change="handleServicesChange">
            ...
            <yid-table-column label="编号" prop="servcode" width="90"></yid-table-column>
            <yid-table-column label="名称" prop="servname" width="140"></yid-table-column>
            <yid-table-column label="所属部门" prop="bname" width="80"></yid-table-column>
            <yid-table-column label="分类" prop="sname" width="100"></yid-table-column>
            ...
            <yid-table-column label="基础定价" :show-overflow-tooltip="false" prop="price" width="120">
                <template slot-scope="scope">
                    <el-row type="flex" justify="space-between">
                        <el-col :span="2"><i class="el-icon-edit" @click="editPrice(scope.row)"></i></el-col>
                        <el-col :span="22">
                            <div v-if="scope.row.price || scope.row.price==0">{{'￥' + scope.row.price}}</div>
                            <div v-else style="color:red">未定价</div>
                            <div>
                                {{scope.row.minPrice || scope.row.minPrice == 0 ? '最低价:' + scope.row.minPrice + '元' : ''}}
                            </div>
                        </el-col>
                    </el-row>
                </template>
            </yid-table-column>
            <yid-table-column label="会员卡折扣价设定" :show-overflow-tooltip="false" prop="discount">
                <template slot-scope="scope">
                    <el-row type="flex" justify="space-between">
                        <el-col :span="22">
                            <div :key="item.id" v-for="item in scope.row.servcards">
                                ...
                                <el-link type="primary" style="margin: 0 10px 0 10px;" @click="deldiscount(item)">[删除]
                                </el-link>
                            </div>
                        </el-col>
            ...
            <yid-table-column label="员工级别价格设置" :show-overflow-tooltip="false" prop="special">
                <template slot-scope="scope">
                    <el-row type="flex" justify="space-between">
                        <el-col :span="22">
                            <div :key="item.id" v-for="item in scope.row.servspecs">
                                ...
                                {{'￥' + item.specialPrice}}
                                <el-link type="primary" style="margin: 0 10px 0 10px;" @click="delspeical(item)">[删除]
                                </el-link>
                            </div>
                        </el-col>
            ...
            <yid-table-column label="门店项目状态" width="80">
                <template slot-scope="scope">
                    <label style="color:blue" @click="setShopServStatus(scope.row)">设置</label>
                </template>
            </yid-table-column>
        </yid-table>

        <yid-dialog :title="addServDialog.title" :visible.sync="addServDialog.visible" width="700px">
            <el-form label-width="100px" ref="copyForm2">
                <el-form-item label="筛选：">
                    <el-select clearable v-model="addServDialog.branch"
                    ...
                    <el-select clearable v-model="addServDialog.serviceType" placeholder="请选择分类"
                    ...
                    <el-button type="primary" @click="addServs()" style="margin-left: 10px;">确认添加</el-button>
                    <el-button type="primary" @click="addServDialog.visible=false">取消</el-button>
                </el-form-item>
                <el-form-item label="">
                    <table border="1" cellspacing="0" style="width: 100%">
                        <tr>
                            ...
                            <th width="25%">分类</th>
                            <th width="55%">
                                <el-checkbox v-model="addServDialog.checkbox" @change="checkAllServ"></el-checkbox>
                                (编号）项目名称
                            </th>
                        </tr>
                        <tr :key="item.servid"
        ...
        <yid-dialog :title="priceDialog.title" :visible.sync="priceDialog.visible" @close="priceDialog.visible=false"
                    width="450px">
            <el-form :model="priceDialog" label-width="150px" ref="copyForm">
                ...
                <el-form-item label="基础定价：">
                    <el-input clearable type="number" min="0" v-model="priceDialog.price" style="width: 200px"/>
                    <br/>
                </el-form-item>
                <el-form-item label="最低价：" width="400px">
                    <el-input clearable type="number" min=0                 :max="priceDialog.price"
        ...
        <yid-dialog :title="pricesDialog.title" :visible.sync="pricesDialog.visible" @close="pricesDialog.visible=false"
                    width="510px">
            <yid-table ref="table1" :data="pricesDialog.servs" @selection-change="handleServicesChange">
                ...
                <yid-table-column label="最低价" prop="minPrice" width="140">
                    <template slot-scope="scope">
                        <el-input clearable type="number" min=0                 :max="scope.row.minPrice"
        ...
        <yid-dialog :title="disCardDialog.title" :visible.sync="disCardDialog.visible" @close="levelCancel"
                    width="550px">
            <el-form :model="disCardDialog" label-width="150px" ref="copyForm">
                ...
                <el-form-item label="设置方式">
                    ...
                    <yid-ask>折扣设置为0时，不打折；输入框为空时表示不设置（没有设置项目的会员卡折扣，项目会员卡价格应用基础折扣）。</yid-ask>
                </el-form-item>
                <template v-if="disCardDialog.type==1">
                    <el-form-item :label="card.cardname" :key="card.cardid" v-for="card in disCardDialog.cards">
                        ...
                        <el-input clearable type="number" min="1" @blur="checkDiscount(card)" v-model="card.value"
                ...
                <template v-if="disCardDialog.type==2">
                    <el-form-item :label="disCardDialog.card.cardname">
                        <el-select clearable style="width: 160px;" @change="checkDiscount(card)"
                                   v-model="disCardDialog.card.dtype">
                            <el-option label="折扣" value="1"></el-option>
                            <el-option label="会员价" value="2"></el-option>
                        </el-select>
                        <el-input clearable type="number" min="1" @blur="checkDiscount(disCardDialog.card)"
                                  v-model="disCardDialog.card.value" style="width: 100px;margin-left:15px"/>
                        <label-wrap v-show="disCardDialog.card.dtype"
        ...
        <yid-dialog :title="speicalDialog.title" :visible.sync="speicalDialog.visible" @close="levelCancel"
                    width="auto">
            <el-row type="flex" justify="space-between">
                <el-form inline>
                    <el-form-item label="项目名称：">
                    ...
                    <el-form-item label="按职位级别">
                        <el-select v-model="speicalDialog.id" @change="changSpeicalData">
                            <el-option :key="item.id" v-for="item in speicalDialog.speicalData" :label="item.name"
            ...
            <yid-table ref="table" style="margin-top: 15px;" :data="speicalDialog.speicalRows">
                ...
                <yid-table-column label="最低价格" min-width="130" prop="minPrice">
                    <template slot-scope="scope">
                        <el-input clearable type="number" min="1" :max="scope.row.specialPrice"
                                  @blur="checkSpecialPrice(scope.row)" placeholder="不必填" v-model="scope.row.minPrice"/>
                    </template>
                </yid-table-column>
                <yid-table-column label="会员卡折扣" min-width="380" align="left">
                    <template slot-scope="scope">
                        ...
                        <el-row type="flex" justify="space-between" class="margin5"
                            ...
                            <el-col :span="6">
                                <el-input type="number" min="1" @blur="checkDiscount(item)" v-model="item.value"
                                          clearable/>
                            </el-col>
                            <el-col :span="2">
                                <label-wrap v-show="item.dtype" v-text="item.dtype==1?'折':'元'"></label-wrap>
                            </el-col>
                        </el-row>
                        <el-row type="flex" justify="space-between" v-show="scope.row.discountType==2" class="margin5">
                            <el-col :span="9">
                            ...
                            <el-col :span="9">
                                <el-input type="number" min="1" @blur="checkDiscount(scope.row)"
                                          v-model="scope.row.value" clearable/>
                            </el-col>
                            <el-col :span="6">
                                <label-wrap v-show="scope.row.dtype" v-text="scope.row.dtype==1?'折':'元'"></label-wrap>
                            </el-col>
                        </el-row>
                    </template>
                </yid-table-column>
                <yid-table-column label="操作" min-width="80">
                    <template slot-scope="scope">
                        <el-link type="primary" style="margin: 0 10px 0 10px;" @click="deleteSpeicalRow(scope.row)">删除
                        </el-link>
                    </template>
                </yid-table-column>
            </yid-table>
        ...
        <yid-dialog :title="mutiPriceDialog.title" :visible.sync="mutiPriceDialog.visible" @close="levelCancel"
                    width="760px">
            ...
            <el-form :model="mutiPriceDialog" label-width="150px" ref="copyForm">
                ...
                <template v-if="mutiPriceDialog.type==1">
                    <el-form-item :label="card.cardname" :key="card.cardid" v-for="card in mutiPriceDialog.cards">
                        ...
                        <el-input type="number" min="1" @blur="checkDiscount(card)" v-model="card.value"
                                  style="width: 100px;margin-left:15px"/>
                        <label-wrap v-show="card.dtype" v-text="card.dtype==1?'折':'元'"></label-wrap>
                    </el-form-item>
                </template>
                <template v-if="mutiPriceDialog.type==2">
                    <el-form-item :label="mutiPriceDialog.card.cardname">
                        <el-select clearable v-model="mutiPriceDialog.card.dtype"
                                   @change="checkDiscount(mutiPriceDialog.card)" style="width: 160px;">
                            <el-option label="折扣" value="1"></el-option>
                            <el-option label="会员价" value="2"></el-option>
                        </el-select>
                        <el-input type="number" min="1" @blur="checkDiscount(mutiPriceDialog.card)"
                                  v-model="mutiPriceDialog.card.value" style="width: 100px;margin-left:15px"/>
                        <label-wrap v-show="mutiPriceDialog.card.dtype"
                                    v-text="mutiPriceDialog.card.dtype==1?'折':'元'"></label-wrap>
                    </el-form-item>
                </template>
                <el-form-item>
        ...
        <yid-dialog :title="shopDialog.title" :visible.sync="shopDialog.visible" width="800px">
            <el-form ref="shopDialog" inline style="margin-top: 8px">
                <el-form-item label="项目：">{{shopDialog.servname}}</el-form-item>
                <br/>
                <el-form-item label="门店：">
                    <el-select clearable v-model.trim="shopDialog.shopid" placeholder="请选择门店" style="width: 160px;">
                        <el-option :key="item.id" :label="item.shopname" :value="item.shopid"
            ...
            <yid-table ref="cardShopList" :data="shopDialog.servshops" style="margin-top: 8px"
        ...
        <!--分组和排序-->
        <yid-dialog :title="addServGroup.title" :visible.sync="addServGroup.visible" width="1000px"
                    v-if="addServGroup.visible">
            <el-row :gutter="20">
                <el-col :span="10">
                    <el-form label-width="" ref="copyForm2" class="shuax">
                        <el-form-item label="筛选">
                            <el-select clearable filterable v-model="formGroup.branch" @change="changeBranch"
                                       placeholder="请选择部门" style="width: 120px;">
                                <el-option
                                        v-for="item in branch"
                                        :key="item.id"
                                        :label="item.bname"
                        ...
                        <el-form-item label="">
                            <el-select clearable filterable v-model="formGroup.scode" placeholder="请选择分类"
                                       style="width: 160px;" @change="changeType">
                                <el-option
                                        v-for="item in serviceType"
                                        :key="item.id"
                                        :label="item.sname"
                    ...
                    <span style="margin-top: 20px; display: flex; align-items: center"><label
                            style="width: 6px; height: 17px;background-color: #00AAEE ; margin-right: 10px"></label>默认</span>
                    <draggable class="moren" tag="div" group="services"
                               v-model="servicesGroup" :sort="true"
                               @start="isDragging3=true" @end="isDragging3=false" @change="moveChange">
                        <span v-for="serv in servicesGroup" :key="serv.servcode">{{serv.servname}}</span></draggable>
                </el-col>
                <el-col :span="14">
                    <div style="display: flex; flex-direction: column; height: 540px; overflow-y: auto">
                        <div v-for="(servtype,serindex) in servicesTypeGroup" :key="servtype.id">
                            <el-row>
                                <label style="width: 6px; height: 17px;background-color: #00AAEE ; margin-right: 10px; color:#00AAEE ">3</label>
                                <el-input v-model="servtype.name" type="text" style="width: 300px;"/>
                                <label style=" cursor: pointer; margin-left: 20px; color: #00AAEE"
                                       @click="deleteGroup(serindex)">删除</label>
                            </el-row>
                            <draggable :class="['group','s'+ servtype.servtid]" tag="div" group="services"
                                       :move="onMove" :sort="true" @remove="reMove" @sort="sort"
                                       v-model="servtype.serviceInfo"
                                       @start="isDragging1=true" @end="isDragging1=false" @change="moveChange">
                                <span v-for="servicein in servtype.serviceInfo"
                                      :key="servicein.servcode">{{servicein.servname}}</span>
                            </draggable>
                        </div>
                    </div>
                    <div>
                        <el-button plain style="width: 100%;" @click="addGroup">+添加分组</el-button>
                    </div>
                </el-col>
            </el-row>
            <el-row style="text-align: center; margin: 20px 0">
                <el-button type="primary" @click="addServGroup.visible = false">取消</el-button>
                <el-button type="primary" style="margin-left: 40px" @click="saveType">保存方案</el-button>
            </el-row>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            openAddServ() {
                service.servicePrice.findServiceList({sptid: this.sptdata.sptid, status: '1'}).then(res => {
            ...
            addServs() {
                ...
                this.addServDialog.services.forEach(item => {
                    ...
                    let flag = true;
                    if (this.addServDialog.branch && this.addServDialog.branch != item.branch) {
                        flag = false;
                    }
                    if (this.addServDialog.serviceType && this.addServDialog.serviceType != item.servtid) {
                        flag = false;
                    }
                    if (flag) {
                        if (item.type) {
            ...
            queryServData() {
                ...
                service.serviceInfo.findServPriceCards({sptid: this.sptdata.sptid}).then(res => {
                    if (res.data) {
            ...
            salePriceDialog() {
                if (Number(this.priceDialog.minPrice) > Number(this.priceDialog.price)) {
                    ...
                    if (this.priceDialog.price === 0) {
                        ...
                        params.price = this.priceDialog.price ? Number(this.priceDialog.price) : null;
                    }
                    if (this.priceDialog.minPrice === 0) {
                        params.minPrice = 0;
                    } else {
                        params.minPrice = this.priceDialog.minPrice ? Number(this.priceDialog.minPrice) : null;
                    }
            ...
            saveDiscount() {
                ...
                let messcode = "";
                let i = 0;
                if (this.disCardDialog.type == '1' && this.disCardDialog.cards && this.disCardDialog.cards.length > 0) {
            ...
            setShopService(row) {
              service.chain.servicePrice.getServShopStatus({servid: row.id}).then(res => {
                    if (res.resp_code == 200) {
            ...
            setShopServStatus(row) {
                service.base.servicebaseInfo.listAll({
            ...
            checkServShopStatus() {
                service.chain.servicePrice.checkServShopStatus(this.shopDialog).then(res => {
                    if (res.data) {
            ...
            saveServShopStatus() {
                service.chain.servicePrice.saveServShopStatus(this.shopDialog).then(res => {
                    if (res.resp_code == "200") {
            ...
            saveServStatus() {
                service.base.servicebaseInfo.saveServShopStatus(this.shopDialog).then(res => {
                    if (res.resp_code == "200") {
                        this.shopDialog.visible = false
                    } else {
                        yid.util.error(res.resp_msg);
            ...
            async getDataType() {
                ...
                service.servicePrice.findServPriceListPage({
                    page: 1,
                    limit: 1000, ...this.formGroup,
                    sptid: this.sptdata.sptid,
                    status: 1
                }).then(res => {
                    if (res.resp_code == "200") {
                        this.servicesGroup = res.data && res.data.length > 0 ? res.data : []
                        if (this.servicesGroup.length > 0 && this.servicesTypeGroup.length > 0) {
                            this.servicesTypeGroup.forEach(sertg => {
                                sertg.serviceInfo.forEach(serinfo => {
                                    let tindex = this.servicesGroup.findIndex(serg => serg.servcode == serinfo.servcode)
                                    if (tindex > -1) {
            ...
            getServicesGroup() {
                let tt = this.sptdata
                service.servicePrice.getServiceGroup({
                    servcode: this.sptdata.spcode,
                    servtid: this.formGroup.scode
                }).then(res => {
                    if (res.resp_code == "200") {
                        this.servicesTypeGroup = res.data && res.data.length > 0 ? res.data : []
