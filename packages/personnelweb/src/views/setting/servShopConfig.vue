  ...
  <div class="dept">
      ...
      <div v-show="listShow">
        <el-form inline>
          <el-form-item label="业务类型：">
            <el-select clearable v-model="form.branch" placeholder="请选择业务类型" label-width="150px">
        ...
        <yid-table ref="table" pagination style="width: 100%;">
          <yid-table-column label="编号" prop="code" width="100"></yid-table-column>
          <yid-table-column label="名称" prop="name" width="180"></yid-table-column>
          ...
          <yid-table-column label="操作" prop="status" width="220">
            <template slot-scope="scope">
              <el-button type="text" @click="editShopPrice(scope.row)"><i class="edit"></i>编辑</el-button>
              <el-button type="text" @click="setServPrice(scope.row)"><i class="view"></i>设置价格</el-button>
              <el-button type="text" @click="setPerfratio(scope.row)"><i class="view"></i>设置业绩</el-button>
              <el-button type="text" @click="copyPrice(scope.row)"><i class="view"></i>复制</el-button>
            </template>
          </yid-table-column>
        </yid-table>
      </div>
    ...
    <yid-dialog :title="addDialog.title" :visible.sync="addDialog.visible" @close="addDialog.visible=false"
      ...
      <el-form ref="addDialog" :model="addDialog" label-width="150px">
        ...
        <el-form-item label="业务类型：" :rules="[{required: true,message:'请选择业务类型'}]">
          <el-select clearable v-model="addDialog.branch" placeholder="请选择业务类型" label-width="150px"
        ...
        <el-form-item>
          <el-button type="primary" :disabled="submitDisabled" @click="saveShopPrice()">保存</el-button>
    ...
    <yid-dialog :title="shopDialog.title" :visible.sync="shopDialog.visible" @close="shopDialog.visible=false"
      ...
      <el-row type="flex" justify="space-between">
        <el-form inline>
          <el-form-item label="门店：">
            <el-input clearable v-model="shopDialog.shop" placeholder="门店名称/编码" style="width: 120px;"></el-input>
          </el-form-item>
          <el-form-item label="业务类型：">
            <el-select clearable v-model="shopDialog.type">
              <el-option :label="type.name" :value="type.value" v-for="type in types"></el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="价格体系：">
            <el-select clearable v-model="shopDialog.isptid" style="width: 100px;">
      ...
      <el-row>
        <el-col :span="14">
          <yid-table pagination ref="table1" style="margin-top: 10px;" @selection-change="handleSelectionChange">
            ...
            <yid-table-column label="价格体系" min-width="100" prop="pricename"></yid-table-column>
          </yid-table>
        </el-col>
    ...
    <yid-dialog :title="importDialog.title" :visible.sync="importDialog.visible" @close="importCancel" width="700px">
      <el-form :model="importDialog.model" label-width="120px" ref="copyForm" :label-position="labelPosition">
        ...
        <el-form-item label="体系类型：">
          <el-select clearable v-model="importDialog.type" style="width: 100px;" @change="handleImportType">
            <el-option label="美发" value="1"></el-option>
            <el-option label="美容" value="2"></el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    deleteShop(row) {
      yid.util.confirm('删除门店，【' + this.spname + '】下的项目将不在【' + row.shopname + '】中显示，确定要删除吗？？', '', '', () => {
        let i = -1;
    ...
    setShop() {
      if (this.flag) {
        yid.util.confirm('选择的门店包括已设置项目价格体系的门店，保存后门店将会从原价格体系移至本体系中！', '', '', () => {
    ...
    setServPrice(row) {
      ...
      this.$refs.servPrice.clear({sptid: row.id, spcode: row.code, spname: row.name, shoplist: row.shoplist});
    },
    //设置业绩比率
    setPerfratio(row) {
      this.listShow = false
      ...
      this.$refs.servPerfratio.clear({
        sptid: row.id, spcode: row.code,
        spname: row.name, shoplist: row.shoplist, status: row.status
      });
    },
    closeDetail() {
      this.listShow = true
    ...
    saveImportSer() {
      if (this.importDialog.services.length == 0) {
        ...
        if (this.importDialog.type == "1") {
          service.chain.servShop.saveSystemMf(this.importDialog.services).then(res => {
            if (res.resp_code == "200") {
              this.queryShopPrice();
              this.importDialog.visible = false;
            } else {
          ...
          service.chain.servShop.saveSystemMr(this.importDialog.services).then(res => {
            if (res.resp_code == "200") {
              this.queryShopPrice();
              this.importDialog.visible = false;
            } else {
    ...
    handleImportType(v) {
      this.excelConvertUrl = v == "1" ? $yid.config.API.BASE + "api-chain/serviceinfo/convertSystemMf" : $yid.config.API.BASE + "api-chain/serviceinfo/convertSystemMr";
    },
    //复制
    copyPrice(row) {
      this.submitDisabled = false,
