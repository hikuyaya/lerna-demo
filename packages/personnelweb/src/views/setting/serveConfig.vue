<template>
    <div class="dept">
        ...
        <yid-table ref="tableService" pagination style="width: 100%;" @selection-change="handleServicesChange">
            ...
            <yid-table-column label="名称" prop="servname" width="160">
            ...
            <yid-table-column label="指定业绩减扣" prop="kyvalue" width="140">
                <template slot-scope="scope">
                    <span v-if='scope.row.kytype=="1"'>{{scope.row.kyvalue}}%</span>
                    <span v-if='scope.row.kytype=="2"'>{{scope.row.kyvalue}}</span>
                </template>
            </yid-table-column>
            ...
            <!--<yid-table-column label="基础定价"  prop="price" width="130">
                <template slot-scope="scope">
            ...
            <yid-table-column label="会员卡折扣价设定"  prop="discount" width="160">
                <template slot-scope="scope">
                    <el-row type="flex" justify="space-between">
                        <el-col :span="22">
            ...
            <yid-table-column label="员工特殊价格设置"  prop="special" width="160">
                <template slot-scope="scope">
                    <el-row type="flex" justify="space-between">
                        <el-col :span="22">
        ...
        <yid-dialog :title="addDialog.title" :visible.sync="addDialog.visible" @close="levelCancel" width="600px">
            <el-form :model="addDialog" label-width="150px" ref="copyForm">
                ...
                <el-form-item label="项目名称：" prop="addDialog.servname" :rules="[{required: true,message:'请填写项目名称'}]">
                    <el-input v-model="addDialog.servname" placeholder="不超过16个汉字"/>
                </el-form-item>
                <el-form-item label="项目分类：" prop="addDialog.branch" :rules="[{required: true,message:'请选择项目分类'}]">
                    <el-select v-model="addDialog.branch" @change="changeServBranch" placeholder="请选择部门"
                               style="width: 160px;">
                        <el-option
                                v-for="item in branch"
                                :key="item.id"
                    ...
                    <el-select v-model="addDialog.servtid" @change="changeServType" placeholder="请选择类别"
...
<!--                        <el-radio :label="3">不计服务业绩</el-radio>-->
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="指标特性：">
                  <el-checkbox v-model="addDialog.isCountYj">不计员工服务业绩</el-checkbox>
                  <el-checkbox v-model="addDialog.isCountNum">不计员工可数</el-checkbox>
                </el-form-item>
                <el-form-item label="指定业绩减扣：">
                    <el-select v-model="addDialog.kytype" placeholder="请选择" style="width: 100px;">
                        <el-option label="默认" value="0"></el-option>
                        <el-option label="按金额" value="2"></el-option>
                        <el-option label="按比例" value="1"></el-option>
                    </el-select>
                    <el-input v-if="addDialog.kytype=='1' || addDialog.kytype=='2'" clearable
                              v-model="addDialog.kyvalue" style="width: 100px;"></el-input>
                    <lebal v-if="addDialog.kytype=='1'">%</lebal>
                    <el-popover style="padding-left: 10px;"
                        ...
                        <ul v-if="addDialog.kytype=='0'" class="tips-content">
                            <li>指定业绩设置为默认时，则按成交金额统计业绩</li>
                            <li>按比率或数值设置业绩比率后。统计业绩将优先取</li>
                            <li>指定业绩设置的数值。</li>
                            <li> 最终获得业绩=(成交额+(加收/减扣)-指定业绩减扣)*支付方式扣除的业绩比率</li>
                        </ul>
                        <ul v-if="addDialog.kytype=='1'" class="tips-content">
                            <li >不可超过100%</li>
                        </ul>
                        <ul v-if="addDialog.kytype=='2'" class="tips-content">
                            <li >如果项目销售价格低于指定扣减值，则该项目销售业绩为0</li>
                        </ul>
                    </el-popover>
                </el-form-item>
                ...
                <el-form-item label="服务工位特殊配置：">
                    <el-switch v-model="addDialog.isOpen"  active-text="开启"
                               inactive-text="关闭" active-value="1" inactive-value="2" active-color="#00AAEE"
                               inactive-color="#DCDFE6" @change="updateStatus(addDialog.isOpen)">
                    </el-switch>
                </el-form-item>
                <el-form-item label="第一工位可选人数：" style="width: 280px;" prop="firstStation">
                  <el-input type="number" placeholder="1" min="1" max="100" v-model="addDialog.firstStation"/><br/>
                </el-form-item>
                <el-form-item label="第二工位可选人数：" style="width: 280px;" prop="twoStation">
                  <el-input type="number" placeholder="1" min="1" max="100" v-model="addDialog.twoStation"/>
                </el-form-item>
                <el-form-item label="第三工位可选人数：" style="width: 280px;" prop="threeStation">
                  <el-input type="number" placeholder="1" min="1" max="100" v-model="addDialog.threeStation"/>
                </el-form-item>
              <el-form-item>
        ...
        <yid-dialog :title="priceDialog.title" :visible.sync="priceDialog.visible" @close="levelCancel" width="450px">
            <el-form :model="priceDialog" label-width="150px" ref="copyForm">
                ...
                <el-form-item label="基础定价：">
                    <el-input clearable type="number" min=0  v-model="priceDialog.price"/>
                    <br/>
                </el-form-item>
                <el-form-item label="最低价：">
                    <el-input clearable type="number" min=0  :max="priceDialog.price" :disabled="!priceDialog.price"
        ...
        <yid-dialog :title="discountDialog.title" :visible.sync="discountDialog.visible" @close="levelCancel"
                    width="550px">
            <el-form :model="discountDialog" label-width="150px" ref="copyForm">
                ...
                <template v-if="discountDialog.type==2">
                    <el-form-item :label="discountDialog.card.cardname">
                        ...
                        <el-input clearable type="number" min="1" @blur="checkDiscount(discountDialog.card)"
                                  v-model="discountDialog.card.value" style="width: 100px;margin-left:15px"/>
                        <label-wrap v-show="discountDialog.card.dtype"
                                    v-text="discountDialog.card.dtype==1?'折':'元'"></label-wrap>
                    </el-form-item>
                </template>
                <el-form-item>
        ...
        <yid-dialog :title="speicalDialog.title" :visible.sync="speicalDialog.visible" @close="levelCancel"
            ...
            <yid-table ref="table" style="margin-top: 15px;" :data="speicalDialog.speicalRows">
                <yid-table-column label="职位" min-width="150" prop="btype">
                    <template slot-scope="scope">
                        <div>
                            [{{scope.row.btype == "1" ? "职务级别" : "个人"}}]{{scope.row.btype == "1" ? scope.row.pslname : scope.row.eename}}
                        </div>
                    </template>
                </yid-table-column>
        ...
        <yid-dialog :title="mutiImportDialog.title" :visible.sync="mutiImportDialog.visible" @close="levelCancel"
                    width="700px">
    ...
    export default {
        ...
        methods: {
            ...
            add() {
                ...
                this.addDialog.issale = 0;
                this.addDialog.isSms = 0;
...
//                })
            ...
            editService(row) {
                ...
                this.addDialog.kytype = row.kytype;
                this.addDialog.kyvalue = row.kyvalue;
                this.addDialog.isOpen = row.isOpen;
                this.addDialog.firstStation = row.firstStation;
                this.addDialog.twoStation = row.twoStation;
                this.addDialog.threeStation = row.threeStation;
                if (row.countYj == 1) {
                  this.addDialog.isCountYj = true
                } else {
                  this.addDialog.isCountYj = false
                }
                if (row.countNum == 1) {
                  this.addDialog.isCountNum = true
                } else {
                  this.addDialog.isCountNum = false
                }
            },
            saveService() {
              var numReg = /^[0-9]*$/
              var numRe = new RegExp(numReg)
              if (Number.isNaN(this.addDialog.firstStation)) {
                this.addDialog.firstStation = 1;
              }
              if (!numRe.test(this.addDialog.firstStation)) {
                this.addDialog.firstStation = 1;
              }
              if (this.addDialog.firstStation < 1 || this.addDialog.firstStation > 100) {
                this.addDialog.firstStation = 1;
              }
              if (Number.isNaN(this.addDialog.twoStation)) {
                this.addDialog.twoStation = 1;
              }
              if (!numRe.test(this.addDialog.twoStation)) {
                this.addDialog.twoStation = 1;
              }
              if (this.addDialog.twoStation < 1 || this.addDialog.twoStation > 100) {
                this.addDialog.twoStation = 1;
              }
              if (Number.isNaN(this.addDialog.threeStation)) {
                this.addDialog.threeStation = 1
              }
              if (!numRe.test(this.addDialog.threeStation)) {
                this.addDialog.threeStation = 1
              }
              if (this.addDialog.threeStation < 1 || this.addDialog.threeStation > 100) {
                this.addDialog.threeStation = 1;
              }
              if (!this.addDialog.servname) {
                ...
                if (this.addDialog.kytype == "1" || this.addDialog.kytype == "2") {
                    if (this.addDialog.kyvalue == null || this.addDialog.kyvalue == "") {
                        yid.util.error("请输入指定业绩减扣");
                        return;
                    }
                    if (this.addDialog.kytype == "1" && this.addDialog.kyvalue > 100) {
                        yid.util.error("按比例不能大于100");
                        return;
                    }
                }
                const params = {};
                ...
                if (this.addDialog.isCountYj) {
                  params.countYj = 1;
                } else {
                  params.countYj = 0;
                }
                if (this.addDialog.isCountNum) {
                  params.countNum = 1;
                } else {
                  params.countNum = 0;
                }
                ...
                params.kytype = this.addDialog.kytype
                params.isOpen = this.addDialog.isOpen;
                params.firstStation = this.addDialog.firstStation;
                params.twoStation = this.addDialog.twoStation
                params.threeStation = this.addDialog.threeStation
                if (this.addDialog.kytype == "0") {
                    params.kyvalue = null
                } else {
                    params.kyvalue = this.addDialog.kyvalue
                }
                this.save(params);
            },
            editPrice(row) {
...
//                }
                if (Number(this.priceDialog.minPrice) > Number(this.priceDialog.price)) {
                    ...
                    service.serviceInfo.saveServPrice(params).then(res => {
                        if (res.resp_code == "200") {
                            yid.util.success(res.resp_msg);
                            this.queryServData();
                            this.levelCancel();
            ...
            async getData(reqParams) {
                const fetch = service.serviceInfo.findServiceListPage
