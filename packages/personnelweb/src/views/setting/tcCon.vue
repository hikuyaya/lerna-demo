<template>
    <div class="tcTon">
        ...
        <el-row>
            <label>适用人员：</label>
            <el-cascader :options="stuffTree.tree" :props="props" :show-all-levels="false" clearable ref="rps"
        ...
        <el-row style="display: flex; margin: 15px 0">
            ...
            <el-checkbox v-model="showGroup.show_pay" v-show="rItem.prtid=='1'||rItem.prtid=='2'">支付方式</el-checkbox>
        ...
        <!--提成项选择-->
        <yid-dialog :title="dialogTcx.title" :visible.sync="dialogTcx.visible" @close="cancelDialogTcx" width="350px">
            <panel-tc-item
                    ref="dialogtcx"
        ...
        methods: {
            ...
            showLevelMemo(level) {
                ...
                for (let i = 0; i < level.subdescList.length; i++) {
                    memo = memo + level.subdescList[i].prnname + "=>" + level.subdescList[i].pvalue + "\n" + (i == level.subdescList.length - 1 ? " " : cond) + "\n"
                }
                return memo
            },
            ...
            setCascaderValue(psList) {
                if (psList instanceof Array) {
                    psList.map(ps => {
                        if (ps.treepath)
                            this.cascaderValue.push(ps.treepath.split(","))
                    })
                }
            }
        }
