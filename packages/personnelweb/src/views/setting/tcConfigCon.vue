    ...
    <el-collapse-transition>
      <div v-show="listShow">
        ...
        <el-row style="margin: 15px 0">
          ...
          <el-col :span="6"
            >状态：{{ rManage.status == '1' ? '启用' : '停用' }}</el-col
          >
        </el-row>
        <yid-table
          pagination
          ...
          <yid-table-column
            label="提成分类"
            width="150"
            prop="prtname"></yid-table-column>
          <yid-table-column label="适用人员">
            <template slot-scope="scope">
              <el-row style="margin: 15px 0">{{
                scope.row.psList.joinField('psname', '、')
              }}</el-row>
            </template>
          </yid-table-column>
          <yid-table-column label="状态" width="160">
            <template slot-scope="scope">
              <el-switch
                ...
                inactive-value="2"
                @change="
                  handleSwitch($event, scope.row.id, scope.row.revision)
                ">
              </el-switch>
            </template>
          </yid-table-column>
          <yid-table-column label="操作" width="150" prop="content">
            <template slot-scope="scope">
              <el-link type="primary" @click="showTcCon(scope.row.id)"
                >编辑</el-link
              >
        ...
        <yid-dialog
          ...
          <div class="rtrtr">
            <div v-for="(type, i) in dt.rType">
              <el-radio v-model="rTypeRadio" :label="i" border>{{
                type.prtname
              }}</el-radio>
            </div>
          </div>
