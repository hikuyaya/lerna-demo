            ...
            <div v-show="visibleList.manage">
                ...
                <el-form ref="searchForm" inline :model="searchForm" style="margin-top: 8px">
                    ...
                    <el-form-item label="员工：" prop="ecodename">
                        <el-input v-model="searchForm.ecodename" clearable  style="width: 200px;" placeholder="员工编码/员工名称"></el-input>
                    </el-form-item>
                ...
                <yid-table pagination ref="tableRm" :data="rManageList" style="margin-top: 8px">
                    ...
                    <yid-table-column label="适用员工" prop="employeeList">
                        <template slot-scope="scope">
                            <div>
                                <el-row gutter="8">共{{scope.row.employeeList ? scope.row.employeeList.length : 0}}人</el-row>
                                <el-row gutter="8"><el-link type="primary" @click="showSelectEmployeeDialog(scope.row)">查看修改</el-link></el-row>
                            </div>
                        </template>
                    </yid-table-column>
                ...
                <yid-dialog :title="dialogType.title" :visible.sync="dialogType.visible" @close="dialogCancel"
                            width="400px">
                    <el-form label-width="80px" ref="formRm" :model="dialogType.model" :rules="rules">
                        <el-form-item label="方案名称" prop="prmname">
                            <el-input clearable v-model="dialogType.model.prmname" placeholder="请输入方案名称"
                                      :rules="rules.prmname"
                            ></el-input>
                        </el-form-item>
                        <el-form-item label="复制方案" prop="copyId" v-if="!dialogType.model.id">
                            <el-select v-model="dialogType.model.copyId" placeholder="选择方案">
                                <el-option
                        ...
                        <el-form-item label="适用人员">
                            <el-button type="primary" @click="openEePanel()">选择员工</el-button>
                            {{dialogType.model.psList.length && dialogType.model.psList.length > 0 ? '共选中'+ dialogType.model.psList.length +'人':''}}
                        </el-form-item>
                ...
                <yid-dialog title="选择员工" :visible.sync="employeeDialog.visible" @close="closeEeDialog"
                            width="1100px">
                    <employee :confirmFunc="checkEmployee" :defaultRightData="employeeDialog.defaultData"></employee>
                </yid-dialog>
            </div>
        </el-collapse-transition>
        ...
        data() {
            return {
                ...
                dialogType: {
                    ...
                    title: '个人提成方案名称',
                    items: [],
                    model: {prmname: null, memo: null, status: "1", copyId: "", psList: [],employeeList:[],id:""}
                },
                paginationParam: {
                    page: 1,
                    limit: 10,
                ...
                rManageList: [],
                rules: {prmname: {required: true, message: '请输入方案名称', trigger: 'blur'}},
        ...
        methods: {
            ...
            showDialogType(model) {
                ...
                if (model) {
                    ...
                    this.dialogType.model = {prmname: null, memo: null, status: "1",psList:[]}
                    service.chain.royaltyManage.getShopManages().then(res => {
                        return res.data;
                    }).then(item => {
                        item.map(v =>{
                            if (v && v.id) {
