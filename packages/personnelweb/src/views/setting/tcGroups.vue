<template>
    <div>
        ...
        <div class="groupDiv" v-if="type=='2'">
                <div class="groups">
                    <scroll-view scroll-x="true" class="gplist">
                        <div class="group" v-for="(g,i) in groupList"
                             @click="chooseGroup(i)">
                            <el-row style="height: 80px;">{{ g.psList.joinField("psname","、",10) }}</el-row>
                            <el-row>
                                <el-button-group>
                                    <el-button type="primary" icon="el-icon-edit" @click.stop="toEdeeGroup(i)"></el-button>
                                </el-button-group>
                            </el-row>
                        </div>
                    </scroll-view>
        ...
        <yid-dialog title="选择员工" :visible.sync="employeeDialog.visible" @close="employeeDialog.visible=false"
                    width="1100px">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            findFullVO(index) {
                $yid.service.chain.royaltyGroup.getById(this.groupList[index].id).then(res => {
            ...
            doCopyGroup(i) {
                ...
                $yid.util.confirm('是否复制提成方案子集[' + group.name + ']', '', '', () => {
                    $yid.service.chain.royaltyGroup.copy({id: group.id, rowindex: this.groupList.length}).then(res => {
                        this.initGroupList(this.rManage.id);
                    })
                })
            },
