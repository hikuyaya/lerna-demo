          ...
          <el-form inline>
            ...
            <el-form-item label="门店：">
              <el-select
                ...
                @blur="clearFormShop"
                clearable
                v-model.trim="queryModel.shopid"
                filterable
    ...
    <el-collapse-transition>
      <div v-if="!visible">
        ...
        <el-row v-for="item in model.itemList">
          <el-row style="color: #303133; margin: 10px 0; font-weight: bold">{{
            item.prtname
          }}</el-row>
          <el-table :data="item.descList" border style="width: 100%">
            <el-table-column
              v-if="item.descList[0].isPosition == '1'"
              label="工位"
              width="80">
              <template slot-scope="scope">
                {{
            ...
            <el-table-column label="提成项" width="180">
              <template slot-scope="scope">
                {{ scope.row.descitemList.joinField('itemname', '，') }}
              </template>
            </el-table-column>
            <el-table-column label="提成方式" width="80">
              <template slot-scope="scope">
                {{ scope.row.normType == '1' ? '按数量' : '按业绩' }}
              </template>
            </el-table-column>
            <el-table-column
              v-if="item.descList[0].isCost == '1'"
              prop="address"
              label="成本"
              width="80">
              <template slot-scope="scope">
                {{
                  scope.row.costType == '1'
                    ? scope.row.cost + '%'
                    : scope.row.cost + '元'
                }}
              </template>
            </el-table-column>
            <el-table-column v-if="item.isPay" label="支付方式" width="180">
              <template slot-scope="scope">
            ...
            <el-table-column
              label="指标"
              v-if="item.descList[0].isSubdesc == '1'">
              <el-table-column label="级别" width="60">
                <template slot-scope="scope">
                  <el-row
                    v-for="level in scope.row.levelList"
                    style="height: 40px">
                    <el-col>
                      {{ level.level }}
                    </el-col>
              ...
              <el-table-column label="指定提成" width="100">
                <template slot-scope="scope">
                  <el-row
                    v-for="level in scope.row.levelList"
                    style="height: 40px">
              ...
              <el-table-column label="不指定提成" width="100">
                <template slot-scope="scope">
                  <el-row
                    v-for="level in scope.row.levelList"
                    style="height: 40px">
    ...
    showLevelMemo(level) {
      ...
      for (let i = 0; i < level.subdescList.length; i++) {
        memo =
          ...
          ' ' +
          level.subdescList[i].pcondition +
          ' ' +
