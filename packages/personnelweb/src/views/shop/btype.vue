        ...
        <el-collapse-transition>
            <div>
                <el-form ref="searchForm" inline :model="searchForm">
                    <el-button @click="editAction(false,'')" type="primary" style="margin-bottom: 10px;">新增</el-button><br/>
                    <el-form-item label="业务类型：" prop="name">
                        <el-input  v-model="searchForm.name" placeholder="业务名称" style="width: 150px;"></el-input>
                    </el-form-item>
                    <el-form-item label="">
                ...
                <yid-table  ref="table" pagination style="width: 100%;margin-top: 20px;">
                    ...
                    <yid-table-column
                        ...
                        <template slot-scope="scope">
                            <el-link type="primary" @click="editAction(true,scope.row)">编辑</el-link>
                        </template>
                    </yid-table-column>
                </yid-table>

                <yid-dialog :title="btypeDialog.title" :visible.sync="btypeDialog.visible" width="550px">
                    <el-form ref="btypeForm" :model="btypeForm"  label-width="140px">
                        <el-form-item label="类型名称：" prop="name" :rules="[{ required: true, message: '类型名称为空'}]">
                            <el-input v-model="btypeForm.name" style="width:200px;"></el-input>
                        </el-form-item>
                        <el-form-item label="对应部门：">
                            <el-checkbox :indeterminate="isIndeterminate" v-model="checkAll" @change="handleCheckAllChange">全选</el-checkbox>
                            <div style="margin: 15px 0;"></div>
                            <el-checkbox-group v-model="checkedBranchs" @change="handleCheckedCitiesChange">
                                <el-checkbox v-for="item in branchList" :label="item.id" :key="item.id">{{item.bname}}
                                </el-checkbox>
                            </el-checkbox-group>
                        </el-form-item>
                        <el-form-item>
            ...
            saveBtype(){
                this.$refs['btypeForm'].validate((valid) => {
                    if (valid) {
                        ...
                        this.btypeForm.bbids = this.btypeForm.bbids.substr(0,this.btypeForm.bbids.length-1);
                        this.btypeForm.bbcodes = this.btypeForm.bbcodes.substr(0,this.btypeForm.bbcodes.length-1);
                        this.btypeForm.bbnames = this.btypeForm.bbnames.substr(0,this.btypeForm.bbnames.length-1);
