<template>
  <div class="region">
    <el-collapse-transition>
      <div v-show="listShow">
        <div class="top">
          ...
          <el-button @click="showRegionShop" style="color: #00b6ff"
            >查看机构下门店</el-button
          >
        </div>
        <br />
          ...
          <div v-if="!showRegion" class="table">
            ...
            <el-form ref="regionForm" :model="regionForm" label-width="140px">
              <el-form-item
                label="机构名称："
                prop="name"
                :rules="[{ required: true, message: '区域名称为空' }]">
                <el-input
                  style="width: 150px"
                  v-model="regionForm.name"></el-input>
              </el-form-item>
              <el-form-item label="上级机构：" prop="parentObj">
                <el-select
                  @clear="clearSelectRegion"
                  clearable
                  :disabled="isEdit"
                  v-model="regionForm.parentObj"
                  placeholder="请选择上级区域">
                  <el-option
                    :value="regionForm.parentObjValue"
                    style="height: auto">
                    <el-tree
                      size="small"
                      show-checkbox
          ...
          <div v-if="showRegion" class="table">
            <yid-table :data="shopData" style="margin-top: 15px">
              ...
              <yid-table-column label="操作" min-width="250" prop="content">
                <template slot-scope="scope">
                  <el-link type="primary" @click="showDetailPage(scope.row)"
                    >详情</el-link
                  >
                </template>
              </yid-table-column>
    ...
    <yid-dialog
      title="转移门店"
      :visible.sync="moveShopDialog.visible"
      width="750px">
      &nbsp;&nbsp;&nbsp;
    ...
    <yid-dialog
      ...
      <el-form ref="shopForm" :model="shopForm" label-width="140px">
        ...
        <el-form-item
          ...
          <el-select
            @clear="clearSelectRegion1"
            clearable
            v-model="shopForm.parentObj"
            placeholder="请选择门店区域">
...
export default {
  ...
  methods: {
    ...
    getTreeAllNoNum() {
      service.chain.region.treeAllNoNum({}).then(res => {
        if (res.resp_code == 200) {
          this.regionDataNoNum = res.data
        }
      })
    },
    getTreeTwo() {
    ...
    editRegion() {
      ...
      if (this.nodeData.parentId != '-1') {
        service.chain.region.getRegionByid(this.nodeData.parentId).then(res => {
          if (res.resp_code == 200) {
            console.log(res)
            let arrLabel = []
            arrLabel.push('[' + res.data.code + ']' + res.data.name)
    ...
    saveRegion() {
      this.$refs['regionForm'].validate(valid => {
        if (valid) {
          if (
            this.regionForm.parentObjValue != null &&
            this.regionForm.parentObjValue.length > 0
          ) {
    ...
    stopRegion() {
      ...
      if (Number(num) === 0) {
        ...
        yid.util.confirm(
          '该门店区域下有门店信息，请转移门店后停用！',
          '提示',
          { confirmButtonText: '转移门店' },
          () => {
            //查询所有门店信息
            const shopParams = {}
            shopParams.regionId = this.nodeData.id
    ...
    batchPb() {
      this.$refs['shopForm'].validate(valid => {
        if (valid) {
          ...
          if (
            this.shopForm.parentObjValue != null &&
            this.shopForm.parentObjValue.length > 0
          ) {
