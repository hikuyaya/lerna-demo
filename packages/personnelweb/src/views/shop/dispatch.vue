    ...
    <div class="dispatchBill">
            ...
            <div v-show="showList">
                ...
                <el-form ref="searchForm" inline :model="searchForm">
                    <el-form-item label="单据编码：" prop="billcode">
                        <el-input clearable v-model="searchForm.billcode" placeholder="单据编码"
                    ...
                    <el-form-item label="门店：" prop="shopid">
                        <el-select clearable v-model.trim="searchForm.shopid" filterable :filter-method="filterShop"
                    ...
                    <el-form-item label="开始卡号：" prop="startno">
                        <el-input clearable v-model="searchForm.startno" placeholder="开始卡号"
                                  style="width: 180px;"></el-input>
                    </el-form-item>
                    <el-form-item label="结束卡号：" prop="endno">
                        <el-input clearable v-model="searchForm.endno" placeholder="结束卡号"
                                  style="width: 180px;"></el-input>
                    </el-form-item>
                    <el-form-item>
                ...
                <yid-table ref="table" pagination style="width: 100%;margin-top: 10px;">
                    ...
                    <yid-table-column label="操作" min-width="150" fixed="right">
                        <template slot-scope="scope">
                            <el-link v-if="scope.row.status == '1'" type="primary" @click="editBill(scope.row)">编辑
                            </el-link>
                            <el-link v-if="scope.row.status == '2'" type="primary" @click="editBill(scope.row)">查看
                            </el-link>
                            <el-link v-if="scope.row.status == '1'" type="primary" @click="deleteBillrow(scope.row)"
                                     style="margin-left: 15px">删除
                            </el-link>
                        </template>
                    </yid-table-column>
                </yid-table>
        ...
        <el-collapse-transition>
            <div v-show="!showList">
                ...
                <el-button v-show="isSp" @click="auditBill" type="primary">审核</el-button>
                <el-button v-show="isSp" @click="deleteBill" type="primary">删除</el-button>
                <div style="margin-top: -5px;">
                    <el-divider/>
                </div>
                <el-form ref="billForm" :model="billForm" style="margin-top: 16px" label-width="100px"
                ...
                <el-row style="text-align: left; margin-top: 10px">
                    <el-link type="primary" v-if="showButton" @click="addRow">添加门店</el-link>
                </el-row>
                <yid-table ref="itemTable" style="margin-top: 15px;" :data="billForm.details">
                    <yid-table-column label="店号" width="150" prop="shopcode">
                        <template slot-scope="scope">
                            <el-input :disabled="!showButton" v-model="scope.row.shopcode"
                                      @blur="setShopName(scope.row)"></el-input>
                        </template>
                    </yid-table-column>
                    <yid-table-column label="店名" width="220" prop="shopname"></yid-table-column>
                    <yid-table-column label="开始卡号" width="240" prop="startno">
                        <template slot-scope="scope">
                            <el-input type="number" :disabled="!showButton" :precision="2" :min="1" :max="9999999999"
                    ...
                    <yid-table-column label="分拨卡张数" width="200" prop="count">
                        <template slot-scope="scope">
                            <el-input type="number" :disabled="!showButton" :precision="2" :min="1"
                                      @change="countEndNo(scope.row)"
                                      v-model="scope.row.count"></el-input>
                        </template>
                    </yid-table-column>
                    <yid-table-column label="结束卡号" width="160" prop="endno"></yid-table-column>
                    <yid-table-column label="操作" width="100" prop="operating">
                        <template slot-scope="scope">
                            <el-button :disabled="!showButton" v-model="scope.$index" type="text"
    ...
    export default {
        ...
        computed: {
            searchFormReq: function () {
                ...
                if (this.searchForm.createTimeRange != null && this.searchForm.createTimeRange.length > 0) {
                ...
                if (this.searchForm.auditTimeRange != null && this.searchForm.auditTimeRange.length > 0) {
        ...
        methods: {
            ...
            addRow() {
                this.billForm.details.push({shopid: '', shopcode: '', shopname: '', startno: '', count: '', endno: ''})
            },
            setShopName(row) {
                let flag = true;
                ...
                if (row.startno) {
                    if (row.startno.length != 10) {
                        ...
                        if (row.startno.length > String(endno).length) {
                            for (let j = 0; j < row.startno.length - String(endno).length; j++) {
                                code += "0"
                            }
                            endno = code + endno
                        }
            ...
            deleteBillrow(row) {
                if (!yid.util.isEmpty(row.billcode)) {
                    yid.util.confirm("您确定要删除[" + row.billcode + "]的单据?", "", "", () => {
            ...
            auditBill() {
                ...
                if (!yid.util.isEmpty(that.billForm.billcode)) {
                    yid.util.confirm("您确定要审核[" + that.billForm.billcode + "]的单据?", "", "", () => {
