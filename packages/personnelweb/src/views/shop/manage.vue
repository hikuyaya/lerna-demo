    ...
    <div>
        <el-collapse-transition>
            <div v-show="listShow">
                ...
                <el-form inline style="margin-top: 15px;" ref="searchForm" :model="searchForm">
                    ...
                    <el-form-item label="组织机构：">
                        <el-select  @clear="clearSelectRegion" clearable ref="selectRegionRef" v-model="selectRegionForm.regionObj.name" placeholder="请选择组织机构" style="width: 180px;">
                            <el-option  hidden :value="selectRegionForm.regionObj.id" style="height: auto"></el-option>
                            <el-tree
                ...
                <yid-table pagination ref="table"  :data="tableData">
                    ...
                    <yid-table-column label="所属组织机构" min-width="140" prop="regionName"></yid-table-column>
                    ...
                    <yid-table-column label="项目价格体系" min-width="140">
                        <template slot-scope="scope">
                            <span v-for="servicePt in scope.row.servicePtshops">{{servicePt.stempName}}<br></span>
                        </template>
                    </yid-table-column>
                    <yid-table-column label="产品价格体系" min-width="140">
                        <template slot-scope="scope">
                            <span v-for="productPt in scope.row.productPtshops">{{productPt.ptempName}}<br></span>
                        </template>
                    </yid-table-column>
                    <yid-table-column label="门店提成方案" min-width="140">
                        <template slot-scope="scope">
                            <span v-for="royaltyPt in scope.row.royaltyShops">{{royaltyPt.rtempName}}<br></span>
                        </template>
                    </yid-table-column>
                    <yid-table-column label="会员卡体系" min-width="140">
                        <template slot-scope="scope">
                            <span v-for="cardPt in scope.row.cardTshops">{{cardPt.ctempName}}<br></span>
                        </template>
                    </yid-table-column>
        ...
        <el-collapse-transition>
            <div v-show="addShow">
                <el-form ref="shopForm" :model="shopForm" style="margin-top: 16px"  label-width="150px" label-position="right">
                    <el-tag>基础信息</el-tag><br>
                    <el-form-item label="门店编码：" prop="shopcode" :rules="[{ required: true, message: '门店编码不能为空'}]">
                        <el-input v-model="shopForm.shopcode" style="width: 350px;" @change="checkShopcode"></el-input>
                    </el-form-item>
                    <el-form-item label="门店名称：" prop="shopname" :rules="[{ required: true, message: '门店名称不能为空'}]">
                        <el-input v-model="shopForm.shopname" style="width: 350px;" placeholder="请输入"/>
                    </el-form-item>
                    <el-form-item label="预约名称：" prop="yyshopname">
                        <el-input v-model="shopForm.yyshopname" style="width: 350px;" placeholder="请输入"/>
                    </el-form-item>
                    <el-form-item label="门店品牌：" prop="brandId" :rules="[{ required: true, message: '门店品牌不能为空'}]">
                        <el-select clearable v-model.trim="shopForm.brandId"   placeholder="请选择" style="width: 200px;">
                            <el-option :key="item.id" :label="item.name" :value="item.id" v-for="item in brandData">
                            </el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="业务类型：" prop="type" :rules="[{ required: true, message: '业务类型不能为空'}]">
                        <el-select clearable v-model.trim="shopForm.type"   placeholder="请选择" style="width: 160px;">
                            <el-option :key="item.id" :label="item.name" :value="item.id" v-for="item in btypeList">
                            </el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="组织机构：" prop="regionObj" :rules="[{ required: true, message: '请选择组织机构'}]">
                        <el-select  ref="selectRegionRef1" v-model="shopForm.regionObj" placeholder="请选择" multiple collapse-tags  @change="selectRegion1" style="width: 180px;">
                    ...
                    <el-form-item label="门店logo：" prop="logo" :rules="[{ required: true, message: '门店logo不能为空'}]">
                        <el-upload
                            ...
                            <img v-if="shopForm.logo" :src="shopForm.logo" class="avatar el-upload" style="width: 200px;height: 200px">
                    ...
                    <el-form-item label="门店电话：" prop="moblie" :rules="[{ required: true, message: '门店电话不能为空'}]">
                        <el-input v-model="shopForm.moblie" style="width: 350px;" placeholder="请输入"/>
                    </el-form-item>
                    <el-form-item label="联系人：" prop="contacts" :rules="[{ required: true, message: '联系人不能为空'}]">
                        <el-input v-model="shopForm.contacts" style="width: 350px;" placeholder="请输入"/>
                    </el-form-item>
                    <el-form-item label="营业时间：" prop="openTime">
                       <el-input v-model="shopForm.openTime" style="width: 350px;" placeholder="请输入"/>
                        <!--<el-time-select
                                :clearable = true
                                style="width: 350px"
                    ...
                    <el-form-item label="门店地址：" prop="pca" :rules="[{ required: true, message: '门店地址不能为空'}]">
                        <el-cascader v-model="shopForm.pca" :options="proCityArea" :props="{value:'code',label:'name'}" style="width: 300px"></el-cascader>&nbsp;&nbsp;&nbsp;&nbsp;
                        <el-input v-model="shopForm.address" style="width: 300px;" placeholder="请输入详情地址"/>
                    </el-form-item>
                    <el-form-item label="经纬度：" class="formItemClass">
                        <el-col>
                            <el-row><el-input v-model="shopForm.lat" placeholder="经度" style="width: 100px"/> , <el-input v-model="shopForm.lng" placeholder="纬度" style="width: 100px"/>
                                <el-link type="primary" @click="shopmap.visible=true" style="margin-left: 8px">经纬度查询</el-link></el-row>
                        </el-col>
                    </el-form-item>
                    <el-form-item label="付款模板：" prop="paytId" :rules="[{ required: true, message: '付款模板不能为空'}]">
                        <el-select clearable v-model.trim="shopForm.paytId"   placeholder="请选择" style="width: 160px;">
                            <el-option :key="item.id" :label="item.name" :value="item.id" v-for="item in payTemplateList">
                            </el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="门店状态" prop="status" :rules="[{ required: true, message: '门店状态不能为空'}]">
                        <el-switch  v-model="shopForm.status" active-value="1" inactive-value="2"  active-text="开" inactive-text="关">
                        </el-switch>
                    </el-form-item>
                    <el-form-item label="是否显示门店" prop="isShow" :rules="[{ required: true, message: '是否显示门店不能为空'}]">
                        <el-switch  v-model="shopForm.isShow" active-value="1" inactive-value="0"  active-text="显示" inactive-text="隐藏">
                        </el-switch>
                        <yid-ask>控制门店信息是否在小程序中显示</yid-ask>
                    </el-form-item>
                    <el-tag>宣传图</el-tag><br>
                    <el-form-item label="门店宣传图">
                        ...
                        <div style="margin-bottom: 10px">
                            <lebal>建议上传图片分辨率为233px *455px；支持图片格式：jpg、png最多仅支持上传6张</lebal>
                        </div>
                    </el-form-item>
                </el-form>
        ...
        <el-collapse-transition>
            <div v-if="addConfigShow">
                <el-form ref="shopConfigForm" :model="shopConfigForm" style="margin-top: 16px"  label-width="150px" label-position="right">
                    <el-tag>门店配置</el-tag><br>
                    <el-form-item label="项目价格体系：" prop="brandId">
                        <el-select clearable v-model.trim="shopConfigForm.sptid"  placeholder="请选择" style="width: 200px;">
                            <el-option :key="item.id" :label="item.name" :value="item.id" v-for="item in servicePricetemps">
                            </el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="产品价格体系：" prop="brandId">
                        <el-select clearable v-model.trim="shopConfigForm.pptid"  placeholder="请选择" style="width: 200px;">
                            <el-option :key="item.id" :label="item.name" :value="item.id" v-for="item in productPricetemps">
                            </el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="会员卡体系：" prop="brandId">
                        <el-select clearable v-model.trim="shopConfigForm.cptid"  placeholder="请选择" style="width: 200px;">
                            <el-option :key="item.id" :label="item.name" :value="item.id" v-for="item in cardTemps">
                            </el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="门店提成方案：" prop="brandId">
                        <el-select clearable v-model.trim="shopConfigForm.rptid"  placeholder="请选择" style="width: 200px;">
                            <el-option :key="item.id" :label="item.prmname" :value="item.id" v-for="item in royaltyManages">
                            </el-option>
                        </el-select>
                    </el-form-item>
                </el-form>
        ...
        methods:{
            ...
            handleNodeClick(data,checked,node){
                if(checked == true) {
                    ...
                    this.getSelectAllRegion(data, this.selectRegionForm.regionObj.childrenIds)
            ...
            getProductPricetemps(){
                service.chain.productPrice.findProductPricetemps({status:"1"}).then(res=>{
                    if(res.resp_code == 200) {
            ...
            getRoyaltyManages(){
                service.chain.royaltyManage.findShopManages({status:'1',type:'1'}).then(res=>{
                    if(res.resp_code == 200) {
            ...
            search(){
                this.$refs['searchForm'].validate((valid) => {
                    if(valid){
                        ...
                        if(this.selectRegionForm.regionObj && this.selectRegionForm.regionObj.id){
                            //大区及以内区域都保存进去
                            this.selectRegionForm.regionObj.childrenIds.push(this.selectRegionForm.regionObj.id)
                            this.searchForm.regionIds = this.selectRegionForm.regionObj.childrenIds.toString();
                        }else{
            ...
            saveShop(){
                this.$refs['shopForm'].validate(valid => {
                    if (valid) {
                        ...
                        if(!this.shopForm.shopImages && this.shopForm.shopImages.length==0){
                        ...
                        if((!this.shopForm.lat && this.shopForm.lat.length == 0) || (!this.shopForm.lng && this.shopForm.lng.length == 0)) {
                        ...
                        //区域
                        if(this.shopForm.regionObjValue != null && this.shopForm.regionObjValue.length > 0){
