<template>
    ...
    <div class="dept">
        ...
        <hr>
        <el-row>
            <el-col :span="3"><el-button type="primary" @click="editShopInfo()" v-if="!this.showButton">修改门店资料</el-button></el-col>
            <el-col :span="3"><el-button type="primary" @click="openShopImage()" v-if="!this.showButton">修改门店宣传图</el-button></el-col>
            <el-col :span="3"><el-button type="primary" @click="openQcode()" v-if="!this.showButton">门店收银开单码</el-button></el-col>
            <!--<el-col :span="3"><el-button type="primary" @click="openQcode()" v-if="!this.showButton">门店推广二维码</el-button></el-col>-->
        </el-row>
        <el-divider></el-divider>
            <el-tag>基础信息</el-tag>
                <el-form :inline="true"  label-width="100px" label-position="left" style="margin-top: 8px">
                    <el-row :gutter="20">
                        <el-col :span="8" v-if="shop.logo">
                            <div class="block" style="width: 360px;">
                                <img  :src="shop.logo" class="avatar el-upload" style="width: 200px;height: 200px">
                            </div>
                        </el-col>
                        <el-col :span="8">
                            <el-row><el-form-item label="门店编号：" class="formItemClass">{{shop.shopcode}}</el-form-item></el-row>
                            <el-row><el-form-item label="业务类型：" class="formItemClass">
                            ...
                            <el-row><el-form-item label="门店电话：" class="formItemClass">
                                {{shop.moblie}}
                            </el-form-item></el-row>
                            <el-row> <el-form-item label="门店状态：" class="formItemClass">
                                {{shop.status == "1" ? "开店":"关店"}}
                            </el-form-item></el-row>
                            <el-row> <el-form-item label="是否显示：" class="formItemClass">
                                {{shop.isShow == "1" ? "展示":"隐藏"}}
                            </el-form-item></el-row>
                            <el-row> <el-form-item label="付款模板：" class="formItemClass">
                                {{shop.paytName}}
                            </el-form-item></el-row>
                        </el-col>
                        <el-col :span="8">
                            <el-row><el-form-item label="门店名称：" class="formItemClass">{{shop.shopname}}</el-form-item> </el-row>
                            <el-row><el-form-item label="预约名称：" class="formItemClass">{{shop.yyshopname}}</el-form-item> </el-row>
                            <el-row><el-form-item label="组织机构：" class="formItemClass">{{shop.regionName}}</el-form-item> </el-row>
                            <el-row><el-form-item label="营业时间：" class="formItemClass">{{shop.openTime}}</el-form-item> </el-row>
                            <el-row><el-form-item label="省市区：" class="formItemClass"> {{shop.provincename +  shop.cityname +  shop.areaname}}</el-form-item> </el-row>
                            <el-row><el-form-item label="门店地址：" class="formItemClass">{{shop.address}}</el-form-item> </el-row>
                            <el-row><el-form-item label="经纬度：" class="formItemClass">{{shop.lat +' , ' + shop.lng}}</el-form-item> </el-row>
                        </el-col>
                    </el-row>
                </el-form>

            <el-tag>门店宣传图</el-tag>
            <div v-if="!shopImages || shopImages.length == 0" style="margin-top: 8px">还没有设置宣传图哦</div>
            <div v-if="shopImages && shopImages.length > 0" class="block" style="width: 360px; margin-top: 16px;">
        ...
        <yid-dialog :title="editshop.title" :visible.sync="editshop.visible" @close="editshop.visible=false" width="800px">
            <el-form :model="shopForm"  :rules="shopRules" label-width="130px" label-position="right" ref="editForm">
                <el-form-item label="门店名称：" prop="shopname"><el-input v-model="shopForm.shopname"/></el-form-item>
                <el-form-item label="预约名称：" prop="yyshopname"><el-input v-model="shopForm.yyshopname" /></el-form-item>
                <el-form-item label="联系人：" prop="contacts"><el-input v-model="shopForm.contacts" /></el-form-item>
                <el-form-item label="联系电话：" prop="moblie"><el-input v-model="shopForm.moblie" /></el-form-item>
                <el-form-item label="省市区：" prop="pca">
                    <el-cascader  @change="handleAddressChange" v-model="shopForm.pca" :options="proCityArea" :props="{value:'code',label:'name'}" style="width: 320px"></el-cascader>
                </el-form-item>
                <el-form-item label="详细地址：" prop="address"><el-input v-model="shopForm.address"/></el-form-item>
                <el-form-item label="营业时间：" prop="openTime"><el-input v-model="shopForm.openTime"/></el-form-item><br/>
                <el-form-item label="经纬度：" class="formItemClass">
                    <el-col>
                        <el-row>{{shop.lat +' , ' + shop.lng}} <el-button type="primary" @click="shopmap.visible=true" style="margin-left: 8px">设置经纬</el-button></el-row>
                    </el-col>
                </el-form-item>
                <el-form-item label="品牌类型：">
                    <el-select clearable v-model.trim="shopForm.brandId"   placeholder="请选择" style="width: 160px;">
                ...
                <el-form-item label="组织机构：">
                    <el-select  ref="selectRegionRef" v-model="selectRegionObj.regionObj" placeholder="请选择组织机构" multiple collapse-tags  @change="selectRegion" style="width: 180px;">
                        <el-option :value="selectRegionObj.regionObjValue" style="height: auto">
                            <el-tree
                                    size="small"
                                    accordion="true"
                                    show-checkbox
                ...
                <el-form-item label="短信签名：">
                  <el-select clearable v-model.trim="shopForm.msgType"  @change="changeMsg(shopForm.msgType)" placeholder="请选择" style="width: 160px;">
                    <el-option :key="item.id" :label="item.name" :value="item.id" v-for="item in msgData" >
                    </el-option>
                  </el-select>
                </el-form-item>
        ...
        <el-dialog :title="shopQCode.title" :visible.sync="shopQCode.visible" @close="shopQCode.visible=false" width="500px" height="500px">
            <el-image
...
<script>
    ...
    import payTemplate from "../../service/modules/chain/payTemplate";
    ...
    export default {
        ...
        data(){
            return {
                ...
                shop:{},
                shopForm:{
                    shopname:'',yyshopname:"",contacts:"",moblie:"",address:"",openTime:"",brandId:"",paytId:"",type:"",id:"",pca:[],status:"",isShow:"",logo:"",code:"",msgType:""
                },
                editshop:{
                    title:'修改门店信息',
                    visible:false
        ...
        methods:{
            ...
            resetData(){
              this.shop={};
              this.shopForm={  shopname:'',yyshopname:"",contacts:"",moblie:"",address:"",openTime:"",brandId:"",paytId:"",type:"",id:"",pca:[],status:"",isShow:"",logo:"",msgType:""};
            },
            async openQcode(){
                if(!this.shopQCode.qrPath){
                    let reqParam= {shopid: this.shop.id,shopcode:this.shop.shopcode,shopname:this.shop.shopname,appid:this.shop.appid}
                    let that = this;
           ...
           async editShopInfo(){
               ...
               for(let i = 0; i < this.regionData.length; i++){
                   ...
                   this.traverseRegion(itemRegion,this.shop.regionId,this.selectRegionObj);
               }
            ...
            toUpdateShop(){
                const that=this;
                if(!$yid.validate.telephone(that.shopForm.moblie) && !$yid.validate.mobile(that.shopForm.moblie)){
                ...
                //区域
                if(this.selectRegionObj.regionObjValue != null && this.selectRegionObj.regionObjValue.length > 0){
            ...
            traverseRegion(regionData, selectRegionId,selectRegionObj){
                ...
                if(parseInt(regionData.id) == selectRegionId){
                    ...
                    that.$refs.templateRegionTree.setCheckedNodes(selectRegionObj.regionObjValue)
                    return
                }
