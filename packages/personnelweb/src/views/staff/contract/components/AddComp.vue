...
<template>
  <div>
    ...
    <yid-table :data="tableData" ref="table" class="mg-t-12">
      ...
      <yid-table-column label="原状态" prop="contractStatus" width="80px">
      ...
      <yid-table-column label="新状态" prop="status2" width="100px">
        <template slot-scope="scope">
          <el-select v-model="scope.row.status2">
            <el-option
              ...
              :label="item.label"
              :value="item.value"
              :disabled="scope.row.contractStatus == item.value">
            </el-option>
          </el-select>
        </template>
      </yid-table-column>
      <yid-table-column label="合同签订日" prop="contdatesigned" width="176px">
        <template slot-scope="scope">
          <el-date-picker
            v-model="scope.row.contdatesigned"
            type="date"
            format="yyyy年MM月dd日"
            placement="bottom"
      ...
      <yid-table-column label="合同生效日" prop="contdatestart" width="176px">
        <template slot-scope="scope">
          <el-date-picker
            v-model="scope.row.contdatestart"
            type="date"
            format="yyyy年MM月dd日"
            placement="bottom"
            value-format="yyyy-MM-dd"
      ...
      <yid-table-column label="合同结束日" prop="contdateend" width="176px">
        <template slot-scope="scope">
          <el-date-picker
...
<script>
import ChooseMultipleStaff from '@src/components/business/ChooseMultipleStaff'
