  ...
  <div class="salaryRewardDetail">
      ...
      <div v-show="showList">
        <el-form ref="searchForm" inline :model="searchForm">
          ...
          <el-form-item label="">
            ...
            <el-button @click="lookup()" type="primary"
              >查看{{ status == '2' ? '已离职' : '在职' }}员工</el-button
            >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <el-button @click="mutiImportEmployee" type="primary"
              >导入</el-button
            >
          </el-form-item>
        </el-form>
        <yid-table ref="table" pagination style="width: 100%; margin-top: 10px">
          <yid-table-column prop="eecode" label="员工编号" width="120">
          </yid-table-column>
          <yid-table-column prop="eename" label="姓名" width="120">
            <template slot-scope="scope">
              <el-link type="primary" @click="editAction(scope.row, true)">{{
                scope.row.eename
              }}</el-link>
            </template>
          </yid-table-column>
          <yid-table-column prop="nickname" label="昵称" width="120">
          </yid-table-column>
          <yid-table-column label="性别" width="60">
            <template slot-scope="scope">
              {{ scope.row.sex == '1' ? '男' : '女' }}
            </template>
          </yid-table-column>
          <yid-table-column prop="mobile" label="手机号" width="120">
          </yid-table-column>
          <yid-table-column prop="psname" label="职务" width="120">
          </yid-table-column>
          <yid-table-column prop="pslname" label="级别" width="120">
          </yid-table-column>
          <yid-table-column prop="shopcode" label="所属机构/门店" width="200">
            <template slot-scope="scope">
              <div v-if="scope.row.otype == '1'">
                {{ '(' + scope.row.shopcode + ')' + scope.row.shopname }}
              </div>
              <div v-if="scope.row.otype == '2'">
                {{ '(' + scope.row.regionCode + ')' + scope.row.regionName }}
              </div>
            </template>
          </yid-table-column>
          <yid-table-column
          ...
          <yid-table-column prop="entryTime" label="入职时间" width="120">
            <template slot-scope="scope">
              {{ $yid.util.formatDate(scope.row.entryTime) }}
            </template>
          </yid-table-column>
          <yid-table-column label="员工状态" width="80">
            <template slot-scope="scope">
          ...
          <yid-table-column label="操作" min-width="150" fixed="right">
            <template slot-scope="scope">
              <el-link type="primary" @click="editAction(scope.row, false)"
                >编辑</el-link
              >
              <el-link
                type="primary"
              ...
              <el-link
                type="primary"
                @click="dimissionAction(scope.row)"
                style="margin-left: 6px">
    ...
    <el-collapse-transition>
      <div v-show="!showList">
        ...
        <el-button @click="edit" type="primary" v-show="showEdit"
          >编辑</el-button
        >
        <el-button @click="save" type="primary" v-show="showSave"
          >保存</el-button
        >
        <el-form
          ref="employeeForm"
          ...
          <el-row :gutter="20">
            <el-col :span="8">
              <el-form-item label="员工头像：" prop="photo">
                <el-upload
                  ...
                  <i
                    v-else
                    class="el-icon-plus avatar-uploader-icon el-upload"></i>
                  <div class="el-upload__tip" slot="tip">
                    仅支持jpg，png格式的图片上传图片尺寸：200px*200px
                  </div>
                </el-upload>
              </el-form-item>
              <el-form-item
                ...
                <el-select
                  placeholder="请选择"
                  v-model.trim="employeeForm.otype"
              ...
              <el-form-item
                ...
                <el-select
                  ...
                  :disabled="this.disabledvalue || !this.isnow">
                  <el-option
                    :value="employeeForm.regionObjValue"
                    style="height: auto">
                    <el-tree
                      size="small"
                      accordion="true"
              ...
              <el-form-item
                ...
                <el-select
                  ...
                  @change="getEditRegPositionLevelList"
                  :disabled="this.disabledvalue">
                  <el-option
                    :key="item.id"
                    :label="item.psname"
              ...
              <el-form-item label="门店做业绩：" prop="shops">
                <el-button
                  type="primary"
                  @click="selectEeShops(employeeForm)"
                  :disabled="this.disabledvalue"
                  >选择门店</el-button
                >
                <lable style="margin-left: 6px">
                  共选中{{
                    employeeForm.shoplist ? employeeForm.shoplist.length : '0'
                  }}家门店
                </lable>
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item
                ...
                <el-input
                  v-model="employeeForm.eecode"
                  :disabled="this.isUser"
                  placeholder="员工编号"
                  style="width: 150px"
                  @change="checkEecode"></el-input>
              </el-form-item>
    ...
    <yid-dialog
      ...
      <el-form
        ...
        <p>操作提示</p>
        <p>
          1、可从管理平台批量导入员工资料，并下载对应模板填写相关信息后保存提交上传；
        </p>
        ...
        <el-form-item>
          <el-button @click="saveImportEmployees" type="primary"
            >保存</el-button
          >
    ...
    <yid-dialog
      ...
      <el-form ref="transferForm" :model="transferForm" label-width="140px">
        <el-form-item
          ...
          <el-select
            placeholder="请选择"
            v-model.trim="transferForm.otype"
            :disabled="this.disabledvalue">
            <el-option label="门店" value="1"></el-option>
            <el-option label="总部" value="2"></el-option>
          </el-select>
        ...
        <el-form-item
          label="调入机构："
          v-if="transferForm.otype == '2'"
          prop="regionObj">
          <el-select
            ref="selectRegionRef"
            placeholder="请选择机构"
            v-model.trim="transferForm.regionObj"
            collapse-tags
            @change="selectRegion">
            <el-option
              :value="transferForm.regionObjValue"
              style="height: auto">
              <el-tree
                size="small"
                accordion="true"
        ...
        <el-form-item
          label="职务："
          prop="regpositionObj"
          v-show="transferForm.otype == '2'">
          <el-select
            value-key="id"
            filterable
            placeholder="请选择"
            v-model.trim="transferForm.regpositionObj"
        ...
        <el-form-item
          label="调入门店："
          v-if="transferForm.otype == '1'"
          prop="shopObj">
          <el-select
            value-key="id"
            filterable
            :filter-method="filterShop3"
            placeholder="请选择"
            v-model.trim="transferForm.shopObj"
            @change="getEditFormBranchList">
            <el-option
              :key="item.id"
              :label="item.shopname"
              :value="item"
              v-for="item in filterShopList3">
              <span style="float: left">{{ item.shopcode }}</span>
              <span style="float: right; color: #8492a6; font-size: 13px">{{
                item.shopname
              }}</span>
        ...
        <el-form-item
          ...
          <el-select
            ...
            filterable
            placeholder="请选择"
            v-model.trim="transferForm.branchObj"
            @change="getEditFormPositionList">
            <el-option
              :key="item.id"
              :label="item.bname"
        ...
        <el-form-item
          ...
          <el-select
            ...
            filterable
            placeholder="请选择"
            v-model.trim="transferForm.positionObj"
            @change="getEditFormPositionLevelList">
            <el-option
              :key="item.id"
              :label="item.psname"
        ...
        <el-form-item
          ...
          <el-select
            ...
            filterable
            placeholder="请选择"
            v-model.trim="transferForm.positionLevelObj"
            @change="selectPositionLevel">
            <el-option
              :key="item.id"
              :label="item.pslname"
...
export default {
  ...
  methods: {
    ...
    getEditFormPositionList(obj) {
      ...
      this.employeeForm.positionLevelObj = {}
      service.position
        .listForChain({ status: '1', bbid: obj.id, isDel: '0' })
        .then(res => {
          if (res.resp_code == 200) {
    ...
    getEditFormPositionLevelList(obj) {
      ...
      service.positionLevel
        .listForChain({ status: '1', psid: obj.id, isDel: '0' })
        .then(res => {
          if (res.resp_code == 200) {
    ...
    async editAction(row, disabledvalue) {
      ...
      await service.branch
        .list({ shopid: row.shopid, isDel: '0', status: '1' })
        .then(res => {
          if (res.resp_code == 200) {
            this.editFormBranchList = res.data
          }
        })
      await service.position
        .list({ status: '1', bbid: row.bbid, isDel: '0' })
        .then(res => {
          if (res.resp_code == 200) {
            this.editFormPositionList = res.data
          }
        })
      await service.positionLevel
        .list({ status: '1', psid: row.psid, isDel: '0' })
        .then(res => {
          if (res.resp_code == 200) {
            this.editFormPositionLevelList = res.data
            this.editRegPositionLevelList = res.data
    ...
    save() {
      ...
      this.$refs['employeeForm'].validate(valid => {
        if (valid) {
          if (
            that.employeeForm.status == null ||
            that.employeeForm.status.length == 0
          ) {
          ...
          if (that.employeeForm.otype == '1') {
            ...
            if (that.employeeForm.positionLevelObj) {
              that.employeeForm.pslname =
                that.employeeForm.positionLevelObj.pslname
              that.employeeForm.pslid = that.employeeForm.positionLevelObj.id
              that.employeeForm.pslcode =
                that.employeeForm.positionLevelObj.pslcode
            } else {
          ...
          if (that.employeeForm.otype == '2') {
            ...
            if (that.employeeForm.regpositionLevelObj) {
              ...
              that.employeeForm.pslcode =
                that.employeeForm.regpositionLevelObj.pslcode
              that.employeeForm.pslname =
                that.employeeForm.regpositionLevelObj.pslname
            } else {
              that.employeeForm.pslid = null
              that.employeeForm.pslcode = null
              that.employeeForm.pslname = null
          ...
          service.employee.save(that.employeeForm).then(res => {
            if (res.resp_code == 200) {
              ...
              if (
                that.employeeForm.id == '' ||
                that.employeeForm.id.length == 0
              ) {
                that.status = '2'
    ...
    saveImportEmployees() {
      if (this.mutiImportDialog.employees.length == 0) {
        ...
        console.log(this.mutiImportDialog.employees)
        service.employee
          .batchImport({ maps: this.mutiImportDialog.employees })
          .then(res => {
            if (res.resp_code == 200) {
    ...
    saveTransfer() {
      ...
      this.$refs['transferForm'].validate(valid => {
        if (valid) {
          if (
            that.transferForm.employee.id &&
            that.transferForm.shopObj.id == that.transferForm.employee.shopid
          ) {
          ...
          if (that.transferForm.otype == '1') {
            ...
            if (that.transferForm.positionLevelObj) {
              ...
              that.transferForm.pslcode =
                that.transferForm.positionLevelObj.pslcode
              that.transferForm.pslname =
                that.transferForm.positionLevelObj.pslname
            } else {
          ...
          if (that.transferForm.otype == '2') {
            ...
            if (that.transferForm.positionLevelObj) {
              ...
              that.transferForm.pslcode =
                that.transferForm.regpositionLevelObj.pslcode
              that.transferForm.pslname =
                that.transferForm.regpositionLevelObj.pslname
            } else {
              that.transferForm.pslid = null
              that.transferForm.pslcode = null
              that.transferForm.pslname = null
    ...
    traverseRegion(regionData, selectRegionId, selectRegionObj) {
      ...
      if (parseInt(regionData.id) == selectRegionId) {
        ...
        that.$refs.templateRegionTree1.setCheckedNodes(
