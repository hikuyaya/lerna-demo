 ...
 * @LastEditors: wqy
 * @LastEditTime: 2022-07-12 14:36:06
 * @FilePath: \personnelweb\src\views\staff\workingage\components\RemoveComp.vue
 * @Description: 批量审批、删除
-->
<template>
    ...
    handleOperate() {
      if (this.type === 'approve') {
        this.$confirm(
          ...
          .then(async () => {
            await service.staff.workingage.censorBill({
              batchNo: this.batchNo
            })
        ...
        this.$confirm(`您确认要移除此批次中所有数据吗？`, `移除确认`, {
          ...
          .then(async () => {
            await service.staff.workingage.deleteBill({
              batchNo: this.batchNo
            })
            this.$message.success('操作成功')
            this.$emit('refresh')
