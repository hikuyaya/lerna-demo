            ...
            <div v-show="showList">
                <el-form ref="searchForm" inline :model="searchForm">
                    ...
                    <el-form-item label="是否允许客装销售：" prop="status">
                        <el-select  clearable filterable placeholder="请选择" v-model.trim="searchForm.iskz" style="width: 100px">
                    ...
                    <el-row :gutter="20">
                        <el-col :span="10">
                            ...
                            <el-form-item label="进价：" prop="cost" :rules="[{ required: true, message: '进价不能为空'}]">
                                <el-input-number type="number" v-model="productForm.cost" placeholder="进价" style="width: 200px;"></el-input-number>
                            </el-form-item>
                            ...
                            <el-form-item label="售价：" prop="price" :rules="[{ required: true, message: '售价不能为空'}]">
                                <el-input-number type="number" v-model="productForm.price" placeholder="售价" style="width: 200px;"></el-input-number>
                            </el-form-item>
                        </el-col>
                        <el-col :span="10">
                ...
                if(data.status=="2"){
                    service.product.checkPackage(data.id).then(res =>{
                        if(res.resp_code=="200" && res.data){
                            yid.util.confirm('该产品现在已在以下套餐中销售:['+res.data+']中包含此项目，停用后该套餐也将停止销售。是否继续停用？', '','', ()=> {
                                service.product.saveProduct(data).then(res=> {
                                    if(res.resp_code == 200) {
                                        let page = this.$refs.table.Pagination.internalCurrentPage;
                                        this.pageInfo.page = page;
