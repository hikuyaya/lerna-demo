    ...
    <div class="productOrder">
        <el-collapse-transition>
            <div v-show="showList">
                ...
                <yid-table  ref="table" pagination style="width: 100%;margin-top: 20px;">
                    <yid-table-column prop="billcode" label="申请单号" width="180"></yid-table-column>
                    <yid-table-column prop="shopname" label="申请门店" width="150"></yid-table-column>
                    <yid-table-column prop="createdBy" label="申请人" width="120"></yid-table-column>
                    <yid-table-column prop="createdTime" label="申请时间" width="180">
                        <template slot-scope="scope">
                            {{$yid.util.formatTime(scope.row.createdTime)}}
                        </template>
                    </yid-table-column>
                    <yid-table-column prop="memo" label="备注" width="200"></yid-table-column>
                    ...
                    <yid-table-column prop="check" label="审核人" width="100"></yid-table-column>
                    ...
                    <yid-table-column
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="scope.row.status != '1'" type="text" @click="showBill(scope.row)" style="margin-left: 6px">明细</el-button>
                        </template>
                    </yid-table-column>
                </yid-table>
            </div>
        ...
        <el-collapse-transition>
            <div v-show="!showList">
                ...
                <el-button v-show="showButton"  @click="alertProductDialog" type="primary">选择产品</el-button>&nbsp;&nbsp;&nbsp;
                <el-button v-show="showButton"  @click="removeProduct" style="background-color: red;color: white">批量移除产品</el-button>&nbsp;&nbsp;&nbsp;
                <el-button v-show="showButton"  @click="saveBill" type="primary">保存</el-button>
                <el-button v-show="showButton" @click="auditBill" type="primary">提交</el-button>
                <el-button v-show="!showButton" @click="excel" type="primary">导出</el-button>
                <el-form ref="billForm" :model="billForm" style="margin-top: 16px" :rules="rules" label-width="100px" label-position="right">
                    ...
                    <span style="width: 200px">订货门店：{{billForm.shopcode}}&nbsp;&nbsp;{{billForm.shopname}}</span>&nbsp;&nbsp;&nbsp;&nbsp;
                    <span style="width: 200px">申请时间：{{billForm.createdTime}}</span>&nbsp;&nbsp;&nbsp;&nbsp;
                    <span style="width: 200px">申请人：{{billForm.opercode}}  {{billForm.oper}}</span><br>&nbsp;&nbsp;&nbsp;&nbsp;
                    <span>备注：<el-input :disabled="!showButton"
                ...
                <yid-table v-show="showButton" ref="itemTable" style="margin-top: 15px;" :data="orderitemData" @selection-change="handleSelectionChange">
                    ...
                    <yid-table-column label="产品名称" min-width="200" prop="pname"></yid-table-column>
                    <yid-table-column label="规格" min-width="100" prop="norms"></yid-table-column>
                    <yid-table-column label="单位" min-width="100" prop="unit"></yid-table-column>
                    <yid-table-column label="门店当前库存" min-width="100" prop="stock"></yid-table-column>
                    <yid-table-column label="要货数" min-width="100" prop="num">
                        <template slot-scope="scope">
                            <el-input onKeypress="return (/[\d]/.test(String.fromCharCode(event.keyCode)))" type="number" v-model="scope.row.num"></el-input>
                        </template>
                    </yid-table-column>
                    <yid-table-column label="操作" min-width="100" fixed="right">
                        <template slot-scope="scope">
                            <el-button type="text" @click="deleteProuct(scope.$index,scope.row)" style="margin-left: 6px;color: red">删除</el-button>
                        </template>
                    </yid-table-column>
                </yid-table>
                ...
                <yid-table v-show="!showButton" ref="showTable" style="margin-top: 15px;" :data="orderitemData" @selection-change="handleSelectionChange">
                    ...
                    <yid-table-column label="单位" min-width="100" prop="unit"></yid-table-column>
                    <yid-table-column label="门店当前库存" min-width="100" prop="stock"></yid-table-column>
                    <yid-table-column label="要货数量" min-width="100" prop="num"></yid-table-column>
                    <yid-table-column label="发货数量" min-width="100" prop="sendnum"></yid-table-column>
                </yid-table>
            </div>
        </el-collapse-transition>

        <el-dialog :title="productDialog.title" :visible.sync="productDialog.visible" width="850px" >
            <div style="display: flex;">
                ...
                <yid-table ref="productData" :data="productData" style="margin-top: 15px;flex: 1;margin-left: 20px;" @selection-change="handleSelectionChange">
                    <yid-table-column type="selection" width="55"></yid-table-column>
                    <yid-table-column label="产品编码" min-width="120" prop="pcode"></yid-table-column>
    ...
    export default {
        ...
        methods:{
            ...
            handleNodeClick(data) {
                ...
                //通过id查询商品列表
                service.stock.product.allList({categrory : categrory,status : "1"}).then(res=>{
                    if(res.resp_code == 200) {
            ...
            addProduct(){
                ...
                for(let n=0 ; n<this.orderitemData.length;n++){
                    ...
                    for(let i=0;i<this.selectProducts.length;i++){
                        ...
                        if(item.pcode == pcode){
                            yid.util.alert("列表中存在【"+this.orderitemData[n].pname+"】不能再次添加");
                            return false;
                        }
                    }
                }
            ...
            excel() {
                ...
                download(yid.config.API.BASE + 'api-chain/productorderitem/list/excel', {poid:this.billForm.id})
            }
        }
    }
</script>
