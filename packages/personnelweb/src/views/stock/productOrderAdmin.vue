    ...
    <div class="productOrderAdmin">
        ...
        <el-collapse-transition>
            <div v-show="!showList">
                ...
                <el-button v-show="showButton" @click="rejectBill" style="background-color: red;color: white">驳回</el-button>
                <el-button @click="excel" style="background-color: green;color: white">导出</el-button>
                <el-form ref="billForm" :model="billForm" style="margin-top: 16px" :rules="rules" label-width="100px" label-position="right">
                    ...
                    <span>备注：<el-input :disabled="true"
                ...
                <yid-table v-show="showTable" ref="itemTable" style="margin-top: 15px;" :data="orderitemData">
                    ...
                    <yid-table-column label="发货数量" min-width="100" prop="sendnum">
                        <template slot-scope="scope">
                            <el-input :disabled="isSendNum" onKeypress="return (/[\d]/.test(String.fromCharCode(event.keyCode)))" type="number" v-model="scope.row.sendnum"></el-input>
                        </template>
                    </yid-table-column>
                </yid-table>

                <yid-table v-show="!showTable" ref="showTable" style="margin-top: 15px;" :data="orderitemData">
                    <yid-table-column label="产品编码" min-width="120" prop="pcode"></yid-table-column>
                    <yid-table-column label="产品名称" min-width="200" prop="pname"></yid-table-column>
                    <yid-table-column label="规格" min-width="100" prop="norms"></yid-table-column>
                    <yid-table-column label="单位" min-width="100" prop="unit"></yid-table-column>
        ...
        methods:{
            ...
            auditBill(){
                ...
                for(let n=0 ; n<this.orderitemData.length; n++){
                    ...
                    if(sendnum == null || sendnum == '' || sendnum.length == 0 || sendnum <= 0){
            ...
            rejectBill(){
                yid.util.confirm('驳回后，门店可以再次修改并提交，确认驳回吗？', '','', ()=> {
                    if(this.billForm.id == '' || this.billForm.id.length == 0){
                        yid.util.alert("单据id为空!")
                        return false;
                    }
