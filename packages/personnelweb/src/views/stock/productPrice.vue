  ...
  <div class="productPrice">
      ...
      <div v-show="listShow">
        ...
        <yid-table ref="table" pagination style="width: 1010px">
          ...
          <yid-table-column label="操作" prop="status" width="120">
            <template slot-scope="scope">
              <el-link type="primary" @click="editShopPrice(scope.row)"
                >编辑</el-link
              >
              <el-link
                type="primary"
                @click="setServPrice(scope.row)"
        ...
        <el-form v-model="productForm" inline style="margin-top: 10px">
          ...
          <el-form-item label="产品类别：">
            <el-select
              ...
              filterable
              placeholder="请选择"
              v-model.trim="productForm.categrory"
              style="width: 120px">
              <el-option-group
          ...
          <el-form-item label="允许客装销售：">
            <el-select
              ...
              filterable
              placeholder="请选择"
              v-model.trim="productForm.iskz"
              style="width: 120px">
              <el-option
                :key="item.value"
                :label="item.label"
                :value="item.value"
                v-for="item in iskzList"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="状态">
            <el-select
          ...
          <el-form-item label="">
            <el-button type="primary" @click="getProductPriceList()"
              >查询</el-button
            >
          </el-form-item>
        </el-form>
        ...
        <yid-table
          ...
          <yid-table-column label="最低限价" min-width="80" prop="nprice">
            <template slot-scope="scope">
              ...
              <i
                class="el-icon-edit"
                @click="alertMinpriceDialog(scope.row)"></i>
            </template>
          </yid-table-column>
          ...
          <yid-table-column label="是否允许客装销售" min-width="80" prop="iskz">
            ...
            <template slot-scope="scope">
              ...
              <el-switch
                v-model="scope.row.iskz"
                active-value="1"
                inactive-value="0"
                active-color="#339933"
                inactive-color="#DCDFE6"
                @change="editIskz(scope.row)">
              </el-switch>
            </template>
          </yid-table-column>
          <yid-table-column
          ...
          <yid-table-column label="门店产品状态" width="110">
            <template slot-scope="scope">
              <label
                style="color: blue"
                @click="setShopService(scope.row)"
      ...
      <el-row type="flex" justify="space-between">
        <el-form inline :model="shopSearchForm" ref="shopSearchForm">
          ...
          <el-form-item label="品牌：">
            <el-select
              clearable
              v-model="shopSearchForm.brandId"
              style="width: 120px">
              <el-option
          ...
          <el-form-item label="区域：">
            <el-select
              ref="selectRegionRef"
              v-model="shopSearchForm.regionObj"
              placeholder="请选择门店区域"
              multiple
              collapse-tags
              @change="selectRegion"
              ...
              <el-option
                hidden
                :value="shopSearchForm.regionObjValue"
          ...
          <el-form-item label="价格体系：">
            <el-select
              clearable
              v-model="shopSearchForm.ispptid"
              style="width: 120px">
        ...
        <el-col :span="14">
          <yid-table
            ...
            <yid-table-column label="价格体系" min-width="120" prop="pricename">
              <template slot-scope="scope">
                {{
                  scope.row.productPtshops.length > 0
                    ? scope.row.productPtshops[0].ptempName
                    : ''
                }}
              </template>
            </yid-table-column>
    ...
    <yid-dialog
      ...
      <el-form label-width="120px" ref="copyForm3">
        ...
        <el-form-item label="">
          <el-button type="primary" @click="addGoods()" v-if="!duoflag"
            >确认添加</el-button
          >
    ...
    <yid-dialog
      :title="npriceDialog.title"
      :visible.sync="npriceDialog.visible"
      width="450px">
    ...
    <yid-dialog
      :title="minpriceDialog.title"
      :visible.sync="minpriceDialog.visible"
      width="450px">
    ...
    <yid-dialog
      :title="batchPriceDialog.title"
      :visible.sync="batchPriceDialog.visible"
      width="600px"
      @close="cancelBatchPriceDialog">
      <yid-table
        ref="batchPriceTable"
        style="margin-top: 15px"
        :data="selectPricesCopy">
        <yid-table-column
          label="产品编码"
          min-width="100"
          prop="pcode"></yid-table-column>
    ...
    <yid-dialog
      :title="shopStatusDialog.title"
      :visible.sync="shopStatusDialog.visible"
      width="800px">
      <el-form ref="shopStatusDialog" inline style="margin-top: 8px">
        ...
        <el-form-item label="门店：">
          <el-select
            clearable
            v-model.trim="shopStatusDialog.shopid"
            @clear="clearShopForm"
            ...
            <el-option
              ...
              :label="item.shopname"
              :value="item.shopid"
              v-for="item in sptdata.shoplist">
      ...
      <yid-table
        ref="cardShopList"
        :data="shopStatusDialog.productShops"
  ...
  methods: {
    init() {
      this.excelConvertUrl =
        $yid.config.API.BASE + 'api-chain/product/convertSystem'

      service.chain.servShop.findbrands({ isDel: '0' }).then(res => {
        this.brands = res.data
      })
      service.chain.servShop
        .findbranchs({ status: '1', isDel: '0' })
        .then(res => {
    ...
    setShop() {
      if (this.flag) {
        yid.util.confirm(
          '选择的门店包括已设置产品价格体系的门店，保存后门店将会从原价格体系移至本体系中！',
          '',
          '',
          () => {
    ...
    openGood() {
      ...
      service.chain.servicepackage
        .product({ pptid: this.productForm.pptid })
        .then(res => {
          this.goodAddDialog.products = res.data
        })
    ...
    addGoods() {
      ...
      for (let i = 0; i < this.goodAddDialog.products.length; i++) {
        if (
          this.goodAddDialog.products[i].type &&
          (!that.goodAddDialog.branch ||
            this.goodAddDialog.products[i].branch ==
              that.goodAddDialog.branch) &&
          (!that.goodAddDialog.productType ||
            this.goodAddDialog.products[i].servtid ==
              that.goodAddDialog.productType)
        ) {
          let flag = true
          ...
          if (flag) {
            for (let n = 0; n < that.productData.length; n++) {
              ...
              if (this.goodAddDialog.products[i].sdcode === pcode) {
                yid.util.info(
                  '列表中存在【' + that.productData[n].pname + '】不能再次添加'
                )
                return false
              }
            }
    ...
    editPrice() {
      if (
        this.priceForm.minPrice != null &&
        this.priceForm.minPrice != '' &&
        Number(this.priceForm.minPrice) < Number(this.priceForm.cost)
      ) {
        return yid.util.error('最低限价不得低于进价!')
      }
      this.$refs['priceForm'].validate(valid => {
        service.chain.productPrice
          .saveOrUpdatePrice(this.priceForm)
          .then(res => {
            if (res.resp_code == '200') {
              this.getProductPriceList()
    ...
    editnPrice() {
      this.$refs['npriceForm'].validate(valid => {
        service.chain.productPrice
          .saveOrUpdatePrice(this.npriceForm)
          .then(res => {
            if (res.resp_code == '200') {
              this.getProductPriceList()
    ...
    editminPrice() {
      ...
      if (
        this.minpriceForm.minPrice != null &&
        this.minpriceForm.minPrice != '' &&
        Number(this.minpriceForm.minPrice) < Number(this.minpriceForm.cost)
      ) {
        return yid.util.error('最低限价不得低于进价!')
      }
      this.$refs['minpriceForm'].validate(valid => {
        service.chain.productPrice
          .saveOrUpdatePrice(this.minpriceForm)
          .then(res => {
            if (res.resp_code == '200') {
              this.getProductPriceList()
    ...
    batchEditPrice() {
      for (let i = 0; i < this.selectPricesCopy.length; i++) {
        if (
          this.selectPricesCopy[i].price === '' ||
          this.selectPricesCopy[i].price.length == 0
        ) {
          yid.util.alert('售价为必填值')
          return
        }
        if (
          this.selectPricesCopy[i].minPrice != null &&
          this.selectPricesCopy[i].minPrice != '' &&
          Number(this.selectPricesCopy[i].minPrice) <
            Number(this.selectPricesCopy[i].cost)
        ) {
      ...
      service.chain.productPrice
        .batchSave({ productPrices: priceList })
        .then(res => {
          if (res.resp_code == '200') {
            this.getProductPriceList()
            this.batchPriceDialog.visible = false
    ...
    setShopService(row) {
      service.chain.productPrice
        .getProductShopStatus({ pid: row.id })
        .then(res => {
          if (res.resp_code == 200) {
    ...
    checkServShopStatus() {
      ...
      service.chain.productPrice
        .checkProductShopStatus(this.shopStatusDialog)
        .then(res => {
          if (res.data) {
    ...
    saveServShopStatus() {
      service.chain.productPrice
        .saveProductShopStatus(this.shopStatusDialog)
        .then(res => {
          if (res.resp_code == '200') {
    ...
    saveImportProducts() {
      if (this.importDialog.products.length == 0) {
        ...
        service.chain.productPrice
          .saveSystem(this.importDialog.products)
          .then(res => {
            if (res.resp_code == '200') {
              this.queryShopPrice()
              this.importDialog.visible = false
