      ...
      <el-form-item label="产品品类：" prop="categrory">
        <el-select
          ...
          <el-option
            ...
            :label="item.cname"
            :value="item.id"
            v-for="item in searchProductcategorylList"></el-option>
        </el-select>
      </el-form-item>
      ...
      <yid-table-column label="安全库存" min-width="200" prop="safeStock">
        <template slot-scope="scope">
          <el-input
            ...
            placeholder="请输入内容"
            @change="updateSafeStock(scope.row)"></el-input>
        </template>
      </yid-table-column>
    </yid-table>
    <yid-dialog
      ...
      <el-form ref="safeForm" :model="safeForm" label-width="120px">
        <el-form-item
          label="安全库存数量"
          prop="safeStock"
          :rules="[{ required: true, message: '安全库存数量为空' }]">
          <el-input-number
            clearable
            min="0"
            v-model.trim="safeForm.safeStock"
      ...
      this.$refs['safeForm'].validate(valid => {
        if (valid) {
          ...
          service.stock.product
            .batchSave({ products: productObjs })
            .then(res => {
              if (res.resp_code == 200) {
                ...
                service.base.product
                  .batchSave({ products: baseProducts })
                  .then(res1 => {
