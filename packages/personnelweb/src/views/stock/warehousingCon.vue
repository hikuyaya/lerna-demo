<template>
  <div>
    ...
    <el-row style="margin-top: 20px" :gutter="20">
      <el-col :span="4"
        ><div class="grid-content bg-purple">
          入库金额：<label inline style="color: red">{{ inAllMoney }}元</label>
        </div></el-col
      >
      <el-col :span="20"
    ...
    <yid-table ref="table" :data="selectProducts" style="margin-top: 15px">
      ...
      <yid-table-column label="入库金额" width="150" prop="money">
        <template slot-scope="scope">
          <el-input-number
            disabled
            v-model="scope.row.count * scope.row.cost"></el-input-number>
        </template>
      </yid-table-column>
  ...
  methods: {
    async initData(obj) {
      ...
      if (this.actionType == 'add') {
        ...
        await service.stock.entryProducts
          .list({ isDel: '0', seid: entryData.id })
          .then(res => {
    ...
    async getTypeDictList() {
      ...
      await service.stock.dictInout
        .list({ type: service.stock.dictInout.inType, isDel: '0' })
        .then(res => {
          if (res.resp_code == 200) {
    ...
    async addNew() {
      ...
      await service.stock.entry.save(reqParms).then(res => {
        if (res.resp_code == 200) {
          ...
          service.stock.entryProducts
            .list({ isDel: '0', seid: this.seid })
            .then(res => {
              if (res.resp_code == 200) {
                this.selectProducts = res.data
              }
