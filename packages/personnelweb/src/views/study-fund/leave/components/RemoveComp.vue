 ...
 * @LastEditors: wqy
 * @LastEditTime: 2022-08-05 11:25:47
 * @FilePath: \personnelweb\src\views\study-fund\leave\components\RemoveComp.vue
 * @Description: 批量审批、删除
-->
...
export default {
  ...
  methods: {
    async onCheck() {
      ...
      const { data } = await service.studyFund.leave.batchQuery({
    ...
    handleOperate() {
      if (this.type === 'approve') {
        this.$confirm(
          ...
          .then(async () => {
            await service.studyFund.leave.batchAduBill({
              batchNo: this.batchNo
            })
        ...
        this.$confirm(`您确认要移除此批次中所有数据吗？`, `移除确认`, {
          ...
          .then(async () => {
            await service.studyFund.leave.batchDelBill({
              batchNo: this.batchNo
            })
            this.$message.success('操作成功')
            this.$emit('refresh')
