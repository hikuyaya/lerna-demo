<template>
  ...
  <avue-crud ref="crud" :option.sync="aoption" :aoption_column="aoption.column" :data.sync="data" :page.sync="page"
             @refresh-change="refresh"
             :cell-style="cellStyle" :table-loading="columnLoading" size="small" @update:search="searchData"
             @search-change="searchChange" @hideChange="hideChange" @sort-change="sortChange"
             :summary-method="summaryMethod" :span-method="spanMethod" :excel-all="loadexcel"
             @search-reset="searchReset"
             @on-load="onLoad">
    <template slot="bottom" v-if="reportHead.isContent">
      <div class="bottom_content" v-html="reportHead.content"></div>
    </template>
    <template v-for="item in reportItems" :slot="item.fieldName" slot-scope="scope">
  ...
  methods: {
    ...
    async sortChange(val) {
      ...
      let datap = {
        ...
        pageSize: this.page.pageSize, ...params
      }
    ...
    async hideChange(columns) {
      ...
      let filter = this.aoption.column.filter(item => item.showColumn).map(item => item.prop).filter(item => {
    ...
    async initTable() {
      ...
      await service.report.getColumns(this.$route.params.code, this.$route.query.test || false).then(res => {


        let {columns, head, udiy} = res.data;
        that.reportItems = columns.filter(item => item.isSearch == true || item.isShow == true);
        ...
        try {
          ...
          that.js.init(that, that.queryParam, that.defaults, that.defaultParams);
        } catch (e) {
          console.error(e)
        }
        ...
        const addItem = (list, columnData) => {
          let item = {
            ...
            renderHeader: (h, {column, index}) => {
              ...
              //文字 尺寸 14 pending 10
              //排序标 宽 24
              column.minWidth = Math.max(20 + f * l + (column.sortable == "custom" ? 24 : 0), columnData.fieldWidth || 30)
//字大小乘个数即长度 ,注意不要加px像素，这里minWidth只是一个比例值，不是真正的长度
//然后将列标题放在一个div块中，注意块的宽度一定要100%，否则表格显示不完全

              return h("div", {class: "table-head", style: {width: "100%", display: "inline"}}, [column.label])
            },
            ...
            pickerOptions: {
              ...
              disabledDate: (time) => {
                if (this.minTime) {
                  return moment(this.minTime).toDate().getTime() < moment().subtract(this.timeLimit, "days").toDate().getTime() || time.getTime() > Date.now();
                } else if (this.currentLimit)
                  return time.getTime() < moment().subtract(this.timeLimit, "days").toDate().getTime() || time.getTime() > Date.now();
            ...
            dicQuery: columnData.dictQuery ? JSON.parse(columnData.dictQuery) : {},
            typeformat: (item, labelKey, valueKey) => {
              if (columnData.fieldType == "select" && columnData.fieldName == "shopcode") {
                return item[valueKey] + "    " + item[labelKey];
              }
              return item[labelKey];
            },
            searchRules: [{
              required: columnData.isNonull, validator: (rule, value, callback) => {//columnData.isNonull

                try {
                  let error = that.js.validator(that, columnData.fieldName, rule, value, callback);
          ...
          if (that.defaults[item.prop] && isObject(that.defaults[item.prop])) {
            Object.assign(item, that.defaults[item.prop])
          }
          if (that.defaults[item.prop] && that.defaults[item.prop].noChange || columnData.isNonull) {
          ...
          that.queryParam[columnData.fieldName] = (!that.$route.query[columnData.fieldName]) ? "" : that.$route.query[columnData.fieldName]
        ...
        that.aoption.column = columnlist.filter(item => item.search == true || item.hide == true);
        ...
        if (udiy) {
          that.udiy.forEach(item => {
            ...
            try {
              let filter = that.aoption.column.filter(item2 => item2.prop === item);
    ...
    async onLoad(page, params) {
      ...
      let queryParam = {
        pageNo: this.page.currentPage,
        pageSize: this.page.pageSize, ...params, ...this.search
      }
    ...
    async loadData(params) {
      ...
      let res = await service.report.getDataFromService(this.reportHead.dbSource, this.$route.params.code, requestData);
      const data = res.data;
      // if (!this.reportHead.isPage) {
