  ...
  <div>
    <avue-crud
      ...
      <template slot="menuLeft" slot-scope="{ size }">
        <el-button type="primary" @click="handleAdd" :size="size"
          >新增</el-button
        >
      ...
      <template slot-scope="{ row, type, size }" slot="menu">
        <el-button :size="size" :type="type" @click="edit(row)">
          编辑</el-button
        >
        <el-button :size="size" :type="type" @click="popReportURL(row.id)">
          地址</el-button
        >
        <el-button
          ...
          :size="size"
          :type="type"
          @click="goPageOnline(row.id)"
          >测试</el-button
        >
        <el-popconfirm
          title="确定删除吗?"
          @confirm="() => handleDelete(row.id)">
          <el-button slot="reference" :size="size" :type="type">
            删除</el-button
          >
        </el-popconfirm>
      </template>
    ...
    <!-- 提示online报表链接 -->
    <el-dialog title="报表访问链接" :visible="visible" @cancel="handleCancel">
      <template slot="footer">
        <el-button @click="handleCancel">关闭</el-button>
        <el-button
          type="primary"
          class="copy-this-text"
          :data-clipboard-text="reportUrlText"
          @click="onCopyUrl"
          >复制
        </el-button>
      </template>
...
export default {
  ...
  methods: {
    async edit(row) {
      ...
      if (!row.id) instance.$on('closed', this.dialogClose)
      instance.edit(row)
    },
    refresh() {
      this.loadData({
    ...
    async loadData(parm) {
      ...
      let res = await service.report.findList(
        filterObj({
          ...this.queryParam,
          ...parm
        })
