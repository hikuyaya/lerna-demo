    ...
    <div class="authority">
            ...
            <div v-show="showList">
                <el-form ref="searchForm" inline :model="searchForm">
                    <el-form-item label="名称：" prop="name">
                        <el-input  clearable v-model="searchForm.name" placeholder="功能点" style="width: 150px;"></el-input>
                    </el-form-item>
                    <el-form-item label="是否开启授权：" prop="status">
                        <el-select  clearable filterable placeholder="请选择" v-model.trim="searchForm.isOpen" style="width: 120px">
                ...
                <yid-table ref="table" pagination style="width: 100%;margin-top: 10px;">
                    ...
                    <yid-table-column
                        ...
                        <template slot-scope="scope">
                            <span v-for="shop in scope.row.shoplist">
                                <span>
                                    {{shop.shopcode}}|{{shop.shopname}}<br>
                                </span>
                            </span>
                        </template>
                    </yid-table-column>
                    <yid-table-column
                            ...
                            width="120">
                        <template slot-scope="scope">
                            <el-switch v-model="scope.row.isOpen" active-value="1" inactive-value="0" active-color="#339933" inactive-color="#DCDFE6" @change="updateIsOpen(scope.row)">
                            </el-switch>
                        </template>
                    </yid-table-column>
                    <yid-table-column
                        ...
                        <template slot-scope="scope">
                            <el-link type="primary" @click="editAuthority(scope.row)">编辑</el-link>
                        </template>
                    </yid-table-column>
                </yid-table>
            </div>
        ...
        <el-collapse-transition>
            <div v-show="!showList">
                ...
                <div>
                    <el-form ref="authorityForm" :model="authorityForm" style="margin-top: 16px"  label-width="150px" label-position="right">
                        <el-form-item label="管理员角色：" prop="roles">
                            <el-select clearable filterable placeholder="请选择" multiple v-model="authorityForm.roles" style="width: 500px">
                                <el-option :key="item.id" :label="item.name" :value="item.id"
                                           v-for="item in roleList"></el-option>
                            </el-select>
                        </el-form-item>
                        <el-form-item label="特殊门店：" prop="shops">
                            <el-button type="primary" @click="selectMoShops(authorityForm)">选择门店</el-button>
                            <lable style="margin-left: 6px">
                                共选中{{authorityForm.shoplist ? authorityForm.shoplist.length : '0'}}家门店
                            </lable>
                        </el-form-item>
                    </el-form>
                </div>
        ...
        <yid-dialog :title="shopDialog.title" :visible.sync="shopDialog.visible" @close="shopDialog.visible=false"
            ...
            <el-row>
                <el-col :span="14">
                    <yid-table pagination ref="tableShop" style="margin-top: 10px;"
