            ...
            <div class="sys-list" v-show="listShow">
                <el-form :model="searchForm" ref="searchForm" inline>
                    <el-form-item label="用户名：">
                        <el-input clearable placeholder="用户名" v-model.trim="searchForm.username"
                                  style="width: 120px;"></el-input>
                    </el-form-item>
                    <el-form-item label="员工">
                        <el-input clearable placeholder="员工编码/姓名/手机号" v-model.trim="searchForm.nickname"
                                  style="width: 190px;"></el-input>
                    </el-form-item>
                    <el-form-item label="系统角色：">
                        <el-select clearable multiple placeholder="请选择" v-model.trim="searchForm.roleIds"
                                   filterable :filter-method="filterRole" style="width: 280px;">
                            <el-option :key="'roleId'+item.id" :label="item.name" :value="item.id"
                                       v-for="item in filterRoleList"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="是否校验登录：">
                        <el-select clearable placeholder="请选择" v-model.trim="searchForm.ismac" style="width: 150px;">
                            <el-option label="是" value="1"></el-option>
                            <el-option label="否" value="0"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="可登录门店：">
                        <el-select clearable placeholder="请选择" v-model.trim="searchForm.shoptype" style="width: 150px;"
                        ...
                        <el-input v-show="showShopcodename" clearable placeholder="请输入门店编码或名称"
                                  v-model.trim="searchForm.shopcodename" style="width: 200px;"></el-input>
                    </el-form-item>
                    <el-form-item label="状态：">
                        <el-select clearable placeholder="请选择状态" v-model.trim="searchForm.enabled"
                                   style="width: 150px;">
                ...
                <yid-table pagination ref="userTable" :data="userData" style="margin-top: 15px;"
                    ...
                    <yid-table-column label="姓名" min-width="70" prop="nickname"></yid-table-column>
                    <yid-table-column label="手机号" min-width="100" prop="mobile"></yid-table-column>
                    <yid-table-column label="账号角色" width="180" prop="roleName">
                        <template slot-scope="scope">
                            <span v-if="scope.row.menus.length>0"><el-link type="primary"
                                                                           @click="showUserMenu(scope.row)">特</el-link></span>
                    ...
                    <yid-table-column label="可登陆门店" width="190" prop="shopname">
                        <template slot-scope="scope">
                            ...
                            <span style="margin-left: 10px"><el-link type="primary"
                                                                     @click="showLoginShop(scope.row)">设置</el-link></span>
                        </template>
                    </yid-table-column>
                    <yid-table-column label="是否校验登录" min-width="70" prop="ismac">
                        <template slot-scope="scope">
                            <el-switch v-model="scope.row.ismac" active-value="1" inactive-value="0"
                                       active-color="#13ce66" @change="updateIsmac(scope.row)">
                            </el-switch>
                        </template>
                    </yid-table-column>
                    ...
                    <yid-table-column label="状态" min-width="70" prop="enabled">
                        <template slot-scope="scope">
                            <el-switch v-model="scope.row.enabled" inactive-color="#DCDFE6"
                    ...
                    <yid-table-column label="登录设备" min-width="70" prop="lastDevice">
                        <template slot-scope="scope">
                            <el-tooltip v-if="scope.row.lastDevice" class="item" effect="dark"
                                        :content="scope.row.lastDevice" placement="top">
                                <el-link :data-clipboard-text="scope.row.lastDevice" class="copymac" type="primary"
                                         @click="copymac">复制
                                </el-link>
                            </el-tooltip>
                    ...
                    <yid-table-column label="操作" min-width="90" fixed="right">
                        <template slot-scope="scope">
                            <el-button @click="resetPassword(scope.row)" type="text"><i class="edit"></i>重置密码
                            </el-button>
                            <el-button @click="openManageDialog(scope.row)" type="text"><i class="edit"></i>数据查看管理
                            </el-button>
                        </template>
                    </yid-table-column>
                </yid-table>

                <yid-dialog :title="dialog.title" :visible.sync="dialog.visible" @close="cancel" width="700px">
                    <el-form ref="userForm" :model="userForm" label-width="80px">
                        <el-form-item label="用户名" prop="username" :rules="[{ required: true, message: '用户名不能为空'}]">
                            <el-input v-model="userForm.username" placeholder="请输入用户名" :disabled="this.disabledvalue"
                                      maxlength="13" show-word-limit @input="checkUsername"
                        ...
                        <el-form-item label="密码" prop="password" :rules="[{ required: true, message: '密码不能为空'}]"
                            ...
                            <el-input clearable maxlength="20" placeholder="请输入密码" v-model.trim="userForm.password"
                                      show-password style="width: 200px;"></el-input>
                        </el-form-item>
                        <el-form-item label="系统角色" prop="roleIds">
                            <el-select clearable multiple placeholder="请选择" v-model.trim="userForm.roleIds"
                                       filterable :filter-method="filterRole2" style="width: 300px;">
                                <el-option :key="'roleId2'+item.id" :label="item.name" :value="item.id"
                                           v-for="item in filterRoleList2"></el-option>
                            </el-select>
                        </el-form-item>
                        <el-form-item label="特色权限">
                            <span><el-link type="primary" @click="editUserMenu()">设置</el-link></span>
                        </el-form-item>
                        <el-form-item label="使用人" prop="nickname" :rules="[{ required: true, message: '使用人不能为空'}]">
                            <el-input v-model="userForm.nickname" placeholder="请输入使用人" @input="checkEename"
                                      style="width: 200px;"></el-input>
                        </el-form-item>
                        <el-form-item label="员工信息" prop="eeInfo">
                            <span v-if="userForm.eeInfo!=null" style="margin-right: 7px">{{userForm.eeInfo.eename}}&nbsp;&nbsp;{{userForm.eeInfo.mobile}} </span>
                            <span v-if="userForm.eeInfo!=null"><el-link type="primary"
                                                                        @click="editBindEmployee(userForm.eeid)">修改</el-link></span>
                            <span v-if="userForm.eeInfo!=null" style="margin-left: 20px"><el-link type="primary"
                                                                                                  @click="clearEmployee()">解绑员工</el-link></span>
                            <span v-if="userForm.eeInfo==null"><el-link type="primary" @click="editBindEmployee('0')">绑定员工</el-link></span>
                        ...
                        <el-form-item label="登录门店">
                            ...
                            <span><el-link type="primary" @click="editLoginShop(userForm.shopid)">设置</el-link></span>
                        </el-form-item>
                        <el-form-item label="校验登录" prop="ismac">
                            <el-select placeholder="请选择" v-model.trim="userForm.ismac" style="width: 150px;">
                                <el-option label="是" value="1"></el-option>
                                <el-option label="否" value="0"></el-option>
                            </el-select>
                        </el-form-item>
                ...
                <yid-dialog :title="roleDialog.title" :visible.sync="roleDialog.visible" @close="roleCancel"
                            width="550px">
                    <el-form :model="roleForm" ref="roleForm" :inline="true" class="demo-form-inline"
                             style="margin-top: 20px;">
                        <el-form-item label="设定对象" prop="userIds">
                            <el-select multiple placeholder="请选择账号：" style="width: 350px;"
                                       v-model.trim="roleForm.userIds" disabled
                                       filterable :filter-method="filterUser">
                                <el-option :key="'user'+item.id" :label="item.username" :value="item.id"
                                           v-for="item in filterUserList"></el-option>
                            </el-select>
                        </el-form-item>
                        <el-form-item label="系统角色" prop="roleIds">
                            <el-select clearable multiple placeholder="请选择" v-model.trim="roleForm.roleIds"
                                       filterable :filter-method="filterRole3" style="width: 350px;">
                                <el-option :key="'roleId3'+item.id" :label="item.name" :value="item.id"
                                           v-for="item in filterRoleList3"></el-option>
                            </el-select>
                        </el-form-item>
                        <el-form-item>
                            <el-button @click="roleCancel" type="" size="mini" style="margin-left: 150px">取消</el-button>
                            <el-button @click="batchSaveRole" type="primary" size="mini">确定</el-button>
                        </el-form-item>
                    </el-form>

                </yid-dialog>

                <yid-dialog :title="pwdDialog.title" :visible.sync="pwdDialog.visible" @close="pwdCancel" width="550px">
                    <el-form :model="pwdForm" ref="pwdForm" :inline="true" class="demo-form-inline"
                             style="margin-top: 20px;">
                        <el-form-item label="新密码" prop="password" :rules="[{ required: true, message: '密码不能为空'}]">
                            <el-input clearable maxlength="20" placeholder="请输入密码" v-model.trim="pwdForm.password"
                                      show-password style="width: 200px;"></el-input>
                        </el-form-item>
                        <el-form-item>
                            <el-button @click="pwdCancel" type="" size="mini" style="margin-left: 150px">取消</el-button>
                            <el-button @click="savePwd" type="primary" size="mini">确定</el-button>
                        </el-form-item>
                    </el-form>

                </yid-dialog>

                <yid-dialog :title="menuDialog.title" :visible.sync="menuDialog.visible" @close="menuCancel"
                            width="600px">
                              ...
                              <span class="el-dropdown-link">
                                <el-button type="primary" icon="el-icon-circle-plus-outline">添加菜单</el-button>
                                    ...
                                    <el-dropdown-menu slot="dropdown">
                                        <el-dropdown-item :command="item" v-for="item in authMenu"
                                                          :key="'menu'+item.id">{{item.name}}
                                        </el-dropdown-item>
                                    </el-dropdown-menu>
                                </el-dropdown>
                            </el-form-item>
                            <el-form-item>
                                <el-button @click="selectAll" type="primary" size="mini" style="margin-left: 40px">全选
                                </el-button>
                                ...
                                <el-button @click="saveMenuConfig" type="primary" size="mini" style="margin-left: 50px">
                                    保存
                                </el-button>
                            </el-form-item>
                        </el-form>
                ...
                <yid-dialog :title="shopDialog.title" :visible.sync="shopDialog.visible" @close="shopCancel"
                    ...
                    <el-row type="flex" justify="space-between">
                        <el-form inline>
                            ...
                            <el-form-item label="区域：">
                                <el-select clearable v-model="shopDialog.region" style="width: 160px;">
                                    <el-option v-for="item in regions" :label="item.name"
                    ...
                    <el-row>
                        <el-col>
                            <yid-table pagination ref="tableShop" :data="shopData" :header-cell-class-name="cellClass"
                                       style="margin-top: 10px;"
                                       @select="handleSelectionChange2" :row-key="getRowKey" class="store-table">
                    ...
                    <el-row justify="space-between" style="margin-top: 10px;">
                        <el-col>
                            <el-button type="primary" @click="saveShop()" style="margin-left: 300px;">保存</el-button>
                            <el-button @click="shopCancel">取消</el-button>
                        </el-col>
                    </el-row>
                </yid-dialog>

                <yid-dialog :title="shopDialog2.title" :visible.sync="shopDialog2.visible" @close="shopCancel2"
                            width="800px">
                    <el-row type="flex" justify="space-between">
                        <el-form inline>
                            <el-form-item label="门店：">
                                <el-input clearable v-model="shopDialog2.shop" placeholder="门店名称/编码"
                                          style="width: 160px;"></el-input>
                            </el-form-item>
                            <el-form-item label="业务类型：">
                            ...
                            <el-form-item label="区域：">
                                <el-select clearable v-model="shopDialog2.region" style="width: 160px;">
                                    <el-option v-for="item in regions" :label="item.name"
                                               :value="item.name"></el-option>
                                </el-select>
                            </el-form-item>
                    ...
                    <el-row>
                        <el-col>
                            <yid-table pagination ref="tableShop2" :data="shopData2" :header-cell-class-name="cellClass"
                                       style="margin-top: 10px;"
                                       @select="handleSelectionChange3" :row-key="getRowKey" class="store-table">
                                <el-table-column type="selection" width="55"></el-table-column>
                                <yid-table-column label="门店编码" min-width="60" prop="shopcode"></yid-table-column>
                                <yid-table-column label="门店名称" min-width="100" prop="shopname"></yid-table-column>
                                <yid-table-column label="门店品牌" min-width="60" prop="brandName"></yid-table-column>
                    ...
                    <el-row justify="space-between" style="margin-top: 10px;">
                        <el-col>
                            <el-button type="primary" @click="saveShop2()" style="margin-left: 300px;">保存</el-button>
                            <el-button @click="shopCancel2">取消</el-button>
                        </el-col>
                    </el-row>
                </yid-dialog>

                <yid-dialog :title="eeDialog.title" :visible.sync="eeDialog.visible" @close="eeCancel" width="800px">
                    <el-row type="flex" justify="space-between">
                        <el-form inline>
                            <el-form-item label="门店：">
                                <el-input clearable v-model="eeDialog.shop" placeholder="门店名称/编码"
                                          style="width: 160px;"></el-input>
                            </el-form-item>
                            <el-form-item label="员工：">
                                <el-input clearable placeholder="员工编码/姓名/手机号" v-model.trim="eeDialog.eename"
                                          style="width: 190px;"></el-input>
                            </el-form-item>
                            <el-form-item label="">
                    ...
                    <el-row>
                        <el-col>
                            <yid-table pagination ref="tableEmployee" :data="employeeData"
                                       :header-cell-class-name="cellClass" style="margin-top: 10px;"
                                       @select="handleSelectionEmployeeChange" class="store-table">
                                <el-table-column type="selection" width="55"></el-table-column>
                                <yid-table-column label="门店编码" min-width="60" prop="shopcode"></yid-table-column>
                                <yid-table-column label="门店名称" min-width="100" prop="shopname"></yid-table-column>
                                ...
                                <yid-table-column label="手机号" min-width="90" prop="mobile"></yid-table-column>
                            </yid-table>
                        </el-col>
                    </el-row>
                    <el-row justify="space-between" style="margin-top: 10px;">
                        <el-col>
                            <el-button type="primary" @click="saveBindEe" style="margin-left: 300px;">保存</el-button>
                            <el-button @click="eeCancel">取消</el-button>
                        </el-col>
                    </el-row>
                </yid-dialog>

                <yid-dialog :title="manageData.title" :visible.sync="manageData.visible"
                            @close="manageData.visible=false"
                            width="800px">
                    <el-row type="flex" justify="space-between">
                        <el-form style="width:600px">
                            <el-form-item label="员工编码/姓名：">
                                <el-label>{{manageData.eecode}} / {{manageData.eename}}</el-label>
                            </el-form-item>
                            <el-form-item label="所在组织机构：">
                                <el-label>{{manageData.orgcode}} {{manageData.orgname}}</el-label>
                            </el-form-item>
                            <el-form-item label="数据查看范围">
                                <el-button type="primary" @click="selectOrgs()">选择组织机构</el-button>
                                <el-button type="primary" @click="selectShops()">选择门店</el-button>
                            </el-form-item>
                        </el-form>
                    </el-row>
                    <el-row>
                        <el-col>
                            <yid-table ref="tableOrg" :data="manageData.orgs"
                                       :header-cell-class-name="cellClass" style="margin-top: 10px;"
                                       @select="handleSelectionEmployeeChange" class="store-table">
                                <yid-table-column label="机构编码" min-width="60" prop="orgcode"></yid-table-column>
                                <yid-table-column label="机构名称" min-width="80" prop="orgname"></yid-table-column>
                                <yid-table-column label="操作" min-width="50">
                                    <template slot-scope="scope">
                                        <el-link type="primary" @click="deleteOrg(scope.row)">删除</el-link>
                                    </template>
                                </yid-table-column>
                            </yid-table>
                            <yid-table ref="tableShop" :data="manageData.shops"
                                       :header-cell-class-name="cellClass" style="margin-top: 10px;"
                                       @select="handleSelectionEmployeeChange" class="store-table">
                                <yid-table-column label="门店编码" min-width="60" prop="orgcode"></yid-table-column>
                                <yid-table-column label="门店名称" min-width="80" prop="orgname"></yid-table-column>
                                <yid-table-column label="操作" min-width="50">
                                    <template slot-scope="scope">
                                        <el-link type="primary" @click="deleteShop(scope.row)">删除</el-link>
                                    </template>
                                </yid-table-column>
                            </yid-table>
                        </el-col>
                    ...
                    <el-row justify="space-between" style="margin-top: 10px;">
                        <el-col>
                            <el-button type="primary" @click="saveManageData()" style="margin-left: 300px;">保存
                            </el-button>
                ...
                <yid-dialog :title="orgDialog.title" :visible.sync="orgDialog.visible" @close="orgDialog.visible=false"
                            width="600px">
                    <el-row type="flex" justify="space-between">
                        <el-form inline>
                            <el-form-item label="组织机构：">
                                <el-input clearable v-model="orgDialog.codename" placeholder="组织机构编码/名称"
                    ...
                    <el-row>
                        <el-col>
                            <yid-table pagination ref="tableOrg" :data="orgDialog.data" style="margin-top: 10px;"
                                       @selection-change="mutlselectOrg">
                                <el-table-column type="selection" width="55"></el-table-column>
                                <yid-table-column label="机构编码" min-width="60" prop="code"></yid-table-column>
                                <yid-table-column label="机构名称" min-width="100" prop="name"></yid-table-column>
                            </yid-table>
                        </el-col>
                    </el-row>
                    <el-row justify="space-between" style="margin-top: 10px;">
                        <el-col>
                            <el-button type="primary" @click="saleOrgs()" style="margin-left: 300px;">保存</el-button>
                            <el-button @click="orgDialog.visible=false">取消</el-button>
                        </el-col>
                    </el-row>
                </yid-dialog>

                <yid-dialog :title="shopDialog3.title" :visible.sync="shopDialog3.visible"
                            @close="shopDialog3.visible=false"
                            width="800px">
                    <el-row type="flex" justify="space-between">
                        <el-form inline>
                            <el-form-item label="门店：">
                                <el-input clearable v-model="shopDialog3.shop" placeholder="门店名称/编码"
                                          style="width: 160px;"></el-input>
                            </el-form-item>
                            <el-form-item label="业务类型：">
                            ...
                            <el-form-item label="区域：">
                                <el-select clearable v-model="shopDialog3.region" style="width: 160px;">
                                    <el-option v-for="item in regions" :label="item.name"
                                               :value="item.name"></el-option>
                                </el-select>
                            </el-form-item>
                    ...
                    <el-row>
                        <el-col>
                            <yid-table pagination ref="tableShop3" :data="shopData3" style="margin-top: 10px;"
                                       @selection-change="mutlselectShops" :row-key="getRowKey">
                                <el-table-column type="selection" width="55"></el-table-column>
                                <yid-table-column label="门店编码" min-width="60" prop="shopcode"></yid-table-column>
                                <yid-table-column label="门店名称" min-width="100" prop="shopname"></yid-table-column>
                                <yid-table-column label="门店品牌" min-width="60" prop="brandName"></yid-table-column>
                    ...
                    <el-row justify="space-between" style="margin-top: 10px;">
                        <el-col>
                            <el-button type="primary" @click="saleShops()" style="margin-left: 300px;">保存</el-button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            checkUsername(value) {
                this.userForm.username = value.replace(/[^\w\.\/]/ig, '');
            },
            checkEename(value) {
            ...
            openManageDialog(row) {
                ...
                service.chain.employee.getManageData({eecode: row.eecode}).then(res => {
                    if (res.resp_code == "200") {
                        ...
                        this.manageData.orgcode = res.data.shopcode ? res.data.shopcode : res.data.regionCode;
                        this.manageData.orgname = res.data.shopname ? res.data.shopname : res.data.regionName;
            ...
            saleOrgs() {
                if (this.orgDialog.select.length > 0) {
                    this.orgDialog.select.forEach(org => {
                        ...
                        if (flag) {
                            this.manageData.orgs.push({orgcode: org.code, orgname: org.name, pathcode: org.pathCode})
                        }
                    })
                }
            ...
            saleShops() {
                if (this.shopDialog3.select.length > 0) {
                    this.shopDialog3.select.forEach(org => {
                        ...
                        if (flag) {
                            this.manageData.shops.push({orgcode: org.shopcode, orgname: org.shopname})
                        }
                    })
                }
