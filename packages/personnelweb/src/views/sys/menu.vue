  ...
  <div class="sys-log">
    <el-form :model="model" inline>
      <el-form-item label="所属应用：">
        <el-select
          ...
          placeholder="请选择"
          style="width: 160px"
          v-model.trim="model.appId">
          <el-option
            :key="'app' + item.id"
            :label="item.name"
            :value="item.appId"
            v-for="item in clientMap"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="供应端">
        <el-select
          ...
          placeholder="请选择"
          style="width: 160px"
          v-model.trim="model.client">
          <el-option
            :key="'client' + item.value"
            :label="item.text"
            :value="item.value"
            v-for="item in clients"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    <div>
      ...
      <span style="margin-left: 30px; color: red"
        >选中一行为上级菜单，不选则为根目录（单击选中，双击取消）</span
      >
    </div>
    <el-table
      ...
      <el-table-column label="菜单URL" min-width="180" prop="url">
      </el-table-column>
      <el-table-column label="菜单PATH" min-width="100" prop="path">
      </el-table-column>
      <el-table-column label="样式" min-width="70" prop="css">
      </el-table-column>
      <el-table-column
        label="请求方法"
        min-width="90"
        prop="pathMethod"></el-table-column>
      <el-table-column
        label="排序号"
        min-width="70"
      ...
      <el-table-column label="操作" min-width="120" fixed="right">
        <template slot-scope="scope">
          ...
          >
          <el-button @click="deleteMenu(scope.row)" type="text"
...
export default {
  ...
  data() {
    return {
      treeMenuData: [],
      model: {
        appId: process.env.VUE_APP_CLIENTID,
        client: ''
      },
      pageInfo: {
        page: 0,
