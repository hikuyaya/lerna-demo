<template>
  <div class="positionlimits">
    ...
    <yid-table
      pagination
      ref="table"
      :data="positionSetingData"
      style="margin-top: 12px">
      <yid-table-column
        label="职务编码"
        min-width="80"
        prop="pscode"></yid-table-column>
      <yid-table-column
        label="职务名称"
        min-width="100"
      ...
      <yid-table-column
        label="分配角色"
        min-width="100"
        prop="roleName"></yid-table-column>
      <yid-table-column label="操作" min-width="100" prop="content">
        <template slot-scope="scope">
          <el-button type="text" @click="editposition(scope.row)"
            ><i class="edit"></i>编辑</el-button
          >
          <el-button type="text" @click="deletePositionSetting(scope.row)"
            ><i class="view"></i>删除</el-button
          >
        </template>
      </yid-table-column>
    </yid-table>
    ...
    <yid-dialog
      :title="positionDialog.title"
      :visible.sync="positionDialog.visible"
      width="800px">
      <el-form ref="positionForm" :model="positionForm" label-width="450px">
        <el-button @click="positionSelectType" type="primary"
          >选择职务</el-button
        >
        <yid-table :data="positionData" style="margin-top: 15px">
          ...
          <yid-table-column label="操作" min-width="100" prop="content">
            <template slot-scope="scope">
              <el-link type="primary" @click="deleteSelectPosition(scope.row)"
                >删除</el-link
              >
            </template>
          </yid-table-column>
        ...
        <yid-table :data="roleData" style="margin-top: 10px">
          ...
          <yid-table-column label="操作" min-width="100" prop="content">
            <template slot-scope="scope">
              <el-link type="primary" @click="deleteSelectRole(scope.row)"
                >删除</el-link
              >
            </template>
          </yid-table-column>
    ...
    <yid-dialog
      :title="editPositionDialog.title"
      :visible.sync="editPositionDialog.visible"
      width="800px">
      <el-form
    ...
    <yid-dialog
      :title="positionSelectDialog.title"
      :visible.sync="positionSelectDialog.visible"
      width="800px">
      <el-form
        ref="positionSelectForm"
        :model="positionSelectForm"
        label-width="100px"
        inline>
        <el-form-item label="同步业务组:">
          <el-select
            clearable
            filterable
            v-model="positionSelectForm.business"
        ...
        <el-form-item>
          <el-input
            clearable
            filterable
            v-model="positionSelectForm.psearch"
            placeholder="请输入职位编码或职位名称"
            style="width: 160px">
          </el-input>
        </el-form-item>
      ...
      <yid-table
        ...
        @selection-change="pSelectionChange">
    ...
    <yid-dialog
      :title="roleSelectDialog.title"
      :visible.sync="roleSelectDialog.visible"
      width="800px">
      <el-form
        ref="roleSelectForm"
        :model="roleSelectForm"
        label-width="100px"
        inline>
        <el-form-item label="角色名称:">
          <el-input
            clearable
            filterable
            v-model="roleSelectForm.rsearch"
            placeholder="请输入角色名称"
