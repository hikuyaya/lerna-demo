<template>
  <div class="role">
      ...
      <div class="table">
        <yid-table
          ...
          <yid-table-column label="操作" min-width="90" fixed="right">
            <template slot-scope="scope">
              <el-button @click="showEdit(scope.row)" type="text"
      ...
      <div class="tree">
        ...
        <div class="auth-config">
          ...
          <div
            ...
            style="margin-top: 50px; margin-left: 70px; color: #cccccc">
            <span v-if="!this.isSelected">点击角色名称，开始配置权限</span>
            <span v-if="this.isSelected"
              >暂无权限，请点击【添加菜单】按钮，为角色配置权限</span
            >
          </div>
          <el-tabs
            v-if="this.treeData.length > 0"
    ...
    <yid-dialog
      :title="rptdialog.title"
      :visible="rptdialog.visible"
      @close="rptdialog.visible = false"
      width="750px">
      <el-checkbox
      ...
      <el-form ref="roleForm1" label-width="80px">
        <el-form-item label=" ">
          <el-button @click="rptdialog.visible = false" type="info"
            >取消</el-button
          >
...
export default {
  ...
  data() {
    return {
      ...
      checkedItems: [],
      rptItemList: [
        { fieldName: '1', fieldTxt: 'A' },
        { fieldName: '2', fieldTxt: 'B' }
      ],
      checkAll: false,
  ...
  methods: {
        ...
        if (valid) {
          ...
          if (this.roleForm.isCopy) {
            ...
            this.treeData.map((item, index) => {
              ...
              halfcheckedArr =
                this.$refs['tree' + index][0].getHalfCheckedKeys()
        ...
        if (res.resp_code == '200') {
          ...
          if (len > 0) {
            text =
              '该角色已被' +
              len +
              '个账号绑定，删除角色时，将影响相关账号功能，删除操作不可撤销。' +
              text
          }
    ...
    getRptItem(headId) {
      const that = this
      service.sysMenu
        .getRptItem({ pageSize: 100, headId: headId })
        .then(res => {
      ...
      this.rptItemList.forEach(item => {
        ...
        if (exist) {
          if (!item.fieldRole) {
            ...
            item.fieldRole &&
            item.fieldRole.indexOf(that.authForm.roleId) < 0
          ) {
    ...
    handleCheckedCitiesChange(value) {
      ...
      this.isIndeterminate =
        checkedCount > 0 && checkedCount < this.rptItemList.length
    }
  }
}
</script>
