...
export function csv2sheet(head, data) {
    ...
    data.forEach(function(row, i) {
        if (i === 0) sheet['!ref'] = 'A1:' + String.fromCharCode(65 + Object.keys(data[0]).length) + (data.length)
        let j = 0
        for (const w in head) {
            sheet[String.fromCharCode(65 + j) + (i + 1)] ={v: row[w] == null ? '' : row[w]}
            j++
        }
    })
    data.shift()
...
/** 导入 file 文件 */
export function imporExecl(file,tableData,heads) {
    const reader = new FileReader()
    reader.readAsBinaryString(file)
    reader.onload = function(e) {
        const data = e.target.result
        const workbook = XLSX.read(data, { type: 'binary' })
        readWorkbook(workbook,tableData,heads)
    }
}

export function readWorkbook(workbook,tableData,heads) {
    const sheetNames = workbook.SheetNames // 工作表名称集合
    const worksheet = workbook.Sheets[sheetNames[0]] // 这里我们只读取第一张sheet
    const csv = XLSX.utils.sheet_to_csv(worksheet)
    csv2table(csv,tableData,heads)
}
