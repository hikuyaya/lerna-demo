    ...
    <div class="employeeSchedule">
        <el-form :model="ruleForm" ref="ruleForm" inline>
            ...
            <el-form-item label="职务：" prop="psid">
                <el-select  clearable filterable placeholder="请选择" v-model.trim="ruleForm.psid" @change="getSearchFormPositionLevelList">
                    <el-option style="width: 100px;"  :key="item.id" :label="item.psname" :value="item.id" v-for="item in searchFormPositionList"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="级别：" prop="pslid">
                <el-select  clearable filterable placeholder="请选择" v-model.trim="ruleForm.pslid">
            ...
            <el-form-item label="人员：" prop="eename">
                <el-input style="width: 120px;" clearable filterable placeholder="姓名/工号" v-model="ruleForm.eename"></el-input>
            </el-form-item>
        ...
        <el-table ref="multipleTable" style="width: 100%;overflow-x: auto;overflow-y: auto;" height="470px" :data="tableData" @selection-change="handleSelectionChange">
            ...
            <el-table-column v-for="(item,i) in monthDayList"
                             align="center"
                             :label="item.dayWeek">
                <template slot-scope="scope">
                    <div @click="chageBc(scope.row,item)" :style="[{'background': getClassesColor(scope.row,item),'cursor':'pointer'}]">
        ...
        <yid-dialog :title="pbDialog.title" :visible.sync="pbDialog.visible" width="450px">
            <el-form ref="pbForm" :model="pbForm"  label-width="140px">
                <el-form-item label="排班人员：" prop="eenames" :rules="[{ required: true, message: '排班人员不能为空'}]">
                    {{pbForm.eenames}}
                </el-form-item>
                ...
                <el-form-item label="班次：" prop="pccid" :rules="[{ required: true, message: '请选择班次'}]">
                    <el-select value-key="id" placeholder="请选择班次" v-model.trim="pbForm.pccid">
                        <el-option :key="item.id" :value="item.id" :label="item.cname" v-for="item in checkClassAll"/>
                    </el-select>
                </el-form-item>
                <el-form-item>
        ...
        <yid-dialog :title="copyDialog.title" :visible.sync="copyDialog.visible" width="450px">
            <el-form ref="copyForm" :model="copyForm"  label-width="140px">
                <el-form-item label="排班人员：" prop="eenames" :rules="[{ required: true, message: '排班人员不能为空'}]">
                    {{copyForm.eenames}}
                </el-form-item>
                <el-form-item label="历史排班：" prop="omon" :rules="[{ required: true, message: '请选择历史排班'}]">
                    <el-select value-key="id" placeholder="请选择班次" v-model.trim="copyForm.omon">
                        <el-option :key="item.id" :value="item.id" :label="item.value" v-for="item in yearmonList"/>
                    </el-select>
                </el-form-item>
                <el-form-item label="应用到：" prop="nmon" :rules="[{ required: true, message: '请选择历史排班'}]">
                    <el-select value-key="id" placeholder="请选择班次" v-model.trim="copyForm.nmon">
                        <el-option :key="item.id" :value="item.id" :label="item.value" v-for="item in yearmonList"/>
                    </el-select>
                </el-form-item>
                <el-form-item>
    ...
    export default {
        ...
        methods:{
            getScheduleList(){
                ...
                service.pers.employeeSchedule.employeeScheduleList(this.ruleForm).then(res=>{
            ...
            chageBc(row,item){
                //改变排班表信息
                const scheduleparam = {pccid:this.checkClassAll[classkey].id,dayStr:item.day,eePhoto:row.photo,eeid:row.id,eecode:row.eecode,eename:row.eename};
                service.pers.employeeSchedule.changePb(scheduleparam).then(res=>{
                    if(res.resp_code == 200) {
                        service.pers.employeeSchedule.employeeScheduleList(this.ruleForm).then(r=>{
                            if(r.resp_code == 200) {
                                r.data.forEach(v=>{
                                    if(row.eecode === v.eecode){
                                        row.employeeSchedules = v.employeeSchedules;
                                        for(var i=0 ; i<row.employeeSchedules.length ; i++){
                                            if(row.employeeSchedules[i].day === item.day){
                                                row.employeeSchedules[i].classes = this.checkClassAll[classkey].cname;
                                                row.employeeSchedules[i].color = this.checkClassAll[classkey].color;
