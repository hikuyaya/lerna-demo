    ...
    <div class="dept">
        <el-tabs v-model="activeName" @tab-click="handleClick" v-show="!detailShow">
            <el-tab-pane label="群发短信" name="msg">
                <el-form :model="qfForm"  label-width="140px" ref="qfForm">
                    ...
                    <el-form-item  prop="messageContent" :rules="{required: true, min:2, max:128, message: '亲,至少要输入两个字', trigger: 'blur' }" label="短信内容：">
                        <el-input
            ...
            <el-tab-pane label="服务项目回访短信模板" name="temp">
                ...
                <yid-table pagination ref="mbTable" style="margin-top: 15px;" :data="modelAddForm.tempData">
                    ...
                    <yid-table-column label="对应类别" min-width="150" prop="servnames"></yid-table-column>
            ...
            <el-tab-pane label="会员生日自动提醒" name="remind">
                <el-form label-width="140px" ref="birthdayFrom">
                    <el-form-item label="自动发送短信：">
                        <el-switch  v-model="birthdayFrom.autosend"
                                active-color="#13ce66"
                    ...
                    <el-form-item style="color: #666;">
                       <p>生日祝福短信最多54个字,短信内容不必包含称谓,系统会自动组合称谓，比如：'尊敬的刘小姐:'，'尊敬的张先生:'</p>
                       <p>设置示例:</p>
                       <p>愿将一切最美好的祝愿都送给您，VMOD SALON全员祝您：生日快乐！人生幸福美满，永浴欢乐年华！越活越年轻越来越漂亮。</p>
                    </el-form-item>
            ...
            <el-tab-pane label="短信发送记录" name="logs">
                ...
                <yid-table pagination ref="sendTable" style="margin-top: 15px;" :data="sendRecord.recordData">
                    <yid-table-column label="发送对象（号码数）" min-width="150" prop="object">
                        ...
                        <template slot-scope="scope">
                            <span v-if='scope.row.ptype==1'>会员顾客({{scope.row.sendcount}})</span>
                            <span v-if='scope.row.ptype==2'>员工({{scope.row.sendcount}})</span>
                            <span v-if='scope.row.ptype==3'>自定义({{scope.row.sendcount}})</span>
                        </template>
                    </yid-table-column>
                    ...
                    <yid-table-column label="短信内容" min-width="350" prop="scontent">
                        <template slot-scope="scope">
                            <span v-if="scope.row.isSet==1">
                                <el-input size="mini"  v-model="sendRecord.newConnet" placeholder="请输入内容">
                                </el-input>
                            </span>
                            <span v-else>{{scope.row.scontent}}</span>
                        </template>
                    </yid-table-column>
                    <yid-table-column label="短信条数" min-width="150" prop="total"></yid-table-column>
                    <yid-table-column label="操作" min-width="250" prop="content">
                        <template slot-scope="scope">
                            <el-link type="primary"  style="margin: 0 10px 0 10px;" v-if='scope.row.status==0' @click="deleteTask(scope.row.id)">取消发送</el-link>


                            <el-link  style="margin: 0 10px 0 10px;" v-if='scope.row.status==0' type="primary" @click="contentChange(scope.row,scope.$index,true)">
                                {{scope.row.isSet?'保存':"修改"}}
                            </el-link>

                            <el-link type="primary" style="margin: 0 10px 0 10px;" @click="replyAlert(scope.row.id)">回复记录</el-link>
                        </template>
                    </yid-table-column>
                </yid-table>
            </el-tab-pane>
            ...
            <el-tab-pane label="短信充值" name="charging">
                <el-alert
                    ...
                    <span style="color: red; font-size: 16px;">6900</span>

                    条

                    已使用 <span style="color: red;">1390</span> 条</el-alert>
                <div style="margin: 15px 0 15px 0;">短信充值：</div>
                <el-row :gutter="40"  >
                    <el-col :span="6"  v-for="(p, index) in chargingList">
        ...
        <yid-dialog :title="tempDialog.title" :visible.sync="tempDialog.visible" @close="" width="600px">
            <el-form :model="modelAddForm" label-width="140px" ref="modelAddForm">
                ...
                <el-form-item prop="modelText" label="模板内容"
                              :rules="{required: true, min:2, max:128, message: '亲,至少要输入两个字', trigger: 'blur' }">
                ...
                <el-form-item label="对应类别：">
                    <el-select @change="modelchangedep" v-model="modelAddForm.modelbranchid"  placeholder="请选择部门">
                        <el-option
                    ...
                    <el-select placeholder="服务类别（可多选）" multiple  v-model.trim="modelAddForm.modelservice"  @change="selectGetFn" style="margin-left: 20px;">
                        <el-option
                ...
                <el-form-item label="发送时间：">
                    消费完成 <el-input-number type="text" v-model="modelAddForm.xfday"  :min="1" :max="10"/> 天后
                </el-form-item>
                <el-form-item>
        ...
        <yid-dialog title="回复记录" :visible.sync="replyDialog.visible" @close="" width="600px">
            <el-alert type="info"
                      :closable="false"></el-alert>
            <yid-table pagination ref="table" style="margin-top: 15px;" :data="replyData">
                <yid-table-column label="回复时间" min-width="33%" prop="time"></yid-table-column>
                <yid-table-column label="回复人" min-width="33%" prop="mobile"></yid-table-column>
                <yid-table-column label="回复内容" min-width="34%" prop="mcontent"></yid-table-column>
            </yid-table>
        </yid-dialog>

        <yid-dialog title="发送明细" :visible.sync="detailDialog.visible" @close="" width="600px">
            <el-alert type="info"
                      :closable="false">
                <div>群发短信</div>
                <div>会员顾客：233</div>
            </el-alert>
            <yid-table pagination ref="table" style="margin-top: 15px;" :data="detailData">
                <yid-table-column label="姓名" min-width="50%" prop="name"></yid-table-column>
                <yid-table-column label="手机号" min-width="50%" prop="phone"></yid-table-column>
            </yid-table>
        </yid-dialog>

        <yid-dialog title="选择发送对象" :visible.sync="objDialog.visible" @close="" width="750px">
            <el-form :model="tempDialog.model"  label-width="120px" ref="copyForm">
                ...
                <template v-if="typeValue == 1">
                    ...
                    <el-form-item label="最少消费次数：">
                        <el-input-number  :min="0" :max="10" v-model="qfForm.xfcs"  style="width:70%"/>
                    </el-form-item>
                ...
                <template v-if="typeValue == 2">
                    <el-form-item label="部门：">
                        <el-select @change="changedep" v-model="qfForm.branchid"  placeholder="请选择">
                        <el-option
                    ...
                    <el-form-item>
                        <el-transfer v-model="qfForm.selectemployees"
        ...
        methods: {
            contentChange(row, index, cg) {
                ...
                else
                {
                    ...
                    yid.service.sms.updateTaskConnetById(pm).then(res => {
            ...
            selectGetFn(val) {
                ...
                for(let i=0;i<=val.length-1;i++){
                    this.modelAddForm.servicetypes.find((item)=>{ //这里的options就是数据源
                        if(item.id == val[i]){
            ...
            changedep()
            {
               ...
               let params={bbid:this.qfForm.branchid,status:'1'}
                console.log("sssss")
                yid.service.sms.getemployeeByBranhid(params).then(res => {
                    // 存储登录信息到本地缓存
            ...
            saveAuto()
            {
                ...
                yid.service.sms.saveBirthdaySms(birthdayParm).then(res => {
            ...
            sendmessage()
            {
                ...
                this.$refs.qfForm.validate((valid) => {
                    if (valid) {
                        ...
                        let params={isAutoName:this.qfForm.addcw,
                        ...
                        if(this.typeValue==1)
                        {
                            ...
                            let qobjparrm={cardId:this.qfForm.ptype,minSerNum:this.qfForm.xfcs,minMoney: this.qfForm.xfze}
                            if(this.qfForm.datasrc!='') {
                            ...
                            yid.service.sms.findConditionSms(qobjparrm).then(res => {
                              ...
                              if(res.data.length==0)
                              {
                                  ...
                                  this.$message({ type: 'error', message: '这是一个无效的群发,没有找到条件的会员'});

                                  return;

                              }
                                yid.service.sms.sendSmsMessage(params).then(res => {
                        ...
                        if(this.typeValue==2)
                        {
                         ...
                         //   params.selectemployees = this.qfForm.selectemployees;
                            ...
                            yid.service.sms.getAllByIds(this.qfForm.selectemployees).then(res => {
                                ...
                                if(res.data.length==0)
                                {
                                    this.$message({ type: 'error', message: '这是一个无效的群发,没有找到条件的员工'});
...
{
    this.$message({ type: 'error', message: '这是一个无效的群发,请确认输入了自定义的号码'});
    return;
}
            ...
            savemodeltempclose(tag = false) {
                this.$refs.modelAddForm.validate((valid) => {
                    if (valid) {
                        ...
                        let parm={servnames:this.modelAddForm.servnames,
            ...
            replyAlert(id) {
                ...
                yid.service.sms.getstaskreply(params).then(res => {
                    // 存储登录信息到本地缓存
            ...
            deleteTask(id) {
                ...
                $yid.util.confirm('你确定要删除此条数据吗？', '','', ()=> {
                    ...
                    yid.service.sms.deleteTaskByID(id).then(res => {
                        // 存储登录信息到本地缓存
                        console.log("res",res)
                        this.$message({ type: 'success', message: '已成功执行'});
...
//第四个
                ...
                if(tab.name == 'charging'){
                    ...
                    yid.service.sms.getCharging(chargingParams).then(res => {
                        // 存储登录信息到本地缓存
                        console.log("记录",res.data)
