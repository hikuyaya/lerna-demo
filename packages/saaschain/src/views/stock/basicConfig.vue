...
<div class="stock-basicConfig">
    <el-tabs v-model="activeName"  @tab-click="handleClick">
        <el-tab-pane label="基础配置" name="first" style="padding:20px 0 0 50px">
            <el-form ref="form" :model="configform" label-width="160px" color="#000" style="color: #000; font-weight: bold">
        ...
        <el-tab-pane label="出库类型配置" name="second" style="padding:20px 0 0 20px">
            <el-row>
                <el-button  type="primary" round @click="showAddOut(false,'')">+新增出库类型</el-button>
            </el-row>
            <yid-table :data="tableDataCk" ref="outtable" style="margin-top: 15px;">
                <yid-table-column label="类型名称" min-width="80" prop="sdiname"></yid-table-column>
                <yid-table-column label="状态" min-width="100" prop="isDel">
                    <template slot-scope="scope">
                        <el-switch :disabled="getIsEditCk(scope.row)" v-model="scope.row.isDel" active-value="0" inactive-value="1" active-color="#409EFF" inactive-color="#DCDFE6" @change="updateStatus(scope.row)">
                        </el-switch>
                    </template>
                </yid-table-column>
                <yid-table-column label="操作" width="150" prop="username">
                    <template slot-scope="scope">
                        <el-button :disabled="getIsEditCk(scope.row)" type="text" size="small" @click="showAddOut(true,scope.row)">编辑</el-button>
                    </template>
                </yid-table-column>
            </yid-table>
            <el-row style="text-align: center; margin-top: 20px">
        ...
        <el-tab-pane label="入库类型配置" name="third" style="padding:20px 0 0 20px">
            <el-row>
                <el-button  type="primary" round @click="showAddIn(false,'')">+新增入库类型</el-button>
            </el-row>
            <yid-table  :data="tableDataRk" ref="intable" style="margin-top: 15px;">
                <yid-table-column label="类型名称" min-width="80" prop="sdiname"></yid-table-column>
                <yid-table-column label="状态" min-width="100" prop="isDel">
                    <template slot-scope="scope">
                    <el-switch :disabled="getIsEditRk(scope.row)" v-model="scope.row.isDel" active-value="0" inactive-value="1" active-color="#409EFF" inactive-color="#DCDFE6" @change="updateStatus(scope.row)">
                    </el-switch>
                </template>
                </yid-table-column>
                <yid-table-column label="操作" width="150" prop="username">
                    <template slot-scope="scope">
                        <el-button :disabled="getIsEditRk(scope.row)" type="text" size="small" @click="showAddIn(true,scope.row)">编辑</el-button>
                    </template>
                </yid-table-column>
            </yid-table>
            <el-row style="text-align: center; margin-top: 20px">
    ...
    <!--出库弹出层-->
    <yid-dialog :title="dialogOut.title" :visible.sync="dialogOut.visible" @close="cancel" width="400px">
        <el-form :model="outForm"  ref="outForm" label-width="80px">
            <el-form-item label="类型名称" prop="sdiname" :rules="[{ required: true, message: '类型名称为空'}]">
    ...
    <!--入库弹出层-->
    <yid-dialog :title="dialogIn.title" :visible.sync="dialogIn.visible" @close="cancel" width="400px">
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            methods: {
                getConfigform(){
                    service.stockConfig.orderConfig().then(res => {
                        if(res.data == null){
