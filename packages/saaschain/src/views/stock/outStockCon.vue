...
<div class="outStockCon">
    ...
    <el-row style="margin-top: 20px">
        <div class="grid-content bg-purple">出库类型：
        <el-select :disabled="!canEditable" v-model="inTypeObj" clearable placeholder="请选择" value-key="id">
            <el-option
    ...
    <el-row style="margin-top: 20px" :gutter="20">
        <el-col :span="4"><div class="grid-content bg-purple">出库金额：<label inline style="color: red">{{outAllMoney}}元</label></div></el-col>
   ...
   <yid-table ref="table" :data="selectProducts" style="margin-top: 15px;">
    ...
    <yid-table-column label="出库数量" width="130" prop="count">
        <template slot-scope="scope">
            <el-input :disabled="!canEditable" type="number" v-model="scope.row.count"></el-input>
        </template>
    </yid-table-column>
    <yid-table-column label="售价" width="130" prop="outcost">
        <template slot-scope="scope">
            <el-input :disabled="!canEditable" type="number" v-model="scope.row.price" ></el-input>
        </template>
    </yid-table-column>
    <yid-table-column label="出库金额" width="130" prop="money">
        <template slot-scope="scope">
            <el-input  disabled type="number" v-model="scope.row.count * scope.row.price"></el-input>
        </template>
    </yid-table-column>
    ...
    <!--出库商品选择-->
    <yid-dialog :title="dialogProduct.title" :visible.sync="dialogProduct.visible" @close="cancel" width="900px">
        ...
        methods: {
            async initData(obj){
                ...
                if(this.actionType == "add"){
                    ...
                    await service.stock.outProducts.list({isDel:"0",soid:entryData.id}).then(res=> {
            ...
            async getTypeDictList(){
                let that = this
                 await  service.stock.dictInout.list({type:service.stock.dictInout.outType,isDel:"0"}).then(res=> {
                    if(res.resp_code == 200) {
            ...
            async addNew(){
                ...
                await  service.stock.out.save(reqParms).then(res=> {
                    if(res.resp_code == 200) {
                        ...
                        service.stock.outProducts.list({isDel:"0",soid:this.soid}).then(res=> {
                            if(res.resp_code == 200){
                                this.selectProducts = res.data;
                            }
                        })
